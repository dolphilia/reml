...
type TomlDocument = Value;
type TomlTable<'a> = &'a Map<String, Value>;
...
fn get_table<'a>(table: &'a Map<String, Value>, key: &str) -> Option<&'a Map<String, Value>> {
    table.get(key).and_then(as_table)
}
...
fn extract_workspace_dependencies(
    workspace_toml: &TomlDocument,
) -> Result<BTreeMap<String, String>, Error> {
    if let Some(table) = workspace_dep_table(workspace_toml) {
        Ok(table
            .iter()
            .map(|(dep_name, dep_value)| {
                let pkg_name = dependency_package_name(dep_name, dep_value);
...
/// Return `[workspace.dependencies]` table if present.
fn workspace_dep_table<'a>(cargo_toml: &'a TomlDocument) -> Option<TomlTable<'a>> {
    let table = as_table(cargo_toml)?;
    get_table(table, "workspace")
        .and_then(|w| get_table(w, "dependencies"))
}
...
fn dependency_package_name<'a>(dep_name: &'a str, dep_value: &'a Value) -> &'a str {
    dep_value
        .as_table()
...
fn dependency_is_workspace(dep_value: &Value) -> bool {
    dep_value
        .as_table()
...
fn open_cargo_toml(path: &Path) -> Result<TomlDocument, Error> {
...
fn extract_crate_names(
    cargo_toml: &TomlDocument,
    workspace_dependencies: BTreeMap<String, String>,
) -> Result<CrateNames, Error> {
    let root_table = cargo_toml
        .as_table()
        .ok_or_else(|| Error::InvalidToml {
            source: toml::de::Error::custom("manifest root is not a table"),
        })?;
    let package_name = extract_package_name(root_table);
    ...
    dep_tables.extend(dep_tables_from_table(root_table));
    dep_tables.extend(target_dep_tables(root_table));

    let dep_pkgs = dep_tables.into_iter().flat_map(|table| {
        table.iter().filter_map(move |(dep_name, dep_value)| {
            let pkg_name = dependency_package_name(dep_name, dep_value);
            ...
            let pkg_name = if dependency_is_workspace(dep_value) {
...
fn extract_package_name(cargo_toml: &Map<String, Value>) -> Option<&str> {
    cargo_toml
        .get("package")
...
fn target_dep_tables<'a>(cargo_toml: &'a Map<String, Value>) -> Vec<TomlTable<'a>> {
    cargo_toml
        .get("target")
        ...
        .flat_map(|target_table| {
            target_table
                ...
                .flat_map(dep_tables_from_table)
        })
        .collect()
}

fn dep_tables_from_table(table: &Map<String, Value>) -> Vec<TomlTable<'_>> {
...
mod tests {
    ...
    macro_rules! create_test {
        (
            ...
            #[test]
            fn $name() {
                let cargo_toml: TomlDocument = toml::from_str($cargo_toml)
                    .expect("Parses `Cargo.toml`");
                let workspace_cargo_toml: TomlDocument = toml::from_str($workspace_toml)
    ...
    // Inline table support should continue to work; add a regression test.
    create_test! {
