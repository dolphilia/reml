...
- Manifest (`reml.toml`) と DSL 署名同期は Rust 版でも動作するものの、パーサープラグイン登録 API（`register_parser` + Capability 公開）は未整備のため、Rust で動作確認する場合はバッチ実行のみを想定し、Streaming/Recover の挙動差分を `do../../notes/parser/core-parse-api-evolution.md#todo-rust-lex-streaming-plugin` へ記録しておく。
