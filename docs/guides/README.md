...
## 開発ワークフロー & ツールチェーン
- [tooling/cli-workflow.md](tooling/cli-workflow.md) — `remlc-ocaml` を使った日常開発フローと診断・トレースの活用手順。
- [tooling/cli-help-template.md](tooling/cli-help-template.md) — CLI ヘルプ/マニュアル/ガイドの同期テンプレート運用方針。
- [tooling/cli-authoring.md](tooling/cli-authoring.md) — `Core.Cli` で宣言的に CLI を構築する最小ガイド。
- [man/remlc-ocaml.1.md](man/remlc-ocaml.1.md) — `remlc-ocaml` の man ページ原稿。
- [tooling/testing.md](tooling/testing.md) — `Core.Test` によるスナップショット/ゴールデンテスト運用。
- [tooling/trace-output.md](tooling/trace-output.md) — `--trace`/`--stats` の出力形式と活用指針。
