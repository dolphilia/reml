...
| 中(ランタイム) | ParseObserver / ParserProfile（ヒット率/BT/Packrat 統計） | 採用（デフォルト OFF） | 観測処理はオプトイン＋定数倍オーバーヘッドに抑え、Packrat キャッシュ鍵を共有することで安全性を維持。 | 仕様化前に `docs/notes/core-parse-api-evolution.md` へ暫定契約を記録。API 追加は非破壊。 |
...
- 観測系 API と recover 同期トークンの暫定契約を `docs/notes/core-parse-api-evolution.md` に追記し、Packrat キャッシュ鍵の整合性テストを計画する。  
