...
1.  **Phase 4 拡張**:
    *   `Core.Dsl.Object` と `Core.Dsl.Gc` の仕様ドラフトを作成。
...
これらの「バッテリー同梱（Batteries-included）」ランタイムキットを提供することで、Reml は開発者がメモリ管理、ディスパッチ、並行処理のために車輪の再発明をすることなく、リッチでパラダイム特化型の言語を構築できるようにします。
