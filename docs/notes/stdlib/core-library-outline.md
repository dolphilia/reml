...
- `Unicode::VERSION` 変更時は `docs/notes/text/unicode-upgrade-log.md` に記録し、`IndexCache.version` と一致しない全キャッシュを即座に破棄する。`tooling/ci/collect-iterator-audit-metrics.py --section text --scenario grapheme_stats --require-success` で `version_mismatch_evictions` を監査ログに記録する。

### 実装・テスト連携
- 参照モデルは `docs/notes/text/text-unicode-ownership.md`（所有権）と `docs/notes/text/text-unicode-segmentation-comparison.md`（セグメンテーション戦略）に追記済み。`IndexCache` は `Vec<usize>` のみを保持し、`Bytes` と共有しないことを仕様化した。  
...
- `FFI/Core Prelude shim` は Phase4 の暫定策であり、`phase4-scenario-matrix.csv` の `FFI-CORE-PRELUDE-001` で回帰監視を行う。Phase5 では `reml_runtime` へ直接依存する `core_prelude` crate を切り出し、`reml_runtime_ffi` から shim を削除する計画を `docs/plans/bootstrap-roadmap/5-1-spec-core-regression-plan.md#フェーズd` と同期させる。移行時は `compiler/rust/frontend/tests/core_iter_*` のスナップショットと `reports/spec-audit/ch5/spec-core-dashboard.md` の KPI を更新する責務を明示しておく。
...
- `collect_cell_ref_effects` セットでは `python3 tooling/ci/collect-iterator-audit-metrics.py --suite collectors --thresholds docs/plans/bootstrap-roadmap/assets/metrics/core_collections_thresholds.json --scenario ref_internal_mutation --require-success --require-cell` に加え、`scripts/validate-diagnostic-json.sh --suite collectors --pattern collector.effect.cell --pattern collector.effect.rc` を定期的に走らせ、`collector.effect.cell` / `collector.effect.rc` が `reports/spec-audit/ch1/core_iter_collectors.json` へ届いていることを確認している。現時点ではスナップショット未整備のため `threshold metric missing: collector.effect.cell_rc` が記録されるが、Run ID を `reports/iterator-collector-summary.md` と `docs/plans/bootstrap-roadmap/assets/metrics/core_collections_persistent.csv` に残しておき、ケース追加後のゼロ遷移をブロッカー解除条件として扱う。
- 2027-03-06 追記（WBS 3.1c-H1）: `Iter::collect_*` 側でも `Collector` の監査メタデータが再現されるよう `compiler/rust/runtime/src/prelude/iter/terminators.rs` に `fn collect_with<C: Collector>` と `CollectOutcome::audit` を導入。`cargo test --manifest-path compiler/rust/frontend/Cargo.toml core_iter_terminators` → `python3 tooling/ci/collect-iterator-audit-metrics.py --section iterator --case terminators --source ../../reports/spec-audit/ch1/core_iter_terminators.json --output ../../reports/iterator-collector-metrics.json --require-success` → `scripts/validate-diagnostic-json.sh --pattern iterator.collect ../../reports/spec-audit/ch1/core_iter_terminators.json` の順に実行し、`collector.effect.mem_reservation`・`Diagnostic.extensions["prelude.collector"]` が Collector 直呼びと一致することを `../../reports/spec-audit/ch0/links.md#iter-terminators-h1`、`../../reports/iterator-collector-summary.md#collect_vec_reserve-h1`、`../../docs/plans/bootstrap-roadmap/assets/prelude_api_inventory.toml`（collect_list/collect_vec/collect_string 行）に記録した。
...
### <a id="iter-g2-flat-zip"></a>Iter G2 flat_map / zip Stage メモ（WBS 3.1c-G2）
- 参照計画: `../plans/bootstrap-roadmap/3-1-core-prelude-iteration-plan.md#4a-アダプタ実装ロードマップw37-後半` と `../plans/bootstrap-roadmap/3-1-iter-collector-remediation.md#6-g2-flat_map--zip-stage-適用w38-前半` を出発点とし、`flat_map` の `effect {mem}` / Stage `beta`、`zip` の `Stage::Stable` + `iterator.error.zip_shorter` を同期する。
- 実装境界: `compiler/rust/runtime/src/prelude/iter/adapters/flat_map.rs` で `IterSeed::FlatMap` を採用し、`EffectLabels::mem_reservation` と `IteratorStageProfile::request_stage(Stage::Beta)` を発火させる。`iter/adapters/zip.rs` は `ZipState::remaining_*` で長さ差を検出し、`Diagnostic.extensions["iterator.zip"]` と `reports/iterator-stage-summary.md` へ `iterator.error.zip_shorter` を転写する。
- テスト/証跡: `compiler/rust/frontend/tests/core_iter_adapters.rs::{flat_map_vec,zip_mismatch}` を `insta` で固定し、`tests/snapshots/core_iter_adapters__core_iter_adapters.snap` を `reports/spec-audit/ch0/links.md#iter-adapters` から逆参照。`scripts/validate-diagnostic-json.sh --pattern iterator.flat_map --pattern iterator.zip`、`collect-iterator-audit-metrics.py --section iterator --case flat_map|zip --output reports/iterator-{flatmap,zip}-metrics.json` を同じジョブで実行する。
- KPI/ログ: `reports/iterator-flatmap-metrics.json`（mem_reservation, stage.required/actual）、`reports/diagnostic-format-regression.md#iterator.zip_mismatch`（zip エラー）、`reports/spec-audit/ch1/core_iter_adapters.json`（adapter coverage）を `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の `iterator.adapter.coverage` に反映する。Nightly 成功ログは `docs-migrations.log` に Run-ID とともに記録し、逸脱は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` に再掲する。
- 2026-02-21 実測: `python3 tooling/ci/collect-iterator-audit-metrics.py --section iterator --case flat_map --case zip --output reports/iterator-flatmap-metrics.json --secondary-output reports/iterator-zip-metrics.json --require-success` の結果を確認。`flat_map_vec` ケースでは `EffectLabels.mem = true`・`EffectLabels.mem_reservation_bytes = 3`・`IteratorStageProfile.required = Stage::Beta (Exact)` を算出し、`reports/iterator-flatmap-metrics.json` と `reports/iterator-stage-summary.md#flat-map` に追記。`zip_mismatch` ケースは `EffectLabels.mut = true`・`iterator.error.zip_shorter = 1` を `reports/iterator-zip-metrics.json` と `reports/diagnostic-format-regression.md#iterator.zip_mismatch` に書き出し、`prelude_api_inventory.toml` の `Iter.zip` 行と `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#iterator-adapter-esc` にも同じリンクを掲載した。

### <a id="iter-g3-buffered-backpressure"></a>Iter G3 buffered/backpressure（WBS 3.1c-G3）
- 実行列:
...
- KPI: `reports/iterator-buffered-metrics.json` に `adapter_metrics.buffered_window.effects.mem_bytes = 2`、`backpressure = {produced: 6, consumed: 4, dropped: 2, ratio: 0.33}`、`iterator.mem.window.bytes = 2`、`iterator.mem.window.backpressure = 0.33` を保存。Criterion ベンチは `reports/benchmarks/iter_buffered-2027-02-22.json` で `windows_per_sec = 1.89e6` / `delta_pct = +0.038`（±10% 内）を確認し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の KPI 表と `docs/plans/rust-migration/3-2-benchmark-baseline.md` §3.2.4 に転記。
- 監査ログ: `reports/spec-audit/ch0/links.md#iter-buffered`、`reports/spec-audit/ch1/core_iter_adapters.json`（`run_id = 2027-02-22-iter-adapter-g3`）、`reports/spec-audit/ch1/iterator.buffered.diagnostics.json`、`reports/iterator-stage-summary.md#iter-buffered` を参照。`prelude_api_inventory.toml` の `meta.last_updated` と `Iter.buffered` 行へ KPI リンクを追加済み。
- リスク管理: `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#iterator-adapter-esc` に `iterator.mem.window` / `iter_buffered` ベンチ逸脱時のテンプレートを追加予定。Nightly で `collect-iterator-audit-metrics.py --case buffered` が失敗した場合は Run-ID をログし、`docs-migrations.log` 経由で Phase 3 M1 に共有する。

### <a id="iter-adapter"></a>Iter Adapter KPI / 文書同期（WBS 3.1c-G4）
- 目的: Adapter API（map/filter/flat_map/zip/buffered/collect_* 等 12 件）の在庫情報を `prelude_api_inventory.toml`、`reports/spec-audit/ch1/core_iter_adapters.json`、`reports/iterator-adapter-metrics.json`、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` で同期し、`iterator.adapter.coverage` / `diagnostic.audit_presence_rate` / `iterator.stage.audit_pass_rate` をセットで監査できるようにする。
- コマンド列:
  1. `cargo xtask prelude-audit --section iter --filter adapter --strict --output reports/spec-audit/ch1/core_iter_adapters.json`
  2. `python3 tooling/ci/collect-iterator-audit-metrics.py --section iterator --case adapters --source reports/spec-audit/ch1/core_iter_adapters.json --output reports/iterator-adapter-metrics.json --require-success`
  3. `scripts/validate-diagnostic-json.sh --pattern iterator.map --pattern iterator.zip reports/spec-audit/ch1/core_iter_adapters.json`
- KPI: `reports/iterator-adapter-metrics.json` に `iterator.adapter.coverage = 1.0`、`diagnostic.audit_presence_rate = 1.0`、`iterator.stage.audit_pass_rate = 1.0` を保存し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の KPI 表へ転記。`prelude_api_inventory.toml` の `meta.last_updated` を `2027-02-24 / WBS 3.1c-G4` へ更新した。
- 監査ログ: `reports/spec-audit/ch0/links.md#iter-adapters-g4` に CLI/スナップショット/Run-ID を集約し、`docs/plans/bootstrap-roadmap/3-0-phase3-self-host.md#iter-adapter`・`docs/plans/bootstrap-roadmap/3-1-core-prelude-iteration-plan.md` §4.a・`docs/plans/bootstrap-roadmap/3-1-iter-collector-remediation.md`（Step1 追記）と連携させた。
