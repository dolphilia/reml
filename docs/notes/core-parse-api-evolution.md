...
- TODO: `CP-WS3-001` 紐付けのため、失敗ケースを追加し `label("identifier"|"number"|"string")` が ExpectedTokens に含まれることを human/JSON で確認する。
...
##### TODO: 位置キャッシュ設計（着手準備）
- `Input` に行頭テーブル（byte → line start）と `g_index/cp_index` の共有キャッシュを持たせ、`advance` では byte オフセットだけ更新し、列/行は要求時にキャッシュへ反映する遅延計算を検討する。
- `span_highlight` は `Input` が提供する行頭テーブルとグラフェム境界参照を再利用し、`char_indices` の先頭走査を排除する。共有キャッシュの無効化コストが大きい場合は、診断整形専用の読み取り API を追加し、ビュー間で参照を共有する。
- キャッシュ更新の責務: `advance` が新しい byte 範囲を進めるときに「最後に観測した行・列」を更新し、`position()` 呼び出し時にのみグラフェム計算を行う。`mark/rewind` ではキャッシュのスナップショット（インデックスへの参照）を O(1) で共有する。
