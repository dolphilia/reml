...
### 進捗ログ（2027-03-29）
- `compiler/rust/frontend/tests/lexer_unicode_identifier.rs` を 12 ケースに拡張し、`UnicodeErrorKind::{InvalidIdentifier,UnsupportedLocale,InvalidUtf8}` が `Span`・`AuditEnvelope.metadata["unicode.error.*"]` と揃って出力されることを確認。結果は `reports/spec-audit/ch1/lexer_unicode_identifier-20270329.json` に保存した。
