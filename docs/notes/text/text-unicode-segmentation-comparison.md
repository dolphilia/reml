...
- UAX #29 rev.40 の GraphemeBreakTest データを `tests/data/unicode/UAX29/GraphemeBreakTest-15.1.0.txt` として同梱し、`cargo test --manifest-path compiler/rust/runtime/Cargo.toml unicode_conformance --features unicode_full` でセグメンテーション互換性を確認。データ投入は `docs/notes/text/unicode-upgrade-log.md` に記録済み。
