...
| 2025-11-25 | emoji, ja-JP | `width_map("👨‍👩‍👧‍👦")` | `unicode-width` では幅 2 固定。CLI では 4+ に見えるケースあり。 | `WidthMode::EmojiCompat` で補正テーブルを適用し、絵文字シーケンスは `grapheme_stats` の計測幅と一致させる | Closed | `compiler/rust/runtime/src/text/data/width_corrections.csv` と `tests/unicode_width_mapping.rs` で監視 |
