...
| --- | --- | --- | --- | --- | --- |
| TUI-001 | tr-TR ロケールでのケース変換が `UnsupportedLocale` になる | Rust runtime ≤20270328 | Resolved | `compiler/rust/runtime/src/text/locale.rs` で `LocaleId::parse("tr-TR")` をサポートし、`docs/notes/text/text-case-width-gap.md` `Closed`。CLI/LSP は `LocaleScope::Case` を指定して再実行する。 | text-case-width-gap.md, reports/spec-audit/ch1/unicode_case_width.rs |
