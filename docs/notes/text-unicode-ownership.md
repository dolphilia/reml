...
| --- | --- | --- | --- | --- |
| `Vec<u8> -> Bytes::from_vec` | `compiler/rust/runtime/src/text/bytes.rs` L12-L27 | ✅ | `false`。`collector.effect.transfer=true` のみ記録 | `Vec` をムーブ。UTF-8 検証は呼ばない。 |
...
- `log_grapheme_stats` の `cache_hits`/`cache_miss` は `GraphemeSeq::stats` の結果をそのまま返すので、Phase 3 では `collector.effect.text_cache_hits` を `AuditEnvelope.metadata["text.grapheme_stats.cache_hits"]` に、`EffectSet` の新ビット `unicode_cache` に写す。  
...
## KPI・監査ログ
- `text.mem.zero_copy_ratio`: `reports/spec-audit/ch1/core_text_mem.json` にゼロコピーケースと `mem_bytes` を書き出し、`collect-iterator-audit-metrics.py --section text --scenario ownership_transfer` で算出。閾値 0.65 以上を Phase3-2 の合格ラインとする。  
- `text.mem.copy_penalty_bytes`: 同 JSON から `copy_cases[].mem_bytes` を 1KB 入力あたりに正規化。1,024B/KB を超えた場合は `0-4-risk-handling.md#core-text-copy-penalty` に記録。  
