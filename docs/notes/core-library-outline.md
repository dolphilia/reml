...
### Iter Generators F1-4 設計メモ（WBS 3.1c-F1-4）

- `../plans/bootstrap-roadmap/3-1-core-prelude-iteration-plan.md#33b-生成-api-実装ステップwbs-31c-f1` に `Iter::unfold`/`Iter::try_unfold` の設計ログを追加し、`docs/spec/3-1-core-prelude-iteration.md:176-221` の `unfold`/`try_unfold` 仕様を `compiler/rust/runtime/src/prelude/iter/generators.rs` へ転写するための作業順（`IterSeed` に状態クロージャを格納、`EffectSet::PURE` vs `EffectSet::with_debug` 切り替え、`IterStepMetadata::flag_error` の追加）を明文化した。
- 実装後に `compiler/rust/frontend/tests/core_iter_generators.rs` へ `unfold_fibonacci_pipeline`・`try_unfold_error_passthrough` を追加し、`cargo test core_iter_generators -- --nocapture` と `cargo insta review --review core_iter_generators` で `iterator.error.kind = "try_unfold"` を Snapshot 固定する。並行して `scripts/validate-diagnostic-json.sh --pattern iterator` と `collect-iterator-audit --section iter --case unfold|try_unfold --output ../../reports/spec-audit/ch1/iter.json#audit_cases.try_unfold` を実行し、`EffectLabels::debug=true` と `AuditEnvelope.metadata.iterator.error.kind` が計測ラインに露出することを KPI 化する。
- `docs/plans/bootstrap-roadmap/assets/prelude_api_inventory.toml` に `Iter::unfold`/`Iter::try_unfold` エントリ（`rust_status=pending`, `wbs = "3.1c F1-4"`）を追加し、`../../reports/spec-audit/ch0/links.md#iter-f1-4` でコマンド履歴・JSON 出力・`cargo xtask prelude-audit --section iter --wbs '3.1c F1-4'` の証跡をまとめる。完了後は `prelude_api_inventory` の `last_updated` を更新し、`../plans/bootstrap-roadmap/3-0-phase3-self-host.md#iter-generators` へ脚注を連携して Phase 3 M1 判定資料と同期させる。
