...
- `collect_cell_ref_effects` セットでは `python3 tooling/ci/collect-iterator-audit-metrics.py --suite collectors --scenario ref_internal_mutation --require-success --require-cell` に加え、`scripts/validate-diagnostic-json.sh --suite collectors --pattern collector.effect.cell --pattern collector.effect.rc` を定期的に走らせ、`collector.effect.cell` / `collector.effect.rc` が `reports/spec-audit/ch1/core_iter_collectors.json` へ届いていることを確認している。結果の KPI は `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` および `docs/plans/bootstrap-roadmap/assets/metrics/core_collections_persistent.csv`（`RefInternalMutation` 行）に転記され、`docs/plans/bootstrap-roadmap/3-2-core-collections-plan.md` §3.2.4 や `docs/plans/rust-migration/p2-spec-compliance-gap.md` の監査ルートにもリンクしている。
