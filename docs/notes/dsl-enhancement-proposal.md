...
**機能:**
*   **Lite Template:** `reml new --template lite` で、`AuditPolicy::None`、`SecurityPolicy::Permissive` が設定済みのプロジェクトを作成。
...
**機能:**
*   **Embedded Parser Interface:** 異なる DSL パーサーを「サブパーサー」として呼び出すための共通インターフェース。
*   **Context Passing:** 親 DSL から子 DSL へ変数スコープや設定を引き継ぐ仕組み。
*   **LSP Delegation:** 埋め込み部分のカーソル位置では、子 DSL の LSP サーバーへリクエストを委譲する仕組み。
