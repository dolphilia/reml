...
- **作業メモ**:
  - `scripts/ci-verify-llvm-link.sh --report tmp/ci-verify-llvm-link.md` をローカル macOS で実行し、レポートを `docs/notes/backend/macos-ci-llvm-link-error-report.md`（新規予定）へ転記する。
