...
| [4-2-registry-distribution.md](../spec/4-2-registry-distribution.md) | レジストリとLLVM tripleの扱い。ターゲットメタデータ配布の設計 | ドラフト |
...
> 2025-11-09 追記: System V / Windows / Darwin の呼出規約・レイアウト差分を整理し、`docs/plans/bootstrap-roadmap/2-6-windows-support.md` ステップ2の成果物に接続。

##### 2.2.2a 呼出規約・レイアウト差分（System V / Windows / Darwin）

| 観点 | System V AMD64 | Windows x64 (MSVC) | Darwin AAPCS64 |
| --- | --- | --- | --- |
| 整数・ポインタ引数の割当 | 第1〜6引数を RDI → RSI → RDX → RCX → R8 → R9 に割り当て、7引数目以降は 8B 単位でスタックに積む[^sysv-abi]。 | 第1〜4引数を RCX → RDX → R8 → R9 に割り当て、5引数目以降は Shadow Space 後方のスタックに積む[^ms64-abi]。 | 第1〜8引数を x0 → x7 に割り当て、それ以降は 16B アライン済みスタックに積む[^aapcs64]。 |
| 浮動小数点引数の割当 | XMM0-7 を SSE クラスへ自動割当（拡張 128bit）。8 本を超える場合はスタックへ spill[^sysv-abi]。 | XMM0-3 を SIMD 引数に利用し、4 本を超える場合は Shadow Space からロードする[^ms64-abi]。 | v0-7 を SIMD/HFA に割り当て、8 本を超える分は 16B 境界のスタックへ配置する[^aapcs64]。 |
| Shadow / Red Zone | 呼出側は 16B アラインを維持し、128B の Red Zone を確保。Shadow Space 要求はなし[^sysv-redzone]。 | 呼出側は常時 32B の Shadow Space（Home Space）を確保し、呼出先が Spill や `va_list` 取得に利用する[^ms64-abi]。 | Shadow Space の概念はなく、呼出側は 16B アラインを維持する[^aapcs64]。 |
| Callee-saved レジスタ | RBX・RBP・R12〜R15 が保存対象。XMM0-15 は caller saved[^sysv-abi]。 | RBX・RBP・RDI・RSI・R12〜R15 に加え XMM6-15 を callee が保存[^ms64-abi]。 | x19〜x28 と v8〜v15 を callee が保存し、x29（FP）・x30（LR）はプロローグで処理[^aapcs64]。 |
| 構造体引数 | 8B 単位のクラス分類（INTEGER/SSE など）で 16B までをレジスタ渡し。複合クラスや 16B 超は `byval align 8` ポインタで渡す[^sysv-abi]。 | 8B を超える集約は呼出側がバッファを確保しポインタ渡し。LLVM では `byval(%T) align 8` を付与[^ms64-abi]。 | HFA/HVA（同種浮動/ベクトル≦4要素）はレジスタ渡し。それ以外は `byval align 16` ポインタ渡し[^darwin-cc]。 |
| 可変長引数 (`varargs`) | 決め打ちの Register Save Area（整数最大6本・浮動小数最大8本）をスタックに確保し、`__va_list_tag` が `reg_save_area` / `gp_offset` / `fp_offset` を参照する。浮動小数引数は 128 ビット幅に拡張される[^sysv-abi]。 | 4 本の整数レジスタ (RCX/RDX/R8/R9) に加え 32B の Shadow Space を常時確保。浮動小数は XMM0-3 へ配置されるが、`va_list` からは Shadow Space を経由して取得する[^ms64-abi]。 | 8 本の整数レジスタ (x0-x7) と 8 本の SIMD レジスタ (v0-v7) を呼び出し側が 64B/128B の Register Save Area に退避し、`va_list` は `__stack` / `__gr_top` / `__vr_top` と `__gr_offs` / `__vr_offs` でアクセスする。浮動小数は 64 ビット単位で右詰めされ、半精度は 32 ビットへ昇格する[^darwin-cc][^aapcs64]。 |
| 構造体戻り値 | サイズが 16B 以下（または HFA/HVA）の場合は RAX/RDX もしくは XMM で直接返却。それ以上は呼出側が用意したバッファポインタを RDI に渡し `sret` 属性を付与する[^sysv-abi]。 | サイズが 8B 以下なら RAX 直接返却。それ以上（および大半の HFA/HVA）は RCX で渡したポインタへ書き込み、`sret` 属性で表現する[^ms64-abi]。 | サイズが 16B 以下、または HFA/HVA（要素数≦4）の場合は x0-x3 あるいは v0-v3 で直接返却。それ以外は呼出側が 16B アライン済みバッファのアドレスを x8 に渡し、LLVM では `sret align 16` として表現する[^darwin-sret]。 |
| 名前マングリング | C 関数は装飾なし（ELF シンボルは先頭 `_` を付与しない）。C++ は Itanium C++ ABI (`_Z...`) を採用。`extern "C"` で抑制[^itanium-abi]。 | C 関数は x64 では装飾なし。C++ は Microsoft Decorated Name (`?Fn@@...`) を採用し、`extern "C"` や `#[link_name]` で制御[^ms-name]。 | Mach-O は先頭 `_` を付与。C++ は Itanium 形式を採用[^mach-mangling]。 |
| 共有ライブラリ公開 | `__attribute__((visibility("default")))` と `-fPIC` が既定。`dlopen`/`dlsym` を利用する[^sysv-abi]。 | `__declspec(dllexport/dllimport)` や `.def` による明示的エクスポートが必須。`LoadLibraryW`/`GetProcAddress` で動的解決[^ms-dll]。 | `__attribute__((visibility("default")))` を推奨し、`dyld` API で動的ロード[^darwin-cc]。 |

**Reml 実装メモ**

- Darwin: `Abi.classify_struct_return` の結果に応じて `sret` 属性と x8 経由の隠し戻り値ポインタをテストで検証（`tests/test_ffi_lowering.ml` の Darwin ケース拡張）。
- Darwin: 可変長引数サポート時は Register Save Area のレイアウトを `VaList` ヘルパで抽象化し、`reports/ffi-macos-summary.md` の計測値と同期して監査ログを更新。
...
| **Phase C**（エコシステム） | 4-1-package-manager-cli.md, 4-2-registry-distribution.md, 4-3-developer-toolchain.md | CLIサブコマンド、レジストリメタデータ |
...
- [4-2-registry-distribution.md](../spec/4-2-registry-distribution.md) - レジストリ
...
[^sysv-abi]: The Santa Cruz Operation et al., *System V Application Binary Interface — AMD64 Architecture Processor Supplement*, Version 1.0, 2018-01-23（最終閲覧 2025-11-09）。
[^sysv-redzone]: *System V Application Binary Interface — AMD64 Architecture Processor Supplement* §3.2.2（Red Zone）（最終閲覧 2025-11-09）。
[^ms64-abi]: Microsoft, *x64 Software Conventions*, Visual Studio 2022 ドキュメント（最終閲覧 2025-11-09）。
[^ms-name]: Microsoft, *Decorated Names*, Visual C++ ドキュメント（最終閲覧 2025-11-09）。
[^ms-dll]: Microsoft, *Exporting from a DLL Using dllexport and dllimport*, Visual Studio 2022 ドキュメント（最終閲覧 2025-11-09）。
[^itanium-abi]: Itanium C++ ABI, §5 Name Mangling, 2023-06-14（最終閲覧 2025-11-09）。
[^mach-mangling]: Apple, *Mach-O Runtime Programming Guide — Symbol Names*, 2022-09-30（最終閲覧 2025-11-09）。
