...
- CLI でストリーミング経路を検証する場合は `cargo run --manifest-path compiler/rust/frontend/Cargo.toml --bin reml_frontend -- --emit-diagnostics docs/spec/1-1-syntax/examples/<sample>.reml --stream chunk=<bytes> --emit-typeck-debug reports/spec-audit/ch1/<sample>-YYYYMMDD-typeck.json` を用い、生成された `streaming_<sample>-YYYYMMDD-diagnostics.json` と `typeck` JSON を `reports/spec-audit/ch1/2025-11-17-syntax-samples.md#2025-11-21-streaming-監査` に追記する。
...
- `trace-coverage-YYYYMMDD.md` では `Trace coverage >= 4`（handle / perform / resume / block の 4 系統）を満たしているかを確認し、`docs/notes/spec-integrity-audit-checklist.md#rust-gap-トラッキング表` の `SYNTAX-003` 行へ証跡リンクを追加する。
