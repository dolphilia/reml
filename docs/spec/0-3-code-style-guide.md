...
- Rust Frontend がまだ受理できない構文は `*_rustcap.reml` のフォールバックを併記し、`docs/notes/spec-integrity-audit-checklist.md#rust-gap-トラッキング表` に `rust-gap` として登録する。`use_nested.reml` については 2025-11-17 時点で `module`/`use`/ブロック/`match` まで解析できるようになり、`TraceEvent::{ModuleHeaderAccepted,UseDeclAccepted}` を `reports/spec-audit/ch1/use_nested-YYYYMMDD-trace.md` に保存している。フォールバック (`use_nested_rustcap.reml`) は参考資料として残しているが、監査では正準サンプルの結果を優先する。
