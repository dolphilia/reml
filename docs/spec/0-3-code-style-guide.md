...
- Chapter 1 のコード片は `docs/spec/1-1-syntax/examples/*.reml` へ配置し、`cargo run --manifest-path compiler/rust/frontend/Cargo.toml --bin poc_frontend -- --emit-diagnostics <sample>` で検証する。Streaming 経路の検証では `cargo test --manifest-path compiler/rust/frontend/Cargo.toml streaming_metrics -- --nocapture` を併用し、コマンドと結果を `reports/spec-audit/ch1/streaming_metrics-YYYYMMDD-log.md`／`reports/spec-audit/summary.md` に記録すること。
