...
[^type-row-metadata-phase25]: 2026-04-22 更新。Phase 2-5 `TYPE-002 Step3` で効果行統合の移行ガードを設計し、`RunConfig.extensions["effects"].type_row_mode = "metadata-only"` を既定値として固定した。`type_row_mode = "ty-integrated"` を要求した場合は診断 `effects.type_row.integration_blocked` を発行し、監査ログへ `effect.type_row.*` キーを出力して試行履歴を追跡する。解除条件は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#type-002-effect-row-integration` で管理し、`TArrow of ty * effect_row * ty` 実装と KPI (`diagnostics.effect_row_stage_consistency`) の導入が揃ったタイミングで脚注撤去を検討する。
...
| `effects.type_row.integration_blocked` | Error | `RunConfig.extensions["effects"].type_row_mode = "ty-integrated"` が要求されたが、Phase 2-5 時点では型表現に効果行を統合していないためガードが発火した。 | `AuditEnvelope.metadata` に `effect.type_row.requested_mode`, `effect.type_row.available_mode`, `effect.type_row.guard_stage` を格納し、`Diagnostic.extensions["effects"]["type_row"]` に同一情報を JSON で写像する。 | `type_row_mode` を `"metadata-only"` に戻し、Phase 2-7 `TYPE-002` が定める解除条件（`TArrow` 拡張・KPI 導入・レビュー完了）を満たすまでガードを維持する[^type-row-metadata-phase25]。 |
...
- `effect.type_row.guard_stage`: `TYPE-002` 移行ガードの段階（例: `"phase2-5-metadata-only"`）。Phase 2-7 で解除された場合は `"phase2-7-ty-integrated"` へ更新し、脚注撤去条件と同期させる。
