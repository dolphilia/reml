...
- **FFI/Core Prelude 回帰カテゴリー**: Phase4 シナリオマトリクスで `FFI-CORE-PRELUDE-001` などのテスト群を追跡する分類。`docs/spec/3-1-core-prelude-iteration.md` で定義される Core Prelude API と `docs/spec/3-6-core-diagnostics-audit.md` の Stage/Capability 契約を `reml_runtime_ffi` 経由で再現できるかを `compiler/rust/frontend/tests/core_iter_*.rs` の snapshot（`cargo test --manifest-path compiler/rust/frontend/Cargo.toml core_iter_effects` など）で検証する。capability shim の維持期間中は Phase4 KPI（spec_core/practical）と連動し、Stage mismatch が復活した場合は `phase4-scenario-matrix.csv` の `resolution` を `impl_fix` へ戻す。 
