...
- **タスク**:
  1.  [x] AST と型表現に参照型を追加し、`&`/`&mut` の構文を解析。
  2.  [x] 型チェックで不変/可変参照の整合性と再代入の制約を検証。
  3.  [x] `mut` 効果との整合（可変参照の導入時に効果タグを付与）。
  4.  [x] Codegen: 参照をポインタとして表現し、読み書きの命令列を定義。
  5.  [x] テスト: `&T` の読み出し、`&mut T` の更新、参照の別名衝突診断。

### 5.6.1 進捗メモ（2026-01-03）
- `&`/`&mut` を lexer/parser/AST に追加し、参照式ノードを導入。
- 参照型 (`REML_TYPE_REF`) を型表現・単一化・変数収集へ反映。
- `&mut` と `var`/`:=` の制約、借用別名衝突の診断を追加。
- codegen で参照をポインタとして扱い、`*`/`:=` の命令列を実装。
- sema/codegen のユニットテストに参照型ケースを追加。
...
### 5.8.1 進捗メモ（2026-01-03）
- 効果集合の型表現を `typeck` に統合し、関数型へ効果行を保持できるようにした。
- `@pure` / `@no_panic` の構文と AST への取り込みを追加し、Sema の契約検査で `REML_DIAG_EFFECT_VIOLATION` を報告するようにした。
- 効果注釈に関する parser/sema の単体テストを追加。
- 効果集合の合成/制約伝搬は最小実装（式内の `mut` 合成）に留まっており、行多相の伝搬は未着手。
- 効果行の行変数・単一化・トップレベル一般化を追加し、`effects.contract.mismatch` 相当の診断と unit テストを追加。

### 5.8.2 残タスク（行多相）
1.  **効果行変数の導入**: `reml_type` の関数型に行変数（`!ε` 相当）を保持する表現を追加し、既存の `effects` ビット集合と併用可能にする。✅ 完了
2.  **行制約の収集**: `perform`/`handle`/呼び出し境界で `Σ_after = (Σ_before - Σ_handler) ∪ Σ_residual` の制約を収集し、制約一覧に蓄積する。
3.  **行の単一化**: 効果行の単一化（集合の包含/一致）を実装し、`reml_type_unify` で `REML_TYPE_FUNCTION` の効果行を解決する。✅ 完了
4.  **一般化と値制限**: トップレベル `let` のみ `!ε` を一般化し、効果がある式は単相化する（値制限と連動）。✅ 完了
5.  **診断拡張**: `effects.contract.mismatch` 相当の診断を追加し、期待/実際の効果集合・行変数を出力する。✅ 完了
6.  **テスト追加**: `@pure`/`@no_panic` 付き関数・ハンドラ境界・行変数一般化のケースを unit テストで網羅する。⚠️ 一部完了（効果行単一化の unit テストを追加、ハンドラ境界は未対応）
...
- 数値リテラルを型変数として扱い、数値制約の伝搬と既定型（Int）への解決を追加。
- ADT/レコード/参照型に対する occurs check と型変数収集を拡張。
- 単項マイナスと range パターンで数値型制約を明示化。
