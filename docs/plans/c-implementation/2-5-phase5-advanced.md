...
### 5.8.2 残タスク（行多相）
1.  **効果行変数の導入**: `reml_type` の関数型に行変数（`!ε` 相当）を保持する表現を追加し、既存の `effects` ビット集合と併用可能にする。
2.  **行制約の収集**: `perform`/`handle`/呼び出し境界で `Σ_after = (Σ_before - Σ_handler) ∪ Σ_residual` の制約を収集し、制約一覧に蓄積する。
3.  **行の単一化**: 効果行の単一化（集合の包含/一致）を実装し、`reml_type_unify` で `REML_TYPE_FUNCTION` の効果行を解決する。
4.  **一般化と値制限**: トップレベル `let` のみ `!ε` を一般化し、効果がある式は単相化する（値制限と連動）。
5.  **診断拡張**: `effects.contract.mismatch` 相当の診断を追加し、期待/実際の効果集合・行変数を出力する。
6.  **テスト追加**: `@pure`/`@no_panic` 付き関数・ハンドラ境界・行変数一般化のケースを unit テストで網羅する。
