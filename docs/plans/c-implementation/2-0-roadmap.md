...
- 詳細: `docs/plans/c-implementation/2-5-phase5-advanced.md` を参照。
- [x] `BigInt` サポートの実装（ライブラリ統合）。
- [x] ADT とレコード型の導入。
- [x] 参照型 (`&T`, `&mut T`) の導入。
- [x] `String` / Unicode 処理の実装。
- [x] パターンマッチングのコンパイル実装（Desugaring）。
- [x] トレイト/型クラスの導入（演算子解決の一般化）。
...
- 依存関係: フェーズ 5 の BigInt/Unicode/ADT/参照型/効果行/効果ハンドラが最低限実装されていること。
