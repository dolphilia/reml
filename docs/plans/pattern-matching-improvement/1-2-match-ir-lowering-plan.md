...
- [ ] M3: バックエンド分岐生成 — **着手済み（MIR ローダー側）**。`compiler/rust/backend/llvm/src/integration.rs` で `Match/Pattern` を含む式木を復元し、`CodegenContext` がブランチサマリ（`branch_plans`）を生成するところまで実装。LLVM ジャンプ生成と実際の分岐コード吐き出しは未実装。  
