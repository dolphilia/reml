...
- 最新同期: Active Pattern の記法・BNF・診断キーに加え、`match` 評価順（Partial Active の `None` フォールスルー含む）を `docs/spec/1-5-formal-grammar-bnf.md` と `docs/guides/compiler/core-parse-streaming.md` 付録Aへ同期。Phase4 の `CH1-ACT-001..003` / `CH1-MATCH-007..018` はマトリクス（run_id 記録含む）と CLI 確認まで完了。クロス実装チェック（`1-2` の M5）は OCaml 実装の更新停止により無効（凍結）。
