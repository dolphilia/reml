...
- Runtime: `compiler/rust/runtime/`
- Frontend MIR 連携: `compiler/rust/frontend/src/semantics/mir.rs` と JSON 出力（Backend の `integration.rs` が読むスキーマ）
...
- 作業ステップ:
  - `compiler/rust/backend/llvm/src/type_mapping.rs` に `Slice` / `Ref { mutable }` を追加する。
  - `layout_of` に `[T]` の `{ptr,len}` を反映する（既存の `String` と同様のサイズ・アラインメント方針を明記）。

3) `parse_reml_type` の拡張
- 目的: Backend 側で `[T]` / `&` / `&mut` を正しく解析する。
- 作業ステップ:
  - `compiler/rust/backend/llvm/src/integration.rs` の `parse_reml_type` に簡易パーサを追加する。
...
- 作業ステップ:
  - `compiler/rust/frontend` 側で `--emit-mir-json` を使ったサンプル出力を取得する。
  - Backend の `generate_snapshot_from_mir_json` で `Slice` / `Ref` / `variadic` が保持されることを確認する。

## 詳細タスクリスト（ファイル単位 TODO）

### Frontend
- `compiler/rust/frontend/src/semantics/typed.rs`: `TypedFunction` に `varargs: bool` を追加し、AST の `FunctionSignature.varargs` を伝搬する TODO。
- `compiler/rust/frontend/src/semantics/mir.rs`: `MirFunction` に `varargs: bool` を追加し、Typed から引き継ぐ TODO。
- `compiler/rust/frontend/src/semantics/mir.rs`: MIR JSON の `schema_version` を更新するか検討する TODO（互換性維持が難しい場合のみ）。

### Backend
- `compiler/rust/backend/llvm/src/type_mapping.rs`: `RemlType::Slice` / `RemlType::Ref { mutable }` を追加する TODO。
- `compiler/rust/backend/llvm/src/type_mapping.rs`: `[T]` のレイアウト（`{ptr,len}`）と `&T` / `&mut T` の ABI 方針を記述する TODO。
- `compiler/rust/backend/llvm/src/integration.rs`: `parse_reml_type` に `[T]` / `&` / `&mut` の簡易パーサを実装する TODO。
- `compiler/rust/backend/llvm/src/ffi_lowering.rs`: `FfiCallSignature` に `variadic: bool` を追加する TODO。
- `compiler/rust/backend/llvm/src/integration.rs`: `FfiCallJson` に `variadic` を追加し、MIR JSON から取り込む TODO。
- `compiler/rust/backend/llvm/src/codegen.rs`: variadic を使う場合の stub 生成方針（未実装なら TODO コメント）を追加する TODO。

### Runtime
- `compiler/rust/runtime/src/ffi/dsl/mod.rs`: `FfiFnSig.variadic` が JSON/DSL 経由で設定される入口を洗い出す TODO。
- `compiler/rust/runtime/src/ffi/dsl/mod.rs`: variadic 呼び出し時の引数数検証・診断方針を仕様と突き合わせる TODO。
- `compiler/rust/runtime/src/ffi/dsl/mod.rs`: `[T]` / `&T` の ABI 表現が必要になった場合の候補構造体をコメントで整理する TODO。
