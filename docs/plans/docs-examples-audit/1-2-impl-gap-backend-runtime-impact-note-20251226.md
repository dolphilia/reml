...
- **Runtime**: キーワード境界判定が `XID_Start/XID_Continue` のみを考慮しており、emoji/ZWJ を識別子継続文字として扱っていない。
  - `compiler/rust/runtime/src/parse/combinator.rs` の `is_ident_continue` / `keyword` 内チェック
- **Backend**: LLVM IR 名にユーザー識別子が直接流入し、非 ASCII を安全に整形する処理がない。
  - `compiler/rust/backend/llvm/src/codegen.rs` の `LlvmBuilder::new_tmp` / `intrinsic_*` / `@{name}` 組み立て
