...
## 対象範囲
- Frontend MIR: `compiler/rust/frontend/src/semantics/mir.rs`
- Backend MIR 取り込み: `compiler/rust/backend/llvm/src/integration.rs`
- Backend コード生成: `compiler/rust/backend/llvm/src/codegen.rs`
- Runtime: `compiler/rust/runtime/`（影響確認のみ）
...
- 作業ステップ:
  - `compiler/rust/frontend/src/semantics/mir.rs` の `MirExprKind` 定義を確認し、各バリアントの JSON 形式（`kind` 名・フィールド）を整理する。
  - `compiler/rust/backend/llvm/src/integration.rs` の `MirExprKindJson` に `EffectBlock` / `Async` / `Await` / `Unsafe` を追加する。
...
- 作業ステップ:
  - `compiler/rust/backend/llvm/src/codegen.rs` の `emit_expr` / `infer_expr_llvm_type` / `describe_expr` で `MirExprKind` の分岐を確認する。
...
- 作業ステップ:
  - `compiler/rust/runtime/src/runtime/async_bridge.rs` と `compiler/rust/runtime/src/prelude/async.rs` を確認し、現在提供している型/API を一覧化する。
  - `await` に相当する橋渡しが runtime/ffi 層に存在するかを確認する（`runtime/ffi/src/registry.rs` の `BridgeIntent::Await` など）。
...
- 作業ステップ:
  - 既存の Backend テスト（`compiler/rust/backend/llvm/src/integration.rs` 内の `tests`）を確認し、MIR JSON を渡す最小ケースを追加する。
  ...
  - `cargo test --manifest-path compiler/rust/backend/llvm/Cargo.toml` を実行し、追加テストが通ることを確認する。
...
- `compiler/rust/backend/llvm/src/integration.rs`
