...
`docs/spec/4-1-system-plugin.md` のサンプル修正で判明した **仕様と実装のギャップ** を整理し、Rust Frontend 側で追随するための対応計画を定義する。
...
## 対象範囲
- 仕様章: `docs/spec/4-1-system-plugin.md`
- サンプル:
  - `examples/docs-examples/spec/4-1-system-plugin/sec_1.reml`
  - `examples/docs-examples/spec/4-1-system-plugin/sec_2.reml`
- 監査ログ: `reports/spec-audit/ch4/docs-examples-audit-YYYYMMDD.md`

## ギャップ一覧（簡略化／回避済み）

### 1. 固定長配列型 `[T; N]` の未対応
- 影響: `[i64; 6]` が `parser.syntax.expected_tokens` で失敗する。
- 該当サンプル: `examples/docs-examples/spec/4-1-system-plugin/sec_1.reml`
- 現状の回避: `SyscallArgs = [i64]` に置き換え、最大 6 引数はコメントで表現。

### 2. 関数型内の引数名の未対応
- 影響: `fn(fd: i32, buf: Ptr<u8>, len: usize)` が構文エラーになる。
- 該当サンプル: `examples/docs-examples/spec/4-1-system-plugin/sec_2.reml`
...
- `compiler/rust/frontend` 内の `Type` 構文定義とパーサ実装を確認し、配列型の分岐点を特定する。
...
### フェーズ 3: サンプル復元と再検証
- `examples/docs-examples/spec/4-1-system-plugin/sec_1.reml` を `[i64; 6]` へ復元し、コメントで回避していた説明を削除する。
- `examples/docs-examples/spec/4-1-system-plugin/sec_2.reml` を引数名付き関数型と `pub struct` 定義へ戻す。
- `docs/spec/4-1-system-plugin.md` の該当コードブロックも同じ正準例へ同期させる。
- `compiler/rust/frontend/target/debug/reml_frontend --output json` で診断 0 件を確認し、差分を監査ログへ追記する。
- 監査ログ `reports/spec-audit/ch4/docs-examples-audit-YYYYMMDD.md` に復元結果と診断 0 件を記録する。
...
- フェーズ 3: `examples/docs-examples/spec/4-1-system-plugin/` の正準例復元は完了。

## 進捗メモ（2025-12-31 フェーズ 3）
- `examples/docs-examples/spec/4-1-system-plugin/sec_1.reml` と `sec_2.reml` を正準例へ復元。
- `docs/spec/4-1-system-plugin.md` のコードブロックを正準例へ同期。
- `reml_frontend --emit-diagnostics` で diagnostics 0 件を確認し、`reports/spec-audit/ch4/docs-examples-audit-20251231.md` に記録。
