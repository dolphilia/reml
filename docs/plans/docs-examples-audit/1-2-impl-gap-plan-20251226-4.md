...
## 対象範囲
- Rust Frontend: `compiler/rust/frontend/`
- Rust Backend (LLVM): `compiler/rust/backend/llvm/`
- Rust Runtime: `compiler/rust/runtime/` / `runtime/`

## 調査結果（要約）
1) `Type.method` の宣言
- Frontend では `ImplDecl` に落とすが、`TypedModule.functions` へは追加されない。
- MIR (`compiler/rust/frontend/src/semantics/mir.rs`) は `TypedModule.functions` のみを対象にするため、Backend には到達しない。
...
1) Typed AST の拡張
- `compiler/rust/frontend/src/semantics/typed.rs` に `TypedExprKind::Lambda` / `TypedExprKind::Rec` を追加。
...
2) MIR の拡張
- `compiler/rust/frontend/src/semantics/mir.rs` に対応する `MirExprKind` を追加。
...
3) JSON 連携の更新
- Backend の `compiler/rust/backend/llvm/src/integration.rs` に新しい MIR JSON を追加。
