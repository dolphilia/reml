...
## 対象範囲
- Backend: `compiler/rust/backend/llvm/src/*`
- Runtime: `compiler/rust/runtime/src/prelude/*`
- Frontend 関連: `compiler/rust/frontend/src/semantics/*`, `compiler/rust/frontend/src/typeck/*`
...
- 作業ステップ:
  - `compiler/rust/frontend/src/semantics` の MIR 定義と lowering 経路を整理し、該当構文がどのレイヤで失われるかを記録する。
...
- 作業ステップ:
  - `compiler/rust/runtime/src/prelude` の公開型、feature gate、Stage/Capability 連動箇所を棚卸しする。
...
##### MIR / Lowering の現状
- Parser AST は `TraitDecl` / `ImplDecl` / `TraitItem` / `ImplItem` を保持する（`compiler/rust/frontend/src/parser/ast.rs`）。
- typeck の出力は `TypedModule`（`compiler/rust/frontend/src/semantics/typed.rs`）で、関数/Active Pattern/Conductor と `DictRef` / `SchemeInfo` のみ。trait/impl 宣言や associated type 宣言は保持されない。
- `TypecheckReport` には `constraints` / `used_impls` / `dict_refs` があるが、`MirModule` 生成は `TypedModule` 由来であり、MIR 側には `dict_ref_ids` だけが残る（`compiler/rust/frontend/src/semantics/mir.rs`）。
...
- Backend 側の MIR 取り込み（`compiler/rust/backend/llvm/src/integration.rs`）は `MirModuleSpec` に関数情報のみを要求し、trait/impl 追加情報を受け取る経路がない。
...
- Runtime Prelude は `Iter<T>` と Stage/Capability/Efffect ラベルを実装済み（`compiler/rust/runtime/src/prelude/iter/mod.rs`）。`io_async` / `async_pending` などの効果ラベルは存在するが、非同期型は未定義。
- `Collector` trait は `compiler/rust/runtime/src/prelude/collectors/mod.rs` に存在し、`type Error` / `new` / `with_capacity` / `push` / `reserve` / `finish` / `into_inner` / `iter_error` を持つ。仕様にない `iter_error` を追加している点が差分。
- `AsyncStream` / `Future` の型定義は `compiler/rust/runtime/src/prelude` 配下に存在しない。`compiler/rust/runtime/Cargo.toml` に `core_async` などの feature gate も未設定。
...
- 作業ステップ:
  - `compiler/rust/runtime/src/prelude` に async 系型の宣言位置を決め、feature gate の運用ルールを定義する。
...
- 作業ステップ:
  - `compiler/rust/runtime/src/prelude/collectors` の現行 API を棚卸しし、trait 化の差分を洗い出す。
...
##### Async 系の型定義（暫定）
- **追加位置**: `compiler/rust/runtime/src/prelude/mod.rs` 直下に `async` モジュールを追加し、`pub mod async;` を公開する。
- **feature gate**: `core_async` / `core-async` を `compiler/rust/runtime/Cargo.toml` に追加し、`prelude::async` は `#[cfg(feature = "core_async")]` で公開する。
- **暫定型**:
