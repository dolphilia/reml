...
- テスト追加:
  - `compiler/rust/frontend/tests/parser/phase1_syntax.rs` で `#[deprecated]`、`unsafe fn`、`async move`、`module`/`macro`/`actor spec` を検証。

### 検証状況
- `cargo fmt --manifest-path compiler/rust/frontend/Cargo.toml` は成功。
- `cargo test --manifest-path compiler/rust/frontend/Cargo.toml --lib` は `ld: Assertion failed: (name.size() <= maxLength)` で失敗（Xcode/SDK は認識済み）。
- 追加で試行した回避策（すべて失敗）:
...
- 推奨コマンド: `RUSTFLAGS="-C link-arg=-fuse-ld=lld" cargo test --manifest-path compiler/rust/frontend/Cargo.toml --lib`

### 次フェーズへの引き継ぎ
- フェーズ 2 の型検査/IR 整合は未着手。
