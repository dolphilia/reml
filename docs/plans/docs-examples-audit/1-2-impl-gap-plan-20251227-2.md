...
- 詳細ステップ:
  ...
  - `compiler/rust/frontend/src/parser/ast.rs` の宣言ノードに `Visibility` を持たせ、`render` に可視性を反映する。
  - `compiler/rust/frontend/src/parser/mod.rs` の宣言パーサで可視性を先読みし、`pub` が付いた宣言を `Visibility::Public` として構築する。
...
- 詳細ステップ:
  - `compiler/rust/frontend/src/parser/ast.rs` の関数定義に「本体なし」の状態を表すバリアントを追加する。
  - `compiler/rust/frontend/src/parser/mod.rs` の関数パーサで `=` / `{` が無い場合にシグネチャのみで確定する分岐を追加する。
  - `compiler/rust/frontend/src/typeck/` の関数登録処理で、本体なしの関数を「未実装/外部定義」として扱えるようにする。
...
- 詳細ステップ:
  - `compiler/rust/frontend/src/parser/ast.rs` の `TraitDecl` / `TraitItem` を拡張し、ジェネリクスと associated type を表現できるようにする。
  - `compiler/rust/frontend/src/parser/mod.rs` の trait パーサで、`type` / `fn` の複数メンバをセミコロン区切りで受理する。
...
- 詳細ステップ:
  - `compiler/rust/frontend/src/parser/ast.rs` に「修飾付き関数名」（例: `QualifiedIdent`）の表現を追加する。
  - `compiler/rust/frontend/src/parser/mod.rs` の関数定義で `Type.method` / `Type::method` を読み取る分岐を追加する。
...
- 詳細ステップ:
  - `compiler/rust/frontend/src/typeck/types.rs` の `BuiltinType` に必要な型種別を追加する。
...
- 詳細ステップ:
  ...
  - `compiler/rust/frontend/target/debug/reml_frontend --output json` で対象サンプルを通し、診断 0 件を確認する。
...
- 進捗欄（運用用）:
  ...
  - [ ] フェーズ 3 完了
  - フェーズ 3: 正準例の復元までは完了。診断 0 件の確認は未達（未解決型と構文エラーが残存）。
