...
## 対象範囲
- Frontend MIR 型トークン: `compiler/rust/frontend/src/semantics/mir.rs`
- Backend 型パース / レイアウト: `compiler/rust/backend/llvm/src/integration.rs`, `compiler/rust/backend/llvm/src/type_mapping.rs`
- Backend コード生成（必要時）: `compiler/rust/backend/llvm/src/codegen.rs`
- Runtime: `runtime/native`（影響確認のみ）
...
- 作業ステップ:
  - `compiler/rust/backend/llvm/src/integration.rs` の `parse_reml_type` の既存分岐（`[T]` や `*T` など）を確認し、`[T; N]` がどこで `Pointer` に落ちているかをメモする。
...
- 作業ステップ:
  - `compiler/rust/backend/llvm/src/type_mapping.rs` の `RemlType` 定義を確認し、既存の `Slice`/`Pointer` と並ぶ形で `Array` を追加する。
...
- 作業ステップ:
  - `compiler/rust/backend/llvm/src/codegen.rs` の型変換（MIR 型 → LLVM 型）の経路を追い、`RemlType::Array` で詰まるポイントを特定する。
...
- 作業ステップ:
  - `compiler/rust/backend/llvm/src/integration.rs` の既存テストに `[i64; 6]` を追加し、`RemlType::Array` が返ることを確認する。
  - 異常系のテストとして `[i64; X]` / `[i64; -1]` / `[i64; 18446744073709551616]` を追加し、診断が出ることを確認する。
  - `compiler/rust/backend/llvm/src/type_mapping.rs` に `layout_of` のテストを追加し、`size` と `align` が期待値になることを確認する。
