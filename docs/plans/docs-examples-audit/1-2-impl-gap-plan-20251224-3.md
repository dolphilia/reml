...
- 進捗:
  ...
  - 診断出力の不変性は未確認（`--emit-diagnostics` の差分未確認）。
- 実装メモ:
...
- 検証:
  - `compiler/rust/frontend/target/debug/reml_frontend examples/docs-examples/spec/2-1-parser-type/sec_*.reml` を再実行し、diagnostics 0 件を確認。
...
- 進捗欄（運用用）:
  ...
  - [ ] フェーズ 2 完了（typeck 通過は実装済み、診断出力の不変性は未確認）
  - [ ] フェーズ 3 完了（サンプル復元未着手）
