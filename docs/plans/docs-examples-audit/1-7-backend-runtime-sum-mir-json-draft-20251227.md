...
1. `compiler/rust/backend/llvm/src/integration.rs` に `MirTypeJson`（構造化型）を追加する。
...
4. `compiler/rust/backend/llvm/src/intrinsics.rs` の `format_reml_type` に `adt` の情報を簡易表示する。
