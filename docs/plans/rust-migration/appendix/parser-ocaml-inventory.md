...
- W2 dual-write で収集した Typed AST/制約ログは `reports/dual-write/front-end/poc/2025-11-07-w2-ast-inventory/` に保存しており、整形済み成果物は `reports/dual-write/front-end/w2-ast-alignment/<case>/` へ同期している（`remlc --emit-parse-debug` フラグで Packrat/SpanTrace を含む CLI 出力を取得）。
...
- W2 の棚卸しでは、上記キーを `reports/dual-write/front-end/w2-ast-alignment/metrics/{streaming,parser}.json` へ出力するための JSON スキーマを `parser_driver` と `collect-iterator` の双方で確認済み。Rust 版では同スキーマを `serde` モジュールで公開し、dual-write ハーネスから直接呼び出せるようにする。
- 2025-11-07 の W2 バッチ結果（OCaml/Rust 双方の parse-debug JSON）は `reports/dual-write/front-end/poc/2025-11-07-w2-ast-inventory/` に格納。OCaml CLI でも `--packrat --emit-parse-debug <path>` を指定することで `packrat_stats`/`span_trace`/`stream_meta` を JSON へ直列化できるようになった。
