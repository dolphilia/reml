...
## 3.0.10 CI ブロッカーとゲート
- `Module Parser Acceptance`: `cargo test --manifest-path compiler/rust/frontend/Cargo.toml parser::module -- --nocapture` が `CI_RUN_ID` 付きで通過し、`reports/spec-audit/ch1/module_parser-YYYYMMDD-parser-tests.md` および `module_parser-YYYYMMDD-dualwrite.md` にログを保存していること。`TraceEvent::ModuleStageEntered(stage)` / `TraceEvent::ModuleDeclAccepted(kind)` の `trace_id = syntax:module-stage::<stage>` が `use_nested-YYYYMMDD-trace.md` へ記録されているかを確認する。失敗時は `needs.module-parser` 依存で dual-write ジョブを停止し、`docs/notes/process/spec-integrity-audit-checklist.md` の `SYNTAX-002/module_parser` 行でステータスを追跡する。
- `Syntax Sample Streaming`: GitHub Actions に `rust-frontend-streaming` ジョブを追加し、`cargo test --manifest-path compiler/rust/frontend/Cargo.toml streaming_metrics -- --nocapture` が成功して `reports/spec-audit/ch1/streaming_metrics-YYYYMMDD-log.md` をアップロードしていること。Streaming サンプルの診断 (`streaming_use_nested-YYYYMMDD-diagnostics.json`, `streaming_effect_handler-YYYYMMDD-diagnostics.json`) を `reports/spec-audit/ch1/` と `ch2/streaming/` に複製し、`scripts/poc_dualwrite_compare.sh use_nested` / `effect_handler` の差分 0 とセットで `reports/spec-audit/ch1/2025-11-17-syntax-samples.md` を更新する。失敗時は `tests/streaming_runner.rs` の回帰確認を実施し、`docs/spec/0-3-code-style-guide.md` に記載したフォールバック手順（`*_rustcap.reml` 復帰条件）を一時的に有効にする。
