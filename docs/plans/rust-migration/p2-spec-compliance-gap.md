...
| `P2G-02.03` | ✅ 契約検証 API | DSL / conductor 契約をまとめて検証する `verify_conductor_contract`、`manifest_path` を参照した監査メタデータ出力を提供する（`docs/spec/3-8-core-runtime-capability.md:141-156`）。 | Rust 側には `manifest_contract.rs` で `run.target.capabilities` を TOML から読み取るパーサと、`verify_conductor_contract` で `verify_capability_stage` の結果を `AuditSink` に `effect.stage.contract`/`effect.stage.capability_mismatch` として出力するコードが追加され、`manifest_path` による差分ログも取得できるようになった。 | `ConductorCapabilityContract` 型・`verify_conductor_contract` を通じて manifest stage/effect/source span の差異を `AuditEvent::CapabilityMismatch` として記録し、`CapabilityError::ManifestMismatch` で通知する仕組みを整備。`registry.rs` 側で正常系・manifest 走査系テストを追加し、`scripts/poc_dualwrite_compare.sh` の diag モードで `effect.stage.*`/`effect.manifest_path` を照合できるようにする。 |
...
- `docs/spec/3-9-core-async-ffi-unsafe.md:1006-1014` で要求されている `status`/`target`/`arch`/`abi`/`expected_abi`/`ownership`/`extern_symbol`/`extern_name`/`link_name`/`return` と `tooling/runtime/audit-schema.json` の `bridge` 定義（`audit_pass_rate`/`platform`/`return.{ownership,status,wrap,release_handler,rc_adjustment}`）をすべて満たす必要がある。まずは現行の FFI トレースコードが保持している値（`RuntimeString` の `ownership`・`Span`・symbol 名・ABI 情報など）と、今後追加すべき `expected_abi`/`link_name`/`return` ラッパ情報の対応表をまとめ、改善対象をドキュメント化する。
- `BridgeAuditMetadata` に `expected_abi`/`arch`/`extern_symbol`/`extern_name`/`link_name` と `return` 用の補助構造体を追加し、`RuntimeString::to_bridge_metadata` から該当値を組み立てる。`AuditContext::log_bridge_metadata` では `bridge.*` の全キーを `metadata` に直接記録し、`bridge.return` へ `ownership/status/wrap/release_handler/rc_adjustment` を JSON として埋め込む。`audited_bridge_call` の `ffi.call.start/end` に加え `ffi.call.result` など段階的なイベントも発行して `AuditContext` に状態遷移を残し、`record_bridge_with_metadata` は `AuditContext` の呼び出しタイミングを集約するヘルパとして再設計する。
- `scripts/poc_dualwrite_compare.sh --mode diag` や `reports/dual-write/<run>/audit*.json` に `bridge.*` キーを含む JSON が出力され、OCaml 側と `jq` で `target`/`arch`/`abi`/`return` オブジェクトを比較できることを確認する。`tooling/ci/collect-iterator-audit-metrics.py` の `ffi_bridge.audit_pass_rate` 算出パスが新しいキーを読み取れるように機能要件を追記し、差分が残る項目は `reports/dual-write/README.md` の TODO に記録して次フェーズで `ffi_bridge.audit_pass_rate` を 1.0 に引き上げる。

### P2G-03.02

- `docs/spec/3-6-core-diagnostics-audit.md:329-337` / `3-9-core-async-ffi-unsafe.md:997-1010` の `ffi.contract.*` 診断要件に照らし、`link_name`/`extern_symbol` の欠落・`#[ownership]` 指定の不整合・期待 ABI のブロックターゲットとの不一致を検出する `ffi_contract` モジュールを Rust 実装内に追加する。`BridgeAuditMetadata` を入力として `SymbolMissing`/`OwnershipMismatch`/`UnsupportedAbi` を返す検証ロジックを用意し、`audited_bridge_call` から呼び出して `BridgeError::Contract` に変換し、`record_bridge_with_metadata` 経由で `ffi.contract.symbol_missing` などのイベントを出力する。拡張メタデータは `Diagnostic.extensions["bridge"]` と同一キー（`expected_abi`/`ownership`/`extern_symbol`/`return`）になるよう維持する。
- `CapabilityRegistry::verify_capability_stage` の失敗（Stage/Effect 逸脱）については `audited_bridge_call` 側で再検証して `effects.contract.stage_mismatch` を明示的に記録し、`effect.stage.required`/`effect.stage.actual`/`effect.stage.required_effects`/`effect.scope`/`effect.stage.missing_effects` を `AuditContext` に埋め込む。`scripts/poc_dualwrite_compare.sh` の `diag` モードおよび `reports/dual-write/<run>/audit*.json`/`collect-iterator-audit-metrics.py` に新イベントが含まれていることを README へ記録し、Go/No-Go 判定で差分がないことを確認する。
- 対象には `compiler/rust/runtime/ffi/tests` 内に `ffi_contract` 検証のユニットテストを追加し、`BridgeAuditMetadata` を組み立てて各違反パスをトリガーする。新しい diag イベントの出力については `AuditSink` モックを使って `reports/dual-write` 用 JSON へ反映されることを検証し、必要なら `scripts/poc_dualwrite_compare.sh` の README や `reports/dual-write/README.md` に新エントリを追加して再現手順を残す。

### P2G-03.03

- `docs/spec/3-9-core-async-ffi-unsafe.md:1012-1014` が `bridge.return` に `ownership`/`status`/`wrap`/`release_handler`/`rc_adjustment` を必須と定めているため、`RuntimeString::to_bridge_metadata` で使う `BridgeReturnAuditMetadata` に `borrowed_wrap`/`transferred_wrap_and_release` といった定型コンストラクタと `failure` ケースを用意し、監査ログの `return` オブジェクトに値を埋められるようにする。
- `ForeignPtr` の `acquire_borrowed_result`/`acquire_transferred_result` で `note_borrowed_return`/`note_transferred_return` を呼び出して thread-local な return tracker に `wrap_foreign_ptr`・`dec_ref`・`rc_adjustment` の組み合わせを記録する。どの返り値経路が使われたかを `BridgeAuditMetadata` に伝える前段として、`compiler/rust/runtime/ffi/src/lib.rs` の `audited_bridge_call` で tracker のクリア／取得ロジックを整備する。
- `audited_bridge_call` では呼び出し前に tracker をクリアし、`body` 実行後に `take_bridge_return_metadata` で記録値を取り出して `metadata.with_return_info(...)` に反映する。成功時は tracker の `status`（`wrap`/`wrap_and_release`）を `ffi.call.result` payload の `return_status` に使い、失敗時は `BridgeReturnAuditMetadata::failure` で `return.status` を `"failure"` に置き換えて `record_bridge_with_metadata` を通じて `AuditContext` に出力する。`return_status` を JSON に含めることで `tooling/ci/collect-iterator-audit-metrics.py` のスキーマを満たす。
 - `compiler/rust/runtime/ffi/src/lib.rs` に録音トラッカーと `BridgeReturnAuditMetadata` のコンストラクタを検証する単体テストを追加し、`note_borrowed_return`/`note_transferred_return` で期待の値が記録され、`bridge.return` のキーセットが常に `ownership`/`status`/`wrap`/`release_handler`/`rc_adjustment` を含むことを担保する。必要に応じて `docs/plans/bootstrap-roadmap` 内の `scripts/poc_dualwrite_compare.sh` や `reports/dual-write/README.md` へ監査スキーマの補足を追記する。

### P2G-04.01

- `docs/spec/3-10-core-env.md:14-41` で規定された `EnvError`/`EnvContext` と `AuditEvent::EnvMutation`（`env.operation`/`env.key`/`env.scope`/`requested_by`）に合わせ、環境 API のエラーハンドリングと監査通知を Rust 側で完結させることを最優先とする。環境変更系 API は `effect {io,security}` を持つため、`RunConfig.extensions["audit"].capture_env=true` で追跡される JSON を経路に含めた状態で `AuditSink` に登録させる。
- `compiler/rust/adapter/env.rs` に今回の設計を落とし込み、`EnvErrorKind`/`EnvError`/`EnvContext` の構造を `reml_adapter` crate へ切り出した。`EnvAuditor` トレイトと `EnvAuditHandle` を使うことで `set_env`/`remove_env` の呼び出し前後に `EnvMutationResult`（成功・失敗）を添えてイベントを登録できるようにし、`EnvMutationResult::failure_reason` で失敗理由を `env.failure_reason` metadata に含める。
- `compiler/rust/runtime/ffi/src/env.rs` では `AuditContext` を `EnvAuditor` として実装し、`env.operation`/`env.key`/`env.scope`/`requested_by`/`env.status`/`env.failure_reason` を補った `metadata` 付きイベントを `env_mutation` で発行するブリッジを用意した。`get_env`/`set_env`/`remove_env` はローカルの `EnvContext::detect(EnvOperation)` を使い、異常時には `EnvError` を期待通りに返す。
- 環境 API の検証には `compiler/rust/adapter/src/env.rs` 内の単体テストを使用し、標準キーに対する成功ケースと NULL バイト入り key の失敗ケースで `RecordingAuditor` が `env_mutation` イベントを記録すること、`reml_adapter::env::get_env` が `Ok(None)` を返すことを確認した。
- 今後のフォローアップとして、`scripts/poc_dualwrite_compare.sh` / `reports/dual-write/env` などに `env_mutation` イベントを差分収集させることで、OCaml 側と同様の `AuditEnvelope.metadata["env.*"]` の JSON を Dual Write ハーネスで比較できる体制を整える。必要があれば `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` の診断集計欄や `docs/notes/` の TODO に `env` 監査の追加を記録する。

### P2G-04.02

- `docs/spec/3-10-core-env.md:120-152` に記された `infer_target_from_env`/`resolve_run_config_target`/`merge_runtime_target` は `RunConfigTarget` を決定して `DiagnosticDomain::Target` との整合を保つ仕組みであり、`docs/plans/rust-migration/2-2-adapter-layer-guidelines.md:24-33` が想定するアダプタ層成果のコアです。まずは OCaml 実装が `REML_TARGET_OS`/`ARCH`/`ABI` などから `TargetProfile` を組み立てて `RunConfig.extensions["target"]` と `Diagnostic.extensions["cfg"]` を更新する流れをトレースし、Rust 側に求められる `target.profile`/`target.config` の属性と診断メタデータの粒度を明示化します。
- `compiler/rust/adapter/target.rs`（仮）として新モジュールを作成し、`TargetProfile` を再現する構造体・パーサー・ユーティリティを置きます。環境変数・コマンドライン補助を `adapter::target::infer_run_config_target` などの公開関数にまとめ、`RunConfig::with_adapter` や CLI 初期化時に呼んで `RunConfigTarget` を構築すると同時に `RunConfig.extensions["target"]` に `profile_id`/`triples`/`req_arch` などを設定します。`DiagnosticDomain::Target` に渡す `Diagnostic.extensions["cfg"]` もこのモジュール経由で整備し、`target.config.mismatch`/`target.profile.missing` が発動する条件を `diagnostic::Builder` で判定できるようにします。
- `adapter::target` では `DiagnosticDomain::Target` への差分比較ロジックも担い、`RunConfigTarget` `PlatformInfo` `TargetProfile` の三者で `os`/`arch`/`abi` が一致しないケースに `extensions["cfg"]` を付与した診断を生成します。`scripts/poc_dualwrite_compare.sh` に `--emit-target-inference` オプションを追加して Rust/OCaml の `RunConfig.extensions["target"]` および `Diagnostic.extensions["cfg"]` を記録し、`reports/dual-write/target-inference/<run-id>/` に JSON を置いて `jq` で `target.config.*`/`cfg.*` の差分を確認します。
- 検証は `compiler/rust/adapter/tests/target_inference.rs` などのユニットテストで `REML_TARGET_*` をセットした環境から正しい `TargetProfile` と `RunConfigTarget` が得られ、診断の `extensions["cfg"]` に `detected`/`requested` フィールドが記録されることを確認します。Dual Write ハーネスを用いて `reports/dual-write/target-inference` の JSON が OCaml 実装と一致しない場合は `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` の該当箇所に TODO を残して次フェーズに引き継ぎます。
