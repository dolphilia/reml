...
P4 フェーズでは Rust 実装の最終差分が仕様・ガイド・ノートへ確実に反映されているかを確認する必要がある。本書は [`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md`](../bootstrap-roadmap/0-3-audit-and-metrics.md) と `docs-migrations.log` を基点とし、Phase 3 Self-Host へ引き継げる情報状態を維持する手順をまとめる。
...
| ガイド (`docs/guides/`) | Rust 実装 + ドキュメント | CLI/CI 手順やランタイムブリッジの更新が必要な時 | `docs/guides/runtime-bridges.md`, `docs/guides/core-parse-streaming.md`, `docs/guides/ai-integration.md` |
| ノート (`docs/notes/`) | 各担当 | 意思決定・調査結果・残課題を共有する時 | `docs/notes/dsl-plugin-roadmap.md`, `docs/notes/core-library-outline.md`, `docs/notes/phase3-handover/` |
...
4. **記録更新**: `docs-migrations.log` に更新対象・理由・関連リスクを追記し、`0-3-audit-and-metrics.md` の該当指標（例: `diagnostic.audit_presence_rate`）へ計測結果を転記する。
...
- [ ] `docs/guides/runtime-bridges.md`・`docs/guides/core-parse-streaming.md` の手順が最新 CI 設定と一致している。
