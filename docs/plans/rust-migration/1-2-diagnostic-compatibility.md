...
4. **diag ハーネスでの強制設定**  \n   - `scripts/poc_dualwrite_compare.sh --mode diag` に `force_type_effect_flags` を実装し、`type_*` / `effect_*` / `ffi_*` ケースでは `--experimental-effects --effect-stage beta --type-row-mode dual-write --emit-typeck-debug <case>/typeck` を OCaml/Rust 共通で注入、Rust 専用に `--emit-effects-metrics <case>/effects` を追加する。`cases.txt` の `#flags` 記述有無に依存せずハーネス側で強制することで、CLI が parser フェーズで停止して `diagnostics.*.json` が空になる事故を防ぐ。  \n   - `collect-iterator-audit-metrics.py --section effects` と `--section parser` の結果（`effect_scope.audit_presence` / `parser.expected_summary_presence`）を `summary.json` の `metrics_ok` 判定へ取り込み、`reports/dual-write/front-end/w4-diagnostics/20280418-w4-diag-effects-r3/summary.md` のように `diag_match` / `metrics_ok` / `typeck_logs.present` を Run ID ごとに公開する。更新内容は `docs/plans/rust-migration/1-0-front-end-transition.md`（§4）と `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#TODO:-diag-rust-06` にも反映した。
...
- 2028-04-10: Run `20280410-w4-diag-streaming-r21`（`reports/dual-write/front-end/w4-diagnostics/20280410-w4-diag-streaming-r21/stream_pending_resume/streaming-metrics.ocaml.json`）で OCaml フロントエンドが `ExpectedTokenCollector.streaming` を Pass（27 件の `expected_tokens` 付き）し、`summary.json` の `metrics_ok=true` を回復。Rust 側は依然 1 件の `parse.expected` のみで `expected_tokens_match=false` が残るため、以降の作業は Rust Recover のマージ処理／`diagnostics.rust.json` への多重出力抑止へ集中する。
- 🆕 2029-04-15: Run `20290415-w4-diag-streaming-recheck2`（`reports/dual-write/front-end/w4-diagnostics/20290415-w4-diag-streaming-recheck2/summary.md`）を Streaming 3 ケースで再実行し、`scripts/poc_dualwrite_compare.sh --mode diag --emit-expected-tokens expected_tokens/stream` が `ExpectedTokenCollector.streaming` の失敗を `summary.json.metrics_ok=false` へ自動反映することを確認した。`stream_pending_resume/streaming-metrics.rust.json` などには `expected_tokens_metric` / `expected_tokens` セクションが追加され、`diag_counts.ocaml=diag_counts.rust=1` を満たしていても `expected_tokens_match=false`（Rust は 1 件のまま）である限り `gating=false` を維持できる。`stream_backpressure_hint` では OCaml 側診断が 0 件のままのため `diagnostics.ocaml.json` 欠落が即座に `metrics_ok=false` へ落ち、残課題が Rust Recover の多重出力削減＋ OCaml Streaming CLI の整備に限定された。
- **アクション**  
 1. `parser_expectation.ml` と Rust `Diagnostic.Builder` に Streaming Pending/Resume 用の `expected` フォールバックを実装し、`parser.expected_summary_presence=1.0` になるまで CLI を再計測する。OCaml 側は `20280410-w4-diag-streaming-r21` で完了済みのため、残差は Rust フロントエンドの Recover 連結（`frontend/src/diagnostic/recover.rs`）と `FrontendDiagnostic` でのマージ処理に集約される。  
 ...
 4. **`diag_counts` 圧縮と Streaming 向け `ExpectedTokenCollector` の実装**  
    - Run `20280318-w4-diag-streaming-r11` の結果では `diag_counts` が `1/11`, `1/4`, `0/5` に分裂し、Rust 側だけが checkpoint ごとに `parse.expected` を複製している。OCaml 側では recover が発生しないため `expected_tokens` が空のまま `expected_tokens/stream.diff.json` に `27 vs 1` などの大差が残る。  
    - `compiler/ocaml/src/parser_expectation.ml` / `core_parse_streaming.ml` / `parser_driver.ml` / `main.ml` に Streaming 専用 Collector を追加し、`expectation_summary_for_checkpoint` が Pending/Resume でも `expected.alternatives` を返すようにする（✅ 2028-04-10: 実装済み）。Rust 版は `frontend/src/diagnostic/recover.rs` に `ExpectedTokenCollector` を移植し、`FrontendDiagnostic::finalize()` で `message_key=parse.expected` + `span` 一致の診断を 1 件に畳み込む。  
    - `scripts/poc_dualwrite_compare.sh --mode diag --emit-expected-tokens <dir>` で保存する `expected_tokens.{ocaml,rust}.json` / `expected_tokens.diff.json` を `collect-iterator-audit-metrics.py --section parser` の入力へ渡し、`parser_expected_summary_presence=1.0` と `diag_counts.ocaml == diag_counts.rust == 1` と `expected_tokens_match=true` の三条件を満たさない場合は `metrics_ok=false` にする。必要な計算は `tooling/ci/collect-iterator-audit-metrics.py:1243-1304` へ `ExpectedTokenCollector.streaming` ブロックとして追加し、`reports/dual-write/front-end/w4-diagnostics/<run>/stream_*/parser-metrics.*.err.log` に詳細ログを出力する。  
    - `p1-front-end-checklists.csv` は `parser.stream.*` フィールド整合に加えて上記 3 指標（`expected_tokens_match` / `diag_counts_match` / `collect-iterator` pass）を受入基準へ組み込み、DIAG-RUST-05 の完了条件をメトリクスレベルで縛る。
...
<a id="ffi-stage-監査diag-rust-06"></a>
## 1.2.16 FFI Stage 監査（DIAG-RUST-06）
1. **再現手順**  
   - `scripts/poc_dualwrite_compare.sh --mode diag --run-id 20280418-w4-diag-effects-r3 --cases docs/plans/rust-migration/appendix/w4-diagnostic-cases.txt --case-filter '^(type_|effect_|ffi_)' --emit-expected-tokens expected_tokens` を実行し、`scripts/dualwrite_summary_report.py --diag-table` で README / `w4-diagnostic-case-matrix.md` を更新する。`collect-iterator-audit-metrics.py --section effects --require-success --source <case>/diagnostics.rust.json` を流すと、`ffi_ownership_mismatch` / `ffi_async_dispatch` では `missing_keys=["effect.stage.required","effect.stage.actual","bridge.stage.required_capabilities","bridge.stage.actual_capabilities"]` が報告され `effects-metrics.rust.err.log`（例: `reports/dual-write/front-end/w4-diagnostics/20280418-w4-diag-effects-r3/ffi_ownership_mismatch/effects-metrics.rust.err.log`）に記録される。  
   - `summary.json` では前述 2 ケースのみ `metrics_ok=false` のままとなり、`effects.regressions=0` でも `effect_scope.audit_presence=0` がブロッカーとして残る。Run ID は `p1-front-end-checklists.csv`（診断カテゴリ行）と `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#TODO:-diag-rust-06` に記載する。
2. **原因**  
   - Rust 側 `Diagnostic` 生成（`compiler/rust/frontend/src/bin/poc_frontend.rs:835-858`）は `effect.stage.*` と `effect.required_capabilities` を空配列のままシリアライズしており、Stage requirement / actual stage / capability detail を一切含めない。  
   - `build_audit_metadata`（同 1010-1074 行）も `bridge.stage.*` / `effect.stage.capability` / `stage_trace` を AuditEnvelope に書き込んでいないため、`collect-iterator-audit-metrics.py` が Stage 監査キーを検知できない。OCaml 実装は `Diagnostic.with_effect_stage_extension`（`compiler/ocaml/src/diagnostic.ml:754-866`）と `Audit_envelope.stage_trace` を通じて CLI フラグ・Capability JSON・Typer/Runtimes の判定を順番に記録しているため、Rust 側 Stage 実装が未移植であることが直接のボトルネックとなっている。
3. **対応方針**（`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#TODO:-diag-rust-06` に連動）  
   1. `TypecheckConfig.effect_context` と `runtime_capabilities` を元に Stage 向けの中間表現（仮称: `StageAuditPayload`）を追加し、`effect.stage.required`（CLI / `@requires_capability(stage=...)`）と `effect.stage.actual`（Capability Registry 由来）を JSON と AuditEnvelope の両方へ出力する。`ffi_async_dispatch` は `cases.txt` に `--runtime-capabilities <macOS capability>` を追加し、Stage 判定の入力を揃える。  
  ...
  - 🆕 2029-06-12: Stage/Capability 拡張は `StageAuditPayload` を `diagnostics.*`・`audit_metadata`・`effects/effects-metrics.{ocaml,rust}.json`・`summary.json.bridge_stage` で共用する単一構造体にまとめ、`bridge_stage.audit_presence` を 1.0 で維持する。`docs/plans/rust-migration/appendix/w4-diagnostic-cases.txt` に登録された `--runtime-capabilities windows.messagebox,windows.ffi,macos.dispatch_queue` を diag ハーネスが自動注入し、`reports/dual-write/front-end/w4-diagnostics/20290427-w4-diag-recover-stream/*/effects-metrics.rust.err.log` に残った `missing_keys` を再現できるようにした上で、再測定（`tmp/w4-diagnostic-effects-stage.txt`）で `python3 tooling/ci/collect-iterator-audit-metrics.py --section effects --require-success --source <case>/diagnostics.rust.json` が `bridge_stage.audit_presence=1.0` / `effect_stage.audit_presence=1.0` を返すまで `summary.json.metrics_ok=false` を維持する。結果は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#TODO:-diag-rust-06` と同期し、`p1-front-end-checklists.csv` の Type/Effect/FFI 行へ Run ID を追記する。  
  - 🆕 2029-06-12: Type/Effect/FFI の `parser.expected_summary_presence` / `expected_tokens_match` が 1.0 へ戻るまで `TypeEffectRecoverLimiter`（`frontend/src/diagnostic/recover.rs`）で recover を 1 件に圧縮し、`ExpectedTokenCollector` の `expected_tokens.{ocaml,rust}.json` を `ffi_*` / `effect_residual_leak` に必須化する。OCaml CLI は `typeck_output.emit_debug_only` 後に `ensure_effect_diag_written` を差し込み、`ffi_async_dispatch` などで `diagnostics.ocaml.json` が空になる既知事象を排除する。Run `20280601-w4-diag-type-effect-rust-typeck-r7/ffi_ownership_mismatch/expected_tokens.diff.json`（`ocaml_count=5` vs `rust_count=2`）と `parser-metrics.rust.json`（`parser.expected_summary_presence=0.0`）を再現指標として利用し、`collect-iterator-audit-metrics.py --section parser --require-success` に `expected_tokens_match` / `parser.expected_summary_presence` / `diag_counts_match` を同時ゲートさせる。
...
### 2029-06 GUI/LSP 差分再チェック
- `reports/dual-write/front-end/w4-diagnostics/20290429-w4-diag-cli-lsp/summary.json` をベースラインとして保持し、CLI 側 `diagnostics.ocaml.json` が空のケースと `lsp/*.diff` に `bridge_stage.audit_presence < 1.0` が残るケースを抽出する。  
- Rust CLI の RunConfig 追加後に `scripts/poc_dualwrite_compare.sh --mode diag --run-id 20290520-w4-diag-cli-lsp --case-filter '^(cli_|lsp_)'` を実行し、`collect-iterator-audit-metrics.py --section parser --require-success --source <case>/diagnostics.{ocaml,rust}.json` で `parser.runconfig_switch_coverage=1.0` / `diag_match=true` / `extensions.config.path` 一致が確認できなければ `summary.json.metrics_ok=false` を明示する。  
- `npm run ci --prefix tooling/lsp/tests/client_compat -- diag-w4 20290520-w4-diag-cli-lsp` と `scripts/report-fixture-diff.mjs` を同じ Run ID で実行し、`reports/dual-write/front-end/w4-diagnostics/<run>/lsp/<case>.diff` と CLI `summary.json` のリンクを `p1-front-end-checklists.csv` / `w4-diagnostic-case-matrix.md` に転記する。  
- Run ID ごとの `diag_match` / `metrics_ok` / `parser.runconfig_switch_coverage` を `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#TODO:-diag-rust-07` と同期し、GUI/LSP 再チェックの成果が Phase 2-8 へそのまま参照できる状態を維持する。
...
- 対応:  
  1. `poc_frontend` に `StageAuditPayload` を追加し、`extensions.effects.stage` / `extensions.bridge.stage` / `audit_metadata` / `typeck/metrics.json` が同一構造体を共有するよう統合した。`collect-iterator-audit-metrics.py` も `effect_stage.audit_presence` / `bridge_stage.audit_presence` / `typeck_debug_match` を新設し、`effects-metrics.rust.err.log` に不足キーと `type_row_mode` ミスマッチを即時記録する。次回 Run（`--case-filter '^(ffi_)'`）で `effect_scope.audit_presence=effect_stage.audit_presence=bridge_stage.audit_presence=1.0` になることを確認する。  
  2. Type/Effect ケースは Rust/OCaml 双方で `typeck/typeck-debug.{rust,ocaml}.json` を生成し、`typeck_debug_match` メトリクスが `type_row_mode=dual-write` を検出できるよう `docs/plans/rust-migration/appendix/w4-diagnostic-cases.txt` を更新した（`ffi_async_dispatch` へ `--runtime-capabilities macos.dispatch_queue` を追加）。`summary.json.typeck_requirements.*` と `reports/.../effects-metrics.*` がそろった Run ID を `p1-front-end-checklists.csv` に記録する。  
...
- *2028-06-01 (Effect/FFI 継続課題)*: Run `reports/dual-write/front-end/w4-diagnostics/20280601-w4-diag-type-effect-rust-typeck-r7/summary.md` では `effect_residual_leak/summary.json` が `ocaml_diag_count=1` / `rust_diag_count=2`・`expected_tokens.ocaml_count=1` / `rust_count=0` を示し、`parser-metrics.rust.json`（同ディレクトリ 1-70 行）でも `parser.expected_summary_presence=0.0` が継続している。`ffi_ownership_mismatch` と `ffi_stage_messagebox` は Rust 側 21 件 / 43 件の recover を出力し、`expected_tokens.diff.json` は `ocaml_count=5/12` 対 `rust_count=2` で乖離、`ffi_async_dispatch/summary.json` は依然 OCaml 診断 0 件 (`expected_tokens.ocaml_count=0`) のまま。これら 4 ケースはいずれも `metrics_ok=false` / `expected_tokens.match=false` で止まり、Rust 側 parser 診断が 20 件超発生する状態が続いている。  
...
- *残タスク整理*:  
  ...
  2. **Rust 診断件数と expected_tokens の縮約** — `effect_residual_leak` と `ffi_ownership_mismatch` の `parser-metrics.rust.json` が `parser.expected_summary_presence < 0.1` を記録している原因は、parser recover が `type_effect_case` でも 2〜43 件生成されるため。`compiler/rust/frontend/src/diagnostic/recover.rs` / `parser/expect.rs` へ `TypeEffectRecoverLimiter` を追加して recover を 1 件に圧縮し、`compiler/rust/frontend/src/typeck/driver.rs` の `TypecheckDriver::emit_effect_violation` が `ExpectedTokenCollector` を経由して `expected_tokens` を OCaml と同じ `":" / "identifier"` 配列に復元できるようにする。`expected_tokens.diff.json`（`ffi_ownership_mismatch`: `ocaml_count=5` vs `rust_count=2`, `ffi_stage_messagebox`: `12` vs `2`）を受入基準に用い、Rust 側 `expected_tokens` が 5/12 件へ揃うまで再実行する。  
  3. **OCaml FFI 入力の再実行** — `ffi_async_dispatch/summary.json` では OCaml 診断 0 件が続いているため、`docs/plans/rust-migration/appendix/w4-diagnostic-cases.txt` の `#flags` を `compiler/ocaml/tests/test_ffi_contract.ml::ffi_dispatch_async` と同一に揃えたうえで `scripts/poc_dualwrite_compare.sh --mode diag --case-filter '^(ffi_async_dispatch)$'` を単体実行し、`diagnostics.ocaml.json` に 1 件の CLI 診断が確実に記録されることを確認する。  
  4. **メトリクスゲート** — `collect-iterator-audit-metrics.py` の `typeck_debug_match` に加え、`parser.expected_summary_presence` と `expected_tokens_match` も CI でゲート化する。`scripts/poc_dualwrite_compare.sh --mode diag --force-type-effect-flags` の post-hook に `python3 tooling/ci/collect-iterator-audit-metrics.py --section effects --require-success --source <case>/diagnostics.<frontend>.json` を組み込み、3 つの指標のいずれかが 1.0 未満の場合は `summary.json.metrics_ok=false` を即時記録する。  
...
## 1.2.21 DIAG-RUST-06/07 直近再測定トラック
- `bridge_stage.audit_presence < 1.0` が `reports/dual-write/front-end/w4-diagnostics/20290427-w4-diag-recover-stream/*/effects-metrics.rust.err.log` で再発しているため、`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#TODO:-DIAG-RUST-06` に記録した Stage/Capability 拡張を優先する。`StageAuditPayload` に `bridge.stage.required_capabilities` / `bridge.stage.actual_capabilities` を `docs/spec/3-8-core-runtime-capability.md` の Registry から補完し、`cases.txt` 側で `--runtime-capabilities windows.messagebox,windows.ffi,macos.dispatch_queue` を自動注入して両 CLI から同じ Stage 情報を出力する。`collect-iterator-audit-metrics.py --section effects --require-success` が `bridge_stage.audit_presence=1.0` / `effect_stage.audit_presence=1.0` を返すまで `scripts/poc_dualwrite_compare.sh --mode diag --cases docs/plans/rust-migration/appendix/w4-diagnostic-cases-recover-stream.txt` を繰り返し、Run 成果を `p1-front-end-checklists.csv` に転記する。  
