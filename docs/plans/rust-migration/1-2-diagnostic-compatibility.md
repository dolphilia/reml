...
- **アクション**  
 ...
 4. **`diag_counts` 圧縮と Streaming 向け `ExpectedTokenCollector` の実装**  
    ...
    - `compiler/ocaml/src/parser_expectation.ml` / `core_parse_streaming.ml` に Streaming 専用 Collector を追加し、`expectation_summary_for_checkpoint` が Pending/Resume でも `expected.alternatives` を返すようにする。Rust 版は `frontend/src/diagnostic/recover.rs`（仮称）へ `ExpectedTokenCollector` を移植し、`FrontendDiagnostic::finalize()` で `message_key=parse.expected` + `span` 一致の診断を 1 件に畳み込む。  
