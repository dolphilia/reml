...
- `bridge_stage.audit_presence < 1.0` が `reports/dual-write/front-end/w4-diagnostics/20290427-w4-diag-recover-stream/*/effects-metrics.rust.err.log` で再発しているため、`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#TODO:-DIAG-RUST-06` に記録した Stage/Capability 拡張を優先する。`StageAuditPayload` に `bridge.stage.required_capabilities` / `bridge.stage.actual_capabilities` を `docs/spec/3-8-core-runtime-capability.md` の Registry から補完し、`collect-iterator-audit-metrics.py --section effects --require-success` で `bridge_stage.audit_presence=1.0` になるまで `scripts/poc_dualwrite_compare.sh --mode diag --cases docs/plans/rust-migration/appendix/w4-diagnostic-cases-recover-stream.txt` を走らせて検証する。  
- Type/Effect/FFI recover の `parser.expected_summary_presence` / `expected_tokens` を 1.0 へ回復し、OCaml CLI の `diagnostics.ocaml.json` が空になる問題を解消する。`TypeEffectRecoverLimiter` による recover 1 件化と `ExpectedTokenCollector` の `expected.alternatives` 復元を `compiler/rust/frontend/src/diagnostic/recover.rs`・`parser/expect.rs` へ導入し、`ffi_ownership_mismatch` / `ffi_stage_messagebox` / `ffi_async_dispatch` について `expected_tokens.diff.json`（例: `ocaml_count=5` vs `rust_count=2`）が消えるまで `scripts/poc_dualwrite_compare.sh --mode diag --case-filter '^(ffi_)'` で再計測する。タイミングを見て `compiler/ocaml/tests/test_ffi_contract.ml` が出力する CLI フラグと `docs/plans/rust-migration/appendix/w4-diagnostic-cases.txt` の `#flags` を突き合わせ、OCaml 側診断が必ず 1 件含まれることを確認する。  
- `reports/dual-write/front-end/w4-diagnostics/20290429-w4-diag-cli-lsp/summary.md` に記録された CLI/LSP の `diag_match=false` には `reports/.../lsp/<case>.diff` を含む GUI/LSP 差分も含まれており、`report-fixture-diff.mjs` / `npm run ci --prefix tooling/lsp/tests/client_compat -- diag-w4 20290429-w4-diag-cli-lsp` の成果を再確認したうえで `extensions.config.*` / `parser.runconfig.*` の同期と `diagnostics.ocaml.json` の出力を揃える。差分が解消した Run ID を `p1-front-end-checklists.csv`（診断行）へ記録し、`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#TODO:-diag-rust-07` のトラッキングも更新する。  
- `summary.json.gating=false` が streaming/type&effect/CLI の 3 カテゴリに残っているため、前述の改善を反映した `scripts/poc_dualwrite_compare.sh --mode diag` の再 run で `summary.json.gating=true` に切り替える。各カテゴリの `diag_match=true` / `metrics_ok=true` / `parser.runconfig_switch_coverage=1.0` を確認し、`reports/dual-write/front-end/w4-diagnostics/summary.md` と `reports/dual-write/front-end/w4-diagnostics/README.md` に新 Run を追加すると同時に `p1-front-end-checklists.csv` の収束判定を更新する。  
