...
- **実施内容**
  ...
  3.1. フロントエンドが出力する MIR JSON（`module`/`functions`/`ffi_calls` を含むスキーマ）を `compiler/rust/backend/llvm::integration::generate_snapshot_from_mir_json` でロードし、`reports/backend-ir-diff/w3-demo-log.json` のような差分ログを生成することで、P1 〜 P2 の Go/No-Go 傾向を JSON で再現できるようにする。
