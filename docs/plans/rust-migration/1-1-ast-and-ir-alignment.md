...
## 1.1.9 Rust AST / Typed AST データモデル草案（W2）
...
- `StageRequirement` は `enum StageRequirement { Exact(IdentId), AtLeast(IdentId) }` で統一し、AST/Typed AST の `stage` フィールドから共有。`EffectMeta` の詳細設計、`TyPool` の実装方式、`dict_ref` 直列化方式は TODO (`TODO-W2-AST-001`〜`003`) として草案末尾に記録し、`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` へ転記予定。  
- 本草案をもって `p1-front-end-checklists.csv` の AST／Typed AST 行は「成果物: typed_ast_schema_draft.md」「完了条件: dual-write AST JSON 差分ゼロ／型 ID・制約リスト一致」として更新し、W2 の達成範囲を固定した。
