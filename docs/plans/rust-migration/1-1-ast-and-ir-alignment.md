...
- W2 の dual-write 出力（Typed AST 含むサマリ）は `reports/dual-write/front-end/poc/2025-11-07-w2-ast-inventory/summary.md` を起点に確認できる。
...
- Rust 側 Packrat/SpanTrace の最新出力は `reports/dual-write/front-end/poc/2025-11-07-w2-ast-inventory/*.rust.parse-debug.json` へ保存済み（OCaml 版は `--packrat` 旗の未整備により 0/0 として記録、CLI 拡張フォローアップ中）。
