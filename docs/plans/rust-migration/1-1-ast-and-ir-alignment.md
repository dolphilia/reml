...
- *2025-12-09 追記*: `Decimal`/`BigInt`/`Ratio` を Rust `Numeric` トレイトで直接扱えるよう、`compiler/rust/runtime/src/numeric/{decimal.rs,mod.rs}` に feature ゲート付きの実装を追加した。`tests/data/numeric/decimal_cases.json` を `scripts/validate-diagnostic-json.sh --suite numeric` の対象に含め、`core_numeric` feature を有効にした `cargo test` で拡張型を行列に載せられることを確認する。フロントエンドの型推論テーブルにもこれらの型名を追記し、`docs/plans/bootstrap-roadmap/3-4-core-numeric-time-gap-plan.md` §1 のバックログと同期する。
...
- P1 のレビュー結果は `docs/plans/rust-migration/1-0-front-end-transition.md` へフィードバックし、必要なら `docs/notes/stdlib/core-library-outline.md` などの関連ノートへリンクを追加する。
