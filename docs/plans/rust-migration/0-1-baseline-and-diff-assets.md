...
1. Phase 2 の最新ブランチで OCaml 実装をビルドし、`dune runtest` + `collect-iterator-audit-metrics.py --require-success` を実行。結果を `reports/diagnostic-format-regression.md` の「Baseline (OCaml)」欄へ反映する。  
   ...
   - ⚠️ 2025-11-06: `scripts/validate-diagnostic-json.sh tmp/diagnostics-output/`・`collect-iterator-audit-metrics.py --require-success` を再実行したが、`parser.core.rule.*` のネスト欠落や `effects.required_capabilities`・RunConfig `lex`・`bridge.*` 監査フィールドの未整備により失敗。ログは `reports/dual-write/20251106-validate-diagnostic-json.md` と `reports/dual-write/20251106-collect-iterator-metrics.json` に保存。OCaml CLI / Parser ランタイムでのメタデータ拡充を継続し、必要なゴールデン更新後に再測定する。
