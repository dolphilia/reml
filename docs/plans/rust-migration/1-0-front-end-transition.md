...
4. **診断 dual-write 実行とメトリクス取得**  
   - `scripts/poc_dualwrite_compare.sh --mode diag --run-id <date>-w4-diag --cases docs/.../w4-diagnostic-cases.txt` を実行し、各ケースごとに OCaml/Rust の JSON を `jq --sort-keys` で整形後 diff を保存。Recover 系は `extensions.recover.*`、Streaming 系は `parser.stream.*`、Type/Effects 系は `effects.*`/`type_row.*` を重点比較する。  
     ...
     - 🆕 2028-01-15: Run ID `20280115-w4-diag-refresh`（`reports/dual-write/front-end/w4-diagnostics/20280115-w4-diag-refresh/summary.md`）で 21 ケースを再実行。Streaming 系は CLI フラグが伝播し `runconfig.extensions.stream.enabled=true` になったが、`collect-iterator-audit-metrics.py` が `parser.stream_extension_field_coverage < 1.0` を継続検出し、`reports/dual-write/front-end/w4-diagnostics/20280115-w4-diag-refresh/stream_pending_resume/parser-metrics.ocaml.err.log` などに `expected_summary_presence < 1.0` が残存。Type/Effect 系は OCaml 側が `E7006` など 1 件ずつ出力するようになった一方、Rust 側は `diagnostics.rust.json` が空のままで `diag_count=0`（`reports/dual-write/front-end/w4-diagnostics/20280115-w4-diag-refresh/type_condition_bool/summary.json`）となり、`diag_match=false` が継続。`reports/dual-write/front-end/w4-diagnostics/README.md` のケース表は `--diag-table` で更新済み。
