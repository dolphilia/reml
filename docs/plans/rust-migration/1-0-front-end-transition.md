...
4. **診断 dual-write 実行とメトリクス取得**  
   - `scripts/poc_dualwrite_compare.sh --mode diag --run-id <date>-w4-diag --cases docs/.../w4-diagnostic-cases.txt` を実行し、各ケースごとに OCaml/Rust の JSON を `jq --sort-keys` で整形後 diff を保存。Recover 系は `extensions.recover.*`、Streaming 系は `parser.stream.*`、Type/Effects 系は `effects.*`/`type_row.*` を重点比較する。  
    ...
    - ✅ 2028-02-10: Run `20280210-w4-diag-recover-else-r4`（`reports/dual-write/front-end/w4-diagnostics/20280210-w4-diag-recover-else-r4/summary.md`）で `recover_else_without_if`/`recover_lambda_body` を個別再測定し、前者の `expected_tokens.diff.json` が `match=true` かつ `ocaml_count=rust_count=27` を達成。`collect-iterator-audit-metrics.py --section parser` も `diag_counts=1/1` と `parser.expected_summary_presence=1.0` を記録しており（`.../recover_else_without_if/parser-metrics.rust.json:22-44`）、Recover 行全体を `Ready + Pass` として確定できた。  
       ...
       - 🆕 2028-03-18: Streaming 系のみを再測定する `scripts/poc_dualwrite_compare.sh --mode diag --run-id 20280318-w4-diag-streaming-r11 --cases docs/plans/rust-migration/appendix/w4-diagnostic-cases.txt --emit-expected-tokens expected_tokens/stream` を実行し、`reports/dual-write/front-end/w4-diagnostics/20280318-w4-diag-streaming-r11/summary.md` を取得。`parser_expected_summary_presence` / `parser.stream_extension_field_coverage` は 3 ケースとも 1.0 を維持したものの、`diag_match=false`（`diag_counts.ocaml/rust = 1/11, 1/4, 0/5`）および `expected_tokens/stream.diff.json`（OCaml 側 0 あるいは 27 件 vs Rust 側 1 件）が残存。成果物と差分ログを `w4-diagnostic-case-matrix.md` と `p1-front-end-checklists.csv` へ転記し、DIAG-RUST-05 の残課題を「Rust 側 streaming recover 多重出力の削減」「OCaml 側 streaming recover への `expected_tokens` 付与」に整理した。
        ...
        4. **`diag_counts` 圧縮と Streaming 向け `ExpectedTokenCollector` の常設** — Streaming recover ケースでは Rust 側が checkpoint ごとに 4〜11 件の `parse.expected` 診断を生成し、OCaml 側は recover 自体が起きず `diag_counts` が 0〜1 件のまま停滞している。  
          - `compiler/ocaml/src/parser_expectation.ml` と `compiler/ocaml/src/core_parse_streaming.ml` に `ExpectedTokenCollector` の Streaming 統合ポイントを設け、Pending/Resume でも `expectation_summary_for_checkpoint` が `expected.alternatives` を返すよう保証する。Rust 側は `frontend/src/diagnostic/recover.rs`（仮称）に Streaming 専用 Collector を追加し、`FrontendDiagnostic::finalize()` で `message_key=parse.expected` ＋ `span` が一致する診断を単一件へマージする。  
