...
- ハンドオーバー: Run ID と CLI コマンドを `reports/dual-write/front-end/<run>/opbuilder/command.json` に残し、決定事項と残課題は `docs/notes/opbuilder-dsl-decisions.md` へ転記する。Self-host 移行時は `docs/plans/bootstrap-roadmap/4-4-field-regression-and-readiness-plan.md` で DSL パイプラインの実行有無を確認する。
