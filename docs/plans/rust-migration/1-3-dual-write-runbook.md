...
## 1.3.7 OpBuilder DSL デュアルライトチェック
- 対象ケース: `examples/spec_core/chapter2/op_builder/core-opbuilder-level-conflict-error.reml`（`CH2-OP-401`）、今後追加する fixity missing / duplicate operator ケースを含めて `.reml` と `expected/` をセットで運用する。
- 手順: `scripts/poc_dualwrite_compare.sh --mode diag --cases <opbuilder-cases.txt>` を Rust/OCaml 両フロントエンドで実行し、`diagnostics.{ocaml,rust}.json` と `expected_tokens.*` を `reports/dual-write/front-end/<run>/opbuilder/` に保存する。`summary.json` の `expected_tokens_match` / `schema_ok` / `gating` を確認し、`core.parse.opbuilder.level_conflict` / `fixity_missing` / `duplicate_operator` が一致しない場合は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` へ TODO を登録する。
- 連携先: `docs/plans/bootstrap-roadmap/4-1-opbuilder-dsl-plan.md` のフェーズ B/C/D 結果（Rust フロントエンド実装・診断ゴールデン・シナリオマトリクス更新）を参照し、回収完了時は `phase4-scenario-matrix.csv` `CH2-OP-401` の `resolution` を `ok` に更新する。
- ハンドオーバー: Run ID と CLI コマンドを `reports/dual-write/front-end/<run>/opbuilder/command.json` に残し、決定事項と残課題は `docs/notes/dsl/opbuilder-dsl-decisions.md` へ転記する。Self-host 移行時は `docs/plans/bootstrap-roadmap/4-4-field-regression-and-readiness-plan.md` で DSL パイプラインの実行有無を確認する。
