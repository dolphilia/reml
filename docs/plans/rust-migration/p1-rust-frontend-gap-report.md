...
| FRG-05 | Streaming & RunConfig | ✅ Closed (P2-8 W39) | `reml_frontend` CLI が `schema_version = "3.0.0-alpha"`、`stage_trace`、`used_impls` を `typeck/typeck-debug.rust.json` へ出力し、Streaming / Packrat メトリクスを CLI から直接収集できる。`scripts/poc_dualwrite_compare.sh` と `tooling/lsp/tests/client_compat/fixtures/*.json` も新 CLI に合わせて更新。 | `compiler/rust/frontend/src/bin/reml_frontend.rs` / `reports/spec-audit/ch1/use_nested-20251122-typeck.json` |
...
| ID | ギャップ | 現状 (Rust) | 期待仕様 / OCaml | 必要対応 |
| --- | --- | --- | --- | --- |
| FRG-12 | HM 実装 | ✅ Closed (P2-8 W39) | `TypecheckDriver::infer_module` が `Option<&Module>` を受け取り、AST 不在時は `typeck.aborted.ast_unavailable` を診断として返す。制約生成・一般化・Impl Registry 出力は `typeck/typeck-debug.rust.json`（`schema_version = "3.0.0-alpha"`）に保存され、`reports/spec-audit/ch1/typeck-fallback-removal-20251122.md` で検証済み。 | `compiler/rust/frontend/src/typeck/driver.rs` / `compiler/rust/frontend/tests/typeck_hindley_milner.rs` |
...
- 実装結果:
  ...
  - `docs/notes/process/spec-integrity-audit-checklist.md` の `SYNTAX-003` 行を `Closed (P2-8)` に更新し、`block_scope`/`effect_handler`/`perform_do` の監査行を追加。
...
> **Status (2025-11-22)**: Closed in Phase 2-8 Step7。`reml_frontend` が AST 欠落時に `typeck.aborted.ast_unavailable` を出力し、`typeck/typeck-debug.rust.json` へ `stage_trace` と `used_impls` を保存する。`reports/spec-audit/ch1/typeck-fallback-removal-20251122.md` を参照。
