...
| SCG-09 | 効果行 / 残余効果 / Capability Stage 監査 (`effects.contract.*`) 未実装 | 1-3 §I, EFFECT-002, `w4` cases | `TypecheckViolation::ResidualLeak` は `perform` の文字列抽出のみ。Stage/Capability コンテキストと連動していない。TPM-TYPE-03 では `cli-callconv`/`ffi-contract` を `poc_dualwrite_compare.sh --mode diag` から `effects-contract` metrics-case で `reports/dual-write/front-end/w4-diagnostics/effects-contract/<run>/<case>` に `diagnostics`/`audit`/`diag-metrics` を収集しているが、Rust 側 `StageContext`/`RuntimeCapability` の JSON 反映が不足している。 | `StageContext` と `RuntimeCapability` を参照し、効果タグと Capability Registry の照合作業を Rust でも実装。`docs/plans/bootstrap-roadmap/p1-test-migration-ffi-cases.txt` と `scripts/poc_dualwrite_compare.sh --mode diag --cases docs/plans/bootstrap-roadmap/p1-test-migration-ffi-cases.txt --force-type-effect-flags` で `effects.contract.*` の監査を `collect-iterator-audit-metrics.py --section diag --metrics-case effects-contract` に渡して gating する。差分が残る `effects.contract.stage_mismatch` は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#effects-change-log` に追記する。 |
