...
| P2 バックエンド統合 | `2-0-llvm-backend-plan.md` | LLVM バックエンド実装、`TargetMachine`/`DataLayout` 整合、MSVC/GNU 対応 | `docs/guides/compiler/llvm-integration-notes.md`, `docs/plans/bootstrap-roadmap/windows-llvm-build-investigation.md` |
...
|  | `../bootstrap-roadmap/3-6-core-diagnostics-audit-plan.md` | CLI/LSP/Audit 出力の `schema_version`・`structured_hints`・`stage_trace` を Rust 実装へ統合し、`examples/core_diagnostics/*.expected.*` をゴールデンとして維持する作業ログ | `docs/spec/3-6-core-diagnostics-audit.md`, `examples/core_diagnostics/README.md`, `tooling/examples/run_examples.sh --suite core_diagnostics --update-golden` |
...
Phase 3 で新設した `examples/core_io` / `examples/core_path` と `tooling/examples/run_examples.sh --suite core_io|core_path` は Rust 実装の CI 検証フローにも取り込み、`docs/plans/bootstrap-roadmap/3-5-core-io-path-plan.md` §6 および本概要の P3 ドキュメントからリンクする。`core_io.example_suite_pass_rate` は `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` に登録済みであり、Rust 移植ラインでも Reader/Writer/Path の監査メタデータ欠落を検知する。

`examples/core_diagnostics/pipeline_success.reml` / `pipeline_branch.reml` も Phase 3 の CLI/Audit 検証に組み込み、`tooling/examples/run_examples.sh --suite core_diagnostics --update-golden` で診断（`*.expected.diagnostic.json`）と監査（`*.expected.audit.jsonl`）の整合を確保する。これらのゴールデンは `docs/spec/3-6-core-diagnostics-audit.md` §9、および `docs/guides/runtime/runtime-bridges.md`・`docs/guides/ecosystem/ai-integration.md` に掲載した例と一致しており、Rust 実装が Stage/Audit KPI を満たしているかを P3 文書群から直接検証できる。
...
4. **用語整合と索引用語の確定**  
   ...
   - ✅ 2025-11-08: `docs/notes/language/glossary-rust-alignment-draft.md` を作成し、所有権・借用・非安全ブロックの仮定義とレビュー方針を整理（履歴メモとして保管）。  
...
5. **P1 フロントエンド作業の段取り確定**  
   ...
   - ✅ 2025-11-20: module parser 再実装 (`ModuleStage::{Header,UseList,DeclList}`) を完了し、`reports/spec-audit/ch1/module_parser-20251119-{parser-tests,dualwrite}.md` / `use_nested-20251119-diagnostics.json` / `effect_handler-20251119-diagnostics.json` をベースラインとして保存。`docs/notes/process/spec-integrity-audit-checklist.md` の `SYNTAX-002/module_parser` を `Closed (P2-8 W38)` に更新し、Phase 1 完了条件へ「Rust Frontend module parser が Chapter 1 サンプルを 0 診断で受理し CI ブロッカーを満たす」項目を追加。
   - ✅ 2025-11-21: Streaming Runner の統合テスト (`compiler/rust/frontend/tests/streaming_metrics.rs`) に `SampleCase` ベースの 3 テストを追加し、`cargo test --manifest-path compiler/rust/frontend/Cargo.toml streaming_metrics -- --nocapture` のログ (`reports/spec-audit/ch1/streaming_metrics-20251121-log.md`) と `streaming_use_nested-20251121-diagnostics.json` / `streaming_effect_handler-20251121-diagnostics.json` を保存。Phase 1 完了条件へ「Streaming 経路（CI ジョブ `rust-frontend-streaming`）が Chapter 1 サンプルを 0 診断で通過し、`StreamFlowState::latest_bridge_signal()` が単一段 `Option` として監査ログに残る」項目を追加。
   - ✅ 2025-11-22: `ParserTraceEventKind` に Expr/Effect/Handler/Resume 系を追加し、`FrontendDiagnostic.extensions.trace_ids` へ `syntax:expr::<kind>` を保存。`scripts/poc_dualwrite_compare.sh effect_handler --trace` のログを `reports/spec-audit/ch1/trace-coverage-20251122.md` に取りまとめ、Phase 1 完了条件へ「Trace coverage >= 4（handle / perform / resume / block）が `docs/notes/process/spec-integrity-audit-checklist.md` の `SYNTAX-003` で Closed」とするチェックを追加。
   - ✅ 2025-11-22: PoC CLI `poc_frontend` を正式名称 `reml_frontend` へ昇格し、`compiler/rust/frontend/src/bin/reml_frontend.rs`・`scripts/poc_dualwrite_compare.sh`・`tooling/lsp/tests/client_compat/fixtures/*.json` を全て新バイナリに合わせた。AST なし時は `typeck.aborted.ast_unavailable` 診断で停止し、`reports/spec-audit/ch1/use_nested-20251122-typeck.json` をはじめとする `typeck` アーティファクトに `schema_version = "3.0.0-alpha" / stage_trace / used_impls` を保存する。Phase 1 完了条件へ「reml_frontend CLI が Stage/Audit 指標を標準化し、FRG-05/FRG-12 を Closed として 3-0 ハンドオーバーを許可する」項目を追加。
