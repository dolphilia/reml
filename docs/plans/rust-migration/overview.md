...
2. **OCaml ベースライン測定と記録更新**  
   ...
   - ⚠️ 2025-11-06: `python3 tooling/ci/collect-iterator-audit-metrics.py --require-success` は `diagnostics array is missing` により失敗。`scripts/validate-diagnostic-json.sh` を用いた診断 JSON の再生成と、`compiler/ocaml/tests/golden/diagnostics/effects/` など効果系サンプルの補完後に再測定するタスクを追加する（同ログ参照）。
