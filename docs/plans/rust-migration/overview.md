...
3. **Windows ツールチェーン監査の先行着手**  
   - `tooling/toolchains/setup-windows-toolchain.ps1 -VerifyOnly` と `check-windows-bootstrap-env.ps1 -EmitJson` を GitHub Actions の `windows-latest` かローカル VM で実行し、`0-2-windows-toolchain-audit.md` の項目順に検証結果を埋める。  
