...
2. **OCaml ベースライン測定と記録更新**  
   ...
   - ✅ 2025-11-07: `with_parser_runconfig_metadata` の空配列初期化を適用し、`scripts/validate-diagnostic-json.sh compiler/ocaml/tests/golden/diagnostics/parser/{expected-summary,parser-runconfig-packrat,streaming-outcome}.json.golden` が通過することを確認（非診断ファイル `effects/syntax-constructs.json.golden` は引き続き除外が必要）。  
   - ℹ️ 2025-11-07: `tooling/ci/collect-iterator-audit-metrics.py --section ffi --source compiler/ocaml/tests/golden/diagnostics/ffi/unsupported-abi.json.golden --audit-source compiler/ocaml/tests/golden/audit/cli-ffi-bridge-{linux,macos,windows}.jsonl.golden --audit-source compiler/ocaml/tests/golden/audit/ffi-bridge.jsonl.golden` にて `ffi_bridge.audit_pass_rate` の欠落フィールドは解消（failures 空）が、指標計算ロジック上 pass_rate=0.0 のまま。`pass_fraction=0.888...` まで回復したため、スクリプト側の分母二重加算を別途 issue 化する。
3. **Windows ツールチェーン監査の先行着手**  
   ...
   - ℹ️ 2025-11-06: `docs/plans/bootstrap-roadmap/2-6-windows-support.md` への監査ログリンク追記は別タスクとしてフォローする。  
4. **用語整合と索引用語の確定**  
...
5. **P1 フロントエンド作業の段取り確定**  
   - `1-0-front-end-transition.md` に沿って、Lexer/Parser・AST/IR・型推論・診断の各ワークストリームに担当者とスプリント目安を割り当てる。  
