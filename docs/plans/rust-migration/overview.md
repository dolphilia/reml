...
5. **P1 フロントエンド作業の段取り確定**  
   ...
   - ✅ 2025-11-20: module parser 再実装 (`ModuleStage::{Header,UseList,DeclList}`) を完了し、`reports/spec-audit/ch1/module_parser-20251119-{parser-tests,dualwrite}.md` / `use_nested-20251119-diagnostics.json` / `effect_handler-20251119-diagnostics.json` をベースラインとして保存。`docs/notes/spec-integrity-audit-checklist.md` の `SYNTAX-002/module_parser` を `Closed (P2-8 W38)` に更新し、Phase 1 完了条件へ「Rust Frontend module parser が Chapter 1 サンプルを 0 診断で受理し CI ブロッカーを満たす」項目を追加。
   ...
   - ✅ 2025-11-22: `ParserTraceEventKind` に Expr/Effect/Handler/Resume 系を追加し、`FrontendDiagnostic.extensions.trace_ids` へ `syntax:expr::<kind>` を保存。`scripts/poc_dualwrite_compare.sh effect_handler --trace` のログを `reports/spec-audit/ch1/trace-coverage-20251122.md` に取りまとめ、Phase 1 完了条件へ「Trace coverage >= 4（handle / perform / resume / block）が `docs/notes/spec-integrity-audit-checklist.md` の `SYNTAX-003` で Closed」とするチェックを追加。
