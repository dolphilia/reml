...
## 背景と狙い
調査メモ `docs/notes/parser/core-parse-improvement-survey.md` では、DSL 実装で頻出する字句処理を `Core.Parse` 側で支援することを推奨している。
...
- 産物（ドキュメント化）
  - 「サンプルが自前実装しているヘルパ一覧」→「Lex 既存 API」→「不足/過剰/重複」の対応表（本計画へ追記、または `docs/notes/parser/core-parse-api-evolution.md` に記録）
...
#### サンプル横断置換方針（確定）
- `lex_pack` をサンプル共通の入口で生成し、`RunConfig.extensions["lex"]` に `space_id/profile/identifier_profile/layout_profile/safety` を格納する。CLI/LSP/回帰が同じトリビア設定を再構成できるようにする。
- `identifier/number/string` はプリセット側で `label` を同梱し、サンプルの自由文 `Parse.fail` を 2-5 準拠の構造化エラーへ寄せる（`Expectation::Class/Rule` を維持）。オーバーフロー・不正値は 2-3 §E-1 の診断変換を用いる。
- `keyword` の大文字小文字無視は **オプションの糖衣**（`keyword_ci(space, kw)` として LexPreset に追加）で提供し、SQL の境界判定を置換。既定はケースセンシティブを維持する。
- TOML ベアキー向けに `IdentifierProfile::toml_key`（`-`/`_` 許容、ASCII/Unicode 両用、NFC/Bidi 安全）を追加し、`ConfigTriviaProfile::toml_relaxed` と組み合わせたプリセットを利用可能にする。
- LayoutProfile は **opt-in 維持**。YAML などオフサイド系サンプルのみ `layout_profile` を RunConfig に渡すが、既定のプリセット適用範囲には含めない（他サンプルへの影響を避ける）。

### Step 2: ヘルパ群を “利用者が迷わない形” に整備する（仕様とサンプルの一致）
ここでは実装追加そのものではなく、計画として「何を揃えると実用になるか」を具体化する。Step2 の結論を基に Step3（サンプル整備）を進める。

#### Step2 で決めたこと
- 最低限揃えるヘルパ（優先順）の粒度とラベル運用を固定した。
  1) `lexeme` / `symbol` / `keyword` は **lex_pack エントリ**で共有し、`RunConfig.extensions["lex"].space_id` を必ず埋める（`with_space` 二重適用を防ぐ）。
  2) `identifier` は `identifier(DefaultId).label("identifier")` をプリセット既定とし、`reserved(profile, set)` で予約語衝突を吸収する（WS2 推奨語彙に一致）。
  3) `number` は `intAuto` / `float` を `label("number")` 付きで束ね、値化失敗は 2-3 §E-1 の診断変換に寄せる（`Expectation::Class("number")` を維持）。
  4) `stringLiteral`（`stringLit`/`stringRaw`/`stringMultiline`）は `label("string")` を付け、エスケープ位置の精度を優先する。
- エラー品質の統一ルールを **プリセット側で強制**する。
  - `identifier`/`number`/`string` の `label` はプリセットに内蔵し、サンプルは追加ラップ不要とする。これにより自由文 `Parse.fail` を排除し、`Expectation::{Keyword,Class,Rule}` へ統一する。
  - `keyword`/`keyword_ci`（後述）は `Expectation::Keyword` を出し、境界判定は Lex 側で一貫させる（Rule 名には頼らない）。
- `lex_pack` の標準形を定義し、RunConfig 連携を必須化する。
  - `LexPack` は `space`（Parser<()>）, `lexeme`, `symbol`, `keyword`, `keyword_ci`, `identifier`, `number`, `string`, `profile`, `identifier_profile`, `layout_profile`, `safety`, `space_id` を保持する record として整理する。
  - エントリポイント（サンプル共通関数）で `lex_pack(profile, identifier_profile, layout_profile, safety)` を生成し、`RunConfig.extensions["lex"]` に **すべてのキー**を格納する。`RunConfig` が無指定の場合は `ConfigTriviaProfile::strict_json` + `IdentifierProfile::default` + layout 無効 + safety オンを採用。
- case-insensitive keyword の糖衣を **LexPreset オプションとして追加**する。
  - 名称: `keyword_ci(space, kw)`（`LexPreset::core` 内の optional API）。`Expectation::Keyword` を保持しつつ `identifier_profile` による境界判定を共有する。SQL サンプルの `string_case_insensitive + notFollowedBy(alnum)` を置換する。
- TOML ベアキー向けプロファイルを **プリセットで吸収**する。
  - `IdentifierProfile::toml_key` を追加し、`-`/`_` を許容しつつ NFC/Bidi/Confusable 安全を維持する。`ConfigTriviaProfile::toml_relaxed` と組み合わせた `lex_pack_toml()` ショートカットを用意し、RunConfig へ `profile="toml_relaxed"`・`identifier_profile="toml_key"` を書き戻す。
- LayoutProfile の扱いを **opt-in 明文化**とし、非オフサイド系への影響を遮断する。
  - 既定は layout 無効。YAML 系のみ `layout_profile` を RunConfig に格納し、`lex_pack` から `with_space` を渡す際に `autoWhitespace` と二重適用しないよう `space_id` を照合する。Layout 由来の token 化は Phase9 までプリセット外とする。

#### Step2 の出口（成果物と次のアクション）
- 仕様サイドの追記方針
  - `docs/spec/2-3-lexer.md` に `lex_pack` 標準形（保持フィールドと RunConfig 共有キー）、`keyword_ci` の糖衣、`IdentifierProfile::toml_key` を追記する。
  - `docs/spec/2-2-core-combinator.md` の `with_space/autoWhitespace` 参照箇所に「`space_id` 共有が前提」の文言を補強し、`label` 付与済みリテラルヘルパの利用例を短く追加する（WS2 の推奨語彙と連動）。
- サンプル／回帰への適用順（Step3 で実施）
  1) `basic_interpreter_combinator.reml`: `lex_pack` 入口を導入し、`identifier/number/string` の `label` 同梱版へ差し替え。自由文 `Parse.fail` を構造化診断へ置換。
  2) `sql_parser.reml`: `keyword_ci` を用いた境界判定へ置換し、予約語拒否を `reserved` で吸収。`RunConfig` へ `space_id/profile/identifier_profile/safety` を書き戻す。
  3) `toml_parser.reml`: `lex_pack_toml()` を導入し、ベアキーを `IdentifierProfile::toml_key` に寄せる。複数行文字列はプリセットへ切替。
  4) `yaml_parser.reml`: Layout 連携を opt-in で試験し、`space_id` 共有だけを先行反映。Layout token 生成は後続（Phase9）へ残す。
- 診断・メトリクスへの影響
  - `label("identifier"|"number"|"string")` 内蔵化により、WS2 の期待語彙を回帰で固定しやすくなる。`phase4-scenario-matrix.csv` へ `CP-WS3-001`（lexeme/space 共有）を追加する際は `space_id` 共有の有無を確認項目に入れる。
  - `keyword_ci` 導入で `Expectation::Keyword` 表示に揺れがないか、SQL サンプルで CLI/LSP 両方の humanized をスポット確認する。
...
- 優先サンプル（lex_pack + ラベル化の適用候補）
  - `basic_interpreter_combinator.reml`: `lexeme/symbol/keyword` 再定義と自由文エラーをプリセット + 構造化エラーへ置換。
  - `sql_parser.reml`: case-insensitive `keyword` 糖衣、予約語チェックを `reserved`/`identifier_profile` へ寄せ、RunConfig 共有キーを導入。
  - `toml_parser.reml`: `ConfigTriviaProfile::toml_relaxed` とベアキー用 `IdentifierProfile` を試験導入し、複数行文字列をプリセット利用へ置換。
  - `yaml_parser.reml`: LayoutProfile との連携可否を検証しつつ、`space_id`/`lexeme` の共有で回帰観点を固定。
