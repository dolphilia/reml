...
- 具体タスク（今回の検討テーマ）
  - `lex_pack` 形式で `RunConfig.extensions["lex"]` へ `space_id/profile/identifier_profile/layout_profile/safety` を注入する導線をサンプルに共通化（autoWhitespace/LSP と再連携）。
  - `identifier/number/string` のプリセットに `label` を同梱し、サンプル側の自由文 `Parse.fail` を 2-5 の構造化エラー（`Expectation::{Class,Rule}` と診断キー）へ置換する方針を定義。
  - `keyword` の大文字小文字無視（case-insensitive）糖衣を LexPreset（または Option）として検討し、SQL サンプルの自前境界判定を置換できるか評価。
  - TOML ベアキー向け `IdentifierProfile`（`-`/`_` 許容）と `ConfigTriviaProfile::toml_relaxed` を組み合わせたプリセットの可否を決める。
  - LayoutProfile の opt-in 連携をどこまで標準に含めるかを明文化（オフサイド不要なサンプルへの影響を最小化）。
