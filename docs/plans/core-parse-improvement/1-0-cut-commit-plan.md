...
- 既存サンプルの現状確認（「どこで attempt に頼っているか」）
  ...
  - `examples/language-impl-comparison/reml/toml_parser.reml`（曖昧な字句分岐での `attempt`、`[`/`{` 開始枝の扱い）
  - `examples/language-impl-comparison/reml/yaml_parser.reml`（構造 vs スカラー分岐、`lookahead` と Cut の必要性）
...
**サンプル側で観測された現状**
- PL/0（`examples/language-impl-comparison/reml/pl0_combinator.reml`）
...
- JSON（`examples/language-impl-comparison/reml/json_parser_combinator.reml`）
...
- TOML（`examples/language-impl-comparison/reml/toml_parser.reml`）
...
- YAML（`examples/language-impl-comparison/reml/yaml_parser.reml`）
...
- **一意トークンの直後**：`[`/`{`/`:`/`=>` などを消費したら「ここからはその構文」と確定し、以降は別枝へ逃げない（特に `attempt(alt)` を安易に広げない）
- **曖昧な入口は attempt、確定後は cut**：キーワード境界や共通接頭辞（`<` と `<=` など）で巻き戻しが必要な場合は入口側に `attempt` を置き、確定地点（閉じ括弧、右項開始、区切り）で `cut_here()` に切り替える

**JSON/YAML 向けの境界例（Step1 へ渡す“型”）**

ここでは、WS1 の棚卸しから得られた「よく間違える置き方」を、**トークン境界として明示**する。
Step1 ではこの節を短く圧縮し、`docs/spec/2-2-core-combinator.md` の指針へ転写する。

- **JSON: `[` を読んだら配列として確定**（`[` の後で value 側へ戻らない）

  ```reml
  // 例: `[` を消費したら、その先は array 文脈で失敗を報告する
  let array =
    sym("[")
      .then(cut_here())
...
- **JSON: `{` を読んだらオブジェクトとして確定**

  ```reml
  let object =
    sym("{")
      .then(cut_here())
      .then(sepBy(member, sym(",")))
      .then(expect("'}'", sym("}")))
  ```

- **JSON: `:` の直後で文脈を確定**（`key:` の後は「値が必要」へ収束させる）

  ```reml
  let member =
    string_lit
      .then(sym(":").then(cut_here()))
      .then(value)
  ```

- **JSON: `,` の直後で要素継続を確定**（`[x, <here>]` は「要素が必要」へ）

  ```reml
  // 末尾区切り許容をしない場合の典型:
  // `,` を読んだら "次の要素" を期待する（value へ戻るが、配列文脈は維持）
  let elements =
    value.then(many(sym(",").then(cut_here()).then(value)))
  ```

- **YAML: `-` を見たら（list item なら）リストとして確定**（入口 attempt を広げすぎない）

  ```reml
  // lookahead で list 開始を判定し、確定後は cut
  let list_item =
    expect_indent(indent)
      .then(lookahead(Lex.string("-")))
      .then(Lex.string("-").then(cut_here()))
      .then(hspace)
      .then(value)
  ```

- **YAML: `:` を消費したら map entry として確定**（`key:` 後は「値」へ）
