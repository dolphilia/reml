...
- [4-3-developer-toolchain.md](../../spec/4-3-developer-toolchain.md) に沿ったツールチェーン整備（パッケージ管理、CI/CD、診断集計）を完了させる。
...
2. **マルチターゲットリリースパイプライン構築**
   - [4-3-developer-toolchain.md](../../spec/4-3-developer-toolchain.md) の手順に従い、ビルド→テスト→署名→配布までを自動化。
   ...
   - 公式リリースノートフォーマット（[4-5-roadmap-metrics.md](../../spec/4-5-roadmap-metrics.md) 参照）でセルフホスト移行の進捗を報告。
3. **ドキュメント更新**
...
4. **エコシステム整備**
   - パッケージレジストリ ([4-2-registry-distribution.md](../../spec/4-2-registry-distribution.md)) にセルフホスト版を登録し、**3 ターゲット全てのバイナリを配布**。x86_64 Linux を推奨ターゲットとして明示。OCaml 版を非推奨マーク。
