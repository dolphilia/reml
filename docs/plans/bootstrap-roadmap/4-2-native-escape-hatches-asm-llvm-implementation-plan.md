...
2. `effect {native, unsafe}` と Capability / 監査ログを統合し、移植性と安全性のリスクを明確化する。
3. LLVM IR 直書きの構文・検証ルールを整理し、`verify_llvm_ir` での検証に耐える形で実装する。

## 現状調査（Rust 実装の状態）
- **フロントエンド**: `compiler/rust/frontend/src/parser/mod.rs` には Inline ASM / LLVM IR 直書きの構文が存在しない。`@unstable("inline_asm")` は属性として保持されるのみで、専用の診断や型検証がない。
- **Typed/MIR**: `compiler/rust/frontend/src/semantics/typed.rs` / `compiler/rust/frontend/src/semantics/mir.rs` に Inline ASM / LLVM IR の式ノードが存在しない（`MirExprKind` は `Unknown` で退避）。
- **バックエンド**: `compiler/rust/backend/llvm/src/unstable.rs` が `unstable:inline_asm` / `unstable:llvm_ir` を検出し、`verify.rs` が `native.unstable.disabled` を出すのみ。IR 生成は未実装。
...
## 設計方針
- **安全性**: Inline ASM / LLVM IR は `effect {native, unsafe}` を必須とし、`unsafe` ブロック外での利用を禁止する。
...
- `docs/spec/1-3-effects-safety.md` に `effect {native, unsafe}` と `@cfg` 要件を追記。
...
3. `docs/spec/1-3-effects-safety.md` に `effect {native, unsafe}` と `@cfg` 要件を追加する。
...
### フェーズB: フロントエンド構文と型検証
1. `compiler/rust/frontend/src/parser/mod.rs` に Inline ASM / LLVM IR の式構文を追加する。
2. `compiler/rust/frontend/src/semantics/typed.rs` と `compiler/rust/frontend/src/semantics/mir.rs` に式ノードを追加する。
3. `compiler/rust/frontend/src/typeck/driver.rs` で型制約を検証する。
   - `effect {native, unsafe}` がない場合は `native.inline_asm.missing_effect` / `native.llvm_ir.missing_effect`
...
### フェーズC: MIR / 変換パス拡張
1. `compiler/rust/frontend/src/semantics/mir.rs` の JSON 形式に Inline ASM / LLVM IR ノードを追加する。
2. `compiler/rust/backend/llvm/src/integration.rs` の MIR ローダに新ノードを反映する。
3. 新ノードに合わせた `MirExprKind` を `compiler/rust/backend/llvm/src/codegen.rs` に追加する。
...
2. LLVM IR 直書きのテンプレートを構文解析し、SSA 名の衝突を防ぐ。
   ...
   - `llvm-as` / `opt -verify` に通らない場合は `native.llvm_ir.verify_failed` を出す。
3. `compiler/rust/backend/llvm/src/verify.rs` に専用診断を追加し、監査ログへメタデータを出力する。

### フェーズE: 監査ログ / Capability / Runtime 連携
1. `compiler/rust/runtime/src/native` に Inline ASM / LLVM IR の監査メタデータ収集を追加する。
...
- [ ] `docs/spec/1-3-effects-safety.md` に `effect {native, unsafe}` と `@cfg` 要件を追加
...
| `effect {native, unsafe}` の乱用 | 安全性の崩壊 | `native.inline_asm.used` / `native.llvm_ir.used` を必須監査キー化し、Capability でゲート |
