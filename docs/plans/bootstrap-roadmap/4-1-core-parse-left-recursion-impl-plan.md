...
1. **CP-WS6-001（左再帰検出）の実行と確認**
   - 実行コマンド（例）:
     - `compiler/rust/frontend/target/debug/reml_frontend --output json --packrat --left-recursion off examples/spec_core/chapter2/parser_core/core-parse-left-recursion-direct.reml`
   - 期待条件:
     - 診断コードに `E4001` が含まれる。
...
- ✅ `phase4-scenario-matrix.csv` の `CP-WS6-002` を `ok` へ更新済み。
- ⏳ `CP-WS6-001` は `pending` へ戻し、左再帰検出（`E4001`）の実装/検証を優先する。
...
## 未着手・残タスク（推奨順）
1. `RunConfig.left_recursion="off"` で左再帰検出（`E4001`）が発火する実装を追加し、`CP-WS6-001` の CLI で診断を確認する。
2. `phase4-scenario-matrix.csv` の `CP-WS6-001` を `ok` へ戻し、`resolution_notes` を最新ログに更新する。
...
## リスクと対策
- **左再帰検出が未実装**: `E4001` を出せない場合、`RunConfig.left_recursion="off"` の検出パスを実装し、診断キーを固定する。
- **profile 出力が出ない**: `Parse.run` の profile 出力経路を CLI が拾うよう調整し、`profile_output` を生成させる。
