...
| --- | --- | --- | --- | --- | --- | --- | --- |
| UCNF-29 | UAX #29 rev.40 | `segment_graphemes`, `TextBuilder::finish` | `tests/data/unicode/segment/grapheme_break_test.txt` | `cargo test grapheme_conformance -- --ignored` | `100% pass` | 進行中 | Unicode 15.1 の GraphemeBreakTest 抜粋を用意済み。全量同期は Phase4 TODO。`docs/notes/unicode-upgrade-log.md` と同期 |
