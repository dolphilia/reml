...
| InvalidIdentifier | `ParseErrorKind::InvalidToken` | `DiagnosticCode::unicode.invalid_identifier` | n/a | `prepare_identifier` の `SpanTagged` から継承 | `unicode.error.kind=invalid_identifier`, `unicode.identifier.raw` | `compiler/rust/frontend/tests/lexer_unicode_identifier.rs` | Green | `prepare_identifier` 経由のエラーを FrontendDiagnostic で検証済み |
