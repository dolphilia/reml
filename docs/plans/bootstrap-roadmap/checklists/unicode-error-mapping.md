...
| InvalidIdentifier | `ParseErrorKind::InvalidToken`（`prepare_identifier` が `identifier` プロファイルで失敗した場合） | `DiagnosticCode::unicode.invalid_identifier` | n/a | `prepare_identifier` の `SpanTagged` から継承 | `unicode.error.kind=invalid_identifier`, `unicode.identifier.raw` | `compiler/rust/frontend/tests/lexer_unicode_identifier.rs`, `reports/spec-audit/ch1/lexer_unicode_identifier-20270329.json` | Green | 非正規化/禁止 Bidi/UnsupportedLocale を 10 ケース以上で検証、`span`/`audit_metadata` の整合を確認 |
| UnsupportedLocale | `ParseErrorKind::UnicodeOption` | `DiagnosticCode::unicode.unsupported_locale` | `IOErrorKind::UnsupportedLocale` | `LocaleId` 宣言箇所の `Span`（`token.span`） | `unicode.locale.requested`, `unicode.locale.supported[]` | `compiler/rust/runtime/tests/unicode_case_width.rs` | In Progress | `text-locale-support.csv` と同期。Parser との統合テストは `examples/core-text/locale_cases.reml` 追加後に実施 |
