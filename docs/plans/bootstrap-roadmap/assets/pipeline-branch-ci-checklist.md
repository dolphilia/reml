...
| 4 | 監査メトリクス | `python3 tooling/ci/collect-iterator-audit-metrics.py --section core_diagnostics --scenario pipeline_branch --diagnostic-source ...pipeline_branch.expected.diagnostic.json --audit-source ...pipeline_branch.expected.audit.jsonl --require-success` | `reports/spec-audit/ch3/pipeline_branch-metrics-YYYYMMDD.json` に `diagnostics=1` / `audit_events=2` / `capability.id=console` を保存し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md#core_diagnostics` の KPI に追記 | `--scenario pipeline_branch` は `core_diagnostics` セクションへ追加しておき、未登録の場合は `collect-iterator-audit-metrics` へ PR を出す。 |
