...
## 既知のギャップ
- ✅ `EffectSet`/`CollectorEffectMarkers` へ `effect {time}` フラグと `time_calls` カウンタを追加済み（`compiler/rust/runtime/src/prelude/iter/mod.rs`, `compiler/rust/runtime/src/prelude/collectors/mod.rs`）。`collector.effect.time` / `collector.effect.time_calls` が診断・監査ログに出力され、`collect-iterator-audit-metrics.py` の `collector.effect.audit_snapshot` 経由で参照できる。
