...
| `fs.watcher.recursive` | `watch_with_limits` の `max_depth`, `exclude_patterns` | `StageRequirement::Exact(StageId::Stable)`（macOS のみ Beta） | `WatchLimits.max_depth` が `>0` の場合に `RecursiveMode::Recursive` で watch。`globset` による除外パターンとリソース制限メモ | `io/watcher.rs` 内で `WatcherAdapter` を抽象化し、`fs.watcher.recursive` Stage を `watch_with_limits` で条件付き要求 | `collect-iterator-audit-metrics.py --section core_io --scenario watcher_audit`、`tests/watcher.rs::watch_with_limits_rejects_invalid_path` | In Progress（Rust runtime、Watcher 限定 Stage テスト） |
