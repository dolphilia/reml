...
| 安全性 | `capability.handle_coverage` | `docs/plans/bootstrap-roadmap/assets/capability-handle-inventory.csv` に登録された 14 行のうち、`status` が「未実装」以外の割合（Stage 検証 PoC などを含む）。Run ID と差分は同 CSV および `docs/notes/runtime-capability-stage-log.md#capability-handle-inventory` に記録する。 | スプリントレビュー（Phase3 W56 以降） | [3-8-core-runtime-capability-plan.md](./3-8-core-runtime-capability-plan.md)#1, [assets/capability-handle-inventory.csv](assets/capability-handle-inventory.csv), [docs/notes/runtime-capability-stage-log.md](../../docs/notes/runtime-capability-stage-log.md) |
