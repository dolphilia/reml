...
本章では Phase 1〜4 に共通する測定指標、診断と監査ログの収集方法、レビュー記録フォーマットを定義する。[3-6-core-diagnostics-audit.md](../../spec/3-6-core-diagnostics-audit.md) と `docs/notes/backend/llvm-spec-status-survey.md` のフォーマットを継承し、各フェーズの完了条件を定量的に確認できるようにする。
...
| 安全性 | `diagnostic.field_coverage` | `docs/plans/bootstrap-roadmap/assets/diagnostics-field-gap.csv` の必須フィールドが「要補完」「未実装」でない割合。Run ID とギャップ一覧を CSV へ記録する。 | スプリント開始時（Phase3 W50 以降） | [3-6-core-diagnostics-audit-plan.md](3-6-core-diagnostics-audit-plan.md) §1.1 |
| 安全性 | `diagnostic.span_highlight_pass_rate` | `Core.Text span_highlight` 単体テスト（`cargo test --manifest-path compiler/rust/runtime/Cargo.toml span_highlight`）でマルチバイト書記素を含むハイライトが 100% 成功する割合。`reports/dual-write/front-end/rust/20290705-span-highlight.md` にログを保存し、CLI/JSON の `primary.highlight` と連携する。 | 週次レビュー（Phase3 W50 以降） | [3-6-core-diagnostics-audit-plan.md](3-6-core-diagnostics-audit-plan.md) §2.1, [reports/dual-write/front-end/rust/20290705-span-highlight.md](../../reports/dual-write/front-end/rust/20290705-span-highlight.md) |
| 安全性 | `capability.handle_coverage` | `docs/plans/bootstrap-roadmap/assets/capability-handle-inventory.csv` に登録された 14 行のうち、`status` が「未実装」以外の割合（Stage 検証 PoC などを含む）。Run ID と差分は同 CSV および `docs/notes/runtime/runtime-capability-stage-log.md#capability-handle-inventory` に記録する。 | スプリントレビュー（Phase3 W56 以降） | [3-8-core-runtime-capability-plan.md](./3-8-core-runtime-capability-plan.md)#1, [assets/capability-handle-inventory.csv](assets/capability-handle-inventory.csv), [docs/notes/runtime/runtime-capability-stage-log.md](../../docs/notes/runtime/runtime-capability-stage-log.md) |
| 安全性 | `runtime.capability_validation` | `python3 tooling/ci/collect-iterator-audit-metrics.py --section runtime --dry-run` の `runtime.capability_validation` エントリで `stage_summary.runtime_candidates[*].stage` と `validation.status` が揃っている割合。結果は `docs/plans/bootstrap-roadmap/assets/metrics/runtime-capability-stage.csv` に追記し、`reports/runtime-capabilities-validation.json` をソースとして保存する。 | 週次レビュー / Phase3 `runtime` ジョブ | [3-8-core-runtime-capability-plan.md](./3-8-core-runtime-capability-plan.md)#1.2, [assets/metrics/runtime-capability-stage.csv](assets/metrics/runtime-capability-stage.csv), [reports/runtime-capabilities-validation.json](../../reports/runtime-capabilities-validation.json) |
| 安全性 | `runtime.capability_ci_pass_rate` | `.github/workflows/rust-runtime.yaml` で実行する `runtime-capability-unit` / `runtime-capability-nextest` / `core-runtime-capability-suite` 各ジョブの成功率。結果は `docs/plans/bootstrap-roadmap/assets/metrics/runtime-capability-ci.csv` へ追記し、`collect-iterator-audit-metrics.py --section runtime --scenario capability_registry` のサマリ (`reports/spec-audit/ch3/runtime_capability-ci-summary-YYYYMMDD.json`) とあわせて管理する。CI 失敗時は `scripts/ci/post_failure_runtime_capability.sh` が `docs/plans/bootstrap-roadmap/0-4-risk-handling.md#runtime-capability` を更新する。 | CI ごと / 週次レビュー | [3-8-core-runtime-capability-plan.md](./3-8-core-runtime-capability-plan.md)#7.3, [assets/metrics/runtime-capability-ci.csv](assets/metrics/runtime-capability-ci.csv), [docs/plans/bootstrap-roadmap/pipeline_branch-stage-mismatch-plan.md](./pipeline_branch-stage-mismatch-plan.md)#7 |
...
| コレクション | `cell_mutations_total` | `CollectOutcome` / `AuditEnvelope.metadata` に記録された `collector.effect.cell=true` の件数。`python3 tooling/ci/collect-iterator-audit-metrics.py --suite collectors --thresholds docs/plans/bootstrap-roadmap/assets/metrics/core_collections_thresholds.json --scenario ref_internal_mutation --require-cell` を実行し、ゼロまたは `metric_missing` が記録された場合はブロッカー扱いとする。 | CI（`tooling/ci/collect-iterator-audit-metrics.py --suite collectors --thresholds docs/plans/bootstrap-roadmap/assets/metrics/core_collections_thresholds.json --scenario ref_internal_mutation --require-success --require-cell`） | [3-2-core-collections-plan.md](./3-2-core-collections-plan.md) §3.2、および `assets/metrics/core_collections_persistent.csv` の `RefInternalMutation` 行 |
| コレクション | `ref_borrow_conflict_rate` | `CollectError::BorrowConflict` を `collector.effect.rc=true` ケースで発生させた割合。`collector.effect.cell_rc` メトリクスから `borrow_conflicts / rc_ops_total` を算出し、CSV にも記録する。`thresholds.json` で `ref_borrow_conflict_rate <= 0.35` を下限とし、超過した場合は CI の `enforcement.failures[]` へ書き出す。 | 週次レビュー時に `python3 tooling/ci/collect-iterator-audit-metrics.py --suite collectors --thresholds docs/plans/bootstrap-roadmap/assets/metrics/core_collections_thresholds.json --scenario ref_internal_mutation --output reports/iterator-collector-metrics.json --require-cell` を実行してログを更新 | [3-2-core-collections-plan.md](./3-2-core-collections-plan.md) §3.2 および `assets/metrics/core_collections_persistent.csv` |
| コレクション | `table_insert_throughput` | `TableCollector` + `EffectfulTable` を用いた `collect_table_csv` ケースで 1 秒あたりに処理できた挿入件数。`collect-iterator-audit-metrics.py --suite collectors --scenario table_csv_import` の `collector.table.csv_import.table_insert_throughput` を取得し、`thresholds.json` で `>= 1000 insert/sec` を要求する。 | CI（Phase3 self-host 判定 / 週次レビュー。`collect_table_csv` スナップショットが存在しない間は `threshold metric missing` を追跡し続ける） | [3-2-core-collections-plan.md](./3-2-core-collections-plan.md) §3.3 |
| コレクション | `csv_load_latency` | `Table.load_csv` が `Core.IO.CsvReader` から 1,000 行 CSV をロードする際の平均遅延。`collect-iterator-audit-metrics.py --suite collectors --scenario table_csv_import` の `collector.table.csv_import.csv_load_latency_ms` を確認し、`<= 250ms` を目標値とする。診断検証自体は `scripts/validate-diagnostic-json.sh --suite collectors --pattern csv_load` で補完する。 | `cargo test --manifest-path compiler/rust/frontend/Cargo.toml core_collections_table` 完走後 / フェーズマイルストーン | [3-2-core-collections-plan.md](./3-2-core-collections-plan.md) §3.3, [3-5-core-io-path-plan.md](./3-5-core-io-path-plan.md) |
| テキスト | `text.mem.zero_copy_ratio` | `Bytes::from_vec` / `String::into_bytes` / `TextBuilder::finish` など `Vec<u8>` をムーブする経路で `collector.effect.transfer=true` かつ `collector.effect.mem_bytes=0` になった割合（0.0〜1.0）。`python3 tooling/ci/collect-iterator-audit-metrics.py --section text --scenario bytes_clone --text-mem-source reports/text-mem-metrics.json --output reports/text-mem-metrics.json --require-success` を実行し、ZeroCopy 入力バイト数 / 全監視入力バイト数を記録する。2027-03-31 時点で `EffectSet::mark_transfer`（`compiler/rust/runtime/src/prelude/iter/mod.rs`）と `collector.effect.transfer` の計測を Rust Runtime に実装済み。 | CI（Phase3 Week41 以降の `phase3-core-text` ジョブ、週次レビュー） | [3-3-core-text-unicode-plan.md](./3-3-core-text-unicode-plan.md) §1.2, [docs/notes/text/text-unicode-ownership.md](../../docs/notes/text/text-unicode-ownership.md) |
...
| テキスト | `text.grapheme.script_mix_ratio` | `reports/spec-audit/ch1/core_text_grapheme_stats.json` の UC-02 行を解析し、`script_mix_ratio >= 0.55` かつ `rtl_ratio >= 0.4` であることを確認する指標。`python3 tooling/ci/collect-iterator-audit-metrics.py --section text --scenario grapheme_stats --text-source reports/spec-audit/ch1/core_text_grapheme_stats.json --require-success --check script_mix` を実行し、`primary_script`/`rtl_ratio` が欠落または閾値未満の場合は CI を失敗させる。 | UC-02/UC-03 再計測時（`text_internal_cache` テスト完走後） | [docs/plans/bootstrap-roadmap/3-3-core-text-unicode-plan.md](./3-3-core-text-unicode-plan.md) §2.2, [docs/notes/text/text-unicode-segmentation-comparison.md](../notes/text-unicode-segmentation-comparison.md), [docs/notes/text/text-unicode-ownership.md](../notes/text-unicode-ownership.md) |
...
| IO/Path | `core_io.capability_matrix_pass_rate` | `docs/plans/bootstrap-roadmap/assets/core-io-capability-map.md` の `<!-- capability-matrix:start -->` 表を `python3 tooling/ci/collect-iterator-audit-metrics.py --section core_io --scenario capability_matrix --matrix docs/plans/bootstrap-roadmap/assets/core-io-capability-map.md --output reports/spec-audit/ch3/core_io_capabilities.json --require-success` で検証し、各 Capability 行に `Stage`/`Provider`/`Effect Scope`/`Hook` が揃っている割合。`Status=missing/blocked` の行が残っている場合は即座に失敗とし、Run ID と `failures[]` を `docs/notes/runtime/runtime-capability-stage-log.md#2025-12-06-coreio-capability-マトリクスrun-id-20251206-core-io-capability-matrix` に記録する。 | Phase3 `core-io-path` ジョブ（CI Nightly）および週次レビュー | [3-8-core-runtime-capability-plan.md](./3-8-core-runtime-capability-plan.md)#5.5, [assets/core-io-capability-map.md](assets/core-io-capability-map.md), [reports/spec-audit/ch3/core_io_capabilities.json](../../reports/spec-audit/ch3/core_io_capabilities.json) |
| Diagnostics | `core_diagnostics.pipeline_branch_stage_pass_rate` | `python3 tooling/ci/collect-iterator-audit-metrics.py --section core_diagnostics --scenario pipeline_branch --diagnostic-source examples/core_diagnostics/pipeline_branch.expected.diagnostic.json --audit-source examples/core_diagnostics/pipeline_branch.expected.audit.jsonl --require-success` の結果として `reports/spec-audit/ch3/pipeline_branch-metrics-YYYYMMDD.json` に保存する pass_rate。`tooling/examples/run_examples.sh --suite core_diagnostics --with-audit --update-golden` が `allowed failure` を抱えていても完走すること、`pipeline_branch.expected.*` に `capability.id` / `effect.stage.*` / `pipeline.*` が揃っていることをあわせて確認する。 | Core.Diagnostics nightly / 週次レビュー | [pipeline_branch-stage-mismatch-plan.md](./pipeline_branch-stage-mismatch-plan.md)#7, [assets/pipeline-branch-ci-checklist.md](assets/pipeline-branch-ci-checklist.md), [reports/spec-audit/ch3/capability_stage-mismatch-20251206.json](../../reports/spec-audit/ch3/capability_stage-mismatch-20251206.json) |
...
| IO/Path | `core_io.example_suite_pass_rate` | `tooling/examples/run_examples.sh --suite core_io` および `--suite core_path` を実行し、`cargo run --bin reml` が `examples/practical/core_io/file_copy/canonical.reml` / `examples/practical/core_path/security_check/relative_denied.reml`（旧 `examples/core_io` / `examples/core_path`）で成功した割合（0.0 or 1.0）。Reader/Writer/Path セキュリティの監査キー欠落が再発した場合はこの指標が 0.0 になり、`core-io-path-plan.md` §6 ドキュメント更新タスクの回帰として扱う。 | Phase3 `core-io-path` ジョブ（Nightly と PR の両方） | [3-5-core-io-path-plan.md](./3-5-core-io-path-plan.md)#6-ドキュメントサンプル更新49-50週目, [examples/practical/core_io/file_copy/canonical.reml](../../examples/practical/core_io/file_copy/canonical.reml), [examples/practical/core_path/security_check/relative_denied.reml](../../examples/practical/core_path/security_check/relative_denied.reml), [tooling/examples/run_examples.sh](../../tooling/examples/run_examples.sh) |
...
| 数値/時間 | `time.timezone.lookup_consistency` | `tests/data/time/timezone_cases.json` の `cases[].platform_offsets` と `offset_seconds` が一致しているかを `python3 tooling/ci/collect-iterator-audit-metrics.py --section numeric_time --scenario timezone_lookup --tz-source tests/data/time/timezone_cases.json --output reports/spec-audit/ch3/time_timezone_lookup.json --require-success` で検証する指標。Linux/macOS/Windows の offset 差異が検出された場合は `docs/notes/runtime/runtime-capability-stage-log.md` へ即時記録し、Capability Stage の再確認を行う。 | Phase3 `core-numeric-time` ジョブ（`timezone_lookup` シナリオ）＋週次レビュー | [docs/plans/bootstrap-roadmap/3-4-core-numeric-time-plan.md](./3-4-core-numeric-time-plan.md) §4.2, [compiler/rust/runtime/src/time/timezone.rs](../../compiler/rust/runtime/src/time/timezone.rs), [docs/notes/runtime/runtime-capability-stage-log.md](../../docs/notes/runtime/runtime-capability-stage-log.md) |
...
| 標準ライブラリ | `core_prelude.panic_path` | `compiler/rust/frontend/tests/panic_forbidden.rs` と `RUSTFLAGS="-Dnon-fmt-panics -Z panic-abort-tests"` で検出した `panic!`/`unwrap_unchecked` 経路数（`effect {debug}` 以外）。0 件が合格で、1 件以上は `6-5-backward-compat-checklist.md` の回帰手順で再検証する。 | Nightly CI + リリース前の `cargo test panic_forbidden`・`scripts/validate-diagnostic-json.sh` 実行後 | [1-3-effects-safety.md](../../spec/1-3-effects-safety.md), [3-1-core-prelude-iteration-plan.md](./3-1-core-prelude-iteration-plan.md) §2.3 |
...
| DX | `diagnostic.total_count` | `tooling/ci/collect-iterator-audit-metrics.py --section diagnostics` が算出する診断件数と Severity 比率（`info_fraction`/`hint_fraction`/`parser_expected_ratio` を含む）。Run ID ごとの結果は `docs/plans/bootstrap-roadmap/assets/metrics/diagnostics_ci.csv` に追記し、週次レビューで傾向を確認する。 | CI（`phase3-core-diagnostics` ジョブ、週次レビュー） | [3-6-core-diagnostics-audit-plan.md](./3-6-core-diagnostics-audit-plan.md) §5.3, [3-6-core-diagnostics-audit.md](../../spec/3-6-core-diagnostics-audit.md) |
| DX | `audit.event_total` | 同スクリプトが `--audit-source` で受け取った監査 JSON(L) のイベント総数および `category_counts`/`missing_event_annotations`。`diagnostics_ci.csv` の `audit_event_total` 列へ Run ID とともに保存し、イベント欠落や `event.kind` 未設定を即時検知する。 | CI（`phase3-core-diagnostics` ジョブ、週次レビュー） | [3-6-core-diagnostics-audit-plan.md](./3-6-core-diagnostics-audit-plan.md) §5.3, [3-6-core-diagnostics-audit.md](../../spec/3-6-core-diagnostics-audit.md) |
...
**Core.Collections KPI 閾値**: `docs/plans/bootstrap-roadmap/assets/metrics/core_collections_thresholds.json` に `collector.effect.cell_rc` と `collector.table.csv_import` の `min/max` 条件を登録し、`collect-iterator-audit-metrics.py --suite collectors --thresholds ...` 実行時に `combined.thresholds.results[]` を生成する。`--require-success` を併用した場合は閾値違反（または `metric_missing`）が `enforcement.failures[]` を通じて CI を失敗させるため、`RefInternalMutation` と `table_csv_import` の双方が揃うまでログをレビューに添付する。
...
- `--section diagnostics` を追加で実行し、`diagnostic.total_count` / `audit.event_total` を `docs/plans/bootstrap-roadmap/assets/metrics/diagnostics_ci.csv` へ追記する。CI から渡した `--audit-source` が欠落している場合は `status=missing` としてログに残し、`missing_event_annotations` が 0 でない Run ID は `3-6-core-diagnostics-audit-plan.md` §5.3 の TODO リストに移送する。Run ID / 日付 / profile / ソースパスを CSV へ記録し、週次レビューでは CSV の最新行と `reports/diagnostic-format-regression.md` を突合する。
- **診断フィールド差分レビュー**（Run ID: `20290601-diag-field-gap`）: `docs/plans/bootstrap-roadmap/assets/diagnostics-field-gap.csv` に `Diagnostic`/`AuditEnvelope`/`Telemetry` のギャップ一覧を追記。`FrontendDiagnostic` 未実装フィールドと `reports/dual-write/front-end/w4-diagnostics/...` の Run を紐付け、`diagnostic.field_coverage` の分母（13 項目）とステータスを更新した。
- **診断フィールド差分レビュー（追跡）**（Run ID: `20290615-diag-field-coverage`）: `FrontendDiagnostic::ensure_id`・`span_trace` 伝搬・`AuditEnvelope.audit_id` の UUID 化を実装し、CSV の `id`/`span_trace`/`extensions map`/`audit_id` を `整合` に更新。`diagnostic.field_coverage` は 13 項目中 9 項目で `整合/整備済み` となり、Parser 診断 JSON の `id`/`span_trace` が CLI/LSP/監査全経路で確認できる。
- **StageAuditPayload 再利用 & 期待集合整備**（Run ID: `20290620-stage-expected`）: `StageAuditPayload` を `diagnostic/effects.rs` へ移設し、CLI/TypeChecker/metrics で同じ Stage/Capability メタデータを共有。あわせて `TypecheckViolation::stage_mismatch`／`iterator_stage_mismatch` が `ExpectedTokenCollector` ベースのトップレベル宣言セットを出力するようにし、`effects-metrics.rust.err.log` の `bridge_stage.audit_presence` と `expected_tokens.diff.json`（`ffi_stage_messagebox` など）の差分解消を確認した。
- **span_highlight 実装確認**（Run ID: `20290705-span-highlight`）: `compiler/rust/runtime/src/text/span_highlight.rs` を追加し、書記素単位のハイライトを `reml_frontend` の `primary.highlight` に組み込んだ。`cargo test --manifest-path compiler/rust/runtime/Cargo.toml span_highlight` のログを `reports/dual-write/front-end/rust/20290705-span-highlight.md` に保存し、`diagnostic.span_highlight_pass_rate` のベースラインを 1.0 で登録した。
...
- **panic 経路監査**: `RUSTFLAGS="-Dnon-fmt-panics -Z panic-abort-tests" cargo test core_prelude_option_result panic_forbidden` を必須化し、`scripts/validate-diagnostic-json.sh` → `reports/diagnostic-format-regression.md` で差分が無いことを確認する。`core_prelude.panic_path = 0` のログは `reports/spec-audit/ch0/links.md#core_prelude` に貼り付け、失敗時は `6-5-backward-compat-checklist.md` の回帰項目へ移送する。
...
| W37 前半 | Chapter 1（構文・型・効果） | Rust Parser WG（#rust-frontend-parser） × Effects Taskforce（#effects-runtime） | `cargo run --manifest-path compiler/rust/frontend/Cargo.toml --bin poc_frontend -- --emit-diagnostics --input examples/docs-examples/spec/1-1-syntax/*.reml` | `reports/spec-audit/ch1/`, `docs/notes/process/spec-integrity-audit-checklist.md#phase-2-8` |
| W37 後半 | Chapter 2（Parser API / Streaming） | Parser API WG（#parser-api） × Diagnostics WG（#diagnostics-audit） | `compiler/rust/frontend/tests/streaming_runner.rs` の結果と Streaming JSON を `reports/spec-audit/ch2/streaming/*.json` へ保存 | `reports/spec-audit/ch2/`, `docs/notes/process/spec-integrity-audit-checklist.md#err-001` |
...
- 2027-01-19 / docs/plans/bootstrap-roadmap/3-1-core-prelude-iteration-plan.md:§2 / WBS 2.3b (`Option/Result` panic 排除) の完了確認として `core_prelude.missing_api = 0`・`core_prelude.panic_path = 0` を KPI 表へ登録し、`reports/spec-audit/ch0/core_prelude_kpi.csv` と `reports/spec-audit/ch0/links.md#core_prelude` に `cargo xtask prelude-audit --strict`・`cargo test panic_forbidden`・`scripts/validate-diagnostic-json.sh` のログを保存。`reports/diagnostic-format-regression.md` には差分なしでスナップショットを追記した。 / monitoring (Phase 3-1)
  ...
  - フォローアップ: `core_prelude.panic_path > 0` または `core_prelude.missing_api > 0` の場合は `6-5-backward-compat-checklist.md#core-prelude-regressions` と `0-4-risk-handling.md#core-prelude-guard` を即再開し、`reports/spec-audit/ch0/core_prelude_kpi.csv` へ失敗サンプルを残す。
...
- サンプル JSON（`SYNTAX-003` 計画書と `docs/notes/effects/effect-system-tracking.md` にも掲載）:
...
- 監視項目と閾値:
  ...
  - `diagnostics.plugin_bundle_ratio` ≥ 0.95: Plugin ドメイン診断で `extensions.plugin.bundle` と `AuditEnvelope.metadata["plugin.bundle.signature"]` が同時に出力されている割合。逸脱時は `docs/notes/dsl/dsl-plugin-roadmap.md` の診断連携節を参照し、バンドル署名の再発行を検討する。
