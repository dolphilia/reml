...
| 標準ライブラリ | `core_prelude.missing_api` | `docs/plans/bootstrap-roadmap/assets/prelude_api_inventory.toml` の `spec` 定義と Rust 実装の対応表で未実装となっている件数。0 件で合格、1 件以上は WBS 2.1b の未完了扱い。2025-11-20 実測値: 2（`Iter::try_fold`, `Iter::try_collect`）を [reports/spec-audit/ch1/iter.json](../../reports/spec-audit/ch1/iter.json) に記録。 | `cargo xtask prelude-audit --strict --wbs 2.1b --baseline docs/spec/3-1-core-prelude-iteration.md`（Nightly + リリース判定前） | [3-1-core-prelude-iteration.md](../../spec/3-1-core-prelude-iteration.md), [3-1-core-prelude-iteration-plan.md](./3-1-core-prelude-iteration-plan.md) §2.1 |
| 標準ライブラリ | `iterator.api.coverage` | Iter/Collector API の実装率（`implemented_entries / total_entries`）。2025-11-20 実測値: 0.9473（36 / 38）。 | `cargo xtask prelude-audit --section iter --strict --baseline docs/spec/3-1-core-prelude-iteration.md`（Nightly + リリース判定前） | [reports/spec-audit/ch1/iter.json](../../reports/spec-audit/ch1/iter.json), [reports/spec-audit/ch0/links.md](../../reports/spec-audit/ch0/links.md#iter-collector-remediation) |
...
- **Iter API 在庫カバレッジ**: `cargo xtask prelude-audit --section iter --strict --baseline docs/spec/3-1-core-prelude-iteration.md` を Nightly で実行し、`reports/spec-audit/ch1/iter.json` の `iterator.api.coverage`（例: 2025-11-20 = 0.9473 / pending: `Iter::try_fold`, `Iter::try_collect`）と `core_prelude.missing_api` を同時に監視する。差分や未実装が検出された場合は `docs/plans/bootstrap-roadmap/assets/prelude_api_inventory.toml` の該当行と `docs/plans/bootstrap-roadmap/3-1-core-prelude-iteration-plan.md` の補修タスクを更新し、`reports/spec-audit/ch0/links.md#iter-collector-remediation` に再実行ログを残す。
...
- 2025-11-19 / docs/plans/bootstrap-roadmap/3-1-core-prelude-iteration-plan.md:§3 / WBS 3.1b F2-3 にて `reports/iterator-collector-summary.md` を作成し、`collect_string_invalid` ケースで `collector.error.invalid_encoding` と `collector.effect.mem` の出現率を `reports/spec-audit/ch0/links.md#collector-f2` と合わせて記録。正常系 KPI `collector.error.invalid_encoding = 0` を維持するための検証手順を `docs/notes/core-library-outline.md` に追記。 / in_progress (Phase 3, M1)
