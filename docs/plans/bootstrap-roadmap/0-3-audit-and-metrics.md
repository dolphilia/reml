...
| Parser | `lexer.identifier_profile_unicode` | Unicode 識別子プロファイルが CLI/LSP/監査ログに揃って適用された割合（0.0〜1.0）。Phase 2-5 では ASCII 限定のため 0.0 を記録し、脚注とレビュー記録で制約を共有する。Phase 2-7 `lexer-unicode` 受け入れテストで 1.0 達成を確認する。 | Phase 2-7 受け入れテスト（`lexer-unicode`）と週次レビュー／Phase 2-5 は記録のみ | [2-3-lexer.md](../../spec/2-3-lexer.md) D-1, [docs/plans/bootstrap-roadmap/2-5-proposals/LEXER-001-proposal.md](./2-5-proposals/LEXER-001-proposal.md), [docs/plans/bootstrap-roadmap/2-5-review-log.md](2-5-review-log.md#lexer-001-step2-仕様脚注と索引の整備2026-02-18) |
