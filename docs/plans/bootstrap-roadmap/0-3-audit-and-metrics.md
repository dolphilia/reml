...
| コレクション | `cell_mutations_total` | `CollectOutcome` / `AuditEnvelope.metadata` に記録された `collector.effect.cell=true` の件数。`collect-iterator-audit-metrics.py --suite collectors --scenario ref_internal_mutation` の結果で測定し、ゼロの場合は実装不備とみなす。 | CI（`tooling/ci/collect-iterator-audit-metrics.py --suite collectors --scenario ref_internal_mutation --require-success`） | [3-2-core-collections-plan.md](./3-2-core-collections-plan.md) §3.2 |
| コレクション | `ref_borrow_conflict_rate` | `CollectError::BorrowConflict` を `collector.effect.rc` ケースで発生させた割合。`reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` の `ref_internal_mutation` セクションから `conflicts / total borrow_mut` を算出する。 | 週次レビュー時に `python3 tooling/ci/collect-iterator-audit-metrics.py --suite collectors --scenario ref_internal_mutation --output reports/iterator-collector-metrics.json` を実行 | [3-2-core-collections-plan.md](./3-2-core-collections-plan.md) §3.2 |
