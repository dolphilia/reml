...
### 2.2 Capability 連携
- `compiler/rust/runtime/src/io/buffered.rs` に `memory.buffered_io` Capability ID を取り込み、初期化時に `FsAdapter::global().ensure_resource_limit_capability()` を呼び `IoContext.with_capability` に設定。
