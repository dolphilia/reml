...
## Step3: Phase4 シナリオ（登録済み）
`docs/plans/bootstrap-roadmap/assets/phase4-scenario-matrix.csv` に、WS5 の計画起点 ID を追加した。

- `CP-WS5-001`（大入力オーダー異常）:
  - 入力: `examples/spec_core/chapter2/parser_core/core-parse-large-input-order.reml`
  - 期待（アンカー）: `expected/spec_core/chapter2/parser_core/core-parse-large-input-order.diagnostic.json`
  - 手順メモ（10MB 生成・観測）: `expected/spec_core/chapter2/parser_core/core-parse-large-input-order.expected.md`
