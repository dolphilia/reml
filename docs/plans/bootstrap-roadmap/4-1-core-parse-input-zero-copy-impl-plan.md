...
- 実行ログ（運用）:
  - `reports/spec-audit/ch4/logs/` に、CP-WS5-001/002 の再実行ログを残す（手順は Step3 で規定）

## 実装対象（Rust）
| 領域 | 主ファイル | 目的 |
| --- | --- | --- |
| `Input`/`Span`/位置更新 | `compiler/rust/runtime/src/parse/combinator.rs` | ゼロコピー前提・境界安全・位置算出のコストを制御する |
| Span ハイライト（列=グラフェム） | `compiler/rust/runtime/src/text/span_highlight.rs` | Unicode 行での下線/列計算を安定させる |
...
- 作業対象（例）:
  - `compiler/rust/runtime/src/parse/combinator.rs`（`Input` / `ParseState::new` / `run`）
- 作業案（優先順）:
...
- 作業対象（例）:
  - `compiler/rust/runtime/src/parse/combinator.rs`（`Input::remaining` / `Input::advance`）
- 作業案（例）:
...
- 作業対象（例）:
  - `compiler/rust/runtime/src/parse/combinator.rs`（位置更新）
  - `compiler/rust/runtime/src/text/span_highlight.rs`（ハイライト生成）
- Phase4 での最小方針:
...
- lldb で単体バイナリを実行すると `core::str::len` で EXC_BAD_ACCESS（stack guard 侵害）で停止。深い再帰/巨大スタックフレームが疑われるが、完全な backtrace 未取得。
- 状態: CP-WS5-001 の 10MB ケースは未固定。1MB までのログは `reports/spec-audit/ch4/logs/` に取得済み。

### 修正に向けた具体ステップ案
1. バックトレース確保  
   - `lldb compiler/rust/frontend/target/debug/reml_frontend -- --output json --emit-diagnostics <5mb>` を用い、`process handle SIGUSR1 -n false -p false -s false` 設定後に `thread backtrace all` を取得。再帰元/ループ箇所を特定。
2. ストリーミング/チャンク実験  
...
1. 生成→実行→ログ保存は `tooling/examples/gen_ws5_large_input.py` に集約する（手作業で 10MB ファイルを作らない）
   ...
   - 生成入力の出力先: `reports/spec-audit/ch4/generated/ws5/CP-WS5-001/`
   - 実行ログの出力先: `reports/spec-audit/ch4/logs/`（例: `spec_core-CP-WS5-001-10mb-YYYYMMDDTHHMMSSZ.diagnostic.json`）
...
- 完了判定（Phase4）:
  - 1KB/100KB/10MB の観測ログを `reports/spec-audit/ch4/logs/` に保存できる
