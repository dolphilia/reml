...
1. CP-WS5-001 のアンカー入力を 1KB/100KB/10MB に拡張したファイルを作り、JSON を保存する
   - 実行コマンド（例）: `cargo run --quiet --manifest-path compiler/rust/frontend/Cargo.toml --bin reml_frontend -- --output json <generated.reml> > <out.json>`
   - ログ置き場: `reports/spec-audit/ch4/logs/`（例: `spec_core-CP-WS5-001-10mb-YYYYMMDDTHHMMSSZ.diagnostic.json`）
2. `out.json` から `packrat_stats` と `farthest_error_offset` を目視で抜き、`expected/spec_core/chapter2/parser_core/core-parse-large-input-order.expected.md` に追記する
