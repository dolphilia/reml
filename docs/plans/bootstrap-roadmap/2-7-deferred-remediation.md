...
- **TODO: DIAG-RUST-05** — Streaming Meta ケース（`stream_pending_resume`, `stream_backpressure_hint`, `stream_checkpoint_drift`）を `w4-diagnostic-cases.txt` に実装し、`parser.stream.*` 系メトリクスの比較を Rust 実装でも実施できるようにする。`reports/dual-write/front-end/w4-diagnostics/20271107-w4-new/` では recover ケースのみで `parser.stream_extension_field_coverage < 1.0` が発生しており、非ストリーミング入力時のセクションスキップ可否も含めて整理する。関連: `appendix/w4-diagnostic-case-matrix.md`。
- **TODO: DIAG-RUST-06** — Effect/Capability/FFI 系ケース（`effect_residual_leak`, `effect_stage_cli_override`, `ffi_stage_messagebox` 等）を整備し、`effects.*` / `bridge.*` 拡張と audit ログの比較を Rust 版でも安定化させる。`type_condition_bool`（inline）で OCaml 側 JSON が生成されていない問題を再現手順付きで切り分け、`20271107-w4-new/type_condition_bool/` の空ファイルを参考に原因を調査する。
- **TODO: DIAG-RUST-07** — CLI RunConfig / LSP RPC ケース（`cli_packrat_switch`, `lsp_hover_internal_error` など）を LSP フィクスチャと `cases.txt` の両方で共有し、`--mode diag` で CLI/LSP の診断 JSON を同時比較できるようにする。`reports/dual-write/front-end/w4-diagnostics/README.md` の diag テーブルへ CLI/LSP 行を追加し、`appendix/w4-diagnostic-case-matrix.md` と同期させる。
