...
- **TODO: DIAG-RUST-05** — Streaming Meta ケース（`stream_pending_resume`, `stream_backpressure_hint`, `stream_checkpoint_drift`）を `w4-diagnostic-cases.txt` に実装し、`parser.stream.*` 系メトリクスの比較を Rust 実装でも実施できるようにする。`2027-11-09`: cases 登録と README 更新を完了。`2027-11-10`: `reports/dual-write/front-end/w4-diagnostics/20271110-w4-diag-naming-check/` で `collect-iterator-audit-metrics.py` が全ケース（例: `recover_missing_semicolon/parser-metrics.ocaml.err.log`）にて `parser.stream_extension_field_coverage < 1.0` で失敗し、OCaml/Rust 双方が `parser.stream.*` 拡張を一切出力していないことを確認。`scripts/poc_dualwrite_compare.sh` に `w4-diagnostic-cases.txt` の `# flags`（`--streaming`, `--stream-resume-hint diag-w4`, `--stream-flow-*` など）を伝播させ、Case ごとに CLI へ確実に適用するタスクを切り出す。対応後は同 Run ID でゲートを再実行し、`reports/dual-write/front-end/w4-diagnostics/README.md` の `parser_audit` 列を 1.0 / 1.0 へ引き上げる。`2027-11-12`: `reports/dual-write/front-end/w4-diagnostics/20271112-w4-diag-m1/stream_pending_resume/schema-validate.log` が `diagnostics[0].expected` 欠落で停止し、3 ケースすべて `parser_audit=0.0`（例: `reports/dual-write/front-end/w4-diagnostics/20271112-w4-diag-m1/stream_backpressure_hint/parser-metrics.rust.err.log`）のまま再現。
- `2028-01-15`: `scripts/poc_dualwrite_compare.sh` の flag 伝播と OCaml CLI エイリアス追加後に Run ID `20280115-w4-diag-refresh/stream_pending_resume` などで `runconfig.extensions.stream.enabled=true` となることを確認したが、`collect-iterator-audit-metrics.py` の `parser.stream_extension_field_coverage` / `parser.expected_summary_presence` が依然 0.0 を報告し、`reports/dual-write/front-end/w4-diagnostics/20280115-w4-diag-refresh/stream_pending_resume/parser-metrics.ocaml.err.log` と `.../stream_backpressure_hint/parser-metrics.rust.err.log` に失敗ログが残存。流量計測そのものが Rust 側 `ParserDriver` に未実装である可能性が高いため、parser streaming 拡張実装タスクへ分解する。
- **TODO: DIAG-RUST-06** — Effect/Capability/FFI 系ケース（`effect_residual_leak`, `effect_stage_cli_override`, `ffi_stage_messagebox` 等）を整備し、`effects.*` / `bridge.*` 拡張と audit ログの比較を Rust 版でも安定化させる。`2027-11-09`: cases 参照を登録し、CLI フラグ（`--experimental-effects --effect-stage beta --runtime-capabilities ...`）を README に追記。`2027-11-10`: `type_condition_bool` では OCaml 側が parser 段階で終了して JSON が空のまま (`reports/dual-write/front-end/w4-diagnostics/20271110-w4-diag-naming-check/type_condition_bool/diagnostics.ocaml.json`) であることを再現。`dune exec remlc -- --packrat --format json --json-mode compact --left-recursion off --type-row-mode dual-write --emit-typeck-debug <path> <input>` を手動実行すると `diagnostics.ocaml.manual.json` に `E7006` が出力されるため、diag ハーネスでも type/effect ケースには `--emit-typeck-debug` を注入して型推論段階まで到達させる必要がある。`scripts/poc_dualwrite_compare.sh` に case メタデータを渡し、OCaml 側にも型推論フラグを適用する計画を追加し、ハーネス修正後に `summary.json` の `diag_match` を回復させる。`2027-11-12`: `reports/dual-write/front-end/w4-diagnostics/20271112-w4-diag-m1/type_condition_bool/summary.json` でも OCaml 側 `diag_count=0` / Rust 側 `diag_count=1`、`reports/dual-write/front-end/w4-diagnostics/20271112-w4-diag-m1/effect_residual_leak/diagnostics.rust.json` では 5 件の lex recover のみが出力される状態を再確認。
- `2028-01-15`: `--emit-typeck-debug` と `--experimental-effects --type-row-mode dual-write --effect-stage beta` を両フロントエンドへ注入した結果、OCaml 側は `type_condition_bool/diagnostics.ocaml.json` で `diag_count=1` を記録できるようになったが、Rust 側は `diagnostics.rust.json` が空 (`reports/dual-write/front-end/w4-diagnostics/20280115-w4-diag-refresh/type_condition_bool/diagnostics.rust.json`) のままで `summary.json` でも `rust_diag_count=0`・`diag_match=false`。Rust TypecheckDriver が effect ケースで診断を生成していないため、Rust 側の `TypecheckDriver::infer_module`／`Diagnostic` 生成経路を優先調査する。
...
- `2028-01-15`: CLI/LSP ケースにもケース固有フラグが適用されるようになったが、`cli_packrat_switch`・`cli_merge_warnings` などは依然 `diag_count=0` のまま（`reports/dual-write/front-end/w4-diagnostics/20280115-w4-diag-refresh/cli_packrat_switch/summary.json`）で、Rust 側は 0〜4 件の診断を出力していても `parser_audit` が 0.0。LSP 系（`lsp_workspace_config` 等）も schema/metrics が `❌` 継続のため、Rust CLI/LSP の schema フィールド埋め込みと `collect-iterator-audit-metrics` インデックス更新を追加フォローアップとして記録する。
