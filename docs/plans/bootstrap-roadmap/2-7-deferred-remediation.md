...
- **TODO: DIAG-RUST-03** — `compiler/ocaml/tests/golden/diagnostics/effects/syntax-constructs.json.golden` は診断 JSON ではなく効果構文 PoC のメトリクスを含むため、`scripts/validate-diagnostic-json.sh` のゲートで必須フィールド欠落として失敗する。`reports/` への移設または validator の除外リスト整備を行い、W4 診断互換試験着手前に OCaml ゴールデン全体をスキャンできるようにする。関連: `docs/plans/rust-migration/1-0-front-end-transition.md#W4`、`reports/dual-write/front-end/w4-diagnostics/baseline/README.md`。
- **TODO: DIAG-RUST-04** — `compiler/ocaml/tests/golden/diagnostics/domain/multi-domain.json.golden` に `cli.change_set` / `schema.version` が存在せず、`collect-iterator-audit-metrics.py --section parser|streaming` の `diagnostic.audit_presence_rate` を 0.7 まで低下させている。Phase 2-7 内で audit メタデータを補完し、`reports/dual-write/front-end/w4-diagnostics/baseline/parser-metrics.ocaml.json` を更新する。
