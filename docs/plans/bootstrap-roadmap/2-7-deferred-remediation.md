...
| [docs/plans/rust-migration/p1-rust-frontend-gap-report.md](../rust-migration/p1-rust-frontend-gap-report.md) | FRG-01〜FRG-14 で W4 の Recover/Streaming/Type/Effect/Diag ケースを棚卸し、`reports/dual-write/front-end/w4-diagnostics/` の triage および `docs/plans/rust-migration/appendix/w4-diagnostic-case-matrix.md` との整合を明文化 | `DIAG-RUST-01`〜`DIAG-RUST-07` の責任区分と `p1-front-end-checklists.csv` の診断行への紐付け（triage ID と成果物パス） | FRG リストのおかげで W4 差分が起点に沿って整理でき、Dual-write run 再開時に triage とチェックリストを同期させる態勢が整った |
| [docs/plans/rust-migration/p1-spec-compliance-gap.md](../rust-migration/p1-spec-compliance-gap.md) | SCG で Type/Effect/Capability/Stage 仕様と OCaml 実装の挙動差分を整理し、`docs/spec/3-6-core-diagnostics-audit.md`/`1-3-effects-safety.md` の注釈と連携した `DIAG-RUST-06` の監査要件を定義 | 本書内では `effects.contract.*`、`stage_mismatch`、`parser.runconfig_switch_coverage` を中心に検証し、`collect-iterator-audit-metrics.py --section effects` での監査指標が未完 | SCG で拾った項目を `reports/dual-write/front-end/w4-diagnostics/` の Type/Effect/FFI ケースに紐づけたため、spec 合致チェックを Gate 化した再度の W4 実行が可能になった |
| [docs/plans/rust-migration/p1-test-migration-plan.md](../rust-migration/p1-test-migration-plan.md) | Rust 側のテスト資産（`p1-test-migration-typeck-cases.txt`、`p1-test-migration-effect-cases.txt` など）と `scripts/poc_dualwrite_compare.sh` のユースケースを明示し、CLI/LSP/Type/Effect/FFI の `reports/dual-write/front-end/w3/w4` への出力フローを整理 | 本書で再実行すべき `DIAG-RUST-05/06/07` ケース群について `p1-front-end-checklists.csv` へ Run ID を転記する作業が残る | テストリストと `scripts/poc_dualwrite_compare.sh` + `collect-iterator-audit-metrics.py` の組み合わせにより、W4 の `summary.json`/`metrics` を再生成し `docs-migrations.log` へ記録する態勢が整備済み |

### W4 最終フェーズ再開の参照体系

`p1-rust-frontend-gap-report.md`、`p1-spec-compliance-gap.md`、`p1-test-migration-plan.md` の 3 つの計画書は、Phase P1 W4 を走り切るための差分トリアージ・仕様チェック・テスト資産を体系的に整理したものです。

`p1-rust-frontend-gap-report.md` では FRG-01 〜 FRG-14 を通じて Recover/Streaming/Type/Effect/Diag それぞれのコード差分と `DIAG-RUST-01`〜`DIAG-RUST-07` の責任区分を明示し、`docs/plans/rust-migration/appendix/w4-diagnostic-case-matrix.md` への Run ID 付与と `p1-front-end-checklists.csv` への triage 反映をガイドしています。

`p1-spec-compliance-gap.md` は Type/Eff/Capability/Stage 観点の SCG を列挙し、`DIAG-RUST-06` で求められる `effects.contract.*`・`stage_mismatch`・`parser.runconfig_switch_coverage` などを `collect-iterator-audit-metrics.py --section effects` に落とし込む仕組みを示します。

`p1-test-migration-plan.md` のテストリストは `scripts/poc_dualwrite_compare.sh` の `--mode diag` 実行と `reports/dual-write/front-end/w4-diagnostics/` への出力を結び付けており、CLI/LSP/Type/Effect/FFI の再実行結果を `docs-migrations.log` に記録する運用の下地になっています。

**このセットアップを活用して W4 最終段階を再開するための確認手順**

1. まず `docs/plans/rust-migration/appendix/w4-diagnostic-case-matrix.md` および `p1-front-end-checklists.csv` の診断行を `reports/dual-write/front-end/w4-diagnostics/` の最新 triage（Recover／Streaming／Type/Effect/FFI／CLI/LSP など）と照らし、`scripts/poc_dualwrite_compare.sh --mode diag --cases docs/plans/rust-migration/appendix/w4-diagnostic-cases.txt --case-filter '^(recover_|stream_)'` などで `expected_tokens` と `parser.stream.*` を確認して `DIAG-RUST-01/05` のゲートを閉じる。
2. `DIAG-RUST-06` については `p1-test-migration-effect-cases.txt` を使って `scripts/poc_dualwrite_compare.sh --mode diag --force-type-effect-flags` を再実行し、`collect-iterator-audit-metrics.py --section effects --require-success` によって `effects-metrics.{ocaml,rust}.json` と `stage`/`bridge.stage` 拡張を検証。結果は `reports/dual-write/front-end/w4-diagnostics/<run>/summary.json`・`effects/effects-metrics.*.json` に保存し、`docs-migrations.log` へ run ID と triage を記録したうえで `2-7-deferred-remediation.md#TODO:-DIAG-RUST-06` へステータスを追記する。
3. `DIAG-RUST-07` では `p1-test-migration-plan.md` の CLI/LSP ケースと `tooling/lsp/tests/client_compat/fixtures/` を同期させ、`npm run ci --prefix tooling/lsp/tests/client_compat -- diag-w4 <run-id>` と `scripts/report-fixture-diff.mjs` による diff 出力で CLI/LSP 両側の `diag_match`・`parser.runconfig_switch_coverage` を確認。成果物パスと Run ID は `p1-front-end-checklists.csv` と `docs/plans/rust-migration/1-2-diagnostic-compatibility.md` に追記し、`docs-migrations.log` / `reports/dual-write/front-end/w4-diagnostics/README.md` に記録して W4.5 のクロージングレビューへ引き継ぐ。

#### 2029-04 W4 診断ラン再実行の履歴

- **20290427-w4-diag-recover-stream**（Recover/Streaming）: `docs/plans/rust-migration/appendix/w4-diagnostic-cases-recover-stream.txt` を使って `scripts/poc_dualwrite_compare.sh --mode diag` を実行。`reports/dual-write/front-end/w4-diagnostics/20290427-w4-diag-recover-stream/*/effects-metrics.rust.err.log` が `bridge_stage.audit_presence < 1.0` を報告し `metrics_ok=false` となったため、`bridge_stage.audit_presence` を満たす `stage`/`capabilities` 拡張の整備（`TODO: DIAG-RUST-06`）が次の焦点になっている。
- **20290428-w4-diag-type-effect**（Type/Effect/FFI）: `--force-type-effect-flags` を付けたケース群で `ffi_stage_messagebox`/`ffi_ownership_mismatch` の `parser-metrics.rust.err.log` が `parser.expected_summary_presence < 1.0` を吐き、`ffi_async_dispatch` では OCaml diagnostics が未出力という状態だった。効果フラグの剛性を高めた本 Run を `collect-iterator-audit-metrics.py --section effects --require-success` で再測定（`reports/dual-write/front-end/w4-diagnostics/<run>/effects`）し、`parser.expected_summary_presence` と `expected_tokens` の整合を gate に組み込んだ運用を継続する。
- **20290429-w4-diag-cli-lsp**（CLI/LSP RunConfig）: CLI と LSP 両チャネルで `scripts/poc_dualwrite_compare.sh --mode diag --cases docs/plans/rust-migration/appendix/w4-diagnostic-cases-cli-lsp.txt` を実行し、`npm run ci --prefix tooling/lsp/tests/client_compat -- diag-w4 20290429-w4-diag-cli-lsp` を続けた。`reports/dual-write/front-end/w4-diagnostics/20290429-w4-diag-cli-lsp/*/effects-metrics.rust.err.log` では `bridge_stage.audit_presence < 1.0` が連続し、`cli_*`/`lsp_*` の `diag_match` も `false`（OCaml CLI diagnostics が存在せず `parser.runconfig_switch_coverage` は 0）のまま。`reports/dual-write/front-end/w4-diagnostics/20290429-w4-diag-cli-lsp/lsp/` 以下に `report-fixture-diff.mjs` の diff 出力が保存されており、`TODO: DIAG-RUST-07` の改善が完了するまでこの Run ID を `p1-front-end-checklists.csv` に保持する。
...
- **TODO: DIAG-RUST-06** — Type/Effect/Capability/FFI 系ケース（`type_condition_*`, `effect_*`, `ffi_*`）で `effects.*` / `bridge.*` 拡張と audit ログを確実に比較できるよう、diag ハーネスが `--experimental-effects --effect-stage beta --type-row-mode dual-write --emit-typeck-debug <case>/typeck` を共通適用し、Rust 側には `--emit-effects-metrics <case>/effects` を強制する。Run `reports/dual-write/front-end/w4-diagnostics/20280601-w4-diag-type-effect-rust-typeck-r7/summary.md` では `effect_residual_leak` が `ocaml_diag_count=1` / `rust_diag_count=2`、`ffi_stage_messagebox` / `ffi_ownership_mismatch` が 21〜43 件、`ffi_async_dispatch` は OCaml 診断 0 件のまま `metrics_ok=false`。`expected_tokens.diff.json`（例: `ffi_ownership_mismatch` で `ocaml_count=5` vs `rust_count=2`）と `parser-metrics.rust.json`（`parser.expected_summary_presence < 0.05`）がブロッカーであるため、以下を継続する。  
  1. `compiler/rust/frontend/src/diagnostic/recover.rs` / `parser/expect.rs` に `TypeEffectRecoverLimiter` を追加し、`type_effect_case=true` の recover を 1 件へ圧縮したうえで `TypecheckDriver::emit_effect_violation` が `ExpectedTokenCollector` を通じて OCaml 並みの `expected_tokens` を埋め戻す。成果確認は `reports/.../effect_residual_leak/expected_tokens.diff.json` と `parser-metrics.rust.json`。  
  2. `compiler/ocaml/src/main.ml`→`typeck_output.ml` の `emit_typeck_debug_if_requested` を効果系 CLI でも常時実行し、`typeck/typeck-debug.ocaml.json` を `effect_residual_leak` / `ffi_*` 3 ケースで出力する（`collect-iterator-audit-metrics.py --section effects --require-success --source <case>/diagnostics.rust.json` の `typeck_debug_match` を 1.0 へ戻す）。  
  3. `docs/plans/rust-migration/appendix/w4-diagnostic-cases.txt` の `ffi_async_dispatch` に記載した CLI フラグを `compiler/ocaml/tests/test_ffi_contract.ml::ffi_dispatch_async` と突き合わせ、`diagnostics.ocaml.json` を必ず 1 件生成することを再検証する。  
  4. `collect-iterator-audit-metrics.py` 側で `parser.expected_summary_presence` と `expected_tokens_match` を DIAG-RUST-06 ゲートに組み込み、`scripts/poc_dualwrite_compare.sh --mode diag --force-type-effect-flags` の post-hook から `python3 tooling/ci/collect-iterator-audit-metrics.py --section effects --require-success --source <case>/diagnostics.rust.json` を呼び出して自動 Fail させる。  
  ...
  - Next actions:  
    ...
    4. `bridge_stage.audit_presence` を満たす `stage`/`capabilities` 拡張を整備し、`reports/dual-write/front-end/w4-diagnostics/20290427-w4-diag-recover-stream/*/effects-metrics.rust.err.log` の `metrics_ok=false` を逆転させる。`StageAuditPayload` に `bridge.stage.required_capabilities` / `bridge.stage.actual_capabilities` を `docs/spec/3-8-core-runtime-capability.md` に定義された Registry から逆引きして注入し、`collect-iterator-audit-metrics.py --section effects --require-success` で `bridge_stage.audit_presence=1.0` を確認できる状態へする。Stage 拡張の適用後は `scripts/poc_dualwrite_compare.sh --mode diag --cases docs/plans/rust-migration/appendix/w4-diagnostic-cases-recover-stream.txt` で `bridge_stage.audit_presence` が 1.0 になることを再検証する。  
  - 🆕 2029-06-12: `bridge_stage.audit_presence < 1.0` を再発させないため、`StageAuditPayload` を `diagnostics.*` / `audit_metadata` / `effects/effects-metrics.{ocaml,rust}.json` / `summary.json.bridge_stage` で共有する単一構造体として定義し直す。Rust CLI には `--runtime-capabilities windows.messagebox,windows.ffi,macos.dispatch_queue` を `docs/plans/rust-migration/appendix/w4-diagnostic-cases.txt` のメタデータから強制注入し、`ffi_stage_messagebox`・`ffi_ownership_mismatch`・`ffi_async_dispatch` がすべて `bridge.stage.required_capabilities` を JSON / Audit の両経路で出力できる状態を作る。直近 Run `20290427-w4-diag-recover-stream` に残っている `effects-metrics.rust.err.log` の `missing_keys` をベースラインとして保存し、新実装のラン（`tmp/w4-diagnostic-effects-stage.txt`）では `python3 tooling/ci/collect-iterator-audit-metrics.py --section effects --require-success --source <case>/diagnostics.rust.json` が `bridge_stage.audit_presence=1.0` を返さない限り `summary.json.metrics_ok=false` と記録する。  
  - 🆕 2029-06-12: `parser.expected_summary_presence` と `expected_tokens_match` を 1.0 へ戻すため、Rust 側 recover を `TypeEffectRecoverLimiter`（`frontend/src/diagnostic/recover.rs`）で 1 件に圧縮し、`ExpectedTokenCollector` を Type/Effect/FFI 入力へ常時適用する。OCaml CLI では `typeck_output.emit_debug_only` の後段に `ensure_effect_diag_written`（仮称）を追加し、`ffi_async_dispatch` など `diagnostics.ocaml.json` が空になるケースを排除する。Run `20280601-w4-diag-type-effect-rust-typeck-r7/ffi_ownership_mismatch/expected_tokens.diff.json` の `ocaml_count=5` / `rust_count=2` を目標値として使用し、`collect-iterator-audit-metrics.py --section parser --require-success` の `parser.expected_summary_presence` / `expected_tokens_match` が 1.0 にならない場合は `summary.json.metrics_ok=false` を必ず残す。  
  - 🆕 2029-06-12: Stage/Recover の修正を揃えたら Streaming（`20290531-w4-diag-streaming-r17` 以降）、Type/Effect/FFI（`20280601-w4-diag-type-effect-rust-typeck-r7` 以降）、CLI/LSP（`20290429-w4-diag-cli-lsp` 以降）の 3 カテゴリを `scripts/poc_dualwrite_compare.sh --mode diag --force-type-effect-flags` で再実行し、`summary.json.gating=true` を更新する。各 Run で `diag_match=true` / `metrics_ok=true` / `parser.runconfig_switch_coverage=1.0` / `bridge_stage.audit_presence=1.0` が揃ったら `reports/dual-write/front-end/w4-diagnostics/summary.md`・`p1-front-end-checklists.csv`・`docs/plans/rust-migration/appendix/w4-diagnostic-case-matrix.md` の該当行に Run ID を追記し、Phase 2-8 へのハンドオーバー条件（CLI/LSP/Type/Stream の `summary.json.gating=true`）を満たしたことを記録する。
  - ✅ 2028-06-03: `compiler/ocaml/src/main.ml` に `emit_typeck_debug_if_requested` を追加し、`--emit-typeck-debug` 指定時はパース失敗や型推論エラーでも `typeck/typeck-debug.ocaml.json` を必ず生成するよう更新（Type/Effect/FFI diag で `typeck_debug_match=false` になる要因を除去）。
  - ✅ 2028-06-03: `scripts/poc_dualwrite_compare.sh` が Type/Effect/FFI ケース完了後に `collect-iterator-audit-metrics.py --section effects --require-success` を自動実行し、`typeck/typeck-debug.metrics.json` を保存。`typeck_debug_match < 1.0` が検出されると `metrics_ok=false` / `gating=false` を即時反映するため、CI での再現ステップが不要になった。
...
- TPM-TYPE-03（`cli-callconv`/`ffi-contract`）は `docs/plans/bootstrap-roadmap/p1-test-migration-ffi-cases.txt` で `effects-contract` メトリクスを定義した。`scripts/poc_dualwrite_compare.sh --mode diag --cases docs/plans/bootstrap-roadmap/p1-test-migration-ffi-cases.txt --force-type-effect-flags` で `reports/dual-write/front-end/w4-diagnostics/effects-contract/<run>/<case>` に `diagnostics`・`audit`・`diag-metrics` を記録し、`collect-iterator-audit-metrics.py --section diag --metrics-case effects-contract` を実行して `effects.contract.stage_mismatch`/`capability_missing`/`ownership` の pass_rate を測定してから `effects-change-log` に差分を転記する。未達のケースは `p1-spec-compliance-gap.md#SCG-09` へ Deferred としてリンクし、次回 Run で `pass_rate=1.0` になればクローズする。

### 診断差分トラッキング {#diagnostic-audit}

- TPM-DIAG-01 では `docs/plans/bootstrap-roadmap/p1-test-migration-diagnostic-cases.txt` で `stage_callconv`/`ffi_effects`/`diagnostic_notes` の CLI ケースを `#metrics-case: cli_diagnostics` に分類し、`scripts/poc_dualwrite_compare.sh --mode diag --cases docs/plans/bootstrap-roadmap/p1-test-migration-diagnostic-cases.txt` で `diagnostics`/`audit`/`diag-metrics` を `reports/dual-write/front-end/w4-diagnostics/cli_diagnostics/<case>` に記録する体制を整備した。`collect-iterator-audit-metrics.py --section diag --metrics-case cli_diagnostics` は `diagnostics.expected_summary_presence`/`diagnostics.count`/`effects.contract.stage_mismatch`/`effects.contract.capability_missing`/`effects.contract.ownership` を ±0.5 ルールで監査し、`scripts/validate-diagnostic-json.sh --frontend rust --schema diagnostics-v2.schema.json` で Schema 準拠を確認、差分は本節で追記したうえで `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md#diagnostic-audit` に転記し Phase 2-8 での継続監査に引き継ぐ。

### effects-change-log

- `2025-11-15`: `TPM-TYPE-03` の `effects-contract` 監査結果を `collect-iterator-audit-metrics.py --section diag --metrics-case effects-contract` で集計し、`reports/dual-write/front-end/w4-diagnostics/effects-contract/<run>/<case>/diag-metrics.{ocaml,rust}.json` に `pass_rate`/`code_counts`/`failures` を残した。
...
- **TODO: DIAG-RUST-07** — CLI RunConfig / LSP RPC ケース（`cli_packrat_switch`, `lsp_hover_internal_error` など）を LSP フィクスチャと `cases.txt` の両方で共有し、`--mode diag` で CLI/LSP の診断 JSON を同時比較できるようにする。`2027-11-09`: `cases.txt` に CLI/LSP ケースを追加し、`reports/dual-write/front-end/w4-diagnostics/README.md` へ CLI/LSP 連携セクションを追記。今後は `scripts/dualwrite_summary_report.py --diag-table` に CLI/LSP 列を拡張する。`2027-11-12`: `reports/dual-write/front-end/w4-diagnostics/20271112-w4-diag-m1/` では CLI/LSP 系 6 ケースすべてが `parser.runconfig_switch_coverage=0.0`／`diag_match=false` のままで、`w4-diagnostic-cases.txt` の `#flags`（`--trace`, `--no-merge-warnings`, `--config ...` など）が CLI 実行時に無視されていることをログで確認（例: `reports/dual-write/front-end/w4-diagnostics/20271112-w4-diag-m1/cli_packrat_switch/parser-metrics.ocaml.err.log`）。  
  ...
  - 🆕 2029-06-12: `reports/dual-write/front-end/w4-diagnostics/20290429-w4-diag-cli-lsp/summary.json` をベースラインに GUI/LSP 差分を再チェックし、`diagnostics.ocaml.json` が空の CLI ケースを再収集する。`scripts/poc_dualwrite_compare.sh --mode diag --run-id 20290520-w4-diag-cli-lsp --case-filter '^(cli_|lsp_)'` を実行した後、`collect-iterator-audit-metrics.py --section parser --require-success --source <case>/diagnostics.{ocaml,rust}.json` で `parser.runconfig_switch_coverage=1.0` / `diag_match=true` / `extensions.config.path` 一致を確認する。並行して `npm run ci --prefix tooling/lsp/tests/client_compat -- diag-w4 20290520-w4-diag-cli-lsp` と `scripts/report-fixture-diff.mjs` を回し、`reports/dual-write/front-end/w4-diagnostics/<run-id>/lsp/<case>.diff` と CLI `summary.json` を `p1-front-end-checklists.csv`・`docs/plans/rust-migration/appendix/w4-diagnostic-case-matrix.md` に転記することで GUI/LSP 再チェックの証跡を `docs/plans/rust-migration/1-2-diagnostic-compatibility.md#1-2-17-cli-lsp-runconfig-整合diag-rust-07` と同期させる。
