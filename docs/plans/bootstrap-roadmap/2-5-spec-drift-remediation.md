...
- `TYPE-003` は Day1（Typer 側辞書復元）を進行中。`compiler/ocaml/src/typed_ast.ml` に `texpr_dict_refs` / `tdecl_dict_refs` を追加し、`make_typed_expr`/`make_typed_decl` の拡張および `attach_decl_dict_args` フローで `solve_trait_constraints` の結果を保持するところまで反映済み。`core_ir` 連携と診断・監査の更新（Day2 以降）は未実施。
