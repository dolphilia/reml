...
- `docs/notes/` : レビュー結果や TODO を記録（例: `docs/notes/process/guides-to-spec-integration-plan.md`）
- `docs/README.md`, `README.md` : 目次・導線の同期
- `docs/plans/repository-restructure-plan.md`, `docs/notes/backend/llvm-spec-status-survey.md` : 作業ログとリスク管理
...
### ストリーミング監査 {#streaming-audit}

- `scripts/poc_dualwrite_compare.sh --mode diag --cases docs/plans/bootstrap-roadmap/p1-test-migration-streaming-cases.txt --emit-expected-tokens streaming_runner` を実行し、`reports/dual-write/front-end/w4-diagnostics/2025-11-28-logos-chumsky/streaming_matches_batch` および `reports/dual-write/front-end/w4-diagnostics/2025-11-28-logos-chumsky/pending_resume_flow` に `streaming_runner.{ocaml,rust}.json`/`streaming_runner.diff.json`/`effects-metrics.{ocaml,rust}.json` を生成した。ただし `pending_resume_flow` の Rust 側では `effects-metrics` が `reports/dual-write/front-end/w4-diagnostics/2025-11-28-logos-chumsky/pending_resume_flow/effects-metrics.rust.err.log` で `[collect-iterator-audit-metrics] bridge_stage.audit_presence < 1.0` を記録しており、Stage に紐づく `audit` メタデータが空のため `collect-iterator` 側の `bridge_stage` チェックに引っかかっている。`diagnostics.{ocaml,rust}.json` は現時点で空配列なので recover/expected token の比較はまだ確認できていないが、同コマンドに `--emit-expected-tokens streaming_runner` を渡すことで将来的に `expected_tokens.{ocaml,rust,diff}.json` が `reports/dual-write/front-end/w4-diagnostics/streaming_runner/<case>` に残るようにしてある。
- `tooling/ci/collect-iterator-audit-metrics.py --section streaming --case streaming_runner` を `streaming_matches_batch/streaming_runner.{ocaml,rust}.json` と `pending_resume_flow/streaming_runner.{ocaml,rust}.json` を `--source` で与えて実行したところ、`diagnostic.audit_presence_rate` が 1 件中 0 件成功（`pending_resume_flow/streaming_runner.rust.json` に `audit`/`cli.audit_id`/`cli.change_set`/`schema.version`/`timestamp` が存在せず、metrics が通らない）と報告された。`parser.stream.*` 系や `ExpectedTokenCollector.streaming` のメトリクスは `streaming_runner.*.json` に `stream_meta`/`expected_tokens` が含まれるようになってから再度収集する予定で、現時点では `diagnostic` 監査の不整合を優先的に解消する必要がある。該当の `streaming_runner.rust.json` は `reports/dual-write/front-end/w4-diagnostics/2025-11-28-logos-chumsky/pending_resume_flow/streaming_runner.rust.json` で確認できるため、`poc_frontend` の Streaming 実行が `StageAuditPayload`（`cli.audit_id` など）を出力し、`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#streaming-recover` へ未解決差分として移行することを次フェーズのフォローアップに含める。
...
- `PARSER-003` は 2-1 型クラス戦略・2-2 効果統合と足並みを揃え、`Core.Parse` 再構成で必要なサポートコード（`rule` ラベル/診断統合）を洗い出し、仕様脚注と TODO を `docs/notes/parser/core-parser-migration.md`（新設予定）へ蓄積する。
...
| `SYNTAX-001` | Chapter 1／識別子 | Unicode 識別子未対応 | Step1 で脚注範囲と拒否メッセージを棚卸済み。Step2（2026-02-24）で BNF・索引・用語集へ ASCII 暫定脚注を追補し、技術的負債 ID 22/23 と Phase 2-7 `lexer-unicode` タスクへ橋渡し済み。Step3 で Unicode 受理テスト雛形とサンプル (`unicode_ident_tests.ml` / `samples/unicode_identifiers.reml`) を作成し、実装後に即時検証できる土台を整備。Step4（2026-03-31）で `lexer.identifier_profile_unicode = 0.0` の測定値を記録し、リスク台帳と Phase 2-7 ロードマップを更新 |
  | `LEXER-001` | Chapter 2／Unicode | `identifier(profile)` が使えない | プロファイル差分と運用ルールを `docs/notes/dsl/dsl-plugin-roadmap.md` §7 に記録 |
...
- `rust-gap` ラベルの付与基準: Rust 実装でのみ再検証可能な差分（OCaml 側で凍結済み）に対し、Chapter 1〜3 で最低 1 件ずつ割り当てる。`reports/spec-audit/diffs/` に差分ノート、`docs/notes/process/spec-integrity-audit-checklist.md` にタスク起票情報を残す。
...
- **PARSER-003 Step6**: 2025-12-24 時点でコアコンビネーター抽出計画のドキュメント同期を実施し、`Core_parse` PoC の状況を仕様・ガイド・索引へ反映する。
  ...
  - `README.md` と `docs/plans/bootstrap-roadmap/2-5-proposals/README.md` の索引を更新し、PARSER-003 の Step6 完了を記録。`docs/notes/parser/core-parse-api-evolution.md` に Step6 セクション、`docs/plans/bootstrap-roadmap/2-5-review-log.md` に 2025-12-24 ログを追加した。
...
- **PARSER-002 / LEXER-002 / DIAG-003 / EFFECT-003 / TYPE-001**: Phase 2-5 中盤で着手し、RunConfig 導入・Lex API 抽出・診断ドメイン拡張・複数 Capability 対応・値制限復元を進める。`PARSER-002` は Week32 Day1-2 で RunConfig 基本型と拡張 API を `compiler/ocaml/src/parser_run_config.ml` に実装済みであり、Week32 Day4-5 のテスト整備で `parser.runconfig_switch_coverage` / `parser.runconfig_extension_pass_rate` を `0-3-audit-and-metrics.md` に登録し、`collect-iterator-audit-metrics.py --require-success` による監視を有効化した[^runconfig-step1-phase25][^runconfig-step5-phase25].
 ...
 2025-11-14 追記: LEXER-002 Step6（TPM-LEX-01）で `core_parse_lex_tests.ml` 相当のトークン列を `compiler/ocaml/tests/golden/core_parse_lex_tests.tokens.json` へゴールデン化し、`compiler/rust/frontend/tests/lexer/core_parse.rs` が `poc_frontend --emit-tokens` 出力と照合して Dual-write 側のトークン安定性を維持する仕組みを整備。`reports/dual-write/front-end/w1-lexer/...` を格納先とし、収集した JSON を `collect-iterator-audit-metrics.py` の `lexer.identifier_profile_*` 指標で監査する流れを計画書へ補足した。
...
- 2025-12-15 更新: ERR-002 Step4 で `docs/spec/2-5-error.md`・`docs/spec/3-6-core-diagnostics-audit.md` の脚注と共有ログ（`docs/plans/bootstrap-roadmap/2-5-review-log.md`）を整備し、Packrat／notes／ストリーミング重複の残課題を `docs/notes/parser/core-parse-streaming-todo.md` と Phase 2-7 計画 §3.4 へ移管した。
...
- **LEXER-001 / SYNTAX-001 / SYNTAX-003 / EFFECT-002 / TYPE-002** など Phase 2-7 移行案件は、Phase 2-5 期間中に脚注整備・ロードマップ作成・必要なノート作成（`docs/notes/effects/effect-system-tracking.md` 等）を完了する。`SYNTAX-003 S4` で整理したハンドオーバー チェックリスト H-O1〜H-O5 を Phase 2-7 計画へ転記済みであるため、進捗確認は同チェックリストに沿って実施する。
...
**更新チェックリスト（優先度順）**
- [x] `docs/spec/3-6-core-diagnostics-audit.md`・`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md`・`docs/guides/runtime/runtime-bridges.md` 間で Severity 4 値化と `DiagnosticDomain` 拡張（DIAG-001〜003）の脚注・メトリクス・ガイド記述が一致しているか再確認し、不整合がある場合は `docs/spec/3-0-core-library-overview.md` と `docs/notes/dsl/dsl-plugin-roadmap.md` も同時に更新する。
- [x] `docs/spec/1-1-syntax.md`・`docs/spec/1-3-effects-safety.md`・`docs/spec/1-5-formal-grammar-bnf.md`・`docs/notes/effects/effect-system-tracking.md` に掲載された `Σ_before`/`Σ_after` PoC 脚注（EFFECT-002 Step4/5）と KPI (`syntax.effect_construct_acceptance`, `effects.syntax_poison_rate`) の説明が揃っているか確認し、差分があれば Phase 2-7 向けの撤去条件を明示する。
- [x] `docs/spec/2-1-parser-type.md`・`docs/spec/2-6-execution-strategy.md`・`docs/guides/compiler/core-parse-streaming.md`・`docs/notes/parser/core-parse-api-evolution.md` で RunConfig（PARSER-002/003 Step6）に関する表記と参照リンクが一致しているか確認し、`parser-runconfig-packrat.json.golden` の更新履歴を脚注に追記する。
...
- `docs/notes/`・`docs/guides/` に追加した参照（`docs/notes/effects/effect-system-tracking.md`, `docs/guides/compiler/core-parse-streaming.md` 等）から仕様書への逆リンクが成立しているか確認する。
...
- [x] `docs/notes/effects/effect-system-tracking.md`・`docs/notes/parser/core-parse-api-evolution.md`・`docs/notes/dsl/dsl-plugin-roadmap.md` に記録した Phase 2-5 の進捗と `docs/plans/bootstrap-roadmap/2-5-review-log.md` のエントリが相互参照できるかを確認し、欠落していればメモを追加する。
...
**引き継ぎ準備メモ**
- Phase 3 で参照する資料（`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md`, `docs/plans/bootstrap-roadmap/3-0-phase3-transition.md`, `docs/notes/effects/effect-system-tracking.md`, `docs/notes/parser/core-parse-api-evolution.md` 等）への導線を `README.md` と `docs/spec/0-0-overview.md` に明示する。
...
[^lexer001-phase25]: `docs/notes/dsl/dsl-plugin-roadmap.md` §7「Unicode プロファイル導入準備」および `docs/plans/bootstrap-roadmap/2-5-review-log.md` 2026-03-04 / 2026-03-21 記録を参照。Phase 2-5 Step3 で ASCII 限定ポリシーと Phase 2-7 フォローアップ TODO を確定し、Step4 で `--lex-profile=ascii|unicode` スイッチ・CI 指標の運用方針を定めた。
