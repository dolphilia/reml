...
2. `compiler/rust/runtime/src/parse/combinator.rs` に CST 収集フラグを追加し、`ParseState` で Trivia バッファを保持する。
...
1. `compiler/rust/runtime/src/text/pretty.rs` に `CstPrinter`（標準プリンタ）を追加する。
...
### 既存実装の範囲
- `compiler/rust/runtime/src/parse/` は `Parser<T>` と `ParseResult<T>` を中心に AST 指向のパースを提供。
