...
- `docs/notes/backend/llvm-spec-status-survey.md`, `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` : ツールチェーンバージョンと測定値の記録先。
...
- Xcode Command Line Tools のバージョンをチェックし `xcode-select --install` の要否を確認、GitHub Actions での差分を `docs/notes/backend/llvm-spec-status-survey.md` に記録。
...
- `otool -L` でリンク先ライブラリを検証し、不要な依存が混入していないかチェックして結果を `docs/notes/backend/llvm-spec-status-survey.md` に追記。
...
1. **依存の文書化**  
   ...
   - `docs/notes/backend/llvm-spec-status-survey.md` へ macOS の LLVM CLI バージョン・`opt`/`llc` 出力を記録し、`docs-migrations.log` にセッションのステップを整理して移行計画のトレースを残す。
...
- Homebrew の LLVM バージョンが頻繁に更新される場合、`brew extract` によるバージョン固定または prebuilt tarball 配布を `docs/notes/backend/llvm-spec-status-survey.md` に追記。
...
- [docs/notes/backend/llvm-spec-status-survey.md](../../notes/llvm-spec-status-survey.md)
