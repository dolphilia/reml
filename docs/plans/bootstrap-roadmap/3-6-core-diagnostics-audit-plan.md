...
#### 1.2 追記（Run ID: 20251224-migration-effect-tag）
- Config/Data 章で導入した `MigrationPlan` API（`reml_runtime::config::migration`）が `effect {migration}` を発火することを `docs/spec/1-3-effects-safety.md` の実装メモと `docs/spec/3-7-core-config-data.md` の補足に追記。Diagnostics 側では `scripts/validate-diagnostic-json.sh --effect-tag migration` をサポート済みのため、Config CLI が `migration` 効果を出力した際も既存の検証パイプラインを流用できる。
- `reports/spec-audit/ch3/migration_plan-pilot.md` を KPI ログに追加し、`cargo test -p reml_runtime --features experimental-migration migration_plan::tests::plan_serialization_roundtrip` の出力を `effect.stage.*` / `config.migration.*` メタデータと紐付けた。今後 `collect-iterator-audit-metrics.py --section config` に `migration_effect_presence` KPI を追加する際の参照点とする。【P:docs/plans/bootstrap-roadmap/3-7-core-config-data-plan.md#5.1】
- Diagnostics 計画としては `MIGRATION_EFFECT_TAG`（Rust 実装が公開する共通定数）を参照し、Config CLI 側が `effect {migration}` を出力した際に Stage 要件（`StageRequirement::AtLeast(StageId::Beta)`）で統一検証できるよう `docs/plans/bootstrap-roadmap/assets/diagnostic-effect-tag-policy.csv` の追補行を登録した。
...
- 監査サンプルとして `examples/core_diagnostics/pipeline_success.reml` / `pipeline_branch.reml` を追加し、`tooling/examples/run_examples.sh --suite core_diagnostics --with-audit` で両ケースを連続実行できるようにした。スクリプトは `compiler/rust/frontend` 配下で `cargo run --bin reml_frontend -- [--emit-audit-log] <example>` を実行しつつ、`pipeline_branch.reml` の `effects.contract.stage_mismatch` による非ゼロ終了のみ `allowed failure` として扱う。
...
#### 8.1 実施結果（Run ID: 20290718-collector-effect-keys）
- `tooling/ci/render-collector-audit-fixtures.py` に `--with-stage` フラグを追加し、`collector.stage.*` 情報から `effect.stage.required|actual|mode|capability|kind|source` を複製するルートを導入した。Stage が埋まっていないスナップショットは即時にエラー化されるため、Effectful コンテナの JSON へ Stage 情報が必ず残るようになった。
- 上記フラグを有効にして `python3 tooling/ci/render-collector-audit-fixtures.py --snapshots compiler/rust/frontend/tests/__snapshots__/core_iter_collectors.snap --output reports/spec-audit/ch1/core_iter_collectors.json --audit-output reports/spec-audit/ch1/core_iter_collectors.audit.jsonl --with-stage` を再実行し、`collector.effect.cell`/`collector.effect.rc`/`collector.effect.mem`/`collector.effect.audit` が JSON/Audit 双方へ流れていることを確認した。`effect.stage.*` の欠落があった場合はスクリプト自体が失敗するため、Stage 付きのスナップショット生成がゲートとして機能する。
- `scripts/tools/list_collector_keys.py` を新設し、`collector.effect.*` キーと出現回数を Markdown に自動出力する仕組みを整備した。`python3 scripts/tools/list_collector_keys.py --output docs/plans/bootstrap-roadmap/assets/collector-effect-keys.md` で `collector.effect.cell` / `collector.effect.rc` / `collector.effect.rc_ops` などの一覧を保存し、監査チームがキー追加時の diff を即座に確認できるようにした。
- `scripts/validate-diagnostic-json.sh --suite collectors --pattern collector.effect.cell --pattern collector.effect.rc` も実行したが、現状の collector フィクスチャは Diagnostic Schema (`primary`/`timestamp`/`audit_metadata`) を満たさないため警告扱いになる点が判明した。`collector.effect.*` 自体は上記キー一覧と `reports/spec-audit/ch1/core_iter_collectors{.json,.audit.jsonl}` の再生成結果で確認済みであり、Schema 適合については Stage=experiment のまま `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` のフォローアップへ切り出す。
