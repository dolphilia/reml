...
- [ ] `CH2-PARSE-901` autoWhitespace/Layout 回帰（`phase4-scenario-matrix.csv` 登録のみ）: `RunConfig.extensions["lex"].profile/layout_profile` を共有する新サンプルと expected を作成し、未設定フォールバックでも診断が変わらないことを PhaseF で確認する。
- [ ] `CH2-PARSE-902` ParserProfile JSON（`phase4-scenario-matrix.csv` 登録のみ）: `core-parse-or-commit-ok.reml` を `RunConfig.profile` / `profile_output` 付きで実行し、Packrat/backtracks/recoveries を best-effort で書き出せることをゴールデン化する。
...
- [x] `examples/language-impl-comparison/reml/pipeline_operator_demo.reml`（期待: 成功 → 2025-12-12 CLI=`cargo run --quiet --manifest-path compiler/rust/frontend/Cargo.toml --bin reml_frontend -- --output human examples/language-impl-comparison/reml/pipeline_operator_demo.reml` / diagnostics=[]。Parser に `|>` パイプ演算子の左結合最弱優先度を実装し、パイプ演算サンプルが通過することを確認）
...
- [x] `examples/language-impl-comparison/reml/basic_interpreter.reml`（期待: 成功 → CLI=`cargo run --manifest-path compiler/rust/frontend/Cargo.toml --bin reml_frontend -- --output human examples/language-impl-comparison/reml/basic_interpreter.reml` / diagnostics=[]）
