...
## 目的
- Phase 4 マイルストーン M3 を支援するため、`README.md`・[0-0-overview.md](../../spec/0-0-overview.md)・`docs/guides/compiler/llvm-integration-notes.md` など主要ドキュメントをセルフホスト版前提に更新する。
...
- 後方互換性への影響（OCaml 実装を参照アーカイブとして扱う方針の明示）
...
- マルチターゲット対応の注意点
- Capability システムとの統合
- 旧 OCaml 実装からの主な移行ポイントを歴史資料として整理
...
- セルフホスト版移行の説明
- マルチターゲット対応の告知
- OCaml 実装を参照アーカイブとして扱う方針と今後の閲覧方法
