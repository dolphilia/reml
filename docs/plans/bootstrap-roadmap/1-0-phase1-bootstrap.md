...
Phase 1 は Reml コンパイラの OCaml 実装によって LLVM IR 生成までの最小パイプラインを確立する段階である。`docs/guides/compiler/llvm-integration-notes.md` の Bootstrap 設計を実行レベルに具体化し、言語仕様 MVP の検証と計測基盤の整備を同時に進める。
...
- ランタイム連携については `docs/guides/compiler/llvm-integration-notes.md` §5 の所有権モデルと最小ランタイム API を準拠実装する。
...
- `docs/notes/backend/llvm-spec-status-survey.md` ほか関連ノートへ測定値・リスクを記録
...
> **ターゲット方針**: Phase 1 の配布ターゲットは **x86_64 Linux (System V ABI)** を優先する。開発環境として macOS (ARM64含む) を使用してもよいが、生成される LLVM IR および成果物は x86_64 Linux 向けとする。これは `docs/guides/compiler/llvm-integration-notes.md` および `docs/notes/backend/llvm-spec-status-survey.md` で x86_64 が主要ターゲットと定義されているため。ARM64 macOS は Phase 3 以降のクロスコンパイル対応で追加予定。
...
2. **Typer 実装**
   - HM 推論の Unifier を OCaml で構築し、`docs/notes/backend/llvm-spec-status-survey.md` で示された成熟度評価に沿ってテスト項目を抽出。
...
4. **LLVM IR 生成とターゲット設定**
   - **`-target x86_64-unknown-linux-gnu`** を既定にした LLVM IR を生成し、System V ABI の呼出規約に合わせて関数シグネチャを構成する（`docs/guides/compiler/llvm-integration-notes.md` §5.0 準拠）。
...
- **MVP 範囲の明確化**: Phase 1 は `docs/guides/compiler/llvm-integration-notes.md` で定義された「MVP（最小実装）」範囲に対応し、期間は約 16 週（2-3ヶ月相当）。「本格実装」は Phase 2 の範囲として扱う。
