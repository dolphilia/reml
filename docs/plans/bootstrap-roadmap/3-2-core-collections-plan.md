...
`docs/guides/runtime-bridges.md` では `core.collections.audit` 登録の手順、Stage/Effect の契約、`RuntimeBridgeDescriptor` が `collector.effect.audit` を伝搬する方法を例示し、`docs/plans/bootstrap-roadmap/3-8-core-runtime-capability-plan.md` の Stage 連携節とのリンクを挿入する【F:../../docs/guides/runtime-bridges.md†L1-L120】。`docs/plans/rust-migration/unified-porting-principles.md` には `effect {audit}` を含む API が Capability を満たす前提でのみ `Core.Collections` を使用できると明記し、不可欠な `collect`/`map` オペレーションを部分的に制限する方針を脚注で追記。`docs/notes/collections-audit-bridge-todo.md` も `Capability` 対応済みのトラッキングリストを更新し、`ChangeSet` 生成→`REML_COLLECTIONS_CHANGE_SET_PATH`→`FormatterContext::change_set` の 3 段階が整備済みであることを確認する。
