...
- Capability 側のドキュメント更新として `docs/guides/runtime-bridges.md` に `Core.Collections` の `audit` フラグを追加し、`docs/plans/rust-migration/unified-porting-principles.md` の「同一性優先」セクションに `effect {audit}` 操作の Capability 要件を追記することで移植チームへ誓約と制限を明示する。
