...
**次のステップ**
- `MapCollector` / `SetCollector` の `finish` で `PersistentMap::diff_change_set` / `PersistentSet::diff` を実際に呼び出し、`CollectOutcome::record_change_set` を通じて `AuditEnvelope.change_set` を埋める（`map_diff` / `set_partition` ケースが OCaml 実装と同じ JSON を出すようにする）。併せて `reports/spec-audit/ch1/*.json` を差し替え、`map_set_persistent` シナリオが `collector.effect.audit=true` で通るよう `cargo test core_iter_collectors` を更新する。
- `Map.merge` / `Set.diff` を利用する上位 API（Config/Data や Table 変換）から `map_diff_to_changes` を呼び、`scripts/poc_dualwrite_compare.sh --target map_diff` で差分を比較できるよう CLI 側を繋ぎ込む。
