...
5. ドキュメント更新: `docs/plans/rust-migration/p1-spec-compliance-gap.md` に `audit_bridge` のギャップ完了を記録し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` へ `collections.audit_bridge_pass_rate`/`collector.effect.audit_presence` の KPI を追加する。また `docs/notes/process/spec-integrity-audit-checklist.md` に「Core.Collections diff JSON の検証」チェックを記載し、`docs-migrations.log` にも `Collections.audit_bridge` の初回実装と `AuditEnvelope` 拡張の日時を残す。
...
- KPI 文書（`reports/iterator-collector-summary.md`, `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md`）に実測値を書き戻し、`docs/notes/process/spec-integrity-audit-checklist.md` の TODO をクローズする。
...
- `scripts/poc_dualwrite_compare.sh --target map_set` を更新し、OCaml 実装との dual-write を自動化する。差分が 5% を超えた場合は `docs/plans/bootstrap-roadmap/5-0-phase4-migration.md` へフォローアップを追加し、`reports/spec-audit/diffs/README.md` に結果を添付する。
...
4. `docs/notes/process/spec-integrity-audit-checklist.md` の `Vec collector.effect.*` 項目や `reports/spec-audit/README.md` の Core Collections 節へ Vec effect に関するログリンクを追記し、`docs-migrations.log` に Vec effect メタデータ追加の記録を残す。
...
3. FFI/RuntimeBridge 経路は `compiler/rust/runtime/ffi/src/handles/ref_handle.rs` に分離し、`docs/guides/runtime/runtime-bridges.md` の契約に従い `RefHandle::clone`/`drop` の監査を `CapabilityRegistry` へ記録する。`docs/plans/bootstrap-roadmap/3-8-core-runtime-capability-plan.md` へ参照を追記する。
4. `reports/iterator-collector-summary.md` では `cell_mutations_total` / `ref_borrow_conflicts` の KPI を追加し、`docs/notes/stdlib/core-library-outline.md` に内部可変性レイヤのメモを残す。`docs/plans/rust-migration/p2-spec-compliance-gap.md` にも `Cell/Ref` の差分を写す。
...
- ベンチマーク側では `collect-iterator-audit-metrics.py --suite collectors --scenario ref_internal_mutation` に `collector.effect.rc`/`collector.effect.mut` の成功判定を置き、`reports/iterator-collector-summary.md` の `ref_borrow_conflict_rate` / `ref_rc_clone_ops` セクションへデータを流す。`docs/notes/stdlib/core-library-outline.md#ref-internal-mutation` に挙動メモを追加し、`docs/plans/rust-migration/p2-spec-compliance-gap.md` と `docs/plans/bootstrap-roadmap/3-2-core-collections-plan.md` で差分のトラッキング行を同期する。

###### 3.2.3 RuntimeBridge と FFI ハンドル
- `compiler/rust/runtime/ffi/src/handles/ref_handle.rs` に `RefHandle` を分離し、`CapabilityRegistry` の `core.collections.ref` エントリを登録する。`RefHandle::clone`/`RefHandle::drop` で `EffectSet::mark_rc()`/`release_rc()` を呼び出し、`docs/guides/runtime/runtime-bridges.md` の `Ref` 契約と `docs/spec/3-9-core-async-ffi-unsafe.md` §4 の `async` 参照ライフタイム要件を満たすようにする。
- `poc_dualwrite_compare.sh --section ref_count` と `reports/spec-audit/diffs/README.md` へ `collector.effect.rc` に関するログを残し、OCaml/ Rust の差分が 5% を超えた場合は `docs/plans/bootstrap-roadmap/5-0-phase4-migration.md` へリスク追加を作成する。`docs/plans/bootstrap-roadmap/3-8-core-runtime-capability-plan.md` の `CapabilityRegistry` を更新し、`core.collections.ref` が `CapabilityStage::Persistent`/`RefHandle` によるアクセス保証を報告するようにする。
- **2027-05-30 Update**: `compiler/rust/runtime/ffi/src/handles/ref_handle.rs`/`handles/mod.rs` と `compiler/rust/runtime/ffi/src/lib.rs` で `RefHandle` の再公開と `register_ref_capability()` の橋渡しを実装し、`compiler/rust/runtime/ffi/src/capability_handle.rs` に `RefCapability` variant を追加して `core.collections.ref` を `CapabilityRegistry` に登録した。ドキュメント側では `docs/guides/runtime/runtime-bridges.md` に `RefHandle` の契約と `collector.effect.rc` への対応を、`docs/plans/bootstrap-roadmap/3-8-core-runtime-capability-plan.md` に新たなステージ連携行を追記して進捗を記録した。
- ✅ 2025-12-06 Update: `compiler/rust/runtime/src/capability/collections.rs` に `CollectionsCapability` を追加し、`CapabilityHandle::Collections`（`core.collections.ref`, Stage=Stable, effect_scope=`["mem","mut","rc"]`）をビルトイン登録。`compiler/rust/runtime/src/collections/mutable/ref.rs` では `OnceCell` ベースの Stage ガードと `EffectfulRef::try_new` / `BorrowError::CapabilityDenied` を導入し、FFI 側も `RefHandle::try_new` で Capability 検証を実行するよう更新した。
- 同アップデートに合わせて `python3 tooling/ci/collect-iterator-audit-metrics.py --suite collectors --scenario ref_internal_mutation --output reports/spec-audit/ch3/collections_ref-20251206.json --require-success --require-cell` を実行し、`collector.effect.cell_rc` KPI（`cell_mutations_total=1`, `rc_ops_total=2`）を取得。スクリプトは既知の `collector.table.csv_import` 閾値不足により `threshold metric missing` を報告するが、Run ログとコマンドは `reports/spec-audit/ch3/collections_ref-20251206.md` にまとめており、本節と `docs/plans/bootstrap-roadmap/3-8-core-runtime-capability-plan.md#5.4-実施結果` から参照できる。

###### 3.2.4 ドキュメント・監査連携
- `reports/iterator-collector-summary.md` の `collect_cell_ref_effects` セクションに `cell_mutations_total` / `ref_borrow_conflict_rate` / `collector.effect.rc` の説明を追記し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の KPI 表および `docs/plans/bootstrap-roadmap/assets/metrics/core_collections_persistent.csv` の `RefInternalMutation` 行で新規メトリクスを追跡する。`docs/notes/stdlib/core-library-outline.md` に内部可変性スタックのメモを立て、`docs/plans/rust-migration/p2-spec-compliance-gap.md` の該当セクションとリンクさせて仕様整合性の監査ルートを明示する。Run ID `20290730-collectors-suite-gate` では `threshold metric missing` をログに残し、ケース追加後にゼロへ遷移することを目標とする。
- `scripts/validate-diagnostic-json.sh --suite collectors --pattern collector.effect.cell --pattern collector.effect.rc` により `collector.effect.cell`/`collector.effect.rc` の存在を gate として強制し、`collect-iterator-audit-metrics.py --suite collectors --thresholds docs/plans/bootstrap-roadmap/assets/metrics/core_collections_thresholds.json --scenario ref_internal_mutation --require-success --require-cell` で `collector.effect.cell`/`collector.effect.rc` を出力したケースを検証する。CI や週次レビューでは `reports/iterator-collector-metrics.json` を生成して `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` に KPI を転記し、`docs/plans/bootstrap-roadmap/3-6-core-diagnostics-audit-plan.md` §8 の `Core Collections effect 連携` から監査チームへ通知する。
...
###### 3.2.5 監査 CI gate 実行ログ（Run ID: 20251203-collectors-gate）
- `python3 tooling/ci/collect-iterator-audit-metrics.py --suite collectors --scenario ref_internal_mutation --require-success --require-cell --output reports/audit/dashboard/collectors-20251203.json` を実行した結果、`collector.effect.cell_rc`（`ref_internal_mutation`）と `collector.table.csv_import`（`table_csv_import`）のいずれも `metric_missing` となり `enforcement.failures` に `threshold metric missing` が追加された。また `diagnostic.audit_presence_rate=0.0` で、`core_iter_collectors.json` に `cli.audit_id`/`cli.change_set`/`schema.version`/`timestamp` が出力されていないことが明確になった。詳細は `reports/audit/dashboard/collectors-20251203.{json,md}` に保存。
- `scripts/validate-diagnostic-json.sh --suite collectors` では 7 件全ての collector snapshot が `primary`/`audit_metadata`/`timestamp` を持たないためスキーマ違反で停止し、`audit.metadata.effect.{required,actual}_capabilities` も欠落していると判定された。`collector.effect.cell`/`collector.effect.rc` を JSON/Audit 双方向に流すまで `--pattern` フィルタは `warning` を返す。
- 本 Run ID を `docs/plans/bootstrap-roadmap/3-6-core-diagnostics-audit-plan.md#8.3-実施結果run-id-20251203-collectors-gate` および `docs-migrations.log (2025-12-03 Core Collections Cell/Ref gate)` へ登録し、Core Diagnostics 計画と同じゲート結果を参照できるようにした。次ステップとして `compiler/rust/frontend/tests/__snapshots__/core_iter_collectors.snap` に `collect_cell_ref_effects`/`table_csv_import` を追加し、`Diagnostic` 生成時に `primary`/`timestamp`/`audit_metadata` を付与するタスクを `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` へフォローアップとして移管した。
...
4. `docs/notes/stdlib/core-library-outline.md` と `docs/plans/rust-migration/3-2-benchmark-baseline.md` へ `table_ordered_insert_ps` / `csv_load_latency` の KPI を追加し、IO チームと共有する。`docs-migrations.log` へ `Table.load_csv` 追加を追記し、再編履歴を残す。
...
3. CI スクリプト（`collect-iterator-audit-metrics.py`、`validate-diagnostic-json.sh`）に `iter.collector.effect.*` チェックを組み込み、`reports/spec-audit/ch1/core_iter_collectors.json` に snapshot を追記する。`docs/notes/stdlib/core-library-outline.md` に dual-write プロセスの結果を記録し、検証ログの再現手順を残す。
...
3. `reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` へ `try_collect` の成功/失敗ケースを追加し、`scripts/validate-diagnostic-json.sh --section iter` で `iter.try_collect.diagnostic` をチェック。`docs/notes/stdlib/core-library-outline.md` に `Collector` 失敗ケースのリストを TODO 頭で残し、追跡用の検証手順を示す。
...
- `docs/notes/stdlib/core-library-outline.md` に `Iter.try_collect` の失敗パターン一覧を TODO として残し、OCaml 実装との dual-write 比較手順（`scripts/poc_dualwrite_compare.sh --target iter_try_collect`）と、`reports/iterator-collector-summary.md` の `Iter.collectors` 表に記録する `status` カラム更新ルールをメモした。【F:../../docs/notes/stdlib/core-library-outline.md†L1-L120】【F:../../reports/iterator-collector-summary.md†L15-L34】
...
4. `compiler/rust/runtime/tests/core_collections_audit_bridge.rs`（新設）で `PersistentMap::merge_with_change_set`/`Set`/`Table` の差分 JSON を検証し、`collect-iterator-audit-metrics.py --scenario map_set_persistent --require-audit` が `collector.effect.audit` を 0 でなく 1 にセットすることを確認するテストを追加する。`scripts/poc_dualwrite_compare.sh --target map_diff` にも `collections.audit_bridge` 出力比較ルートを組み込み、OCaml 実装との差分を監査結果として報告する。
   ...
   - `ConfigMergeOutcome` の JSON 出力を `write_change_set_to_temp_dir` で一時ファイル化し、CLI 実行前に `REML_COLLECTIONS_CHANGE_SET_PATH` を注入するランタイム経路の設計を `docs/notes/stdlib/collections-audit-bridge-todo.md` に記録した。`collect-iterator-audit-metrics.py` の `map_set_persistent` シナリオが期待する `collections.diff.total/items` を満たすテンプレートも同ノートで追跡しており、次のコード実装ではこのテンプレートを具現化する予定である。
...
- `docs/notes/process/spec-integrity-audit-checklist.md` の `Core.Collections 監査 TODO` セクションに `config.diff.bridge` 項目を追加し、`collection_diff` の導入・`schemas_diff` チェック・`scripts/validate... --section config` の順で追跡できるようにした。今後 `schema_diff` フィールドの追加ステップが発生した場合はこの TODO を更新する。

#### 5.3 `effect {audit}` による Capability チェックと運用ガード
- `tables`, `maps`, `collections` が `effect {audit}` を発行する API（`emit_metric`, `collect_table` の `audit` モード、差分ブリッジ経由の `change_set` 出力など）では `CapabilityRegistry` に `core.collections.audit` を登録し、`docs/plans/bootstrap-roadmap/3-8-core-runtime-capability-plan.md` と同期して Stage/Effect の検証経路を明確化する【F:../../docs/plans/bootstrap-roadmap/3-8-core-runtime-capability-plan.md†L57-L110】。`scripts/poc_dualwrite_compare.sh --target audit_bridge` を通じて `CapabilityRegistry::check("core.collections.audit")` が `false` の場合に `CollectError::CapabilityDenied` を返すパスと Diagnostic の `effect {audit}` フラグの欠落を監査する。
- `effect {audit}` を伴うテスト/CI は `tooling/ci/collect-iterator-audit-metrics.py --section collectors --scenario audit_cap` で `collector.effect.audit=true` を期待し、`scripts/validate-diagnostic-json.sh --pattern collector.effect.audit` で `reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` の `collections` セクションを検証する。`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の Phase3 `Capability` 列に `core.collections.audit` を追加し、`reports/iterator-collector-summary.md` の `status` カラムで `audit_cap` ケースの経過を追跡する。
- Capability 側のドキュメント更新として `docs/guides/runtime/runtime-bridges.md` に `Core.Collections` の `audit` フラグを追加し、`docs/plans/rust-migration/unified-porting-principles.md` の「同一性優先」セクションに `effect {audit}` 操作の Capability 要件を追記することで移植チームへ誓約と制限を明示する。

##### 5.3.1 CapabilityRegistry と CollectOutcome の接続
`core.collections.audit` は `CapabilityDescriptor` に Stage=Stable/EffectScope=`["audit","mem"]` として登録し、`CollectOutcome` の `effect_labels` を `CapabilityRegistry::check` で検証するヘルパ（`collector.requires_audit_capability()` など）を `compiler/rust/runtime/src/prelude/collectors` に追加する。`CollectorBridge`/`AuditChangeBridge` は `CollectorAuditTrail.metadata["collector.effect.audit"]` を受け取り、`AuditEnvelope` への写像前に `CapabilityRegistry::check` を呼び出して不足時は `CollectError::CapabilityDenied` へフォールバックするルートを用意する。`CollectOutcome::audit` の呼び出し時には `collections.diff.*` の `ChangeSet` JSON を作成し、`REML_COLLECTIONS_CHANGE_SET_PATH` 経由で `FormatterContext::change_set` に渡すフローと併せて `Core.Diagnostics` 側に `collector.effect.audit=true` が反映されることを保証する【F:../../docs/notes/stdlib/collections-audit-bridge-todo.md†L1-L40】。

###### 5.3.1.1 実装ステップ
1. `collector.effect.audit` を検出した `CollectOutcome` に対し、`crate::registry::CapabilityRegistry` を使って `core.collections.audit` の Stage/Effect を検証するユーティリティ（`collectors::ensure_audit_capability`）を `compiler/rust/runtime/src/prelude/collectors/mod.rs` に追加する。
2. `CollectOutcome::audit` を `compiler/rust/runtime/src/prelude/collectors/mod.rs` に実装し、`collections.diff` `ChangeSet` を `crate::config::set_collections_change_set_env` 経由で `REML_COLLECTIONS_CHANGE_SET_PATH` に書き込みつつ `CollectError::CapabilityDenied` のエラー型を返すようにする。
3. `CompilerFrontend` 側（`compiler/rust/frontend/src/diagnostic/formatter.rs`）では `FormatterContext::change_set` の生成時に `collections` ペイロードを `metadata`/`extensions` にコピーし、`scripts/validate-diagnostic-json.sh` の `collections.diff` チェックと `tooling/ci/collect-iterator-audit-metrics.py` の `audit_cap` シナリオで検証できるように結線する。

##### 5.3.2 `CollectError::CapabilityDenied` と実行パス
`ListCollector`/`MapCollector`/`TableCollector` などの `finish`/`collect` 処理は、`effect {audit}` を含む操作（`emit_metric("collections.audit", ...)`, `collect_table(..., audit=true)` など）で `CapabilityRegistry::require("core.collections.audit")` を呼び出し、チェックが失敗した場合は `CollectError::CapabilityDenied` を返す。`Diagnostic.extensions["collector.capability"]` に `core.collections.audit` を書き込むことで `scripts/poc_dualwrite_compare.sh` の `--target audit_bridge` シナリオが `collector.effect.audit` の有無と `CapabilityDenied` を突き合わせられる状態にする。`reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` には `capability` / `collections.change_set.total` / `collector.effect.audit` の期待値を含むケースを追加し、`scripts/validate-diagnostic-json.sh --pattern capability --pattern collector.effect.audit` で CI gate とする。

###### 5.3.2.1 実装ステップ
1. `collectors` モジュールに `collector.check_audit_capability` などのヘルパを追加し、`ListCollector::finish` 等の `effect {audit}` 経路から `CapabilityRegistry::require("core.collections.audit")` を呼ぶ。失敗時は `CollectError::CapabilityDenied` を補足して `Diagnostic.extensions["collector.capability"]` を埋め、`collector.effect.audit=false` のログを `AuditEnvelope` へ残す。
2. `CollectOutcome::new`/`CollectError` に `collectors::AuditErrorKind::CapabilityDenied` を追加して `GuardDiagnostic` の `extensions`/`metadata` に `collector.capability` と `collector.effect.audit` の状態を記録し、`scripts/poc_dualwrite_compare.sh --target audit_bridge` でこの payload を検出可能にする。
3. `reports/iterator-collector-summary.md` の `status` カラムに `audit_cap` ケースを追記し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` Phase3 Capability 行に `core.collections.audit` pass rate・`collectors.audit_cap` latency を追加する。`scripts/validate-diagnostic-json.sh` と `tooling/ci/collect-iterator-audit-metrics.py --scenario audit_cap` で `collector.effect.audit`, `collector.capability`, `collections.change_set.total` のキーを検証して CI gate を構築する。

##### 5.3.3 テストとメトリクスの自動化
`tooling/ci/collect-iterator-audit-metrics.py --section collectors --scenario audit_cap` では `collector.effect.audit` を 1、`collections.change_set.total` を 0 以上、`collector.capability` を含む JSON セルを検証し、`REML_COLLECTIONS_CHANGE_SET_PATH` から読み込んだ `ChangeSet` が `change_set.total`/`items` を満たすことを保証する。`scripts/validate-diagnostic-json.sh --suite collectors --pattern collector.effect.audit --pattern collector.capability --pattern collections.change_set.total` を Phase3 CI gate へ組み込み、失敗時に `reports/iterator-collector-summary.md` の `status=audit_cap` に `collector.capability`/`effect.audit` の値と `reason` を記録して `reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` の `capability` ケースと連動させる。
`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の Phase3 Capability 行には `core.collections.audit` pass rate、`collector.effect.audit_presence`、`collections.change_set.total` の平均 latency を追加し、`reports/iterator-collector-summary.md` の `audit_bridge` 表と `reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` の KPI セクションへ同一値を写す。`docs/notes/process/spec-integrity-audit-checklist.md` の `Collections.audit_bridge` 項目も `effect {audit}` カバレッジのチェックリストに `audit_cap` ケースを追記し、`reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` の `capability` ケース ID と `scripts/poc_dualwrite_compare.sh --target audit_bridge` のログへのリンクを記録する【F:../../docs/notes/process/spec-integrity-audit-checklist.md†L20-L26】。

##### 5.3.4 ドキュメントと運用ガードの補強
`docs/guides/runtime/runtime-bridges.md` では `core.collections.audit` 登録の手順、Stage/Effect の契約、`RuntimeBridgeDescriptor` が `collector.effect.audit` を伝搬する方法、`collect-iterator-audit-metrics.py --scenario audit_cap` によるモニタリングを例示し、`docs/plans/bootstrap-roadmap/3-8-core-runtime-capability-plan.md` の Stage 連携節とのリンクと `REML_COLLECTIONS_CHANGE_SET_PATH` の注入ルートを挿入する【F:../../docs/guides/runtime/runtime-bridges.md†L1-L120】。`docs/plans/rust-migration/unified-porting-principles.md` には `effect {audit}` を含む API が Capability を満たす前提でのみ `Core.Collections` を使用できると明記し、不可欠な `collect`/`map` オペレーションを部分的に制限する方針を脚注で追記。`docs/notes/stdlib/collections-audit-bridge-todo.md` も `Capability` 対応済みのトラッキングリストを更新し、`ChangeSet` 生成→`REML_COLLECTIONS_CHANGE_SET_PATH`→`FormatterContext::change_set` の 3 段階が整備済みであることを確認する。また `reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` の `case=audit_cap` を `docs/notes/process/spec-integrity-audit-checklist.md` にリンク付きで列挙し、`scripts/poc_dualwrite_compare.sh --target audit_bridge` や CI KPI の `collector.capability` 検証と同期する運用手順を補強する。
...
## リスクとフォローアップ
- Finger tree 実装が性能目標を満たさない場合、代替構造 (RRB-Tree 等) の調査を `docs/notes/stdlib/core-library-outline.md` に記録し、Phase 4 で検討する。
