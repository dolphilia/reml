...
###### 3.2.4 ドキュメント・監査連携
- `reports/iterator-collector-summary.md` の `collect_cell_ref_effects` セクションに `cell_mutations_total` / `ref_borrow_conflict_rate` / `collector.effect.rc` の説明を追記し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の KPI 表および `docs/plans/bootstrap-roadmap/assets/metrics/core_collections_persistent.csv` の `RefInternalMutation` 行で新規メトリクスを追跡する。`docs/notes/core-library-outline.md` に内部可変性スタックのメモを立て、`docs/plans/rust-migration/p2-spec-compliance-gap.md` の該当セクションとリンクさせて仕様整合性の監査ルートを明示する。Run ID `20290730-collectors-suite-gate` では `threshold metric missing` をログに残し、ケース追加後にゼロへ遷移することを目標とする。
