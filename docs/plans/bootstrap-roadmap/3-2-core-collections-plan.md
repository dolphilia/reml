...
##### 5.3.3 テストとメトリクスの自動化
`tooling/ci/collect-iterator-audit-metrics.py --section collectors --scenario audit_cap` では `collector.effect.audit` を 1、`collections.change_set.total` を 0 以上、`Classifier` で `REML_COLLECTIONS_CHANGE_SET_PATH` に出力した JSON が読み込まれることを検証する。`scripts/validate-diagnostic-json.sh --suite collectors --pattern collector.effect.audit --pattern collections.change_set.total` を `Phase3` CI gate へ組み込み、失敗時は `reports/iterator-collector-summary.md` の `status` カラムへ `audit_cap` のテスト名と失敗理由を記録する。`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の Phase3 メトリクス表には `audit_cap` の pass rate と `collector.effect.audit_presence` を追加し、`docs/notes/spec-integrity-audit-checklist.md` の `Collections.audit_bridge` 項に `effect {audit}` カバレッジを TODO として追記する【F:../../docs/notes/spec-integrity-audit-checklist.md†L20-L30】。
