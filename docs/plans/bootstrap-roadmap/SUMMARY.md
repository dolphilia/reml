...
5. **[4-1-dsl-composability-standard-plan.md](4-1-dsl-composability-standard-plan.md)**: DSL Composability Standard の仕様化と実装計画
6. **[4-1-core-lsp-derive-plan.md](4-1-core-lsp-derive-plan.md)**: Auto-LSP Derive（補完/アウトライン/ハイライト導出）
7. **[4-1-core-parse-cst-plan.md](4-1-core-parse-cst-plan.md)**: CST サポートとロスレスパース計画
8. **[4-1-core-parse-lex-helpers-impl-plan.md](4-1-core-parse-lex-helpers-impl-plan.md)**: LexPreset/lex_pack 置換と WS3 Step3 回帰接続
9. **[4-1-core-parse-error-labeling-impl-plan.md](4-1-core-parse-error-labeling-impl-plan.md)**: 期待集合ラベル統一（CP-WS2-001）
10. **[4-1-core-parse-input-zero-copy-impl-plan.md](4-1-core-parse-input-zero-copy-impl-plan.md)**: Input/Zero-copy の不変条件チェックと回帰接続（WS5）
11. **[4-1-core-parse-left-recursion-impl-plan.md](4-1-core-parse-left-recursion-impl-plan.md)**: 左再帰検出/ガードの回帰接続（WS6）
12. **[4-2-practical-execution-pipeline-plan.md](4-2-practical-execution-pipeline-plan.md)**: `run_examples.sh`/`cargo test -p reml_e2e` による実行パイプライン
...
2. **[5-1-jit-backend-plan.md](5-1-jit-backend-plan.md)**: JIT バックエンド調査・設計
3. `tooling/bootstrap/self_host_runner.py`（予定）: Stage チェーン自動化
4. `.github/workflows/rust-self-host.yml`（予定）: 週次 Self-host ジョブ
5. `reports/self-host/`（予定）: Stage ごとのビルド・診断・監査レポート
...
- 79-88週: JIT バックエンド調査を並行で実施し、Phase 6 以降の拡張計画へ引き継ぐ
