...
| ✅　F1-4 `unfold`/`try_unfold` | `Iter::unfold` は状態クロージャを `IterSeed` に格納。`try_unfold` は `Result<Option<(T, State)>, E>` を受け、`Err(E)` を `IterStep::Error` として `EffectLabels::residual = ["debug"]`、`AuditEnvelope.metadata.iterator.error.kind = "try_unfold"` を設定。 | `.../iter/generators.rs`, `.../iter/effects.rs` | `core_iter_generators.rs::try_unfold_error_passthrough` で `iterator.error.kind = "try_unfold"` を snapshot 化。`scripts/validate-diagnostic-json.sh --pattern iterator` で `iterator.error.kind` が検出される。 |
| F1-5 API 在庫同期 | `prelude_api_inventory.toml` に `module = "Iter"` エントリを追加し `rust_status` を `working`→`implemented` に段階更新。`cargo xtask prelude-audit --section iter --baseline docs/spec/3-1-core-prelude-iteration.md --wbs 3.1c-F1` を nightly job へ登録し、JSON 出力を `reports/spec-audit/ch1/iter.json` に保存、`reports/spec-audit/ch0/links.md` から参照する。 | `docs/plans/bootstrap-roadmap/assets/prelude_api_inventory.toml`, `reports/spec-audit/ch1/iter.json` | `prelude_api_inventory` の `last_updated = "37w / WBS 3.1c-F1"` を記録し、`reports/spec-audit/ch0/links.md#iter-generators` に `cargo xtask prelude-audit` の証跡が追加される。 |
...
- **F1-4 設計ログ（2025-12-19 着手）**: `docs/spec/3-1-core-prelude-iteration.md:176-221` で定義された `unfold`/`try_unfold` の条件を `compiler/rust/runtime/src/prelude/iter/generators.rs` の `IterSeed`/`IterSource` レイヤへ落とし込み、`IterSeed::new(label, stage)` に状態クロージャを保存する `IterSeedUnfold<S, T>`（内部構造体）の追加、`IterStepMetadata` で `iterator.error.kind` を後付けできる `fn flag_error(&mut self, kind: &str)` を実装するタスクを整理した。`unfold` は `EffectSet::PURE` を維持し、`collect-iterator-audit --section iter --case unfold --output reports/spec-audit/ch1/iter.json#audit_cases.unfold` で `iterator.stage.audit_pass_rate=1.0` を確認する。`try_unfold` は `EffectSet::PURE.with_debug()` を `IterState::with_effects` から注入し、`Err(E)` 経路で `EffectLabels { debug: true, .. }` を `attach_effects` へ渡しながら `AuditEnvelope.metadata.iterator.error.kind = "try_unfold"` を残す。テスト面では `compiler/rust/frontend/tests/core_iter_generators.rs` に `unfold_fibonacci_pipeline` と `try_unfold_error_passthrough`（`insta` snapshot で `iterator.error.kind` を固定）を追加し、`scripts/validate-diagnostic-json.sh --pattern iterator` と `cargo xtask prelude-audit --section iter --wbs '3.1c F1-4'` を完了条件に設定。計測結果は `reports/spec-audit/ch1/iter.json#audit_cases.try_unfold` へ追加し、`docs/plans/bootstrap-roadmap/assets/prelude_api_inventory.toml` の `Iter::unfold`/`Iter::try_unfold` エントリ（`rust_status=pending`, `wbs = "3.1c F1-4"`, `last_updated` 参照）を更新、`docs/notes/core-library-outline.md#iter-generators-f1-4` と `reports/spec-audit/ch0/links.md#iter-f1-4` から逆引き可能にする。
