...
- F3 は `compiler/rust/frontend/tests/core_iter_pipeline.rs` の 6 シナリオ snapshot と `scripts/validate-diagnostic-json.sh`/`tooling/ci/collect-iterator-audit-metrics.py --module iter --section collectors --output reports/iterator-stage-summary.md` を組み合わせた検証を行い、`iterator.effect.debug = 0`／`iterator.stage.audit_pass_rate = 1.0`／`collector.effect.mem = 0` を確認する。テスト結果と snapshot へのリンクは `reports/spec-audit/ch0/links.md#iter-f3-snapshotkpi` に集約し、KPI 値は `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` へ転記する。逸脱は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` に記録し、M1 Go/No-Go 判定に反映する。
...
- 生成 API 実装後は `compiler/rust/frontend/tests/core_iter_generators.rs` と `core_iter_pipeline.rs` を増設し、`Iter::from_list |> Iter.collect_list`、`Iter::range |> Iter.take |> Iter.collect_vec`、`Iter::buffered` の `effect {mem}` 計測を `collect-iterator-audit-metrics.py` で固定。結果 JSON は `reports/spec-audit/ch1/iter.json` へ保存し `reports/spec-audit/ch0/links.md` から逆引きする。
