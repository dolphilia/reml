...
- この KPI セットは `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` に同期され、`collector.effect.mem=0`／`collector.stage.audit_pass_rate=1.0`／`collector.error.invalid_encoding=0`（正常系）を通過条件とした F2 の品質ゲートとして扱う。`docs/notes/core-library-outline.md#collector-f2-監査ログ` でもイテレータ／コレクタ実装状況として同じログを参照している。
- M1 レビューでは `reports/spec-audit/ch0/links.md#collector-f2-監査ログ` から監査ログを参照し、`Collector` トレイトおよび標準コレクタで `rust_status=implemented` となった API と KPI 再現性を確認する。このセクションは `docs/notes/core-library-outline.md#collector-f2-監査ログ` とのクロスリンクにより、Collector F2 の証跡を 2 方向でたどれるようにしている。
...
- F3 では本来 `compiler/rust/frontend/tests/core_iter_pipeline.rs` の 6 シナリオ snapshot と `scripts/validate-diagnostic-json.sh`/`tooling/ci/collect-iterator-audit-metrics.py --module iter --section collectors --output reports/iterator-stage-summary.md` を組み合わせた検証を実施するが、2026-W05 現在は `core_iter_pipeline.rs` および `core_iter_pipeline__*.snap` が存在せず、従来のリンク（`reports/spec-audit/ch0/links.md#iter-f3-snapshotkpi`）も無効である。TODO: pipeline snapshot 作成後に当該小節を復活させ、`iterator.effect.debug = 0`／`iterator.stage.audit_pass_rate = 1.0`／`collector.effect.mem = 0` をエビデンス付きで確認し、KPI 値を `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` へ転記する。逸脱は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` で追跡する。
...
### 3.0.3a Prelude/Iter WBS 3.1c 連携
...
- 生成 API 実装後は `compiler/rust/frontend/tests/core_iter_generators.rs` に加えて `core_iter_pipeline.rs` を新設し、`Iter::from_list |> Iter.collect_list`、`Iter::range |> Iter.take |> Iter.collect_vec`、`Iter::buffered` の `effect {mem}` 計測を `collect-iterator-audit-metrics.py` で固定する予定だったが、当該テスト／snapshot はまだ作成されていない。TODO: pipeline ファイル作成後に `reports/spec-audit/ch1/iter.json` と `reports/spec-audit/ch0/links.md` を実データで再接続し、`collector.effect.mem`／`iterator.stage.audit_pass_rate` を M1 根拠として活用できる状態へ戻す。
