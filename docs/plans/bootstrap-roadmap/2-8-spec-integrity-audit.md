...
5. **Parser API 本格実装と CLI 連携（ブラッシュアップ提案①対応）**
   ...
   - **検証とクローズ条件**:
      - `cargo run --bin poc_frontend -- --emit-diagnostics docs/spec/1-1-syntax/examples/use_nested.reml --parse-debug-output tmp/use_nested-parse.json` の結果に `packrat_cache` と `legacy_error` が含まれ、OCaml 実装の JSON とフィールド差分 0 である。
      - `reports/spec-audit/ch1/use_nested-YYYYMMDD-parse-result.json` に `RunConfig.require_eof`・`left_recursion`・`lex.identifier_profile` が保存され、`git rev-parse HEAD` を脚注に残している。
      - `docs/plans/rust-migration/overview.md` Phase 1 セクションへ「Parser API 完全化」チェック項目を追加し、`docs/spec/2-1-parser-type.md` へのリンクとともに PR を提出済み。
