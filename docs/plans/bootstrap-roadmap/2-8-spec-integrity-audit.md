...
- `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` の「差分分類」を Phase 2-8 起動週のベースラインとして固定し、以降の更新は `reports/spec-audit/diffs/` と `docs/notes/process/spec-integrity-audit-checklist.md` へログを残す。
...
- `docs/notes/process/spec-integrity-audit-checklist.md` を Phase 2-8 仕様で更新し、`Phase 2-8 初動チェック` と `rust-gap トラッキング表` を追加。`reports/spec-audit/summary.md` と相互参照し、Chapter ごとの TODO を可視化する。
...
- CLI 実行手順（`cargo run --manifest-path compiler/rust/frontend/Cargo.toml --bin poc_frontend -- --input examples/docs-examples/spec/1-1-syntax/*.reml` 等）を `reports/spec-audit/ch1/README.md` に記し、`docs/spec/1-1-syntax.md` のコード片と 1:1 で突き合わせる。
- エラー発生時は差分リストに追記し、修正案を `docs/notes/process/spec-integrity-audit-checklist.md` に記録。OCaml 実装での再現確認は任意かつリファレンス使用のみに留める。
...
- `docs/guides/compiler/core-parse-streaming.md` の内容と Chapter 2 の記述を同時更新。
...
- `compiler/rust/frontend/tests/streaming_metrics.rs` で定義されている `streaming_expected_token_snapshot_matches` / `streaming_diagnostics_inject_expected_tokens` を Rust 版で実行し、`docs/spec/2-5-error.md` と `docs/guides/compiler/core-parse-streaming.md` の `parse.expected` 仕様に沿って `ExpectationSummary` が埋め込まれているか確認する。
...
4.3. **ガイド・ノートとの同期**
- `docs/guides/dsl/plugin-authoring.md`, `docs/guides/runtime/runtime-bridges.md` などを章更新内容に合わせて調整。
- `docs/notes/dsl/dsl-plugin-roadmap.md`, `docs/notes/stdlib/core-library-outline.md` に監査結果とフォローアップ TODO を記載。
...
- 大規模修正は PR 単位でレビューし、承認ログを `docs/notes/process/spec-integrity-audit-checklist.md` に記録。
...
- Phase 3 で扱うべき TODO を `docs/notes/process/spec-integrity-audit-checklist.md` に残す。
...
- 外部公開向けチェック未整備: Phase 3 での公開を見据え、ライセンス・記法・翻訳関連の TODO を `docs/notes/process/spec-integrity-audit-checklist.md` に残す。
...
2. Trace/diagnostic 拡張: `ParserTraceEventKind` に式・効果・`operation/resume` を追加し、`TraceEvent::ExprEnter`/`ExprLeave` を `syntax:expr-*` 命名規約で固定。`reports/spec-audit/ch1/effect_handler-*.json` へ Streaming/非 Streaming 双方のトレースを焼き付け、`docs/notes/process/spec-integrity-audit-checklist.md` の `rust-gap SYNTAX-003` 行を「Trace 取得済み」でクローズできる状態を Phase 2-8 のクローズ条件に含める。
...
1. **Rust Parser のトップレベル拡張**
   ...
   - `module_parser` 序盤で `module` と `use` を確定させ、`examples/docs-examples/spec/1-1-syntax/use_nested.reml` が構文エラーにならない状態を確認する。`cargo run --bin poc_frontend -- --emit-diagnostics examples/docs-examples/spec/1-1-syntax/use_nested.reml --trace-output reports/spec-audit/ch1/use_nested-YYYYMMDD-trace.md` を実行し、`reports/spec-audit/ch1/use_nested-YYYYMMDD-diagnostics.json` を差し替えて `rust-gap SYNTAX-002` のステータスを `docs/notes/process/spec-integrity-audit-checklist.md` に反映する。
   ...
   - **検証とクローズ条件**:
     - `cargo run --bin poc_frontend -- --emit-diagnostics examples/docs-examples/spec/1-1-syntax/use_nested.reml` が診断 0 件で完了し、`use_nested_rustcap.reml` が不要になったことを `docs/spec/1-1-syntax.md` 脚注で案内できる状態。
     - `reports/spec-audit/diffs/SYNTAX-002-ch1-rust-gap.md` に「Expected Result」「Current Result」「対処ステータス: In Progress→Closed」まで記入し、`docs/notes/process/spec-integrity-audit-checklist.md` の `SYNTAX-002` 行に `owner: Rust Parser WG / due: W37` を追記。
...
2. **式/ブロック/効果構文の段階的実装**
   ...
   - ハンドラ構文では `operation log(args, resume) { ... }` を `DeclKind::Handler` と `OperationDecl` に展開し、`resume` の型検証を `TypeAnnot` と共有する。`examples/docs-examples/spec/1-1-syntax/effect_handler.reml` を受理できることを Rust Frontend CLI で再現し、`reports/spec-audit/ch1/effect_handler-YYYYMMDD-diagnostics.json` を保存する。
   - 受理後に `docs/notes/process/spec-integrity-audit-checklist.md` の `rust-gap SYNTAX-003` をクローズし、`docs/plans/rust-migration/p1-rust-frontend-gap-report.md` へ差分結果を逆参照する。
   - **実務タスク**（W37 後半の 3 日を上限）:
     ...
     4. `examples/docs-examples/spec/1-1-syntax/effect_handler.reml` / `examples/docs-examples/spec/1-1-syntax/effect_logger.reml` を対象に `scripts/poc_dualwrite_compare.sh effect_handler` を追加実行し、OCaml 実装との診断同等性を `reports/spec-audit/ch1/effect_handler-YYYYMMDD-dualwrite.md` へ記録する。`docs/plans/rust-migration/1-2-diagnostic-compatibility.md` の「効果診断ギャップ」節に Rust 実装の進捗を追記し、`p1-rust-frontend-gap-report.md` では `SYNTAX-003` の残作業を `Closed(P2-8)` に切り替える。
      5. `docs/notes/process/spec-integrity-audit-checklist.md` に `block_scope`, `effect_handler`, `perform_do` の監査行を新設し、`owner`, `evidence(log)` を埋める。併せて `reports/spec-audit/diffs/SYNTAX-003-ch1-rust-gap.md` へ AST・診断の差分ログを追加し、`docs/plans/bootstrap-roadmap/2-5-proposals/PARSER-002-proposal.md` から参照できるよう脚注リンクを追記する。
   - ✅ 進捗メモ（2025-11-18 更新）: `ExprParser` 分離と `TraceEvent::ExprEnter/Leave` 拡張を完了し、`block_scope.reml`（`reports/spec-audit/ch1/block_scope-20251118-diagnostics.json`）と `effect_handler.reml`（`effect_handler-20251118-diagnostics.json`）の診断 0 件ログを取得。dual-write 結果は `reports/spec-audit/ch1/effect_handler-20251118-dualwrite.md` に保存し、`reports/spec-audit/diffs/SYNTAX-003-ch1-rust-gap.md` を作成。`docs/notes/process/spec-integrity-audit-checklist.md` の `SYNTAX-003` 行を `Closed(P2-8)` に更新済み。
   - **検証とクローズ条件**:
      - `cargo test --manifest-path compiler/rust/frontend/Cargo.toml parser::expr`（新設）と `cargo run --bin poc_frontend -- --emit-diagnostics examples/docs-examples/spec/1-1-syntax/effect_handler.reml` がいずれも 0 exit / 診断一致で完了し、`reports/spec-audit/ch1/effect_handler-YYYYMMDD-diagnostics.json` が `CI_RUN_ID` 付きで保存されている。
...
3. **module_parser の再実装マイルストーン**
   ...
   - **実務タスク**（W37→W38, 4 日を上限）:
      ...
      2. `compiler/rust/frontend/tests/parser.rs` に `module_header_accepts_use_nested`, `module_decl_accepts_effect_handler`, `module_operation_resume_roundtrip` などの統合テストを追加し、`cargo test --manifest-path compiler/rust/frontend/Cargo.toml parser::module -- --nocapture` を `reports/spec-audit/ch1/module_parser-YYYYMMDD-parser-tests.md` に保存する。テストケースは `examples/docs-examples/spec/1-1-syntax/*.reml`（`use_nested`、`effect_handler`、`block_scope`）を直接読み込んで JSON 診断を比較する。
   ...
   - **検証とクローズ条件**:
      ...
      - ✅ 2025-11-20: `examples/docs-examples/spec/1-1-syntax/README.md`・`docs/spec/0-3-code-style-guide.md` に module_parser テスト保存ルールを追記し、`docs/plans/rust-migration/overview.md` から Phase 1 完了条件として参照できるようリンクした（`module_parser` の監査証跡は `docs/notes/process/spec-integrity-audit-checklist.md` `SYNTAX-002/module_parser` 行で `Closed` 管理）。
4. **Streaming テストと差分ログ更新**
   ...
   - **実務タスク（W38 前半 / 3 日上限）**:
      ...
      2. `cargo test --manifest-path compiler/rust/frontend/Cargo.toml streaming_metrics -- --nocapture | tee reports/spec-audit/ch1/streaming_metrics-YYYYMMDD-log.md` を実行し、`reports/spec-audit/ch1/` のログと `reports/spec-audit/diffs/SYNTAX-002-ch1-rust-gap.md` に `Streaming acceptance` セクションを追加する。同時に `examples/docs-examples/spec/1-1-syntax/README.md` のチェックリストへ Streaming 実行の項目を追加し、`*_rustcap.reml` の削除条件（Streaming テストと CLI 実行がともにグリーンであること）を記載する。
   ...
   - **検証とクローズ条件**:
      ...
      - `reports/spec-audit/ch1/streaming_use_nested-YYYYMMDD-diagnostics.json` / `streaming_effect_handler-YYYYMMDD-diagnostics.json`（および `ch2/streaming/` の同名ファイル）が生成され、`docs/notes/process/spec-integrity-audit-checklist.md#rust-gap-トラッキング表` の `ERR-001` 行に `Closed (P2-8 W38)` を記入済み。
...
5. **Parser API 本格実装と CLI 連携（ブラッシュアップ提案①対応）**
   ...
   - **成果物・ログ管理**:
     ...
     - `docs/notes/process/spec-integrity-audit-checklist.md` の `FRG-07` と `FRG-11` に `owner: Parser WG`・`artifact: parser_api-YYYYMMDD-log.md` を記録し、進捗を W38 後半のレビューで共有する。
   ...
   - **関連ドキュメント更新**:
     ...
     - `examples/docs-examples/spec/1-1-syntax/README.md` に `--emit-parse-result` と `RunConfig` JSON の保存手順を追加し、Chapter 1 サンプルの ParseResult ログ参照先を `reports/spec-audit/ch1/` に揃える。
   - **実務タスク（W38 後半 / 3 日上限）**:
      ...
      3. `cargo test --manifest-path compiler/rust/frontend/Cargo.toml parser::api parser::driver -- --nocapture` を `reports/spec-audit/ch1/parser_api-YYYYMMDD-log.md` へ保存し、`docs/notes/process/spec-integrity-audit-checklist.md` の `FRG-07` 行を `Closed(P2-8)` へ変更。`docs-migrations.log` へ Parser API 切替を記録。
   - ✅ 進捗メモ（2025-11-22 更新）: `compiler/rust/frontend/src/parser/api.rs` を `pub mod api` として再公開し、`State<'src>` に `PackratMemo`（`FxHashMap<usize, PackratCacheEntry>`）を追加。`Reply<T>` の `Err` へ `committed: bool` を導入して `parser/mod.rs` の `cut()`/`attempt()` マクロから利用できるようにした。`ParserDriver::parse_with_options_and_run_config` は `Parser::<Module>::parse(state, run_config)` へ統一し、`ParseResult.packrat_cache` と `RunConfig` を CLI 経由で保存する経路を `poc_dualwrite_compare.sh --emit-parse-result` と `collect-iterator-audit-metrics.py` 双方に配線した。`reports/spec-audit/ch1/parser_api-20251122-log.md`（`CI_RUN_ID=rust-frontend-parser-api-20251122.2`, `git rev-parse HEAD = 7a1ab812`）および `parser_api-20251122-parse-diff.md` / `use_nested-20251122-parse-result.json` を追加し、`docs/notes/process/spec-integrity-audit-checklist.md` の `FRG-07`/`FRG-11` を `Closed (P2-8 W38)` へ更新。`docs/plans/rust-migration/1-0-front-end-transition.md`（W3）と `p1-rust-frontend-gap-report.md` にも PackratMemo 組み込み完了ログをリンクし、`docs-migrations.log` へ「Parser API 切替 / RunConfig JSON 保存ルール / poc_dualwrite_compare.sh parse_result 拡張」を追記済み。
   - **検証とクローズ条件**:
      - ✅ 2025-11-22: `cargo run --bin poc_frontend -- --emit-diagnostics examples/docs-examples/spec/1-1-syntax/use_nested.reml --parse-debug-output tmp/use_nested-parse.json` を実行し、`tmp/use_nested-parse.json.packrat_cache` と `legacy_error` が OCaml CLI の JSON とフィールド差分 0 であることを `reports/spec-audit/ch1/parser_api-20251122-parse-diff.md` に記録。
      ...
      - ✅ 2025-11-22: `docs/plans/rust-migration/overview.md` Phase 1 テーブルへ「Parser API 完全化 (PackratMemo + RunConfig JSON)」を追記し、`docs/spec/2-1-parser-type.md` の脚注・`examples/docs-examples/spec/1-1-syntax/README.md` の `--emit-parse-result` ガイドとともに PR を発行。`README` 反映後 `docs-migrations.log` へ差分 ID `DOCS-20251122-PAI` を登録。
...
6. **Trace / Diagnostics 拡張（ブラッシュアップ提案②対応）**
   ...
   - **実務タスク（W39 前半 / 2 日上限）**:
      ...
      2. `scripts/poc_dualwrite_compare.sh effect_handler --trace` を実行してトレース差分を `reports/spec-audit/ch1/effect_handler-YYYYMMDD-trace.md` に保存。`examples/docs-examples/spec/1-1-syntax/README.md` の「監査ログ」節にトレース命名規約を追加。
      3. `docs/notes/process/spec-integrity-audit-checklist.md` の `SYNTAX-003` 行へ `Trace coverage >= 4` を満たした証拠を追記し、`reports/spec-audit/diffs/SYNTAX-003-ch1-rust-gap.md` にトレース比較結果を貼付。
   - ✅ 進捗メモ（2025-11-22 更新）: `parser/mod.rs` で `ParserTraceEventKind::{ExprEnter,ExprLeave,EffectEnter,EffectExit,HandlerAccepted,OperationResume}` を実装し、`FrontendDiagnostic.extensions.trace_ids` に重複のない `syntax:expr::<kind>` を出力。`scripts/poc_dualwrite_compare.sh effect_handler --trace` のログを `reports/spec-audit/ch1/trace-coverage-20251122.md` へ集約し、`Trace coverage >= 4`（handle / perform / resume / block）を `effect_handler-20251118-trace.md` / `block_scope-20251118-trace.md` と突き合わせた。`examples/docs-examples/spec/1-1-syntax/README.md` / `docs/spec/0-3-code-style-guide.md` / `docs/plans/rust-migration/unified-porting-principles.md` に命名規約と保存手順を追記済み。
   - **検証とクローズ条件**:
      ...
      - `examples/docs-examples/spec/1-1-syntax/README.md` の脚注と `docs/spec/0-3-code-style-guide.md` が更新され、他章からもトレース命名規約を参照できる状態。
...
7. **CLI プロモーションと型推論の厳格化（ブラッシュアップ提案③対応）**
   ...
   - **検証とクローズ条件**:
      ...
      - `docs/plans/rust-migration/overview.md` / `docs/spec/0-3-code-style-guide.md` / `examples/docs-examples/spec/1-1-syntax/README.md` に新 CLI 名称が掲載され、`docs/notes/process/spec-integrity-audit-checklist.md` の `FRG-05`/`FRG-12` 行が `Closed` へ更新済み。`docs-migrations.log` に CLI 名称変更と Stage/Audit トレース更新を記録。
...
[^streaming-guide]: `docs/guides/compiler/core-parse-streaming.md` §3/§7（Streaming パーサの監査とメトリクス管理）。
