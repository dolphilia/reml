...
3. **module_parser の再実装マイルストーン**
   ...
   - ✅ 進捗メモ（2025-11-19 更新）: `module_parser.rs` を `ModuleStage` 単位に再構成し、`TraceEvent::ModuleStageEntered` / `TraceEvent::ModuleDeclAccepted` を保存できる状態にした。`compiler/rust/frontend/tests/parser.rs` に 6 本の統合テスト（`module_header_accepts_use_nested`, `module_use_reports_shadowing`, `module_decl_accepts_effect_handler`, `module_decl_reports_resume_without_operation`, `module_decl_blocks_roundtrip`, `module_decl_dualwrite_snapshot`）を追加し、`reports/spec-audit/ch1/module_parser-20251119-parser-tests.md` に `cargo test ... parser::module -- --nocapture` のログ（`git rev-parse HEAD = f9e10ae6` を添付）を保存済み。dual-write 比較結果は `reports/spec-audit/ch1/module_parser-20251119-dualwrite.md` と `reports/spec-audit/ch1/2025-11-17-syntax-samples.md#2025-11-19-module_parser-再実装ログ` へ反映し、`docs/notes/spec-integrity-audit-checklist.md` の `SYNTAX-002/module_parser` 行を `In Review (P2-8 W38)` に設定した。
   - **検証とクローズ条件**:
      - `cargo test --manifest-path compiler/rust/frontend/Cargo.toml parser::module parser::expr -- --nocapture` が CI 上で緑化し、`reports/spec-audit/ch1/module_parser-YYYYMMDD-parser-tests.md` に `CI_RUN_ID`・`git rev-parse HEAD`・主要テストケースの統計を記録している。
      - `scripts/poc_dualwrite_compare.sh use_nested` と `effect_handler` がいずれも 0 diff を報告し、`reports/spec-audit/ch1/module_parser-YYYYMMDD-dualwrite.md` に結果を保存する。`docs/plans/rust-migration/3-0-ci-and-dual-write-strategy.md` の「CI ブロッカー」節に Module Parser チェックを追加し、差分レビューを `Closed(P2-8)` とする。
      - `docs/spec/1-1-syntax/examples/README.md` と `docs/spec/0-3-code-style-guide.md` に module_parser テストの保存ルールを追記し、`docs/plans/rust-migration/overview.md` から Phase 1 完了条件（Rust Frontend module parser の受理範囲）が参照できる。
