...
1. **Rust Parser のトップレベル拡張**
   ...
   - `module_parser` 序盤で `module` と `use` を確定させ、`examples/docs-examples/spec/1-1-syntax/use_nested.reml` が構文エラーにならない状態を確認する。`cargo run --bin poc_frontend -- --emit-diagnostics examples/docs-examples/spec/1-1-syntax/use_nested.reml --trace-output reports/spec-audit/ch1/use_nested-YYYYMMDD-trace.md` を実行し、`reports/spec-audit/ch1/use_nested-YYYYMMDD-diagnostics.json` を差し替えて `rust-gap SYNTAX-002` のステータスを `docs/notes/spec-integrity-audit-checklist.md` に反映する。
...
3. **module_parser の再実装マイルストーン**
   ...
   - **検証とクローズ条件**:
      ...
      - ✅ 2025-11-20: `examples/docs-examples/spec/1-1-syntax/README.md`・`docs/spec/0-3-code-style-guide.md` に module_parser テスト保存ルールを追記し、`docs/plans/rust-migration/overview.md` から Phase 1 完了条件として参照できるようリンクした（`module_parser` の監査証跡は `docs/notes/spec-integrity-audit-checklist.md` `SYNTAX-002/module_parser` 行で `Closed` 管理）。
...
7. **CLI プロモーションと型推論の厳格化（ブラッシュアップ提案③対応）**
   ...
   - **検証とクローズ条件**:
      ...
      - `docs/plans/rust-migration/overview.md` / `docs/spec/0-3-code-style-guide.md` / `examples/docs-examples/spec/1-1-syntax/README.md` に新 CLI 名称が掲載され、`docs/notes/spec-integrity-audit-checklist.md` の `FRG-05`/`FRG-12` 行が `Closed` へ更新済み。`docs-migrations.log` に CLI 名称変更と Stage/Audit トレース更新を記録。
