...
- `reports/spec-audit/ch5/` にダッシュボード（`spec-core-dashboard.md`, `practical-suite-index.md`, `perf-summary.md` 等）を追加し、Phase 5/6 のレビューで参照できる統一フォーマットを提供する。
...
## スコープ
- **含む**: `tooling/ci/collect-iterator-audit-metrics.py` の拡張、`reports/spec-audit/ch5/` ダッシュボード生成スクリプト、`docs/spec/0-3-audit-and-metrics.md` の KPI 追記、`AuditEnvelope` へのメタデータフィールド追加方針の文書化。
...
- `reports/spec-audit/ch5/spec-core-dashboard.md`, `reports/spec-audit/ch5/practical-suite-index.md`, `reports/spec-audit/ch5/perf-summary.md` を生成するスクリプト（`scripts/gen_phase4_dashboard.py` 等）を整備し、CI から自動更新できる。
...
- `scripts/gen_phase4_dashboard.py` を追加し、`reports/spec-audit/ch5/` に Markdown ダッシュボードを生成。`spec_core_dashboard` は Chapter 1〜3 の Pass/Fail 表、`practical_suite_index` はカテゴリ別 KPI とリンク、`perf_summary` は parse/memory の中央値を掲載。
...
- CI アーティファクトとして `reports/spec-audit/ch5/*.md`, `metrics/phase4-practical.json`, `metrics/phase4-perf.json` を保存し、`reports/spec-audit/ch5/README.md` に再取得手順を記載。
...
- KPI 更新手順を `reports/spec-audit/ch5/README.md` と `docs/plans/bootstrap-roadmap/5-4-field-regression-and-readiness-plan.md` にリンクし、レビュー時の参照ポイントを統一。
...
- **ログ肥大化**: `reports/spec-audit/ch5/*.md` と JSON ログが肥大化する場合は、圧縮アーカイブ (`.tar.zst`) と最新サマリ（`.md`）を分離し、`0-4-risk-handling.md` のストレージ対策ガイドに従う。
