...
- 実装: `compiler/rust/runtime/ffi/src/core_prelude/*.rs`, `compiler/rust/runtime/src/prelude/**/*`
- テスト: `compiler/rust/frontend/tests/core_prelude_option_result.rs`, `core_iter_*.rs`
...
- **現状**: `compiler/rust/runtime/src/prelude/iter/mod.rs:297-305` で `IterStep::Error(_)` を受け取っても `Ok(collector.finish())` を返しているため、`collect` 系 API が iterator 内部の失敗を観測できない。
...
- **対応ステップ**:
  1. `Result.ensure` 相当の Guard 実装 (`compiler/rust/runtime/src/prelude/ensure.rs`) をインベントリへ反映し、`rust_status` を `implemented` に更新。
...
- **対応ステップ**:
  1. `compiler/rust/frontend/tests/core_iter_generators.rs` に `iter_empty_returns_none`、`iter_once_delivers_single_value` を追加し、`stage_snapshot`/`effect_labels` を snapshot で固定。
