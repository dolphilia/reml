...
- Core.IO & Path (3-5 §6) のドキュメント・サンプル更新タスクを着手し、`examples/core_io/file_copy.reml` / `examples/core_path/security_check.reml` と `tooling/examples/run_examples.sh --suite core_io|core_path` を追加。`docs/spec/3-5-core-io-path.md`・`docs/guides/runtime-bridges.md`・`docs/guides/plugin-authoring.md` にサンプル参照や `IoContext` 運用例を追記し、`core_io.example_suite_pass_rate` KPI を `0-3-audit-and-metrics.md` に登録した。

### Phase 4: 移行完了と運用体制

Rust セルフホスト実装を正式版として採用し、エコシステムを移行します（期間: 約18週）。
...
| [4-5-backward-compat-checklist.md](4-5-backward-compat-checklist.md) | M4: 旧実装アーカイブ | 後方互換チェック、移行ガイド | 18週 |
| [4-6-support-policy.md](4-6-support-policy.md) | M4: 旧実装アーカイブ | サポートポリシー、アーカイブ周知 | 18週 |
...
| Phase 4 M1 (6週) | Rust セルフホスト版の最終互換性承認 | 4-1, 0-3 | 正式版採用/旧実装アーカイブ可否 |
