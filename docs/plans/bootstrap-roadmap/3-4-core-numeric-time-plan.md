...
> - `tooling/scripts/update_time_locale_table.py` を追加し、`docs/plans/bootstrap-roadmap/assets/time-format-locale-map.csv` から `compiler/rust/runtime/src/time/locale_table_data.rs` を自動生成するよう変更。`TIME_LOCALE_TABLE` は `LocaleStatus`（Supported/Planned/Deprecated）と `supports_{rfc3339,unix,custom}` を保持し、未サポートロケールやフォーマット種別を `TimeError` で拒否するよう `format_with_locale` を更新した。【F:../../tooling/scripts/update_time_locale_table.py†L1-L87】【F:../../compiler/rust/runtime/src/time/locale_table_data.rs†L1-L43】【F:../../compiler/rust/runtime/src/time/format.rs†L1-L250】
> - `tests/data/time/format/format_cases.json` に `tr-TR` カスタムケースを追加し、`time::tests::planned_locale_is_rejected` で `az-Latn` が `TimeErrorKind::InvalidFormat` を返すことを固定。結果サマリを `reports/spec-audit/ch3/time_format-locales.md` に記録し、T-1 の成果物として共有した。【F:../../tests/data/time/format/format_cases.json†L1-L28】【F:../../compiler/rust/runtime/src/time/mod.rs†L250-L400】【F:../../reports/spec-audit/ch3/time_format-locales.md†L1-L5】
> - `compiler/rust/runtime/src/time/format/icu.rs` を追加し、ICU パターンを `time` 記法へ変換するトランスレータを実装。`tests/data/time/format/icu_cases.json` と `time::tests::{time_format_icu_cases_from_dataset,time_parse_icu_cases_from_dataset}` で往復テストを行い、`reports/spec-audit/ch3/time_format-locales.md` に実行ログを追記した。【F:../../compiler/rust/runtime/src/time/format/icu.rs†L1-L116】【F:../../tests/data/time/format/icu_cases.json†L1-L18】【F:../../compiler/rust/runtime/src/time/mod.rs†L331-L690】
