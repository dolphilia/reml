...
- Unicode 字句診断: `compiler/rust/frontend/tests/lexer_unicode_identifier.rs` 由来の JSON (`reports/spec-audit/ch1/lexer_unicode_identifier-20270329.json`) を `scripts/validate-diagnostic-json.sh --pattern unicode.error.kind --pattern unicode.identifier.raw` で検証し、`unicode.diagnostic.display_span` KPI（`0-3-audit-and-metrics.md`）へ転記する。`Diagnostic.Builder` 側では `unicode.*` 拡張キーを `AuditEnvelope.metadata` と `extensions` の両方に書き込むユーティリティを追加する。
