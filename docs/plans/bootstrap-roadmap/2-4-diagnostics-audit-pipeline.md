...
- 監査永続化（`Cli.Audit_persistence`）が生成する `reports/audit/index.json` / `history/*.jsonl.gz` に対し、拡張キーの欠落チェックと `retained_entries` の再計算を行うスクリプト（`tooling/ci/verify-audit-metadata.py` 仮称）を作成する。
