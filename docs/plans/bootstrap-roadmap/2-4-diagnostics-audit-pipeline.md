...
**進捗状況 (2025-10-29 更新)**
- `Cli.Audit_persistence` に永続ストア初期化と付随ファイル更新を実装し、`summary.md`・`index.json` が `audit_store` ごとに自動更新されるよう調整。
- CI プロファイルでは最新 20 件を gzip 化した `reports/audit/history/<target>.jsonl.gz` を生成し、`~outcome:Failure` 指定時に `reports/audit/failed/<build-id>/` へ監査ログと `entry.json` を退避。
- `Audit_envelope.append_events` が空イベントでもファイルを生成するよう修正し、`index.json` 更新の前提を保証。
- gzip 書き出し用に `camlzip` 依存を追加済み。CI/ローカル双方で `opam install camlzip` を必須リストへ追記予定。
### 4. メタデータ統合（28-29週目）
**担当領域**: 拡張メタデータ
