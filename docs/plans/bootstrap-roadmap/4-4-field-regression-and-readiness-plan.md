...
- `docs/notes/dsl-plugin-roadmap.md`, `docs/guides/dsl/plugin-authoring.md`, `docs/guides/runtime/runtime-bridges.md` に掲載されている DSL/Plugin 例を Phase 4 スイートへ取り込み、実運用シナリオでの再現性を検証する。
...
- `examples/practical/bundle_runtime_bridge` では `docs/guides/runtime/runtime-bridges.md` の `FlowController` サンプルを実行し、`reports/spec-audit/ch4/practical-bundle-runtime-bridge.md` に結果と監査ログを添付。
...
- Chapter 1 の仕様を参照するテストについては `reports/spec-audit/ch4/spec-core-dashboard.md` の Pass/Fail を必ず添付し、境界テスト/意地悪テストが欠落していないかをレビュー項目に含める。
