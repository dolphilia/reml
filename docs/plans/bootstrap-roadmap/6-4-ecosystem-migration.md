...
- 旧 OCaml 実装を参照用として明確にアーカイブし、レジストリやガイドでは非推奨ステータスを付与して Rust セルフホスト版への移行を促す。
...
- パッケージメタデータの作成（[4-2-registry-distribution.md](../../spec/4-2-registry-distribution.md) §2 に準拠）
...
1.3. **OCaml 版参照アーカイブの明示**
- 既存 OCaml 版パッケージに `deprecated: true` / `archived: true` フラグを追加
- アーカイブメッセージで参照用途のみであることと移行先情報を明記
- `advisories` に参照制限と移行告知を追加
- ダウンロード統計を最終記録し、以降は自動配布対象から除外
...
**成果物**: レジストリ登録済みセルフホスト版、参照アーカイブ通知付き OCaml 版メタデータ、配布優先度設定
...
- 旧 OCaml 版からの移行ポイント（ABI 差分、呼び出し規約）を参考情報として整理
- マルチターゲット対応の実装パターン
...
2.4. **後方互換性ガイドの追加**
- 旧 OCaml 版との差異一覧（参照アーカイブ向け）
- 移行時の注意点（Breaking Changes）
- 互換レイヤの提供有無
- OCaml 版が非サポートである旨と参照方法の明示
...
- Rust ベースラインとの出力比較スクリプト（必要に応じて OCaml 版の結果を参考値として添付）
- 診断メッセージ差分チェック
- 性能比較ツール
...
6.4. **互換レイヤの検討**
- 旧 OCaml 版との互換レイヤの必要性評価（最小限に留める）
