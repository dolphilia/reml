...
10. `compiler/rust/runtime/src/parse/embedded.rs` の単体テスト（境界検出・空入力・`end` 欠落）を `compiler/rust/runtime/tests/parse_embedded.rs` に追加する。
...
### 既存実装の範囲
- `compiler/rust/runtime/src/parse/` に `combinator.rs` と `cst.rs` が存在するが、埋め込み DSL を扱う共通インターフェースは未定義。
...
### モジュール分割案
- `compiler/rust/runtime/src/parse/embedded.rs`: `EmbeddedDslSpec` と境界検出ロジック。
- `compiler/rust/runtime/src/parse/combinator.rs`: `embedded_dsl` コンビネーターの公開 API。
- `compiler/rust/runtime/src/diagnostics/dsl.rs`: `Diagnostic.source_dsl` の付与と `AuditEnvelope` の補助。
- `compiler/rust/runtime/src/lsp/embedded.rs`: Span から LSP 委譲先を引くルーティングモデル。
