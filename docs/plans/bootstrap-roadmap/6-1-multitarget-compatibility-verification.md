...
## 目的
- Phase 4 マイルストーン M1 を達成するため、x86_64 Linux/Windows/ARM64 macOS の 3 ターゲットで Rust セルフホスト実装の LLVM IR・バイナリ・診断の差分を検証し、承認閾値内に収束させる。
- リリース前の最終互換性チェックリストを運用し、Rust 実装のみを基準としたマルチターゲット品質リスクを解消する（OCaml 実装は参照用に限る）。
...
- Rust セルフホスト実装の LLVM IR・バイナリ（canonical build）
- CLI 診断出力（サンプルコード実行結果）
- 性能メトリクス（コンパイル時間、実行時間、メモリ使用量）
- Phase 3 から引き継いだゴールデンテスト結果

3.2. **Windows x64 成果物**
- MSVC/GNU ABI 対応の Rust バイナリ
- PE 形式の実行ファイルと付随する PDB
- Windows 固有の診断メッセージ・ログ
- Linux 基準への差分比較用メタデータ（コンパイルフラグ、ツールチェーンバージョン）

3.3. **ARM64 macOS 成果物**
- ARM64 ABI 対応の Rust バイナリ
- Mach-O 形式の実行ファイルと dSYM
- macOS 固有の診断メッセージ
- codesign/Notary 情報（Phase 4-2 との連携）

3.4. **成果物の整理と保存**
- ターゲット別ディレクトリ構造の統一
- 命名規則の徹底（`remlc-rust-{target}-{version}`）
- CI アーティファクトとしての保存
- ローカル検証用のダウンロード手順
...
4.1. **IR 比較実行**
- 各ターゲットの Rust ビルド vs x86_64 Linux 基準の IR 比較
- 差分箇所の抽出と分類
- 許容差分の除外処理
- 未解決差分のフラグ付け
...
4.4. **性能比較**
- コンパイル時間・実行時間・メモリ使用量の比較（Linux 基準 vs Windows/macOS）
- Phase 3 ベースラインとの比較（±5% 目標）
- 差分原因のラベリング（ツールチェーン、OS、ハードウェア）
- メトリクス記録とダッシュボード更新
