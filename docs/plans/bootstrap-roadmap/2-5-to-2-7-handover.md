...
- **ゲート／リスク**: `0-4-risk-handling.md` 登録「効果構文 Stage 昇格遅延」（期限: 2026-09-30）。脚注撤去は KPI 達成と `docs/notes/effects/effect-system-tracking.md` H-O1〜H-O5 完了が条件。
...
- **ERR-002 回復系 FixIt**（`docs/plans/bootstrap-roadmap/2-5-proposals/ERR-002-proposal.md`）: CLI/LSP の自動修正インターフェイス刷新を Phase 2-7 `diagnostics` と連携して実装し、Packrat 経路のカバレッジと翻訳整備を `docs/notes/parser/core-parse-streaming-todo.md` に従って完了させる。
...
- 進捗は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` と `docs/notes/effects/effect-system-tracking.md` のチェックリストで月次レビューを実施。
