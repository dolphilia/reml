...
### 2. 差分・確認事項
- 診断ドメイン拡張（DIAG-001〜003）: `docs/spec/3-6-core-diagnostics-audit.md`, `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md`, `docs/guides/runtime-bridges.md`, `docs/notes/dsl-plugin-roadmap.md` の脚注とメトリクス定義が揃っているか Step7 チェックリストで確認する必要がある。
- RunConfig / Streaming（PARSER-002/003）: `docs/spec/2-1-parser-type.md`, `docs/spec/2-6-execution-strategy.md`, `docs/guides/core-parse-streaming.md`, `docs/notes/core-parse-api-evolution.md` の記述が `parser-runconfig-packrat.json.golden` と同期しているかを検証し、未同期の場合はゴールデンと脚注を更新する。
...
- Phase 2-7 移行チーム向け TODO を整理（`collect-iterator-audit-metrics.py` の PASS 更新タイミング、`docs/guides/plugin-authoring.md` と VS Code 拡張ドキュメントのエイリアス周知フロー更新、`RunConfig.extensions["lex"]` テンプレート提供計画）。
