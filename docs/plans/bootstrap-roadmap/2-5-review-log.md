...
- `RunConfig` の `extensions["effects"].type_row_mode = "metadata-only"` 設定と脚注 `[^lexer-ascii-phase25]`, `[^effects-syntax-poc-phase25]` の撤去条件を再確認し、条件が `2-7-deferred-remediation.md` 0.3 節へ反映されていることを確認した。（TYPE-002 脚注は 2026-12-18 に撤去済み）
...
- 既存の設計メモに記載された AST/Parser 拡張案と CI 指標の導入計画を整理し、PoC スコープと基準値を `docs/notes/effects/effect-system-tracking.md` に反映した（docs/notes/effects/effect-system-tracking.md:1）。
...
### 3. フォローアップ
1. `docs/notes/effects/effect-system-tracking.md` に PoC スコープ表・メトリクス基準値・実装差分を記録し、Step2 以降のタスクと Phase 2-7 への引き継ぎ条件を更新済み（docs/notes/effects/effect-system-tracking.md:9）。
...
## TYPE-002 Step1 効果行統合棚卸（2026-04-10）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/TYPE-002-proposal.md`](./2-5-proposals/TYPE-002-proposal.md#5-実施ステップ)

### 1. 仕様確認
- Chapter 1-2 の効果行規定を再読し、関数型が `A -> B ! Σ` として効果集合を内包する前提で `@handles` や行多相 `∀ε. τ ! ε` が説明されていることを確認（docs/spec/1-2-types-Inference.md:158）。
- Chapter 1-3 §I の残余効果計算 `Σ_after = (Σ_before - Σ_handler) ∪ Σ_residual` と Stage/Capability 連携が、型と効果を同時に扱うことを要件としている点を整理（docs/spec/1-3-effects-safety.md:244）。脚注追加時は両節を同期させる必要がある。

### 2. 実装差分
- 型表現 `ty` は `TArrow of ty * ty` を維持しており、効果行を保持しない（compiler/ocaml/src/types.ml:48）。`generalize` / `instantiate` / `Type_unification` 系の処理では効果集合を比較できない。
- `Type_inference` で関数型を構築する際も効果情報を落としており（compiler/ocaml/src/type_inference.ml:2691）、`Effect_analysis.collect_from_fn_body` → `Type_inference_effect.resolve_function_profile` で収集したタグは `typed_fn_decl.tfn_effect_profile` へ分離される（compiler/ocaml/src/type_inference.ml:2712, compiler/ocaml/src/typed_ast.ml:167）。
- `record_effect_profile` が Capability Stage 整合に用いられるものの（compiler/ocaml/src/type_inference.ml:2723）、型スキームへ伝播しないため `@handles` / `@requires_capability` の比較は外付けロジックに依存している。

### 3. 残課題とタグ
1. 差分タグ `TYPE-002-S1` を発行し、本エントリと `docs/notes/effects/effect-system-tracking.md`（Phase 2-5 TYPE-002 Step1 節）を紐付けた。
2. Step2 では `compiler/ocaml/docs/effect-system-design-note.md` に `TArrow of ty * effect_row * ty` 案と `effect_row` データ構造比較（リスト／ビットセット／RowVar）をまとめる。
3. Step3 で Chapter 1-2 / 1-3 に脚注を追加し、「Phase 2-5 時点は型スキームへ効果行を統合していない」旨と解除条件を明文化、`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` に引き継ぎ条件を登録する。

## TYPE-002 Step2 型表現統合ドラフト（2026-04-18）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/TYPE-002-proposal.md`](./2-5-proposals/TYPE-002-proposal.md#5-実施ステップ)

### 1. 調査と整理
- `Effect_analysis.collect_from_fn_body`（compiler/ocaml/src/type_inference.ml:292）で返却する `tag list` と `typed_fn_decl.tfn_effect_profile` の転記ロジックを読み直し、`effect_row` が同一タグ正規化（`normalize_effect_name`）を共有できることを確認。
- `string list` / `StringSet.t` / `row_var` の 3 候補を比較し、`Σ_after` 演算を高速化するには `StringSet` ベースが妥当だが、診断表示では宣言順を保持する配列が必要という整理に至った。行多相を扱う `row_var` は Phase 2-7 で予約実装とし、現段階では `None` 固定で保持する案を採用。

### 2. 作成したドラフト
- `compiler/ocaml/docs/effect-system-design-note.md` へ「## 3. 型表現統合ドラフト（TYPE-002 Step2, 2026-04-18）」を追加し、`effect_row` レコード案（`declared` / `residual` / `canonical` / `row_var`）と `TArrow of ty * effect_row * ty` 拡張のコードスケッチを掲載。`generalize` / `instantiate` / `Type_unification` / `solve_trait_constraints` / `core_ir/desugar` など影響範囲を表形式で整理した。
- データフロー図を追加し、`Effect_analysis` → `effect_row` → 型スキーム → 診断／監査という一貫経路を視覚化。診断側の `typed_fn_decl.tfn_effect_profile` は暫定的に併存させつつ、型比較を優先する運用へ段階移行する方針を明示した。

### 3. 連携更新とフォローアップ
- `docs/plans/bootstrap-roadmap/2-5-proposals/README.md` に脚注 `[^type-002-row-design]` を追加し、設計ノートの新章へ導線を設定。  
- `docs/plans/bootstrap-roadmap/2-5-proposals/TYPE-002-proposal.md` Step2 を更新し、暫定採用案と Phase 2-7 への宿題（RowVar 実装、`diagnostics.effect_row_stage_consistency` KPI 定義）を記録。  
- 次ステップでは仕様脚注（Chapter 1-2 / 1-3）と `0-3-audit-and-metrics.md` の KPI 追記を行うため、`docs/notes/effects/effect-system-tracking.md` に RowVar 対応と指標シリアライズの TODO を登録。

**差分タグ**: `TYPE-002-S2`（Phase 2-5 Step2 完了）

## TYPE-002 Step3 効果行脚注と移行ガード（2026-04-22）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/TYPE-002-proposal.md`](./2-5-proposals/TYPE-002-proposal.md#5-実施ステップ)

### 1. 仕様更新
- `docs/spec/1-2-types-Inference.md` §A.2 / §C.6、`docs/spec/1-3-effects-safety.md` §A / §I、`docs/spec/3-6-core-diagnostics-audit.md` §2.4.2 に移行ガード脚注を追加し、Phase 2-5 時点では効果行が `ty` へ統合されていないこと、診断メタデータとして暫定運用することを明文化。2026-12-18 更新で脚注を撤去し、本文へ恒常運用を反映。
- `docs/spec/README.md` と `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` に同脚注を導線として追記し、索引と差分リストから TYPE-002 の暫定措置へ遷移できるようにした。

### 2. 移行ガード設計
- `RunConfig.extensions["effects"].type_row_mode` の既定値を `"metadata-only"` に固定し、`type_row_mode = "ty-integrated"` を要求した際に `effects.type_row.integration_blocked` を発行するポリシーを `docs/spec/3-6-core-diagnostics-audit.md` に記載。監査ログには `effect.type_row.requested_mode` / `available_mode` / `guard_stage` を記録する仕様を併記。
- `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#type-002-effect-row-integration` を新設し、Phase 2-7 で実施すべき `TArrow` 拡張・KPI 実装・脚注撤去の順序と検証条件をまとめた。

### 3. フォローアップ
- `docs/notes/effects/effect-system-tracking.md` に Step3 セクションを追加し、脚注・ガード・監査キーの更新点と Phase 2-7 への宿題（`diagnostics.effect_row_stage_consistency` KPI 実装、脚注撤去チェックリスト）を記録。
- `docs/plans/bootstrap-roadmap/2-5-proposals/README.md` の TYPE-002 項および脚注 `[^type-002-row-design]` を更新し、Step3 での脚注整備・診断追加をハイライト。

**差分タグ**: `TYPE-002-S3`（Phase 2-5 Step3 完了）


## TYPE-002 Step4 実装ロードマップとテスト観点（2026-04-24）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/TYPE-002-proposal.md`](./2-5-proposals/TYPE-002-proposal.md#5-実施ステップ)、`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#type-002-effect-row-integration`

### 1. ロードマップ確定
- Phase 2-7 を 3 スプリント（Sprint A: 型表現＋`effect_row` 基盤、Sprint B: `generalize` / `instantiate` / `Type_unification` 拡張と診断連携、Sprint C: Core IR・監査・Windows/macOS クロスチェック）に分割し、着手順序と担当モジュール（`types.ml`, `typed_ast.ml`, `type_inference.ml`, `constraint_solver.ml`, `core_ir/desugar.ml`, `tooling/ci/collect-iterator-audit-metrics.py` など）を `2-7-deferred-remediation.md` へ記録。  
- `RunConfig.extensions["effects"].type_row_mode` の移行手順を「`metadata-only` → `dual-write`（診断と型の両方へ出力） → `ty-integrated`」と定義し、各段階でのガード診断 (`effects.type_row.integration_blocked`) / ロールバック条件 / 監査キー更新 (`effect.type_row.requested_mode`, `...available_mode`, `...guard_stage`) を整理。  
- Core IR 経路では `core_ir/desugar_fn.ml` と `runtime/effect_registry.ml` のメタデータ伝播を `Sprint C` の必須項目として設定し、`effect_system_design_note.md` に追記したデータフロー図と整合させた。

### 2. テスト観点と KPI
- `compiler/ocaml/tests/test_type_inference.ml` に `type_effect_row_equivalence_*` シリーズ（行等価判定・残余効果保持・`@handles` 整合）を追加する計画を確定し、`StringSet` 正規化と表示順維持を両立するゴールデンの整備手順をまとめた。  
- `compiler/ocaml/tests/streaming_runner_tests.ml` には Stage／Capability 監査が `effect_row` 集計後も一致するかを確認する `streaming_effect_row_stage_consistency` ケースを追加予定とし、CLI/LSP/監査 JSON の diff を `reports/diagnostic-format-regression.md` でレビューするフローを定義。  
- `tooling/ci/collect-iterator-audit-metrics.py` に新規 KPI `diagnostics.effect_row_stage_consistency`（診断と監査の効果行一致率）、`type_effect_row_equivalence`（型同値テストの成功率）、`effect_row_guard_regressions`（`metadata-only` ガード違反件数）を追加し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` に指標定義・収集タイミング・合格基準を記載。CI は `--require-success` で 1.0 の維持を必須化する。

### 3. フォローアップ
- `docs/notes/effects/effect-system-tracking.md` に Step4 サマリを追記し、Phase 2-7 着手前に実施する前提タスク（dual-write 実験ブランチ作成、Windows 監査プロファイル拡張、RowVar 評価メモ更新）を TODO として登録。  
- Step5（ハンドオーバー）では Windows/macOS CI で `effect_row_guard_regressions = 0` を確認する ToR を作成し、`docs/plans/bootstrap-roadmap/2-4-to-2-5-handover.md` の後継欄へリンクする。  
- 行多相（`RowVar`）と `Constraint_solver` 拡張は Phase 3 評価項目として `effect-system-tracking.md#phase-3-以降の検討` に移管し、Phase 2-7 では予約値 `Open row_var` の API を封印する方針を維持。

**差分タグ**: `TYPE-002-S4`（Phase 2-5 Step4 完了）

## TYPE-002 Step5 ハンドオーバー準備とリスク登録（2026-04-24）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/TYPE-002-proposal.md`](./2-5-proposals/TYPE-002-proposal.md#5-実施ステップ)

### 1. 調査と整理
- `compiler/ocaml/docs/technical-debt.md` の H1（`type_mapping` TODO）および効果プロファイル管理の未解決課題を再確認し、効果行統合の遅延が Type/Runtime/監査へ及ぼす影響（Self-host CI 判定、`@handles` 整合性、Stage 検証）を再整理した。
- Phase 2-7 の実装スプリント（A: dual-write, B: 型操作対応, C: IR/監査）で必要となる前提条件とブロッカーを棚卸し、`type_row_mode` を `metadata-only` のまま維持した際の暫定運用手順を文書化した。

### 2. ハンドオーバー資料
- `docs/plans/bootstrap-roadmap/2-5-to-2-7-type-002-handover.md` を作成し、Gate 条件（設計レビュー完了、脚注と `type_row_mode` の整合確認、テスト基盤準備、リスクレビュー）と Sprint 計画を記録。  
- `docs/plans/bootstrap-roadmap/2-4-to-2-5-handover.md` に追記して Phase 2-7 週次レビューからアクセスできる導線を確保。

### 3. リスク登録と KPI
- `docs/plans/bootstrap-roadmap/0-4-risk-handling.md` にリスク ID `TYPE-002-ROW-INTEGRATION`（期限 2026-10-31, 状態 Open）を追加し、TYPE/EFFECT 双方の週次レビュー対象とした。EFFECT 系リスクとの依存関係をコメントで明示。  
- `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` で `diagnostics.effect_row_stage_consistency` / `type_effect_row_equivalence` / `effect_row_guard_regressions` を正式 KPI として登録し、Phase 2-7 Sprint A 着手時点で参照できるよう計測経路を点検した。

### 4. フォローアップ
1. Phase 2-7 キックオフで `TYPE-002-G1` レビュー（Type/Effect/Runtime リード参加）を開催し、`TArrow` 拡張の実装案を承認する。  
2. `type_row_mode` を `dual-write` へ切り替える前に `collect-iterator-audit-metrics.py` の拡張が CI 上で動作することを検証し、`effect_row_guard_regressions = 0` を確認する。  
3. KPI 達成後に脚注撤去する際の手順を `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` とリスク台帳へ反映する（2026-12-18 に完了し、記録を更新済み）。

2026-12-18 追記: Phase 2-7 Sprint C で Core IR・Runtime への効果行伝播とマルチプラットフォーム監査が完了。`type_row_mode` 既定値を `"ty-integrated"` へ更新し、関連脚注を撤去。`collect-iterator-audit-metrics.py --section effects --require-success` が Linux/Windows/macOS すべてで `effect_row_guard_regressions = 0` を維持することを確認した。

**差分タグ**: `TYPE-002-S5`（Phase 2-5 Step5 完了）

## EFFECT-002 Step2 AST/Parser PoC 実装（2026-04-12）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/EFFECT-002-proposal.md`](./2-5-proposals/EFFECT-002-proposal.md#5-実施ステップ)

- 構文拡張: `compiler/ocaml/src/parser.mly#L1225-L1352` に `perform` / `do` / `handle ... with` 規則を追加し、`Experimental_effects_disabled` 例外で `-Zalgebraic-effects` 無効時の使用を拒否。効果参照分解ヘルパ（`split_effect_module_path` / `effect_target`）を導入し、Menhir 規則へ組み込んだ。
- AST/Typed AST: `compiler/ocaml/src/ast.ml#L69-L156` と `compiler/ocaml/src/typed_ast.ml#L109-L140` で効果呼び出し・ハンドラ用のノード／補助型を定義し、`ast_printer.ml#L102-L140` で簡易表示を更新。`handler_decl` に `handler_span` を追加し式位置の `handler` リテラルでも位置を保持。
 - RunConfig/ドライバ: `compiler/ocaml/src/parser_run_config.ml#L76-L116` および `parser_run_config.mli#L34-L37` に `experimental_effects` フィールドと setter を追加。`compiler/ocaml/src/parser_driver.ml#L200-L258` でフラグを設定・リセットし、禁止時に `effects.syntax.experimental_disabled` 診断を生成するガードを実装。
 - テスト: `compiler/ocaml/tests/test_parser.ml#L638-L677` に experimental フラグ有無のユニットテストを追加し、`perform` と `handle` が PoC フラグ下でのみ受理されることを確認。
 - フォローアップ: `menhir --explain compiler/ocaml/src/parser.mly` の差分確認と CLI/監査ゴールデン更新は Step4（診断・CI 計測整備）で対応する。

## EFFECT-002 Step3 Typer/効果解析 PoC 実施（2026-04-15）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/EFFECT-002-proposal.md`](./2-5-proposals/EFFECT-002-proposal.md#5-実施ステップ)

- 型推論: `compiler/ocaml/src/type_inference.ml` に `PerformCall` / `Handle` 分岐と `infer_handler_literal` 系ヘルパを追加し、Typed AST 上で `TEffectPerform`／`THandle` を構築できるようにした。ハンドラ本体は文ごとに推論し、残余効果集合へ追記する PoC のみを実装（戻り値型は暫定的に `()` 固定）。
- 効果解析: `Type_inference.Effect_analysis` が効果操作のタグを正規化して記録し、ハンドラ本体のブロックも再帰的に走査するよう更新。`Σ_handler` の差集合と stage 整合は未着手で、Phase 2-7 に繰越。
- テスト: `compiler/ocaml/tests/effect_handler_poc_tests.ml` を新設し、`perform` で `console` タグが残余集合に載ること・`handle` 式から `THandle` が生成されることを確認。`dune build tests/effect_handler_poc_tests.exe` を試行したところ、既存 `ast.ml` の相互再帰が `rectypes` 前提であるため `Unbound type constructor "expr"` によりビルド失敗（要環境整備）。PoC のコードパス自体はソース解析で手動検証済み。
- フォローアップ: `Σ_after = (Σ_before - Σ_handler) ∪ Σ_residual` の本実装、効果操作の戻り値推論、CI 指標導入は Step4/Step5 で継続。`docs/notes/effects/effect-system-tracking.md` に TODO を追記。

## EFFECT-002 Step4 診断・CI 計測整備（2026-04-18）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/EFFECT-002-proposal.md`](./2-5-proposals/EFFECT-002-proposal.md#5-実施ステップ)

### 1. 調査結果
- `compiler/ocaml/src/diagnostic.ml:781` の `set_extension "effects"` は Stage / Capability 情報のみを格納しており、`Σ_before`・`Σ_handler`・`Σ_after` を保持するフィールドが未定義。`set_audit_metadata` が呼び出される範囲（`compiler/ocaml/src/diagnostic.ml:838-904`）も Stage 連携に限定されているため、`effect.sigma.*` と `effect.syntax.constructs.*` を追加する余地を確保する必要がある。  
- `tooling/ci/collect-iterator-audit-metrics.py:115`〜`tooling/ci/collect-iterator-audit-metrics.py:474` を確認したところ、RequiredField 定義と `check_extension_fields` は Stage / Iterator の必須項目検証のみで、効果構文受理率を算出するロジックが存在しない。`syntax.effect_construct_acceptance` / `effects.syntax_poison_rate` の指標追加には新たなデータ抽出ルーチンが必須。  
- `scripts/validate-diagnostic-json.sh:109` 以降で実行される Python 検証は `parse_result.recovered` や値制限関連フィールドのみを対象としており、効果構文向けの `extensions.effects.sigma` や `constructs` 配列の検証が行われていない。PoC サンプルのゴールデンも未整備である。

### 2. 診断フィールドとメトリクス設計
- 仕様の `Σ_after = (Σ_before - Σ_handler) ∪ Σ_residual`（docs/spec/1-3-effects-safety.md:12315）に合わせ、以下のフィールド構成で残余効果と構文統計を記録する方針を確定した。`sigma.*` はタグ名を小文字配列で表現し、`extensions` と `audit.metadata` の両方で同じキーを出力する。

  | 出力先 | キー | 内容 | 備考 |
  | --- | --- | --- | --- |
  | `extensions.effects` | `sigma.before` | ハンドラ適用前の潜在効果集合 `Σ_before` | `perform` / `do` 解析結果を集計 |
  | `extensions.effects` | `sigma.handler` | ハンドラ宣言が捕捉する集合 `Σ_handler` | `@handles` 属性と同期 |
  | `extensions.effects` | `sigma.residual` | ハンドラ本体で発生した `Σ_residual` | `effect.contract.residual_snapshot` と連携 |
  | `extensions.effects` | `sigma.after` | 適用後の残余集合 `Σ_after` | 空集合か否かで accept/poison を判断 |
  | `extensions.effects` | `constructs` | 各 `perform` / `handle` の詳細 (`kind`/`tag`/`span`/`handled_by`/`diagnostics`) | CI 集計とデバッグで利用 |
  | `audit.metadata` | `effect.sigma.before|handler|residual|after` | `sigma.*` のミラー | 監査ダッシュボードで差分確認 |
  | `audit.metadata` | `effect.syntax.constructs.total` | 効果構文の総数 | `int` |
  | `audit.metadata` | `effect.syntax.constructs.accepted` | `Σ_after = ∅` の構文数 | PoC 中は 0 |
  | `audit.metadata` | `effect.syntax.constructs.poisoned` | `Σ_after ≠ ∅` の構文数 | PoC 中は total と同値 |
  | `audit.metadata` | `effect.syntax.constructs.residual_tags` | 捕捉漏れタグ一覧 | `string list` |

- 上記データを前提に指標を定義。`syntax.effect_construct_acceptance = accepted / total`、`effects.syntax_poison_rate = poisoned / total` とし、PoC 期間は (0.0 / 1.0)、本実装後は (1.0 / 0.0) を `--require-success` の閾値とする。`docs/notes/effects/effect-system-tracking.md` と `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` にゲート条件を追記した。
- `tooling/ci/collect-iterator-audit-metrics.py` へ `iter_effect_constructs`（仮称）を追加し、`constructs` 配列を巡回して指標と `effects.contract.*` 診断の整合を同時に検証する設計をまとめた。
- `scripts/validate-diagnostic-json.sh` に効果構文専用バリデータを追加し、`extensions.effects.sigma` の配列形式、`constructs[*].kind` の列挙（`perform`/`handle`）、`Σ_after` と `effect.syntax.constructs.accepted` の一致を検証する。検証用ゴールデンは `compiler/ocaml/tests/golden/diagnostics/effect-handler-poc.json.golden`（仮称）を新設して固定する。

### 3. フォローアップ
1. `diagnostic.ml` / `diagnostic_serialization.ml` に `sigma` / `constructs` 出力ロジックを追加し、`effect.contract.residual_snapshot` など既存診断との整合を確認する（Phase 2-7 `EFFECT-003` タスクへ移管）。
2. CI ツール群（`collect-iterator-audit-metrics.py`・`validate-diagnostic-json.sh`）とダッシュボード更新タスクを Phase 2-7 `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` の `H-O1` / `H-O2` に連携。ゴールデン更新時には `reports/diagnostic-format-regression.md` のチェックリストに効果構文の確認項目を追加する。
3. `docs/notes/effects/effect-system-tracking.md` の PoC 指標節を更新し、`syntax.effect_construct_acceptance` / `effects.syntax_poison_rate` の遷移条件と `Σ` 記録仕様を Phase 2-7 キックオフレビュー資料へ引用できる状態に整備する。

## EFFECT-002 Step5 ドキュメント整合とハンドオーバー（2026-04-20）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/EFFECT-002-proposal.md`](./2-5-proposals/EFFECT-002-proposal.md#5-実施ステップ)

### 1. 実施内容
- Chapter 1（`docs/spec/1-1-syntax.md`, `docs/spec/1-3-effects-safety.md`, `docs/spec/1-5-formal-grammar-bnf.md`）に `Σ_before`/`Σ_after` 記録と PoC 指標 (`syntax.effect_construct_acceptance`, `effects.syntax_poison_rate`) の参照脚注を追加し、Step4 設計（`extensions.effects.sigma.*`, `effect.syntax.constructs.*`）と仕様の同期を完了した。
- 概要 (`docs/spec/0-0-overview.md`) と索引 (`docs/spec/README.md`) へ Experimental Stage の位置付けと KPI 参照ルートを追記し、読者が PoC 運用と Phase 2-7 への移行条件を辿れるようにした。
- `docs/notes/effects/effect-system-tracking.md` に Step5 ログと参照表を追加し、引き継ぎチェックリスト（H-O1〜H-O5）と紐付け。

### 2. 連携更新
- `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md`「EFFECT-002」節へ Step5 完了メモを追記し、PoC KPI の参照先と Phase 2-7 以降の残課題（ハンドラ実装・Stage 昇格）を整理した。
- `docs/plans/bootstrap-roadmap/2-5-proposals/README.md` に Step5 更新履歴を追加し、計画書全体から PoC KPI の導線を確保した。
- `docs/plans/bootstrap-roadmap/0-4-risk-handling.md` に効果構文 Stage 遷移遅延リスク（ID: EFFECT-POC-Stage, 期限: 2026-09-30, 状態: Open）を登録。

### 3. フォローアップ
1. Phase 2-7 で `collect-iterator-audit-metrics.py` と `scripts/validate-diagnostic-json.sh` に効果構文 KPI を実装し、`syntax.effect_construct_acceptance = 1.0` / `effects.syntax_poison_rate = 0.0` を達成する。
2. Stage 昇格が確定した時点で脚注 `[^effects-syntax-poc-phase25]` と `[^effects-sigma-poc-phase25]` を撤去し、索引・リスク台帳を更新する。
3. `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` の H-O1〜H-O5 を定期レビューし、PoC から正式実装への切り替え条件を監視する。

## Step7-9 ドキュメント整合準備レビュー（2026-04-28）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md`](./2-5-spec-drift-remediation.md#7-ドキュメント更新の実施33-34週目) §7〜§9

### 1. 調査と整理
- Phase 2-5 で実施した提案（DIAG-001〜003, PARSER-002/003, EFFECT-002, TYPE-002）に由来する仕様・ガイド・ノート・メトリクスの更新箇所を棚卸しし、コア仕様／RunConfig／効果 PoC／診断メタデータの 4 クラスタで整合ポイントをチェックリスト化した。
- `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` §7〜§9 にチェックボックス付きタスクリストと参照メモを追加し、進捗の横断確認と Phase 2-7 引き継ぎ時の欠損検知を容易にした。併せて 9.2 / 9.3 の見出しが「8.x」と誤記されていたため修正した。
- KPI とリスク管理の連携を明示するため、`0-3-audit-and-metrics.md`・`0-4-risk-handling.md`・`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` を相互参照するメモを追記し、Phase 3 での参照動線を整理した。

### 2. 差分・確認事項
- 診断ドメイン拡張（DIAG-001〜003）: `docs/spec/3-6-core-diagnostics-audit.md`, `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md`, `docs/guides/runtime/runtime-bridges.md`, `docs/notes/dsl/dsl-plugin-roadmap.md` の脚注とメトリクス定義が揃っているか Step7 チェックリストで確認する必要がある。
- RunConfig / Streaming（PARSER-002/003）: `docs/spec/2-1-parser-type.md`, `docs/spec/2-6-execution-strategy.md`, `docs/guides/compiler/core-parse-streaming.md`, `docs/notes/parser/core-parse-api-evolution.md` の記述が `parser-runconfig-packrat.json.golden` と同期しているかを検証し、未同期の場合はゴールデンと脚注を更新する。
- 効果構文 PoC（EFFECT-002）: `docs/spec/1-1-syntax.md`, `docs/spec/1-3-effects-safety.md`, `docs/spec/1-5-formal-grammar-bnf.md`, `docs/notes/effects/effect-system-tracking.md` に掲載した `Σ` 脚注と KPI (`syntax.effect_construct_acceptance`, `effects.syntax_poison_rate`) が同一条件を指しているか確認し、齟齬があれば Phase 2-7 向け TODO を更新する。
- Type Row 移行（TYPE-002）: `docs/spec/1-2-types-Inference.md`, `docs/spec/3-6-core-diagnostics-audit.md` の `type_row_mode = "metadata-only"` 脚注と `effects.type_row.integration_blocked` KPI の文言が一致しているか確認し、解除条件は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` の H-O3 と同期する。
- リスク・引き継ぎ: `0-4-risk-handling.md` の `EFFECT-POC-Stage`, `TYPE-002-ROW-INTEGRATION`, `LEXER-Unicode-Stage`, `STREAMING-Capability-Sync` の状態ラベルと担当セクションが最新になっているか点検する。

### 3. フォローアップ
1. Step7〜Step9 のチェックボックスを消化する際は、更新対象ファイル（仕様・ガイド・ノート・メトリクス・リスク台帳）の変更リンクとレビュア確認を本ログに追記する。特に KPI 再計測結果は `tooling/ci/collect-iterator-audit-metrics.py --require-success` のログと併せて記録する。
2. 索引・README 整合確認（Step8.1）はリンク切れ調査を含むため、`scripts/validate-diagnostic-json.sh` 実行後のサマリと照合し、修正が必要な場合は #link-check タグを付けて監視する。
3. Phase 3 引き継ぎ資料（Step9.3）は `docs/plans/bootstrap-roadmap/3-0-phase3-transition.md` と `docs/notes/effects/effect-system-tracking.md` の対象節を明示し、今回追加したタグ（例: `Phase3:effects`, `Phase3:parser-streaming`, `Phase3:diagnostics`）を活用して移行計画を立てる。
...
### 1. 対応表の作成
- `docs/notes/parser/core-parser-migration.md` に Menhir 規則と 15 個のコアコンビネーターの対応を追加し、代表的な規則と不足メタデータを整理した（`Phase 2-5 Core コンビネーター棚卸し` セクション参照）。  
...
## SYNTAX-001 Step1 Unicode 識別子差分棚卸（2025-11-02）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-001-proposal.md`](./2-5-proposals/SYNTAX-001-proposal.md#step-1-chapter-11-5-差分棚卸)

### 1. 仕様側の確認
- Chapter 1 は `XID_Start + XID_Continue*` を前提とし、脚注 `[^lexer-ascii-phase25]` で Phase 2-7 まで ASCII 制限が残ると明記しているが適用範囲は §A.3 のみ（`docs/spec/1-1-syntax.md:27-42`）。  
- Chapter 1.5 BNF でも `Ident ::= *Unicode XID*` を宣言し、ASCII 制限の脚注が無いため仕様読者には完全実装済みと読める（`docs/spec/1-5-formal-grammar-bnf.md:279-295`）。

### 2. 実装側の現状
- `lexer.mll` は `xid_start/xid_continue` を ASCII のみに固定し、`_` 規則で単一バイトの想定外文字を拒否する現行挙動を保持している（`compiler/ocaml/src/lexer.mll:77-84`、`compiler/ocaml/src/lexer.mll:231-234`）。  
- `Core_parse.Lex` ブリッジは Trivia プロファイル同期のみを実装し、Unicode 識別子プロファイルや正規化チェックは未接続であることを確認した（`compiler/ocaml/src/core_parse_lex.ml:1-160`）。

### 3. 検証とエビデンス
- `compiler/ocaml/tests/test_lexer.ml:55-229` に Unicode 識別子拒否の再現テストを追加。`Char.chr 0xE8`（UTF-8 先頭バイト）に基づく現行メッセージ `Unexpected character: è` と Span `4-5` をゴールデンとして固定化し、ASCII 制約の証拠を残した。

### 4. 未検証事項の洗い出し
- Chapter 1.4 で要求される NFC 正規化/Bidi 制御/Confusable 警告は Phase 2-5 時点で未検証であり、Lexer/`Core.Parse` 経路ともにフックが存在しない（`docs/spec/1-4-test-unicode-model.md:168-202`）。SYNTAX-001 Step2 で脚注拡張とロードマップ整備が必要。

### 5. フォローアップ
1. Chapter 1.5 と `docs/spec/0-2-glossary.md` に ASCII 制限の脚注を追加する作業を Step2 で実施。  
2. `Core_parse` の Lex 拡張に Unicode プロファイル入力フックを設ける案を Phase 2-7 `lexer-unicode` タスクへ提案する（技術的負債 ID 22/23 と連携）。  
3. `0-3-audit-and-metrics.md` の `lexer.identifier_profile_unicode` 指標を Phase 2-5 の評価に追加し、測定値 0.0 を記録する準備を進める。

## SYNTAX-001 Step2 仕様脚注・索引整備（2026-02-24）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-001-proposal.md`](./2-5-proposals/SYNTAX-001-proposal.md#step2-仕様脚注索引用語集の整備週31-32docsチーム)

### 1. 仕様脚注の拡張
- `docs/spec/1-5-formal-grammar-bnf.md` の `Ident` 産出規則に `[^lexer-ascii-phase25]` を追加し、Phase 2-5 時点は ASCII (`[A-Za-z0-9_]+`) が暫定プロファイルであることを脚注化。  
- 脚注本文を `SYNTAX-001 Step2` として更新し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md`（`lexer.identifier_profile_unicode`）と `docs/plans/repository-restructure-plan.md` のリンク整合手順を参照する形へ整備した。

### 2. 用語集・索引の同期
- `docs/spec/0-2-glossary.md` に「Unicode 識別子プロファイル（暫定）」エントリを追加し、`LEXER-001` と `SYNTAX-001` を統合した脚注を掲載。  
- `docs/spec/README.md` と ルート `README.md` に Unicode 識別子の暫定運用導線を追記し、仕様全体で同じ注意喚起と Phase 2-7 `lexer-unicode` タスクへの導線を共有した。

### 3. 検証とフォローアップ
- Markdown の脚注番号衝突とリンク切れを手動確認し、`docs/plans/repository-restructure-plan.md` の Phase 2-5 方針（相対パス維持・索引同期）に抵触しないことを確認。  
- Step3 ではテスト雛形の整備と `lexer.identifier_profile_unicode` 指標の記録更新を継続予定。Phase 2-7 `lexer-unicode` サブタスクで CLI/LSP と Unicode プロファイル実装を追跡する。

## SYNTAX-001 Step3 Unicode テスト雛形整備（2026-02-24）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-001-proposal.md`](./2-5-proposals/SYNTAX-001-proposal.md#step3-検証用サンプルとテストシナリオ整備週32compilertesting)

### 1. テストケースの準備
- `compiler/ocaml/tests/unicode_ident_tests.ml` を追加し、`REML_ENABLE_UNICODE_TESTS` 環境変数で実行切り替えを行うスキップ機構を導入。デフォルトでは `[unicode_pending]` を出力して成功扱いとし、Phase 2-7 以降に Unicode 実装が揃った時点で即座に有効化できる状態にした。  
- docs/spec/1-1-syntax.md §A.3 と docs/spec/1-4-test-unicode-model.md §K に基づき、日本語・ギリシャ語・キリル・ハングル・アラビア語（ZWJ）・合成アクセントを含むケースを `acceptance_cases` と `supplementary_cases` に整理。正規化期待値は `normalized` フィールドで NFC 形を明記した。

### 2. フィクスチャと Dune 設定
- `compiler/ocaml/tests/samples/unicode_identifiers.reml` を新設し、複数スクリプトの識別子宣言を含むサンプルモジュールを用意。テスト起動時にファイル存在を検証することで、将来 parser 連携テストへ転用する土台を確保した。  
- `compiler/ocaml/tests/dune` に `unicode_ident_tests` を登録し、既存テスト群と同じ依存ライブラリでビルド可能であることを確認。実行条件は環境変数で制御するため、Phase 2-5 でもテストスイートが失敗しない。

### 3. フォローアップ
- Step4 で `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の `lexer.identifier_profile_unicode` 指標へ Phase 2-5 測定値（0.0）とレビュー日を記録し、`0-4-risk-handling.md` の技術的負債 ID 22/23 と連携する。  
- Phase 2-7 では CI ジョブで `REML_ENABLE_UNICODE_TESTS=1` を設定し、Unicode 識別子受理・正規化・ZWJ/Bidi ガードの検証を実施する。

## SYNTAX-001 Step4 記録とメトリクス連携（2026-03-31）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-001-proposal.md`](./2-5-proposals/SYNTAX-001-proposal.md#step-4-記録とメトリクス連携週32末docsqa)

### 1. メトリクス記録
- `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` に「Phase 2-5 Step4 記録」を追加し、`lexer.identifier_profile_unicode = 0.0` と週次レビュー日（2026-03-31）を明記。`tooling/ci/collect-iterator-audit-metrics.py --summary` の `parser.runconfig.lex.profile` 出力が全て `ascii` であることをログで確認した。  
- 監視継続のため、Phase 2-7 で指標が 1.0 へ達した際は同セクションを更新するよう TODO を残置。

### 2. リスク・ロードマップ連携
- `docs/plans/bootstrap-roadmap/0-4-risk-handling.md` に Unicode XID 実装未完了リスク（期限: 2026-08-31, 状態: Open）を登録し、`lexer.identifier_profile_unicode` の監視結果と連動させた。  
- `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` に「Unicode 識別子プロファイル移行」セクションを追記し、XID テーブル生成、`REML_ENABLE_UNICODE_TESTS=1` 常時化、脚注撤去を Phase 2-7 の成果物として明文化。

### 3. フォローアップ
- CLI/LSP ゴールデンから ASCII 制限文言を撤去するタスクと、`docs/notes/dsl/dsl-plugin-roadmap.md` チェックリスト更新を Phase 2-7 へ連携。  
- 指標が 1.0 へ達した際にレビュー記録へ追記すること、ならびに Phase 2-8 で脚注撤去作業を実施することを了承。

## SYNTAX-003 S1 パーサ PoC 設計（2026-03-12）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-003-proposal.md`](./2-5-proposals/SYNTAX-003-proposal.md#5-実施ステップと調査計画phase-2-5-内)

### 1. 設計結果
- `expr_base` に `perform` / `do` / `handle` を追加する挿入位置と優先順位を確定し、既存の制御構文（`if`/`match` 等）と同列の扱いでパースする方針を整理。`do` は `perform` と同一 AST バリアントへ射影し、発生源を区別できる補助フィールドを追加する設計とした。
- `EffectPath ::= Ident { "." Ident }` を `effect_path` ヘルパ（`module_path option * ident`）で表現する案をまとめ、`make_effect_ident` の正規化ルールと併せて `Console.log` や `Core.Console.log` を一貫して処理する。
- `handle` 式はトップレベル `handler_decl` を再利用しつつ、式位置では属性・可視性・`resume` 多重呼び出しを禁止する PoC 制限を設定。これらの拡張は Phase 2-7 で再検討する。
- `parser_run_config` に `experimental_effects: bool` を追加し、`-Zalgebraic-effects` 無効時は構文段階で `effects.syntax.experimental_disabled` 診断を返すガードを導入する計画を明文化。
- `menhir --explain` の衝突件数は現行 31 件のまま維持される想定。実装後に差分が発生した場合は本ログへ追記することを決定。

### 2. ドキュメント・ノート更新
- `compiler/ocaml/docs/parser_design.md` §3.3.1 に PoC 設計を追記し、優先順位・PoC 制限・診断方針を反映。
- `compiler/ocaml/docs/effect-system-design-note.md` のモジュール別タスクに S1 結果を追加し、Typer/Runtime 連携の前提を共有。
- `docs/notes/effects/effect-system-tracking.md` を新設し、PoC ステージと `-Zalgebraic-effects` フラグ運用、Phase 2-7 引き継ぎ TODO、CI 指標（`syntax.effect_construct_acceptance`）の導入方針を整理。

### 3. フォローアップ
1. SYNTAX-003 S2 で `Type_inference_effect` に新 AST バリアントを取り込み、`Σ_before`/`Σ_after` 更新ロジックを PoC として定義する。
2. `tooling/ci/collect-iterator-audit-metrics.py` へ `syntax.effect_construct_acceptance` 指標を追加し、PoC 実装完了までは 0.0 を許容・正式導入時には 1.0 を必須とする運用を確立する。
3. `parser_run_config` 実装時に CLI フラグ連携テストを追加し、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` や `reports/diagnostic-format-regression.md` の検証手順を更新する。
...
### 1. 公開シグネチャ案
- `docs/notes/parser/core-parse-api-evolution.md` に `Core_parse` モジュール署名案を作成し、`Id`/`State`/`Reply`/`Registry` を分離した構造を提示。`Parser<T> = State.t -> Reply.'a t * State.t` の形で純関数とし、`rule`/`label`/`cut`/`recover` が `Reply` の `consumed`/`committed` フラグに反映されることを明記。  
...
[^core-parse-api-note]: `docs/notes/parser/core-parse-api-evolution.md` Phase 2-5 Step2 Core_parse シグネチャ草案。`Id` 採番と `State`/`Reply` の公開 API を記載。
...
## LEXER-001 Step1 ASCII 実装棚卸（2025-12-12）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/LEXER-001-proposal.md`](./2-5-proposals/LEXER-001-proposal.md#phase-2-5差分可視化と準備)

### 1. ASCII 限定実装の確認
- `compiler/ocaml/src/lexer.mll:41-78` では識別子の規則が ASCII のみ（`['a'-'z' 'A'-'Z' '_']` / `['a'-'z' 'A'-'Z' '0'-'9' '_']`）に固定され、Unicode XID を参照する拡張ポイントが存在しない。Phase 1 TODO コメントが残ったままになっており、仕様の `identifier(profile=DefaultId)` と乖離している。  
- `Core.Parse.Lex.Bridge` はトリビアプロファイル再構成に限定され、RunConfig から `IdentifierProfile` を復元する経路を持たない（`compiler/ocaml/src/core_parse_lex.ml:8-115`）。`identifier(profile=...)` API を実装側で切り替える術が無く、仕様上のプロファイル拡張が適用できない。  
- Unicode 文字（例: `解析器`）が入力されると、`token` 規則末尾のフォールバックに到達して `Lexer.Lexer_error ("Unexpected character: " ^ String.make 1 c, span)` が送出される。UTF-8 先頭バイトが `�`（U+FFFD）として表示されることをテストで確認した。

### 2. 再現スニペットと現在の失敗メッセージ
- フィクスチャ `compiler/ocaml/tests/samples/lexer_unicode_identifier.reml` を追加し、`let 解析器 = 1` の最小ケースで ASCII 限定挙動を再現。  
- `compiler/ocaml/tests/test_lexer.ml:29` にフィクスチャ読込ヘルパを追加し、`compiler/ocaml/tests/test_lexer.ml:88` で `lexer error: unicode identifier rejected` テストを実装して、現在の失敗メッセージ（`Unexpected character: �`）とエラー位置（`span 4-5`）を保存した。  
- `dune exec tests/test_lexer.exe`（作業ディレクトリ `compiler/ocaml`）で全テストが成功することを確認し、再現手順を CI 共有可能な形で確定した。

### 3. フォローアップ / TODO
1. Step2 で `docs/spec/2-3-lexer.md` D-1/D-2 と `docs/spec/1-1-syntax.md` の Identifier 節へ ASCII 暫定脚注を追加し、仕様読者へ制限を通知する。  
2. `docs/spec/0-2-glossary.md` に `Unicode identifier profile` の暫定定義と Phase 2-7 `lexer-unicode` タスクの参照を追加する。  
3. DSL/プラグイン連携資料（`docs/notes/dsl/dsl-plugin-roadmap.md`）へ Capability 名の Unicode 別名と互換モード運用の暫定ルールを追記し、Phase 2-7 以降の実装計画へ差分を引き継ぐ。

## LEXER-001 Step2 仕様脚注と索引の整備（2026-02-18）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/LEXER-001-proposal.md`](./2-5-proposals/LEXER-001-proposal.md#step-2-仕様脚注と索引の整備週31-32-docs-チーム)

### 1. 仕様・索引の反映
- `docs/spec/2-3-lexer.md` D-1 に ASCII 制約脚注 `[^lexer-ascii-phase25]` を設置し、Phase 2-7 `lexer-unicode` へ移行する計画とレビュー記録の参照を明示した。  
- `docs/spec/1-1-syntax.md` の識別子節と `docs/spec/0-2-glossary.md`（構文セクション）に同脚注を連携し、索引 `docs/spec/README.md` からも暫定制約へ誘導できるよう更新。  
- 既存の用語集フットノートと干渉しないよう、ASCII 制約用に新しい脚注 ID を割り当て、Phase 2-7 で除去する際の差分把握を容易にした。

### 2. メトリクスとレビュー記録
- `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` に `lexer.identifier_profile_unicode` 指標を追加し、Phase 2-5 時点では 0.0（ASCII 限定）を保持する運用と、Phase 2-7 で 1.0 を到達条件とする方針を設定。  
- `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` の `LEXER-001` エントリへ Step2 完了情報と参照文書を追記し、差分リストから参照できるようにした。  
- 本ログ条項を追加し、仕様・索引・メトリクスの更新点を Phase 2-7 チームへ引き継いだ。

### 3. TODO / 引き継ぎ
1. Step3（DSL/プラグインチーム連携）で `docs/notes/dsl/dsl-plugin-roadmap.md` の Unicode 別名ポリシーと CLI ガイド更新を実施する。  
2. Phase 2-7 着手時に `lexer.identifier_profile_unicode` のベースラインを更新し、脚注削除タイミングとメトリクス 1.0 達成条件を同期する。  
3. Unicode プロファイル導入後に脚注を削除する際、`docs/spec/README.md` と用語集の該当項目が最新状態へ戻るようチェックリストを用意する。

## SYNTAX-003 Step0 ステージ定義整備（2026-02-26）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-003-proposal.md`](./2-5-proposals/SYNTAX-003-proposal.md#5-実施ステップと調査計画phase-2-5-内)

### 1. 実施内容と調査
- Chapter 1 B.5・Chapter 1.5・Chapter 3.8 の効果構文節を確認し、PoC ステージの脚注が Phase 2-5 の暫定機能（`-Zalgebraic-effects` 必須）であることを明記する必要がある点を整理した。  
- `parser.mly` で `PerformExpr` / `HandleExpr` を有効化する際の挿入候補と `%prec` 設定を洗い出し、`menhir --list-errors` の衝突検証を計画。`parser_design.md` と `effect-system-design-note.md` の該当節を参照し、PoC で許容する制約（`resume` 多重呼び出し・`do` 同義扱い）をまとめた。  
- `Type_inference_effect`・`effect_analysis.ml`・`diagnostic.ml` の拡張ポイントを調査し、PoC の検証には `test_type_inference.ml`・`streaming_runner_tests.ml`・`reports/diagnostic-format-regression.md` を更新する必要があることを確認した。

### 2. 成果物
- `SYNTAX-003` 計画書へ S0〜S4 の実施ステップ表を追加し、`syntax.effect_construct_acceptance` / `effects.syntax_poison_rate` など新規メトリクスを `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` に追記する準備タスクを明記。  
- Phase 2-7 へ引き継ぐチェックリスト草案（脚注解除条件・CI 指標達成条件・`-Zalgebraic-effects` 運用）を記録し、差分リストから参照できるよう脚注 ID を割り当てた。

### 3. TODO / フォローアップ
1. S1 完了後に `parser.conflicts` の差分と CLI ゴールデンの更新案を記録する。  
2. `syntax.effect_construct_acceptance` を `collect-iterator-audit-metrics.py` に導入する際、`diagnostic.info_hint_ratio` との整合検証を Phase 2-7 へ依頼する。  
3. `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` に効果構文 PoC から正式化へ移行するためのステージ表を追加する。

## SYNTAX-003 S0 ステージ脚注同期（2026-03-30）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-003-proposal.md`](./2-5-proposals/SYNTAX-003-proposal.md#5-実施ステップと調査計画phase-2-5-内)

### 1. 実施内容
- `docs/spec/1-1-syntax.md` B.5 に脚注 `[^effects-syntax-poc-phase25]` を追加し、`-Zalgebraic-effects` フラグ有効時のみ利用できる PoC であることを明記。  
- `docs/spec/1-5-formal-grammar-bnf.md` の効果構文産出規則と脚注、`docs/spec/3-8-core-runtime-capability.md` の段階的導入ポリシーへ同脚注を同期し、Capability Stage と Experimental フラグの紐付けを文章化した。  
- 索引 `docs/spec/README.md` に効果構文 PoC の案内を追記し、`docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` 差分リストへ脚注 ID を登録。`SYNTAX-003` 計画書本体にも S0 完了メモを追加した。

### 2. エビデンス
- `docs/spec/1-1-syntax.md:203`、`docs/spec/1-5-formal-grammar-bnf.md:214`、`docs/spec/3-8-core-runtime-capability.md:16` に脚注参照を挿入済み。  
- `docs/spec/README.md` では 1.x および 3.x の該当項目に PoC 表記を追加。  
- `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` は `SYNTAX-003` 行へ脚注 `[^effects-syntax-poc-phase25]` を反映した。

### 3. TODO / 引き継ぎ
1. S1 以降で `parser.mly` へ構文を実装する際、この脚注を削除・更新する条件と手順を `docs/notes/effects/effect-system-tracking.md` に明記する。  
2. `syntax.effect_construct_acceptance` 指標を導入した時点で脚注を更新し、PoC から正式化へのエビデンスを `0-3-audit-and-metrics.md` へ記録する。  
3. Phase 2-7 開始時のレビューで脚注維持の是非と Stage 昇格条件（`StageId::Beta` 以上）を再確認する。

## SYNTAX-003 Step2 型・効果解析 PoC 接続（2026-03-19）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-003-proposal.md`](./2-5-proposals/SYNTAX-003-proposal.md#5-実施ステップと調査計画phase-2-5-内)

### 1. 実施内容と調査
- `compiler/ocaml/src/type_inference_effect.ml` を確認し、現状は Stage 判定と Capability 名正規化を行う関数のみ実装されている点を洗い出した。式レベルで `perform` / `handle` を解析するには Typed AST に効果ノードを追加し、`Type_inference_effect` 側に `Σ_before` と `Σ_after` を返却する補助関数を設計する必要があると結論づけた。  
- `docs/spec/1-3-effects-safety.md` §I と `compiler/ocaml/docs/effect-system-design-note.md` を突合し、PoC 範囲では「単一タグ捕捉」「`resume` 1 回」「未宣言タグ禁止」を前提とすれば既存の `effects.contract.*` 診断と Stage 確認を流用できることを確認した。  
- `reports/diagnostic-format-regression.md` の効果診断サンプルと `tooling/ci/collect-iterator-audit-metrics.py` の指標一覧を確認し、TypeChecker が `Σ_after` を JSON 拡張へ書き出すだけで PoC 指標 `syntax.effect_construct_acceptance` を算出できることを検証した。

### 2. 成果物
- `SYNTAX-003` 計画書に PoC で通過させる型規則表を追加し、`perform` / `handle` / `handler` / `@handles` それぞれについて型制約・効果集合計算・Stage/診断フックを明文化した。  
- `compiler/ocaml/tests/test_type_inference.ml` に効果構文テストの草案コメントを追加し、成功 2 ケース・失敗 1 ケースを具体例として列挙。診断比較方法と `run_test` ベースでの実装方針を共有した。  
- `docs/notes/effects/effect-system-tracking.md` に PoC 指標の JSON 断片と記録方法を追記し、Phase 2-7 で CI へ接続する際の参照元を整備した。

### 3. TODO / フォローアップ
1. Phase 2-7 で Typed AST に `TEffectPerform` / `TEffectHandle` ノードを追加し、`Type_inference` と `Type_inference_effect` の両方で効果集合を扱えるよう改修する。  
2. `Type_inference_effect` へ式用の `resolve_expr_profile`（仮称）を実装し、`effects.contract.residual` 診断に `Σ_before`・`Σ_after`・捕捉タグ一覧を添付する。  
3. `reports/diagnostic-format-regression.md` に PoC 用の効果診断サンプルを追加し、`collect-iterator-audit-metrics.py` で `syntax.effect_construct_acceptance` を計測する際のゴールデンを Phase 2-7 着手前に用意する。

## SYNTAX-003 Step3 診断・CI 計測整備（2026-03-27）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-003-proposal.md`](./2-5-proposals/SYNTAX-003-proposal.md#s3-診断-ci-計測整備2026-03-27)

### 1. 実施内容と調査
- `compiler/ocaml/src/diagnostic.ml` と `parser_diag_state.ml` を確認し、効果構文の PoC では `extensions.effects` と `audit_metadata` に追加フィールドを出力できる余地があること、既存の `Diagnostic.Builder` API を利用すれば CLI/LSP/監査ログのゴールデンを汎用的に更新できることを整理した。  
- `tooling/ci/collect-iterator-audit-metrics.py` の RequiredField 設定と `effects` 集計処理を調査し、新指標 `syntax.effect_construct_acceptance` / `effects.syntax_poison_rate` を `effect_syntax.constructs` から算出する設計を作成。`--require-success` では 1.0 を達成値として扱い、逸脱時にフェイルさせるガードを追加する方針を定めた。  
- `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` と `reports/diagnostic-format-regression.md` に効果構文計測の項目が無いことを確認し、Phase 2-7 へ連携するための KPI 定義・ゴールデン更新手順・サンプル JSON の追加箇所を洗い出した。`docs/notes/effects/effect-system-tracking.md` では診断/CI セクションを更新し、S3 以降の TODO を整理する計画とした。

### 2. 成果物
- PoC 向けのメトリクス例 (`effect_syntax.constructs` / `metrics`) を JSON で作成し、`syntax.effect_construct_acceptance = 0.0`・`effects.syntax_poison_rate = 1.0` を Phase 2-5 の基準値とするサンプルを本計画書と `docs/notes/effects/effect-system-tracking.md` に添付。  
- `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の KPI 表へ新指標を追加し、CI での収集タイミングと逸脱時のフォローアップ手順を明文化。`reports/diagnostic-format-regression.md` では CLI/LSP ゴールデン更新時に効果構文サンプルを含めるチェックリストを追記した。  
- `docs/notes/effects/effect-system-tracking.md` の Stage 表を更新し、診断・CI 計測が「計測設計確定」ステージへ進んだことを記録。Phase 2-7 への引き継ぎ TODO として、監査ダッシュボード更新とゴールデン整備の担当割り当てを追記した。

### 3. TODO / フォローアップ
1. Phase 2-7 で `collect-iterator-audit-metrics.py` に新指標を実装し、`--require-success` 実行時に 1.0 以外で失敗するフェイルセーフを導入する。フェイル時は `0-4-risk-handling.md` へ即時登録する運用に切り替える。  
2. CLI/LSP/監査ログの効果構文サンプルをゴールデン化し、`reports/diagnostic-format-regression.md` のチェックリストに沿って `diagnostic.info_hint_ratio` の影響を確認する。サンプル追加後は `tooling/ci/collect-iterator-audit-metrics.py --section effects` の出力を CI アーティファクトとして保存する。  
3. `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` にメトリクスの本実装タスクとダッシュボード更新タスクを登録し、Phase 2-7 着手時のレビューチェックリストへ連携する。

## SYNTAX-003 S4 Phase 2-7 引き継ぎ準備（2026-04-03）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/SYNTAX-003-proposal.md`](./2-5-proposals/SYNTAX-003-proposal.md#s4-phase-2-7-への引き継ぎ準備2026-04-03)

### 1. 実施内容と調査
- `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` を更新し、効果構文 PoC 実装・CI メトリクス更新・脚注撤去のタスクをまとめた §8 を新設。`collect-iterator-audit-metrics.py --require-success` に effect 指標を組み込む条件と `-Zalgebraic-effects` フラグ運用を明文化した。  
- `docs/notes/effects/effect-system-tracking.md` を再整理し、S4 引き継ぎパッケージとして Stage 表と Phase 2-7 実行チェックリスト（H-O1〜H-O5）を表形式で掲載。CLI/LSP/CI の各担当が参照すべきファイルを列挙した。  
- 仕様差分リスト（`2-5-spec-drift-remediation.md`）とカタログ（`2-5-proposals/README.md`）へ S4 完了メモを追記し、脚注 `[^effects-syntax-poc-phase25]` の撤去条件と Stage 遷移シナリオを参照できるようにした。

### 2. 成果物
- `SYNTAX-003` 計画書に S4 セクションと引き継ぎチェックリストを追加し、PoC から Phase 2-7 への移行条件と `-Zalgebraic-effects` フラグ運用 TODO を整理。  
- `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` §8 で効果構文 PoC 移行タスクをブレークダウンし、CI/CLI/LSP/ドキュメントの担当割り当てと期待メトリクスを明記した。  
- `docs/notes/effects/effect-system-tracking.md` に H-O1〜H-O5 のチェックリストを追加し、作業履歴へ S4 完了を記録。Phase 2-7 で消化すべき実行条件を一元化した。

### 3. TODO / フォローアップ
1. Phase 2-7 で H-O1〜H-O5 を順次消化し、完了時に `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` へ更新メモを追加する。  
2. `collect-iterator-audit-metrics.py --require-success` と LSP フィクスチャで `-Zalgebraic-effects` の挙動差異が無いかを定期確認し、逸脱時は `0-4-risk-handling.md` に登録する。  
3. Stage = Stable へ昇格した段階で脚注 `[^effects-syntax-poc-phase25]` を撤去し、`docs/spec/README.md` と `docs/notes/dsl/dsl-plugin-roadmap.md` のチェックリストを更新する。撤去判断は Phase 2-7 終盤レビューで行う。
...
- `compiler/ocaml/src/core_parse.ml:5` 時点で Packrat キャッシュ機構が未実装であることを確認し、`Cache_key = (Id.fingerprint, byte_offset)` を前提に `Core_parse.State` へキャッシュコンテキストを保持する案を `docs/notes/parser/core-parse-api-evolution.md` へ追加。  
...
### 2. 回復同期トークンと診断連携
- `parser_run_config.ml:240` の `Recover` モジュールと `parser_diag_state.ml:8` の `record_recovery` を突合し、`Recover_config.sync_tokens` を `Core_parse.recover` へ伝搬するフローを `docs/notes/parser/core-parser-migration.md` に追記。  
...
- `docs/guides/dsl/plugin-authoring.md` と `docs/guides/compiler/core-parse-streaming.md` にコンビネーター利用例と RunConfig 共有手順を追記し、CLI/LSP/ストリーミングが同じ `parser.core.rule.*` メタデータを生成する運用ガイドを整備した。  
- `docs/notes/parser/core-parse-api-evolution.md` へ Step6 セクションを追加し、ドキュメント同期と Phase 2-7 への引き継ぎ事項を記録。
...
### 2. ノート・フォローアップ整理
- `docs/notes/types/type-inference-roadmap.md` に Step4 メモを追加し、Stage/Capability 要件・RunConfig CLI スイッチ・Phase 2-7 への引き継ぎ内容（execution-config / effect-metrics）を明示した。【N:docs/notes/types/type-inference-roadmap.md†L33-L74】
...
3. Phase 3 で Reml 実装へ移植する際、`RunConfig` の `effects.value_restriction_mode` と診断拡張を同梱できるよう移植手順を `docs/notes/parser/core-parser-migration.md`（予定）に追記する。

## LEXER-001 Step3 DSL/プラグインチーム連携（2026-03-04）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/LEXER-001-proposal.md`](./2-5-proposals/LEXER-001-proposal.md#step-3-dslプラグインチーム連携週32-dsl-チーム)

### 1. ASCII 運用ポリシーの整理
- `docs/notes/dsl/dsl-plugin-roadmap.md` §7 を新設し、Phase 2-5 中はプラグイン ID・Capability 別名を ASCII 限定で登録する暫定ポリシーを定義。Unicode 別名は Phase 2-7 `lexer-unicode` 完了後に再評価する方針とした。  
- Stage/Capability 監査との整合を明文化し、`Runtime Bridge` 連携時は ASCII ID のみを CI 指標（`diagnostics.effect_stage_consistency` など）に集計する運用を記録。  
- `compiler/ocaml/docs/technical-debt.md` ID22/ID23（Windows Capability Stage / macOS FFI）の監査ログ更新と衝突しないよう、Unicode 別名導入時の再確認手順を TODO として追記。

### 2. フォローアップと共有事項
- `docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md` の `LEXER-001` 行へ Step3 完了脚注を追加し、Phase 2-7 での Unicode 実装着手条件（`lexer.identifier_profile_unicode` 指標を PASS 化、CLI/LSP 周知の両立）を共有。  
- Phase 2-7 移行チーム向け TODO を整理（`collect-iterator-audit-metrics.py` の PASS 更新タイミング、`docs/guides/dsl/plugin-authoring.md` と VS Code 拡張ドキュメントのエイリアス周知フロー更新、`RunConfig.extensions["lex"]` テンプレート提供計画）。

### 3. リスクと次のアクション
- Phase 2-6 週次レビューで Unicode 別名を先行導入したプラグインが存在しないか確認するチェック項目を追加。  
- Phase 2-7 で脚注削除と API 公開を行う際は、`docs/spec/2-3-lexer.md` / `docs/spec/1-1-syntax.md` の再検証と `lexer_unicode_identifier.reml` のゴールデン更新を必須条件とする。

## LEXER-001 Step4 測定指標と CI スイッチの定義（2026-03-21）

関連計画: [`docs/plans/bootstrap-roadmap/2-5-proposals/LEXER-001-proposal.md`](./2-5-proposals/LEXER-001-proposal.md#step-4-測定指標と-ci-スイッチの定義週32-metrics-チーム)

### 1. メトリクス整備
- `tooling/ci/collect-iterator-audit-metrics.py` に `lexer.identifier_profile_unicode` を追加し、`ascii` / `unicode` / `other` の件数内訳と `--lex-profile` スイッチ仕様を JSON 出力へ含めるプレースホルダを実装。Phase 2-5 は `pass_rate = null` の監視モードとし、Phase 2-7 で 1.0 を達成した時点でゲート条件へ昇格させる。  
- `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の該当行を更新し、`parser.runconfig.lex.profile` と `AuditEnvelope.metadata["parser.runconfig.lex.profile"]` の一致を指標要件として明文化。CI で `--lex-profile=ascii|unicode` を切り替えて差分を検知する手順を追記した。

### 2. CI スイッチとテスト戦略
- ASCII 回帰テストでは `--lex-profile=ascii` を既定にし、Unicode リハーサル時は `--lex-profile=unicode` を明示してゴールデン差分を確認する手順を本記録と計画書へ周知。`scripts/validate-diagnostic-json.sh` のスキップ機構と合わせて Phase 2-7 着手前に切替リハーサルを行う。  
- 差分計画書（`docs/plans/bootstrap-roadmap/2-5-spec-drift-remediation.md`）へ Step4 脚注を追加し、`expected_pass_fraction=1.0` を Phase 2-7 `lexer-unicode` の受け入れ条件として登録。

### 3. フォローアップ
- Phase 2-7 `lexer-unicode` 開始時に `collect-iterator-audit-metrics.py` の `pass_rate` を 1.0 測定へ切り替える TODO を残し、CLI/LSP ゴールデン更新と連動させる。  
- `docs/notes/dsl/dsl-plugin-roadmap.md` §7 に CLI フラグ運用を追記し、プラグインチームへ ASCII → Unicode 切替ロードマップを共有する。
...
- 残課題として、Packrat キャッシュ経路の FixIt 生成（`Parser_expectation.Packrat`）と `recover` notes の多言語翻訳ルールを Phase 2-7 へ移管。`docs/notes/parser/core-parse-streaming-todo.md` に追記し、`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` に引き継ぎ項目を登録した。
...
### 2. 共有と後続登録
- `docs/notes/parser/core-parse-streaming-todo.md` を更新し、Packrat 経路・notes ローカライズ・ストリーミング重複検証の TODO を Phase 2-7 へ移管した旨と手順を追記。  
...
- Phase 2-7 で実装する Packrat スナップショット・多言語テンプレートの検証フローは §2.7 計画および `docs/notes/parser/core-parse-streaming-todo.md` にTODOとして残し、本フェーズではドキュメント共有まで完了した。
...
## EXEC-001 Step3 ストリーミング制御ループ PoC（2026-01-16）
...
- `docs/spec/2-7-core-parse-streaming.md` へ PoC 状態の脚注を追加する準備を進める（Step5 で対応）。`docs/guides/compiler/core-parse-streaming.md` §6 のサンプルコードにも `StreamDriver` の擬似コードを反映する。
...
- `docs/guides/compiler/core-parse-streaming.md` と `docs/guides/dsl/plugin-authoring.md` を更新し、ストリーミング経路およびプラグイン API が `ExpectationSummary` をそのまま活用できる運用ガイドを追記。S4 時点の CLI/LSP ゴールデンは再利用し、ドキュメント差分のみで完結しているため追加のスナップショット生成は不要と判断。
- フォローアップ共有として `docs/notes/process/spec-integrity-audit-checklist.md` の草案を作成し、Phase 2-8 で利用する監査チェックリストに期待集合モニタリング項目（`parser.expected_summary_presence` / `parser.expected_tokens_per_error`）を登録できるよう TODO セクションを整備。
...
## ERR-002 Step0 回復フロー棚卸し（Week 32 Day2）
...
- RunConfig 移行時に残っている Packrat/左再帰シムの未実装事項を `docs/notes/parser/core-parser-migration.md` に TODO として記録し、今後の追跡先（`PARSER-003`・`LEXER-002`）を明文化。
...
- `docs/guides/compiler/core-parse-streaming.md` §9 を更新し、RunConfig 共有時に CLI 側の JSON 設定をストリーミング経路へ引き渡すワークフローと `parser-runconfig-packrat.json.golden` を用いた検証手順を紹介した。

### 2. レビュー記録とリンク
- `docs/notes/parser/core-parser-migration.md` に Phase 2-5 RunConfig 移行ステップを追記し、完了タスク（Step1〜6）と残課題（Packrat 実装、LSP 自動テスト、監査指標拡張）を一覧化した。今後の検証先として `PARSER-003`・`LEXER-002`・`EXEC-001` を明示。
...
- Streaming 系タスクとの依存関係（`ParserId` の安定化、`RunConfig` の伝播、Streaming PoC での lex 再利用）を整理し、`docs/notes/parser/core-parse-streaming-todo.md` に共有メモを追加した。
...
- `docs/notes/parser/core-parse-streaming-todo.md` へ `space_id` round-trip の決定事項を追記。
...
- 字句性能計測ノート（docs/notes/parser/lexer-performance-study.md:1-32）を作成し、`scripts/benchmark-parse-throughput.sh` による測定手順と `remlc` 未構築により計測を後続へ持ち越した事情、再計測 TODO を整理した。
...
- `docs/notes/parser/lexer-performance-study.md:1-32`
...
- ガイド `docs/guides/compiler/core-parse-streaming.md` に `9.2 Core.Parse.Lex プロファイル共有サンプル` を追加し、ストリーミング経路で `Core.Parse.Lex.Bridge.derive` と `Core.Parse.Lex.Api.lexeme` を利用する手順を記載。RunConfig ビルダーと `lexer.shared_profile_pass_rate` の活用方法を共有した。
...
- `docs/guides/compiler/core-parse-streaming.md` の新セクションが `RunConfig` 共有手順（9.1）と矛盾しないことをレビュー。`Core.Parse.Lex.Api` 利用例が既存の `StreamDriver` 設計と整合することを確認した。
...
- Step5 で `docs/spec/3-6-core-diagnostics-audit.md` / `docs/guides/runtime/runtime-bridges.md` 等の脚注更新を実施し、OCaml 実装への反映日と依存関係を明記する。
...
- `docs/spec/3-8-core-runtime-capability.md:254` / `docs/guides/runtime/runtime-bridges.md:9` / `docs/notes/dsl/dsl-plugin-roadmap.md:121` に `Target` / `Plugin` / `Lsp` ドメインの整合メモと脚注を挿入し、RunConfig/Capability/Plugin 計画への依存関係を明文化。  
- `docs/plans/bootstrap-roadmap/2-5-proposals/DIAG-003-proposal.md` の Step5 に結果要約を追加し、計画内で完了日時と更新対象を明示。

### 2. 検証と共有
- 脚注内で参照するドキュメント同士（`docs/spec/3-6-core-diagnostics-audit.md`, `docs/spec/3-8-core-runtime-capability.md`, `docs/guides/runtime/runtime-bridges.md`, `docs/notes/dsl/dsl-plugin-roadmap.md`）のリンクが成立していることを相互チェック。  
...
### 1. 更新内容
- 仕様・ガイド脚注: `docs/spec/2-6-execution-strategy.md`・`docs/spec/2-7-core-parse-streaming.md` に Phase 2-5 PoC の制限と監視指標を追加し、`RunConfig.extensions["stream"]` の既定キーと残課題（Packrat 共有、バックプレッシャ自動化、Pending/Error 監査）の状態を明示。`docs/guides/compiler/core-parse-streaming.md` §10 を新設し、PoC 状態と既知制限をまとめた。
- ランタイム連携: `docs/guides/runtime/runtime-bridges.md` にストリーミング信号（`DemandHint` / `StreamMeta`）を Runtime Bridge へ伝達する手順と監査ログ (`parser.stream.*`) の拡張方針を追加。Phase 2-7 で Stage 監査と連携するための Capability 設定を記載した。
...
### 3. TODO / 引き継ぎ
1. Packrat キャッシュ共有とバックプレッシャ自動化は `docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md` §6 に引き継ぎ済み。実装後は `docs/guides/compiler/core-parse-streaming.md` §10 の脚注を更新し、既知制限一覧から該当項目を削除する。
...
3. CLI JSON / `Cli.Stats` に `stream_meta` を組み込み、`scripts/validate-diagnostic-json.sh` の検証項目を拡張する。実装時は `docs/spec/2-6-execution-strategy.md` と `docs/guides/runtime/runtime-bridges.md` の監査要件を同期。
...
- `nl -ba docs/guides/runtime/runtime-bridges.md | sed -n '200,240p'` で Runtime Bridge との連携手順と監査拡張を確認。Phase 2-7 フォローアップとリンクしてあることをチェック。
