...
### 3. 残課題とタグ
1. 差分タグ `TYPE-002-S1` を発行し、本エントリと `docs/notes/effect-system-tracking.md`（Phase 2-5 TYPE-002 Step1 節）を紐付けた。
...
- 次ステップでは仕様脚注（Chapter 1-2 / 1-3）と `0-3-audit-and-metrics.md` の KPI 追記を行うため、`docs/notes/effect-system-tracking.md` に RowVar 対応と指標シリアライズの TODO を登録。
...
### 3. フォローアップ
- `docs/notes/effect-system-tracking.md` に Step3 セクションを追加し、脚注・ガード・監査キーの更新点と Phase 2-7 への宿題（`diagnostics.effect_row_stage_consistency` KPI 実装、脚注撤去チェックリスト）を記録。
...
### 3. フォローアップ
- `docs/notes/effect-system-tracking.md` に Step4 サマリを追記し、Phase 2-7 着手前に実施する前提タスク（dual-write 実験ブランチ作成、Windows 監査プロファイル拡張、RowVar 評価メモ更新）を TODO として登録。  
...
- フォローアップ: `Σ_after = (Σ_before - Σ_handler) ∪ Σ_residual` の本実装、効果操作の戻り値推論、CI 指標導入は Step4/Step5 で継続。`docs/notes/effect-system-tracking.md` に TODO を追記。
...
- 上記データを前提に指標を定義。`syntax.effect_construct_acceptance = accepted / total`、`effects.syntax_poison_rate = poisoned / total` とし、PoC 期間は (0.0 / 1.0)、本実装後は (1.0 / 0.0) を `--require-success` の閾値とする。`docs/notes/effect-system-tracking.md` と `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` にゲート条件を追記した。
...
3. `docs/notes/effect-system-tracking.md` の PoC 指標節を更新し、`syntax.effect_construct_acceptance` / `effects.syntax_poison_rate` の遷移条件と `Σ` 記録仕様を Phase 2-7 キックオフレビュー資料へ引用できる状態に整備する。
...
- `docs/notes/effect-system-tracking.md` に Step5 ログと参照表を追加し、引き継ぎチェックリスト（H-O1〜H-O5）と紐付け。
...
### 2. 差分・確認事項
- 診断ドメイン拡張（DIAG-001〜003）: `docs/spec/3-6-core-diagnostics-audit.md`, `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md`, `docs/guides/runtime/runtime-bridges.md`, `docs/notes/dsl-plugin-roadmap.md` の脚注とメトリクス定義が揃っているか Step7 チェックリストで確認する必要がある。
- RunConfig / Streaming（PARSER-002/003）: `docs/spec/2-1-parser-type.md`, `docs/spec/2-6-execution-strategy.md`, `docs/guides/compiler/core-parse-streaming.md`, `docs/notes/core-parse-api-evolution.md` の記述が `parser-runconfig-packrat.json.golden` と同期しているかを検証し、未同期の場合はゴールデンと脚注を更新する。
- 効果構文 PoC（EFFECT-002）: `docs/spec/1-1-syntax.md`, `docs/spec/1-3-effects-safety.md`, `docs/spec/1-5-formal-grammar-bnf.md`, `docs/notes/effect-system-tracking.md` に掲載した `Σ` 脚注と KPI (`syntax.effect_construct_acceptance`, `effects.syntax_poison_rate`) が同一条件を指しているか確認し、齟齬があれば Phase 2-7 向け TODO を更新する。
...
3. Phase 3 引き継ぎ資料（Step9.3）は `docs/plans/bootstrap-roadmap/3-0-phase3-transition.md` と `docs/notes/effect-system-tracking.md` の対象節を明示し、今回追加したタグ（例: `Phase3:effects`, `Phase3:parser-streaming`, `Phase3:diagnostics`）を活用して移行計画を立てる。
...
### 3. フォローアップ
- CLI/LSP ゴールデンから ASCII 制限文言を撤去するタスクと、`docs/notes/dsl-plugin-roadmap.md` チェックリスト更新を Phase 2-7 へ連携。  
...
- `docs/notes/effect-system-tracking.md` を新設し、PoC ステージと `-Zalgebraic-effects` フラグ運用、Phase 2-7 引き継ぎ TODO、CI 指標（`syntax.effect_construct_acceptance`）の導入方針を整理。
...
3. DSL/プラグイン連携資料（`docs/notes/dsl-plugin-roadmap.md`）へ Capability 名の Unicode 別名と互換モード運用の暫定ルールを追記し、Phase 2-7 以降の実装計画へ差分を引き継ぐ。
...
### 3. TODO / 引き継ぎ
1. Step3（DSL/プラグインチーム連携）で `docs/notes/dsl-plugin-roadmap.md` の Unicode 別名ポリシーと CLI ガイド更新を実施する。  
...
### 3. TODO / 引き継ぎ
1. S1 以降で `parser.mly` へ構文を実装する際、この脚注を削除・更新する条件と手順を `docs/notes/effect-system-tracking.md` に明記する。  
...
- `docs/notes/effect-system-tracking.md` に PoC 指標の JSON 断片と記録方法を追記し、Phase 2-7 で CI へ接続する際の参照元を整備した。
...
- `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` と `reports/diagnostic-format-regression.md` に効果構文計測の項目が無いことを確認し、Phase 2-7 へ連携するための KPI 定義・ゴールデン更新手順・サンプル JSON の追加箇所を洗い出した。`docs/notes/effect-system-tracking.md` では診断/CI セクションを更新し、S3 以降の TODO を整理する計画とした。

### 2. 成果物
- PoC 向けのメトリクス例 (`effect_syntax.constructs` / `metrics`) を JSON で作成し、`syntax.effect_construct_acceptance = 0.0`・`effects.syntax_poison_rate = 1.0` を Phase 2-5 の基準値とするサンプルを本計画書と `docs/notes/effect-system-tracking.md` に添付。  
...
- `docs/notes/effect-system-tracking.md` の Stage 表を更新し、診断・CI 計測が「計測設計確定」ステージへ進んだことを記録。Phase 2-7 への引き継ぎ TODO として、監査ダッシュボード更新とゴールデン整備の担当割り当てを追記した。
...
- `docs/notes/effect-system-tracking.md` を再整理し、S4 引き継ぎパッケージとして Stage 表と Phase 2-7 実行チェックリスト（H-O1〜H-O5）を表形式で掲載。CLI/LSP/CI の各担当が参照すべきファイルを列挙した。  
...
- `docs/notes/effect-system-tracking.md` に H-O1〜H-O5 のチェックリストを追加し、作業履歴へ S4 完了を記録。Phase 2-7 で消化すべき実行条件を一元化した。
...
3. Stage = Stable へ昇格した段階で脚注 `[^effects-syntax-poc-phase25]` を撤去し、`docs/spec/README.md` と `docs/notes/dsl-plugin-roadmap.md` のチェックリストを更新する。撤去判断は Phase 2-7 終盤レビューで行う。
...
### 1. ASCII 運用ポリシーの整理
- `docs/notes/dsl-plugin-roadmap.md` §7 を新設し、Phase 2-5 中はプラグイン ID・Capability 別名を ASCII 限定で登録する暫定ポリシーを定義。Unicode 別名は Phase 2-7 `lexer-unicode` 完了後に再評価する方針とした。  
...
- `docs/notes/dsl-plugin-roadmap.md` §7 に CLI フラグ運用を追記し、プラグインチームへ ASCII → Unicode 切替ロードマップを共有する。
...
- `docs/spec/3-8-core-runtime-capability.md:254` / `docs/guides/runtime/runtime-bridges.md:9` / `docs/notes/dsl-plugin-roadmap.md:121` に `Target` / `Plugin` / `Lsp` ドメインの整合メモと脚注を挿入し、RunConfig/Capability/Plugin 計画への依存関係を明文化。  
- `docs/plans/bootstrap-roadmap/2-5-proposals/DIAG-003-proposal.md` の Step5 に結果要約を追加し、計画内で完了日時と更新対象を明示。

### 2. 検証と共有
- 脚注内で参照するドキュメント同士（`docs/spec/3-6-core-diagnostics-audit.md`, `docs/spec/3-8-core-runtime-capability.md`, `docs/guides/runtime/runtime-bridges.md`, `docs/notes/dsl-plugin-roadmap.md`）のリンクが成立していることを相互チェック。  
