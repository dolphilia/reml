...
3. `cargo xtask prelude-audit --section iter --strict --baseline docs/spec/3-1-core-prelude-iteration.md` を実行し、出力を `reports/spec-audit/ch1/iter.json` に JSON 形式で保存。`reports/spec-audit/ch0/links.md` と `docs/notes/docs-migrations.log` にコマンドライン・日付・成果を追記する。
...
- 2027-02-24 更新: `cargo xtask prelude-audit --section iter --filter adapter --strict --output reports/spec-audit/ch1/core_iter_adapters.json` を実行し、adapter 12 API の `missing_entries = []` を `run_id = 2027-02-24-iter-adapter-g4` として保存。`reports/spec-audit/ch1/core_iter_adapters.json` へ `inventory_snapshot` を追加し、`iterator.adapter.coverage = 1.0` を `reports/iterator-adapter-metrics.json`・`docs/guides/tooling/audit-metrics.md`（KPI 表）・`docs/notes/stdlib/core-library-outline.md#iter-adapter` に連携した。CLI ログとスナップショット参照は `reports/spec-audit/ch0/links.md#iter-adapters-g4`、更新履歴は `docs/notes/docs-migrations.log` に記録済み。
...
3. 上記変更を `docs/notes/docs-migrations.log` に記述し、`docs/plans/bootstrap-roadmap/3-1-core-prelude-iteration-plan.md` §3 の状況説明を更新（例: 「2026-W05 時点で pipeline snapshot 未整備」など）。
...
- `docs/plans/bootstrap-roadmap/assets/prelude_api_inventory.toml` の `Iter.map` / `Iter.filter` 行に KPI・テスト名・効果タグの取り扱いを追記し、`rust_status = "working"` → `implemented` への遷移条件を明示する。更新後は `docs/notes/docs-migrations.log` に「WBS 3.1c-G1 map/filter 立ち上げ」を追記する。
...
- 2026-02-21 更新: 上記 G2 実装は `cargo test core_iter_adapters -- --include-ignored flat_map_vec zip_mismatch` と `python3 tooling/ci/collect-iterator-audit-metrics.py --section iterator --case flat_map --case zip --output reports/iterator-flatmap-metrics.json --secondary-output reports/iterator-zip-metrics.json --require-success` で実測済み。`reports/iterator-flatmap-metrics.json` には `adapter_metrics.flat_map_vec.effects.mem = true` / `mem_reservation_bytes = 3` を、`reports/iterator-zip-metrics.json` には `adapter_metrics.zip_mismatch.iterator.error.zip_shorter = 1` / `stage.required = Exact("stable")` を記録。`reports/spec-audit/ch1/core_iter_adapters.json`・`reports/spec-audit/ch0/links.md#iter-adapters`・`docs/plans/bootstrap-roadmap/assets/prelude_api_inventory.toml`・`docs/guides/tooling/audit-metrics.md`・`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md`・`docs/notes/stdlib/core-library-outline.md#iter-g2-flat-zip` へ同じ Run-ID を反映し、`docs/notes/docs-migrations.log` に WBS 3.1c-G2 完了ログを追記した。
...
- 補正文書: `docs/notes/stdlib/core-library-outline.md`, `docs/plans/bootstrap-roadmap/3-0-phase3-self-host.md`, `docs/notes/docs-migrations.log`
...
3. `docs/guides/tooling/audit-metrics.md` の KPI 表へ `iterator.mem.window` を追加し、`docs/plans/rust-migration/3-2-benchmark-baseline.md` §3.2.4 とリンクさせる。Run-ID は `reports/spec-audit/ch0/links.md#iter-buffered` / `reports/iterator-stage-summary.md#iter-buffered` / `docs/notes/docs-migrations.log` に残し、`docs/notes/stdlib/core-library-outline.md#iter-g3-buffered-backpressure` と `docs/plans/bootstrap-roadmap/3-0-phase3-self-host.md#iter-adapter` から参照できるようにする。
...
5. 2027-02-22 実施結果: `cargo test --manifest-path compiler/frontend/Cargo.toml core_iter_adapters -- --include-ignored buffered_window` → `cargo bench -p compiler-rust-frontend iter_buffered -- warmup-time 3 --measurement-time 10` → `python3 tooling/ci/collect-iterator-audit-metrics.py --section iterator --case buffered --output reports/iterator-buffered-metrics.json --require-success` を完了。`reports/iterator-buffered-metrics.json` に `adapter_metrics.buffered_window.effects.mem_bytes = 2` / `backpressure = {"produced": 6, "consumed": 4, "dropped": 2, "ratio": 0.33}` を記録し、Criterion 結果 `reports/benchmarks/iter_buffered-2027-02-22.json` で `windows_per_sec = 1.89e6` / `delta_pct = +0.038`（±10% 以内）を確認した。Run-ID `2027-02-22-iter-adapter-g3` を `reports/spec-audit/ch0/links.md#iter-buffered` と `reports/iterator-stage-summary.md#iter-buffered`、`docs/notes/docs-migrations.log`、`docs/plans/bootstrap-roadmap/assets/prelude_api_inventory.toml` に反映済み。
