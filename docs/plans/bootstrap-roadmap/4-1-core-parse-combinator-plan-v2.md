...
- `docs/notes/parser/core-parse-combinator-survey.md`（Phase 0 調査結果）  
...
## フェーズ計画
### Phase 7: 調査結果の統合方針策定
- `docs/notes/parser/core-parse-combinator-survey.md` の調査項目を「高: API 拡張 / 中: ランタイム追加 / 低: 糖衣」に再編し、採用/見送り理由と影響範囲（仕様改訂の有無、破壊的変更の有無）を明文化。性能・安全性（`docs/spec/0-1-project-purpose.md`）に反するものは保留に回す。  
- 導入候補と方針  
  - **高（API 拡張）**:  
...
- 仕様・ドキュメントへの反映方針  
  ...
  - 観測系 API は `docs/notes/parser/core-parse-api-evolution.md` に暫定契約を残し、回帰指標（期待集合、バックトラック率）の計測ポイントを明文化してから仕様化する。
- 実施記録  
  - `docs/notes/parser/core-parse-combinator-survey.md#phase-7-統合方針（優先度再編と採否理由）` に Phase 7 の採否テーブルと影響度を集約済み。性能/安全性の判断基準は `docs/spec/0-1-project-purpose.md` に準拠。  
  - 脚注ドラフトの起票先: `docs/spec/2-2-core-combinator.md`（優先度ビルダー/`recoverWith`/左再帰ガード）、`docs/spec/2-0-parser-api-overview.md`（`RunConfig.lex/layout/profile`）。観測系 API は `docs/notes/parser/core-parse-api-evolution.md` に暫定契約を追記する。  
...
- Phase 7 採否テーブル（`docs/notes/parser/core-parse-combinator-survey.md#phase-7-統合方針（優先度再編と採否理由）`）を前提に、演算子優先度ビルダーを「非破壊追加」として設計する。`committed` フラグの扱いと期待集合抑制を `chainl1/chainr1` の巻き戻し規約に統合する。  
...
- OpBuilder: 新ビルダー/autoWhitespace を `OpBuilder` DSL に統合するための変更点を洗い出し、回帰テストを更新。互換性リスクを `docs/notes/parser/core-parse-api-evolution.md` に記載。
...
- OpBuilder/autoWhitespace: RunConfig での演算子テーブル上書き方針とプラグイン設定衝突時の回帰監視案を `docs/notes/parser/core-parse-api-evolution.md#phase-4-1-phase11-pluginstreamingopbuilder-連携メモ` に追記。`phase4-scenario-matrix.csv` への追加と Phase 12 のフォローアップを次ステップとして残した。
...
- ハンドオーバー: 未着手/保留項目は `docs/notes/parser/core-parse-api-evolution.md` に TODO として残し、次フェーズの入口を明示。
