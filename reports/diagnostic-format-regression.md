...
- レビュー: `docs/plans/bootstrap-roadmap/3-1-core-prelude-iteration-plan.md` §4.a、`docs/notes/stdlib/core-library-outline.md#iter-g2-flat-zip`、`docs/plans/bootstrap-roadmap/2-7-deferred-remediation.md#iterator-adapter-esc` に参照ログを残し、zip の長さ差が診断・監査の両面で報告されているか二重チェックする。

### `iterator.buffered_window` 差分ログ（WBS 3.1c-G3）
- コマンド: `cargo test --manifest-path compiler/rust/frontend/Cargo.toml core_iter_adapters -- --include-ignored buffered_window` → `scripts/validate-diagnostic-json.sh --pattern iterator.buffered reports/spec-audit/ch1/core_iter_adapters.json`。
- 出力: `reports/spec-audit/ch1/core_iter_adapters.json` と `reports/spec-audit/ch1/iterator.buffered.diagnostics.json` に `EffectLabels.mem_bytes = 2`、`iterator.backpressure.{produced=6,consumed=4,dropped=2,ratio=0.33}`、`effect.stage.iterator.required = "Exact(experimental)"` / `actual = "experimental"` を記録。`reports/iterator-buffered-metrics.json` の `adapter_metrics.buffered_window.backpressure` と一致していることを確認。
- レビュー: `docs/plans/bootstrap-roadmap/3-1-core-prelude-iteration-plan.md` §4.a G3、`docs/notes/stdlib/core-library-outline.md#iter-g3-buffered-backpressure`、`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md`、`docs/plans/bootstrap-roadmap/3-0-phase3-self-host.md#iter-adapter` にリンクを貼り、リングバッファ容量・バックプレッシャ・Stage 情報がすべてのドキュメントで同期していることを監査する。
