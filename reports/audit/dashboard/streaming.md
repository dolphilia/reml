...
|------------------|-----------|----------|------|
| linux-x86_64 | 1.0 | `dune runtest compiler/ocaml/tests/streaming_runner_tests.ml` → `tooling/ci/collect-iterator-audit-metrics.py --section streaming --require-success --source compiler/ocaml/tests/golden/diagnostics/parser/streaming-outcome.json.golden` | `resume_lineage = ["feeder.await"]` を維持したまま成功 |
