...
## 2025-12-29 docs-examples-audit Core.Lsp サンプル再検証
| JST 時刻 | コマンド | 結果 | 備考 |
|----------|----------|------|------|
...
| 22:51 | `sec=sec_3; f="examples/docs-examples/spec/3-18-core-system/${sec}.reml"; out="reports/spec-audit/ch3/3-18-core-system__${sec}-20260101-diagnostics.json"; RUSTFLAGS="-C link-arg=-fuse-ld=lld" cargo run --manifest-path compiler/rust/frontend/Cargo.toml --bin reml_frontend -- --output json "$f" > "$out"` / `for f in examples/docs-examples/spec/3-18-core-system/sec_4_1.reml examples/docs-examples/spec/3-18-core-system/sec_6.reml; do sec=$(basename "$f" .reml); out="reports/spec-audit/ch3/3-18-core-system__${sec}-20260101-diagnostics.json"; RUSTFLAGS="-C link-arg=-fuse-ld=lld" cargo run --manifest-path compiler/rust/frontend/Cargo.toml --bin reml_frontend -- --output json "$f" > "$out"; done` | ✅ diagnostics 0 件 | `sec_3`/`sec_4_1`/`sec_6` を修正し再採取。 |
