...
- **Metrics command**: `python3 tooling/ci/collect-iterator-audit-metrics.py --suite collectors --module iter --case wbs-31b-f2 --output reports/iterator-collector-metrics.json`（`--suite collectors` が未指定時は `--section collectors --source ... --audit-source ...` を明示する）。
- **Core.Collections gate**: `python3 tooling/ci/collect-iterator-audit-metrics.py --suite collectors --thresholds docs/plans/bootstrap-roadmap/assets/metrics/core_collections_thresholds.json --scenario ref_internal_mutation --require-success --require-cell`。`collector.effect.cell_rc` のメトリクスが存在しない場合は `threshold metric missing` が `enforcement.failures[]` に記録される。
...
- **現状**: `core_iter_collectors.snap` には `collect_cell_ref_effects` ケースが未収録のため、上記コマンドは `threshold metric missing: collector.effect.cell_rc (ref_internal_mutation)` を記録する。Run ID `20290730-collectors-suite-gate` ではこのメッセージを `reports/iterator-collector-metrics.json` の `thresholds.failures[]` に残し、データ追加後にゼロへ遷移することを Go/No-Go 条件とした。
...
- **現状**: `table_csv_import` スナップショットも未収録のため、`threshold metric missing: collector.table.csv_import (table_csv_import)` が `thresholds.failures[]` に残る。`core_collections_thresholds.json` を更新した際はこのメッセージを参考にし、KPI が計測できた Run ID を `reports/spec-audit/ch0/links.md#collector-f2` に追加する。

### audit_bridge
- **目的**: `Core.Collections` の `Map.diff`/`Set.diff`/`Table.to_map` 操作が `Collections.audit_bridge` を経由して `AuditEnvelope.change_set` の `collections.diff.*` エントリを生成し、`collector.effect.audit`/`collector.effect.mem_bytes` と `AuditEnvelope.metadata` が連動することを保証する。詳細計画は `docs/plans/bootstrap-roadmap/3-2-core-collections-plan.md` §5.1 に記載されている【F:../docs/plans/bootstrap-roadmap/3-2-core-collections-plan.md†L316-L333】。
- **手順**: `python3 tooling/ci/collect-iterator-audit-metrics.py --section collectors --scenario map_set_persistent --require-audit` を実行し、`reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` の `collections.diff` オブジェクトと `collector.effect.audit`/`collector.effect.mem_bytes` の整合性を検証する【F:../tooling/ci/collect-iterator-audit-metrics.py†L3595-L3665】【F:../reports/spec-audit/ch1/core_iter_collectors.audit.jsonl†L1-L7】。
- **検証**: `scripts/validate-diagnostic-json.sh --pattern collections.diff --pattern collector.effect.audit --pattern collector.effect.mem_bytes reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` で `collections.diff.items` の `kind`/`key`/`value`、`metadata` に `collector.effect.audit` が `true`（差分あり）または `false`（ゼロ差分）であることを確認するルートを整備する【F:../scripts/validate-diagnostic-json.sh†L1-L200】。

### audit_cap
- **目的**: `core.collections.audit` Capability を必要とする差分付与パス（`collections.diff.*` + `effect {audit}`）が `collector.capability`/`collector.effect.audit`/`collections.change_set.total` を全て記録し、` audits/ audit.jsonl` で `collector.capability` が `core.collections.audit` になっていることを担保する。
- **手順**: `python3 tooling/ci/collect-iterator-audit-metrics.py --section collectors --scenario audit_cap --source reports/spec-audit/ch1/core_iter_collectors.json --audit-source reports/spec-audit/ch1/core_iter_collectors.audit.jsonl --require-success` を実行。`scripts/validate-diagnostic-json.sh --pattern collector.capability --pattern collector.effect.audit --pattern collections.diff.total reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` で `case=audit_cap` の `collector.capability`/`collector.effect.audit`/`collections.diff.total` を検証する。
- **KPI**: `collector.capability.audit_pass_rate = 1.0`、`collector.effect.audit_presence = 1.0`、`collections.change_set.total >= 1`。結果は `reports/spec-audit/ch1/core_iter_collectors.audit.jsonl` の `case=audit_cap` entry と `docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md` の Phase3 Capability 行に反映する。
