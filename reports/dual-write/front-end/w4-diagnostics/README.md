...
- **20290427-w4-diag-recover-stream**: `docs/plans/rust-migration/appendix/w4-diagnostic-cases-recover-stream.txt` を用いて Recover/Streaming ケースを再実行。`reports/dual-write/front-end/w4-diagnostics/20290427-w4-diag-recover-stream/*/effects-metrics.rust.err.log` が `bridge_stage.audit_presence < 1.0` を吐き、`metrics_ok=false` のままなので `bridge_stage.audit_presence` 用の Stage/Capability 拡張が引き続きブロッカー。
- **20290428-w4-diag-type-effect**: `--force-type-effect-flags` を付けて Type/Effect/FFI ケースを再測定したところ、`ffi_stage_messagebox` / `ffi_ownership_mismatch` の `parser-metrics.rust.err.log` が `parser.expected_summary_presence < 1.0` を報告。加えて `ffi_async_dispatch` では OCaml CLI diagnostics が未出力のままで `diag_match=false`。`collect-iterator-audit-metrics.py --section effects --require-success --source <case>/diagnostics.{ocaml,rust}.json` で `effect_scope.audit_presence` を検証しつつ、`expected_tokens` と `parser.expected_summary_presence` を再ゲート化する。
- **20290429-w4-diag-cli-lsp**: CLI/LSP ケースを `docs/plans/rust-migration/appendix/w4-diagnostic-cases-cli-lsp.txt` で Dual-write したあと、`npm run ci --prefix tooling/lsp/tests/client_compat -- diag-w4 20290429-w4-diag-cli-lsp` を実行し `tooling/lsp/tests/client_compat/scripts/report-fixture-diff.mjs` で `reports/dual-write/front-end/w4-diagnostics/20290429-w4-diag-cli-lsp/lsp/<case>.diff` を出力。`reports/dual-write/front-end/w4-diagnostics/20290429-w4-diag-cli-lsp/*/effects-metrics.rust.err.log` 全件に `bridge_stage.audit_presence < 1.0` が記録され `metrics_ok=false`、さらに CLI 側 `diagnostics.ocaml.json` が空のケースもあり `parser.runconfig_switch_coverage` は 0 から変化せず `diag_match=false` が継続しているため `TODO: DIAG-RUST-07` の改善が必須。
