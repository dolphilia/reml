...
- `StatisticsError` → `Diagnostic` のブリッジは `compiler/rust/runtime/src/numeric/error.rs` で `column`/`aggregation`/`audit_id` と `numeric.statistics.*`/`data.stats.*` の両メタデータへ書き込むように進捗し、`encode_sample_value` で `NaN`/`±Infinity` を JSON 文字列化。`scripts/validate-diagnostic-json.sh --suite numeric` を追加して `tests/data/numeric/`・`tests/expected/numeric_*.json` を `diagnostic-v2` スキーマで検証できるようにしました（Plan §3.2）。
