...
- `docs/spec/3-3-core-text-unicode.md` §9 に `examples/core-text` への注記を追記し、`reports/spec-audit/ch1/core_text_examples-YYYYMMDD.md` から実行ログを参照できるようにしています。サンプルの進捗記録は `docs/plans/bootstrap-roadmap/3-3-core-text-unicode-plan.md` §5 と `docs/notes/process/docs-update-log.md` で管理します。
...
- `MetricPoint` → `AuditSink`、`StatisticsError` → `Diagnostic`、`Timestamp` → `IO` の 3 経路について、`docs/notes/runtime/core-numeric-time-gap-log.md` にバックログ（2025-12-01 付）を登録し、README・Phase3 Self-Host・監査計画 (`docs/plans/bootstrap-roadmap/0-3-audit-and-metrics.md`) から追跡できるようにしました。
...
- `docs/spec/3-5-core-io-path.md` §4.2/§7 や `docs/spec/3-0-core-library-overview.md` にサンプル参照を追記し、Reader/Writer + `IoContext` と Path セキュリティヘルパの監査ポイントが一目で追えるようにしました。ガイド (`docs/guides/runtime/runtime-bridges.md`, `docs/guides/dsl/plugin-authoring.md`) では `IoContext.helper` と Capability チェックのベストプラクティスを追加しています。
- `docs/plans/bootstrap-roadmap/3-5-core-io-path-plan.md`、`docs/plans/rust-migration/overview.md`、`docs/plans/bootstrap-roadmap/3-0-phase3-self-host.md` にも本タスクの完了ログと依存関係を明記し、`docs/notes/runtime/runtime-bridges-roadmap.md` で Runtime Bridge とサンプル/CI Runbook の突合を管理します。

## Core.Diagnostics 進捗

- `docs/spec/3-6-core-diagnostics-audit.md` §9 を Rust Frontend の最新 CLI 出力に合わせて更新し、`examples/core_diagnostics/pipeline_*.reml` から生成した `CliDiagnosticEnvelope`/`AuditEvent` の抜粋を掲載しました。Chapter 1 の概要 (`docs/spec/1-0-language-core-overview.md`) と Chapter 3 の概要にも同サンプルへの参照を追加しています。
- `tooling/examples/run_examples.sh --suite core_diagnostics --update-golden` で診断・監査ゴールデン（`*.expected.diagnostic.json` / `*.expected.audit.jsonl`）を再生成できるようにし、`examples/core_diagnostics/README.md` に手順をまとめました。CI はこれらのゴールデンを基準に Stage/Audit 伝搬の回帰を検出します。

## Core.Runtime & Capability 進捗

- `reml_capability list --format json > reports/spec-audit/ch3/capability_list-20251205.json` と `scripts/capability/generate_md.py --output docs/spec/3-8-core-runtime-capability.md` を組み合わせ、3.8 章と本 README の Capability テーブルを同じ JSON から再生成できるようにしました（Run ID: `20251230-capability-doc-sync`, ログ: `docs/notes/runtime/runtime-capability-stage-log.md`）。`reml_capability describe <id>` を利用すると CLI から Stage/EffectScope/Provider を確認できます。
- `examples/core_diagnostics/pipeline_branch.reml` で `effects.contract.stage_mismatch` を再現し、`reports/spec-audit/ch3/capability_stage-mismatch-20251206.json` に `capability.id=console` / `effect.stage.required` / `bridge.stage.*` を保持した CLI/Audit ログをまとめました。`tooling/examples/run_examples.sh --suite core_diagnostics --with-audit` に `allowed failure` を組み込んだため、Stage mismatch のサンプルを更新しても他の例がブロックされません。
- `docs/plans/bootstrap-roadmap/assets/capability-stage-flow.{mmd,svg}` で RunConfig→CapabilityRegistry→StageAuditPayload→Diagnostics/Audit/KPI の流れを図示し、CI では `python3 tooling/ci/collect-iterator-audit-metrics.py --section runtime --require-success --matrix docs/plans/bootstrap-roadmap/assets/core-io-capability-map.md` を使って `runtime.capability_validation` を定期測定しています。利用手順は `docs/plans/bootstrap-roadmap/3-0-phase3-self-host.md#303c-capability-registry-完成条件` に記録しました。
