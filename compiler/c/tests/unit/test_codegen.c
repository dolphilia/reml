...
static void test_codegen_string_literal(void **state) {
  (void)state;

  const char *source = "let x = \"ab\" + \"cd\"; x;";
  reml_compilation_unit *unit = parse_source(source);
...
static void test_codegen_match_tuple_record(void **state) {
  ...
  {
    const char *source =
        "let x = (1, 2); match x with | (1, 2) -> 1 | (2, _) -> 2 | _ -> 3;";
    reml_compilation_unit *unit = parse_source(source);
  ...
  {
    const char *source =
        "let x = { a: 1, b: 2 }; match x with | { a: 1, b: 2 } when true -> 1 | _ -> 2;";
    reml_compilation_unit *unit = parse_source(source);
  ...
  {
    const char *source =
        "let x = (1, 2); match x with | (1, 2) when true -> 1 | _ -> 2;";
    reml_compilation_unit *unit = parse_source(source);
...
static void test_codegen_string_unicode_escape(void **state) {
  (void)state;

  const char *source = "let x = \"\\u{1F600}\"; x;";
  reml_compilation_unit *unit = parse_source(source);
...
void test_codegen(void **state) {
  ...
  test_codegen_string_literal(state);
  test_codegen_string_unicode_escape(state);
