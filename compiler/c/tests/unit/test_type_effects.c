...
static void test_type_function_effects_unify(void **state) {
  ...
  reml_type *left = reml_type_make_function(&ctx, left_params, result, REML_EFFECT_MUT);
  reml_type *right = reml_type_make_function(&ctx, right_params, result, REML_EFFECT_MUT);
  assert_true(reml_type_unify(&ctx, left, right));

  UT_array *mismatch_params = make_params(param);
  reml_type *mismatch =
      reml_type_make_function(&ctx, mismatch_params, result, REML_EFFECT_IO);
