...
(** 型付き関数引数 *)
and typed_arg =
  | TPosArg of typed_expr
  | TNamedArg of ident * typed_expr
...
and typed_fn_body =
  | TFnExpr of typed_expr
  | TFnBlock of typed_stmt list
...
let make_typed_decl attrs vis kind scheme span =
  { tdecl_attrs = attrs; tdecl_vis = vis; tdecl_kind = kind;
    tdecl_scheme = scheme; tdecl_span = span }
...
let rec string_of_typed_expr texpr =
  ...
  | TLiteral _ ->
      Printf.sprintf "(Literal : %s)" ty_str
  | TVar (id, _) ->
      Printf.sprintf "(%s : %s)" id.name ty_str
  | TModulePath (_, id) ->
      Printf.sprintf "(Path::%s : %s)" id.name ty_str
  | TCall (fn, args) ->
      Printf.sprintf "(Call %s [%d args] : %s)"
        (string_of_typed_expr fn) (List.length args) ty_str
  | TLambda (params, _, body) ->
      Printf.sprintf "(Lambda [%d params] %s : %s)"
        (List.length params) (string_of_typed_expr body) ty_str
  | TPipe (e1, e2) ->
      Printf.sprintf "(%s |> %s : %s)"
        (string_of_typed_expr e1) (string_of_typed_expr e2) ty_str
  | TBinary (_, e1, e2) ->
  ...
  | TFieldAccess (e, field) ->
      Printf.sprintf "(%s.%s : %s)"
        (string_of_typed_expr e) field.name ty_str
  | TTupleAccess (e, idx) ->
      Printf.sprintf "(%s.%d : %s)"
        (string_of_typed_expr e) idx ty_str
  | TIndex (e1, e2) ->
      Printf.sprintf "(%s[%s] : %s)"
        (string_of_typed_expr e1) (string_of_typed_expr e2) ty_str
  | TPropagate e ->
      Printf.sprintf "(%s? : %s)" (string_of_typed_expr e) ty_str
  | TIf (cond, then_e, else_e) ->
      ...
      Printf.sprintf "(if %s then %s%s : %s)"
        (string_of_typed_expr cond) (string_of_typed_expr then_e) else_str ty_str
  | TMatch (e, arms) ->
      Printf.sprintf "(match %s [%d arms] : %s)"
        (string_of_typed_expr e) (List.length arms) ty_str
  | TWhile (cond, body) ->
      Printf.sprintf "(while %s %s : %s)"
        (string_of_typed_expr cond) (string_of_typed_expr body) ty_str
  | TFor (pat, _, body) ->
      Printf.sprintf "(for %s in ... %s : %s)"
        (string_of_typed_pattern pat) (string_of_typed_expr body) ty_str
  | TLoop body ->
  ...
  | TReturn None ->
      Printf.sprintf "(return : %s)" ty_str
  | TReturn (Some e) ->
      Printf.sprintf "(return %s : %s)" (string_of_typed_expr e) ty_str
  | TDefer e ->
      Printf.sprintf "(defer %s : %s)" (string_of_typed_expr e) ty_str
  | TAssign (lhs, rhs) ->
      Printf.sprintf "(%s := %s : %s)"
        (string_of_typed_expr lhs) (string_of_typed_expr rhs) ty_str

(** 型付きパターンの文字列表現（簡易版） *)
and string_of_typed_pattern tpat =
  let ty_str = string_of_ty tpat.tpat_ty in
  let bindings_str = String.concat ", " (List.map (fun (name, ty) ->
    Printf.sprintf "%s: %s" name (string_of_ty ty)
  ) tpat.tpat_bindings) in
  match tpat.tpat_kind with
  | TPatLiteral _ ->
      Printf.sprintf "(PatLit : %s)" ty_str
  | TPatVar id ->
      Printf.sprintf "(%s : %s [%s])" id.name ty_str bindings_str
  | TPatWildcard ->
      Printf.sprintf "(_ : %s)" ty_str
  | TPatTuple pats ->
      Printf.sprintf "(PatTuple [%d] : %s)" (List.length pats) ty_str
  | TPatRecord (fields, has_rest) ->
      let rest_str = if has_rest then " .." else "" in
      Printf.sprintf "(PatRecord [%d fields%s] : %s)"
        (List.length fields) rest_str ty_str
...
let string_of_typed_decl tdecl =
  ...
  | TTraitDecl _ ->
      Printf.sprintf "trait ... : %s" scheme_str
  | TImplDecl _ ->
      Printf.sprintf "impl ... : %s" scheme_str
  | TExternDecl _ ->
      Printf.sprintf "extern ... : %s" scheme_str
  | TEffectDecl _ ->
      Printf.sprintf "effect ... : %s" scheme_str
  | THandlerDecl _ ->
