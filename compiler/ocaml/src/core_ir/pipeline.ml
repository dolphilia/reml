...
(** 単一パスの実行（定数畳み込み + DCE） *)
let run_single_pass (config: pipeline_config) (stats: pipeline_stats) (fn: function_def)
    : function_def =
  let fn_after_const_fold =
    if config.enable_const_fold then begin
      let start_time = Unix.gettimeofday () in
      let fold_config = { Const_fold.max_iterations = 3; verbose = config.verbose } in
      let optimized, fold_stats = Const_fold.optimize_function ~config:fold_config fn in
      let end_time = Unix.gettimeofday () in
      stats.total_const_fold_time <- stats.total_const_fold_time +. (end_time -. start_time);
      stats.total_folded_exprs <- stats.total_folded_exprs + fold_stats.folded_exprs;
  ...
  let fn_after_dce =
    if config.enable_dce then begin
      let start_time = Unix.gettimeofday () in
      ...
      stats.total_removed_bindings <- stats.total_removed_bindings + dce_stats.removed_bindings;
      stats.total_removed_blocks <- stats.total_removed_blocks + dce_stats.removed_blocks;
      if config.verbose then
...
(** 関数に対してパイプラインを実行 *)
let optimize_function ?(config = config_o1) (fn: function_def) : function_def * pipeline_stats =
  let stats = create_pipeline_stats () in
  let optimized =
    if config.opt_level = O0 then
      fn
    else
      run_to_fixpoint config stats fn
  in
  (optimized, stats)

(** モジュール全体に対してパイプラインを実行 *)
let optimize_module ?(config = config_o1) (m: module_def) : module_def * pipeline_stats =
  let stats = create_pipeline_stats () in
  let optimized_fns = List.map (fun fn ->
    ...
    stats.total_const_fold_time <- stats.total_const_fold_time +. fn_stats.total_const_fold_time;
    stats.total_dce_time <- stats.total_dce_time +. fn_stats.total_dce_time;
    stats.total_folded_exprs <- stats.total_folded_exprs + fn_stats.total_folded_exprs;
    stats.total_removed_bindings <- stats.total_removed_bindings + fn_stats.total_removed_bindings;
    stats.total_removed_blocks <- stats.total_removed_blocks + fn_stats.total_removed_blocks;
    optimized
  ) m.function_defs in
