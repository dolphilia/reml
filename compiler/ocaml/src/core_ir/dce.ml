...
let create_stats () : dce_stats =
  { removed_bindings = 0; removed_blocks = 0; removed_stmts = 0 }
...
let rec collect_used_vars (e : expr) : VarSet.t =
  ...
  | Match (scrut, cases) ->
      ...
      List.fold_left
        (fun acc case -> VarSet.union acc (collect_used_vars case.case_body))
        scrut_vars cases
...
let collect_stmt_used_vars (stmt : stmt) : VarSet.t =
  ...
  | Store (_, e) -> collect_used_vars e
  | Alloca _ -> VarSet.empty
...
let collect_block_used_vars (block : block) : VarSet.t =
  let stmt_vars =
    List.fold_left
      (fun acc stmt -> VarSet.union acc (collect_stmt_used_vars stmt))
      VarSet.empty block.stmts
  in
...
let collect_function_used_vars (fn : function_def) : VarSet.t =
  List.fold_left
    (fun acc block -> VarSet.union acc (collect_block_used_vars block))
...
let rec has_side_effect (e : expr) : bool =
  ...
  | Let (_, bound, body) -> has_side_effect bound || has_side_effect body
  | Match (scrut, cases) ->
      has_side_effect scrut
      || List.exists (fun case -> has_side_effect case.case_body) cases
...
let rec dce_expr (used_vars : VarSet.t) (stats : dce_stats) (e : expr) : expr =
  ...
  | Let (var, bound, body) ->
      ...
      if (not (VarSet.mem var used_vars)) && not (has_side_effect bound') then (
        stats.removed_bindings <- stats.removed_bindings + 1;
        body')
      else make_expr (Let (var, bound', body')) e.expr_ty e.expr_span
  | Match (scrut, cases) ->
...
(** 文の DCE *)
let dce_stmt (used_vars : VarSet.t) (stats : dce_stats) (stmt : stmt) :
    stmt option =
  match stmt with
  | Assign (var, e) ->
      let e' = dce_expr used_vars stats e in
      (* 変数が使用されていない、かつ副作用がない場合は代入を削除 *)
      if (not (VarSet.mem var used_vars)) && not (has_side_effect e') then (
        stats.removed_stmts <- stats.removed_stmts + 1;
        None)
      else Some (Assign (var, e'))
  | Store (var, e) ->
      let e' = dce_expr used_vars stats e in
      Some (Store (var, e'))
  | Alloca _ -> Some stmt
  | ExprStmt e ->
      let e' = dce_expr used_vars stats e in
      (* 副作用がない式文は削除 *)
      if not (has_side_effect e') then (
        stats.removed_stmts <- stats.removed_stmts + 1;
        None)
      else Some (ExprStmt e')
  | Return e -> Some (Return (dce_expr used_vars stats e))
  | (Jump _ | Branch _ | Phi _ | EffectMarker _) as s -> Some s

(** 終端命令の DCE *)
let dce_terminator (used_vars : VarSet.t) (stats : dce_stats)
...
(** 到達可能なブロックラベルを収集 *)
let collect_reachable_labels (entry_label : label) (blocks : block list) :
    label list =
  ...
  let rec visit label =
    if not (Hashtbl.mem reachable label) then (
...
(** 到達不能ブロックを削除 *)
let remove_unreachable_blocks (stats : dce_stats) (blocks : block list) :
    block list =
  if List.length blocks = 0 then blocks
  else
    ...
    let reachable_set =
      List.fold_left
        (fun acc lbl ->
          Hashtbl.add acc lbl ();
          acc)
        (Hashtbl.create 32) reachable_labels
    in
    let reachable_blocks =
      List.filter (fun block -> Hashtbl.mem reachable_set block.label) blocks
    in
...
let optimize_function (fn : function_def) : function_def * dce_stats =
  ...
  (* 診断保護フラグをチェック *)
  if fn.fn_metadata.opt_flags.preserve_for_diagnostics then (fn, stats)
  else
    (* 1. 使用される変数を収集 *)
    ...
    List.map
      (fun fn ->
        ...
        stats.removed_bindings <-
          stats.removed_bindings + fn_stats.removed_bindings;
