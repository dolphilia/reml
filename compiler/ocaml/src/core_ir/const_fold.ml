 ...
 * 最適化結果は10進表記で出力。
 *)
let int64_to_literal (i: int64) : literal =
  Int (Int64.to_string i, Base10)

(** float → Ast.literal への逆変換 *)
let float_to_literal (f: float) : literal =
  Float (Float.to_string f)

(* ========== 定数評価エンジン ========== *)

(** 整数演算のオーバーフローチェック *)
let check_int_overflow (_op_name: string) (result: int64) (_span: span) : int64 =
  (* OCaml の int64 は自動的にラップアラウンドするため、
   * 明示的なオーバーフロー検出は複雑。Phase 1 では簡易実装 *)
  result

(** 算術演算の畳み込み *)
let fold_arithmetic_op (op: prim_op) (args: expr list) (span: span) : expr option =
  match op, args with
  | (PrimAdd | PrimSub | PrimMul | PrimDiv | PrimMod),
    [{ expr_kind = Literal lit_a; expr_ty; _ }; { expr_kind = Literal lit_b; _ }] ->
      (* 整数演算を試す *)
      begin match literal_to_int64 lit_a, literal_to_int64 lit_b with
      | Some a, Some b ->
          let result =
            match op with
      ...
      | _ ->
          (* 浮動小数演算を試す *)
          begin match literal_to_float lit_a, literal_to_float lit_b with
          | Some a, Some b ->
              let result =
                match op with
                | PrimAdd -> a +. b
...
(** 比較演算の畳み込み *)
let fold_comparison_op (op: prim_op) (args: expr list) (span: span) : expr option =
  match op, args with
  (* 整数比較 *)
  | PrimEq, [{ expr_kind = Literal lit_a; _ };
  ...
  | PrimNe, [{ expr_kind = Literal lit_a; _ };
             { expr_kind = Literal lit_b; _ }] ->
      begin match literal_to_int64 lit_a, literal_to_int64 lit_b with
      | Some a, Some b ->
          Some (make_expr (Literal (Bool (a <> b))) ty_bool span)
  ...
  | PrimLt, [{ expr_kind = Literal lit_a; _ };
             { expr_kind = Literal lit_b; _ }] ->
      begin match literal_to_int64 lit_a, literal_to_int64 lit_b with
      | Some a, Some b ->
          Some (make_expr (Literal (Bool (a < b))) ty_bool span)
  ...
  | PrimLe, [{ expr_kind = Literal lit_a; _ };
             { expr_kind = Literal lit_b; _ }] ->
      begin match literal_to_int64 lit_a, literal_to_int64 lit_b with
      | Some a, Some b ->
          Some (make_expr (Literal (Bool (a <= b))) ty_bool span)
  ...
  | PrimGt, [{ expr_kind = Literal lit_a; _ };
             { expr_kind = Literal lit_b; _ }] ->
      begin match literal_to_int64 lit_a, literal_to_int64 lit_b with
      | Some a, Some b ->
          Some (make_expr (Literal (Bool (a > b))) ty_bool span)
  ...
  | PrimGe, [{ expr_kind = Literal lit_a; _ };
             { expr_kind = Literal lit_b; _ }] ->
      begin match literal_to_int64 lit_a, literal_to_int64 lit_b with
      | Some a, Some b ->
          Some (make_expr (Literal (Bool (a >= b))) ty_bool span)
  ...
  | _ ->
      ...
      | [{ expr_kind = Literal lit_a; _ }; { expr_kind = Literal lit_b; _ }] ->
          begin match literal_to_bool lit_a, literal_to_bool lit_b with
          | Some a, Some b ->
              begin match op with
              | PrimEq -> Some (make_expr (Literal (Bool (a = b))) ty_bool span)
              | PrimNe -> Some (make_expr (Literal (Bool (a <> b))) ty_bool span)
              | _ -> None
              end
          | _ ->
              (* 文字列比較 *)
              begin match literal_to_string lit_a, literal_to_string lit_b with
              | Some a, Some b ->
                  begin match op with
                  | PrimEq -> Some (make_expr (Literal (Bool (a = b))) ty_bool span)
                  | PrimNe -> Some (make_expr (Literal (Bool (a <> b))) ty_bool span)
...
(** 論理演算の畳み込み *)
let fold_logical_op (op: prim_op) (args: expr list) (span: span) : expr option =
  match op, args with
  (* 論理積 *)
  | PrimAnd, [{ expr_kind = Literal lit_a; _ };
              { expr_kind = Literal lit_b; _ }] ->
      begin match literal_to_bool lit_a, literal_to_bool lit_b with
      | Some a, Some b ->
          Some (make_expr (Literal (Bool (a && b))) ty_bool span)
  ...
  (* 論理和 *)
  | PrimOr, [{ expr_kind = Literal lit_a; _ };
             { expr_kind = Literal lit_b; _ }] ->
      begin match literal_to_bool lit_a, literal_to_bool lit_b with
      | Some a, Some b ->
          Some (make_expr (Literal (Bool (a || b))) ty_bool span)
  ...
  | PrimNot, [{ expr_kind = Literal lit_a; _ }] ->
      begin match literal_to_bool lit_a with
      | Some a ->
          Some (make_expr (Literal (Bool (not a))) ty_bool span)
      | _ -> None
      end
...
let fold_primitive (op: prim_op) (args: expr list) (span: span) : expr option =
  ...
  | PrimAnd | PrimOr | PrimNot ->
      fold_logical_op op args span
  | _ -> None
...
let create_const_env () : const_env =
  Hashtbl.create 64
...
let rec fold_expr (env: const_env) (stats: fold_stats) (e: expr) : expr =
  ...
  (* 変数参照: 定数環境を検索 *)
  | Var var ->
      begin match lookup_const env var with
      | Some lit ->
  ...
  | Primitive (op, args) ->
      ...
      begin match fold_primitive op folded_args e.expr_span with
      | Some folded ->
          stats.folded_exprs <- stats.folded_exprs + 1;
          folded
      | None ->
          make_expr (Primitive (op, folded_args)) e.expr_ty e.expr_span
      end
  ...
  | If (cond, then_e, else_e) ->
      ...
      begin match folded_cond.expr_kind with
      | Literal (Bool true) ->
      ...
      | _ ->
          ...
          make_expr (If (folded_cond, folded_then, folded_else)) e.expr_ty e.expr_span
      end
  ...
  | Match (scrut, cases) ->
      ...
      let folded_cases = List.map (fun case ->
...
let fold_stmt (env: const_env) (stats: fold_stats) (stmt: stmt) : stmt =
  ...
  | Assign (var, e) ->
      ...
      (* 代入が定数なら環境に追加 *)
      begin match folded_e.expr_kind with
  ...
  | ExprStmt e ->
      ExprStmt (fold_expr env stats e)

  | Return e ->
      Return (fold_expr env stats e)

  | Jump _ | Branch _ | Phi _ | EffectMarker _ as s -> s

(** 終端命令の畳み込み *)
let fold_terminator (env: const_env) (stats: fold_stats) (term: terminator) : terminator =
  match term with
  | TermReturn e ->
      TermReturn (fold_expr env stats e)
...
(** ブロックが変化したかを判定 *)
let block_changed (b1: block) (b2: block) : bool =
  b1 != b2
...
let default_config : fold_config = {
  max_iterations = 5;
  verbose = false;
}

(** 不動点に達するまで畳み込みを反復 *)
let fold_to_fixpoint (config: fold_config) (stats: fold_stats) (fn: function_def) : function_def =
  let rec loop iteration fn_prev =
    if iteration >= config.max_iterations then
      fn_prev
    else begin
      let fn_next = fold_function stats fn_prev in
      if function_changed fn_prev fn_next then begin
...
(** 関数に対して定数畳み込みを適用 *)
let optimize_function ?(config = default_config) (fn: function_def) : function_def * fold_stats =
  let stats = create_stats () in
  let optimized = fold_to_fixpoint config stats fn in
  (optimized, stats)

(** モジュール全体に対して定数畳み込みを適用 *)
let optimize_module ?(config = default_config) (m: module_def) : module_def * fold_stats =
  let stats = create_stats () in
