...
let rec string_of_literal = function
  ...
  | Ast.Array exprs ->
      Printf.sprintf "[%s]" (join_with ", " string_of_ast_expr_shallow exprs)
  | Ast.Record fields ->
      let field_strs = List.map (fun (id, e) ->
...
let rec string_of_simple_pattern = function
  ...
  | PConstructor (name, pats) ->
      if List.length pats = 0 then
        ...
        Printf.sprintf "%s(%s)" name (join_with ", " string_of_simple_pattern pats)
...
let rec string_of_expr ?(depth=0) expr =
  ...
  | Var var ->
      Printf.sprintf "%s(var %s)" ind (string_of_var_id var)

  | App (fn, args) ->
      let fn_str = string_of_expr ~depth:(depth + 1) fn in
      let args_str = join_with "\n" (fun arg ->
        string_of_expr ~depth:(depth + 1) arg
      ) args in
  ...
  | Let (var, bound, body) ->
      ...
      Printf.sprintf "%s(let %s =\n%s\n%sin\n%s)" ind
        (string_of_var_id var) bound_str ind body_str
  ...
  | Match (scrut, cases) ->
      ...
      let cases_str = join_with "\n" (string_of_case ~depth:(depth + 1)) cases in
      Printf.sprintf "%s(match : %s\n%s\n%s)" ind ty_str scrut_str cases_str

  | Primitive (op, args) ->
      let args_str = join_with " " (fun arg ->
        string_of_expr ~depth:0 arg |> String.trim
      ) args in
      Printf.sprintf "%s(prim %s %s : %s)" ind (string_of_prim_op op) args_str ty_str
  ...
  | ADTConstruct (name, fields) ->
      let fields_str = join_with ", " (fun f ->
        string_of_expr ~depth:0 f |> String.trim
      ) fields in
...
and string_of_case ?(depth=0) case =
  ...
  let guard_str = match case.case_guard with
...
let string_of_stmt ?(depth=0) = function
  ...
  | Phi (var, incoming) ->
      let ind = indent depth in
      let incoming_str = join_with ", " (fun (lbl, v) ->
  ...
  | EffectMarker { effect_tag; effect_expr } ->
      let ind = indent depth in
      let expr_str = match effect_expr with
...
let string_of_terminator ?(depth=0) = function
  ...
  | TermBranch (cond, then_lbl, else_lbl) ->
      let ind = indent depth in
      let cond_str = string_of_expr ~depth:0 cond |> String.trim in
      Printf.sprintf "%sbr %s ? %s : %s" ind cond_str
        (string_of_label then_lbl) (string_of_label else_lbl)

  | TermSwitch (expr, cases, default) ->
      let ind = indent depth in
      let expr_str = string_of_expr ~depth:0 expr |> String.trim in
      let cases_str = join_with ", " (fun (lit, lbl) ->
        Printf.sprintf "%s => %s" (string_of_literal lit) (string_of_label lbl)
      ) cases in
...
let string_of_block ?(depth=0) block =
  let ind = indent depth in
  let params_str = if List.length block.params = 0 then ""
    else "(" ^ (join_with ", " string_of_var_id block.params) ^ ")"
  in
  let stmts_str = join_with "\n" (string_of_stmt ~depth:(depth + 1)) block.stmts in
...
let string_of_param param =
  let default_str = match param.param_default with
    | Some e -> " = " ^ (string_of_expr ~depth:0 e |> String.trim)
    | None -> ""
  in
...
let string_of_function fn =
  ...
  Printf.sprintf "fn %s(%s) -> %s {\n%s\n}" fn.fn_name params_str ret_ty_str blocks_str
...
let string_of_global global =
  ...
  Printf.sprintf "%s %s = %s" mutability
    (string_of_var_id global.global_var) init_str

let string_of_variant variant =
  if List.length variant.variant_fields = 0 then
...
let string_of_type_def tdef =
  let params_str = if List.length tdef.type_params = 0 then ""
...
let string_of_module mod_def =
  ...
  let functions_str = join_with "\n\n" string_of_function mod_def.function_defs in

  Printf.sprintf "module %s\n\n%s\n\n%s\n\n%s"
    mod_def.module_name types_str globals_str functions_str
