...
let rec string_of_type ty =
  ...
  | TyTuple tys ->
      tys |> List.map string_of_type |> String.concat ", "
      |> Printf.sprintf "(%s)"
  | TyRecord fields ->
      fields
      |> List.map (fun (id, ty) ->
             Printf.sprintf "%s: %s" (string_of_ident id) (string_of_type ty))
...
let rec string_of_literal = function
  | Int (value, base) ->
      Printf.sprintf "int(%s:%s)" value (string_of_int_base base)
  | Float f -> Printf.sprintf "float(%s)" f
  | Char c -> Printf.sprintf "char(%S)" c
  | String (s, kind) ->
      Printf.sprintf "string[%s](%S)" (string_of_string_kind kind) s
  ...
  | Record fields ->
      ...
      |> List.map (fun (id, expr) ->
             Printf.sprintf "%s = %s" (string_of_ident id) (string_of_expr expr))
      |> String.concat ", "
...
and string_of_pattern pat =
  ...
  | PatWildcard -> "_"
  | PatTuple pats ->
      pats |> List.map string_of_pattern |> String.concat ", "
      |> Printf.sprintf "(%s)"
  ...
  | PatConstructor (id, args) ->
      if args = [] then string_of_ident id
        ...
        args |> List.map string_of_pattern |> String.concat ", "
        |> Printf.sprintf "%s(%s)" (string_of_ident id)
  | PatGuard (pat, expr) ->
...
and string_of_stmt = function
  ...
  | AssignStmt (lvalue, rvalue) ->
      Printf.sprintf "assign %s := %s" (string_of_expr lvalue)
        (string_of_expr rvalue)
...
and string_of_unary_op = function Not -> "!" | Neg -> "-"
...
and string_of_expr expr =
  ...
  | ModulePath (path, id) ->
      Printf.sprintf "%s.%s" (string_of_module_path path) (string_of_ident id)
  | Call (fn, args) ->
  ...
  | Lambda (params, ret, body) ->
      ...
      let ret_str =
        match ret with
        | None -> ""
        | Some ty -> Printf.sprintf " -> %s" (string_of_type ty)
      in
      Printf.sprintf "lambda(%s%s => %s)" params_str ret_str
        (string_of_expr body)
  | Pipe (lhs, rhs) ->
      Printf.sprintf "pipe(%s |> %s)" (string_of_expr lhs) (string_of_expr rhs)
  | Binary (op, lhs, rhs) ->
      Printf.sprintf "binary(%s %s %s)" (string_of_expr lhs)
        (string_of_binary_op op) (string_of_expr rhs)
  | Unary (op, expr) ->
      Printf.sprintf "unary(%s%s)" (string_of_unary_op op) (string_of_expr expr)
  | FieldAccess (expr, id) ->
      Printf.sprintf "%s.%s" (string_of_expr expr) (string_of_ident id)
  | TupleAccess (expr, idx) -> Printf.sprintf "%s.%d" (string_of_expr expr) idx
  | Index (expr, idx) ->
      Printf.sprintf "%s[%s]" (string_of_expr expr) (string_of_expr idx)
  | Propagate expr -> Printf.sprintf "%s?" (string_of_expr expr)
  | If (cond, then_, else_) ->
      let else_str =
        match else_ with None -> "none" | Some e -> string_of_expr e
      in
      Printf.sprintf "if(%s, %s, %s)" (string_of_expr cond)
        (string_of_expr then_) else_str
  | Match (expr, arms) ->
      let arms_str =
        arms
        |> List.map (fun arm ->
               let guard =
                 ...
                 | None -> ""
                 | Some g -> " if " ^ string_of_expr g
               in
               Printf.sprintf "%s%s => %s"
                 (string_of_pattern arm.arm_pattern)
                 guard
                 (string_of_expr arm.arm_body))
        |> String.concat " | "
      in
  ...
  | Assign (lvalue, rvalue) ->
      Printf.sprintf "%s := %s" (string_of_expr lvalue) (string_of_expr rvalue)

and string_of_param param =
  let pat = string_of_pattern param.pat in
  let ty =
    match param.ty with None -> "" | Some ty -> ": " ^ string_of_type ty
  in
  let default =
...
and string_of_handler_entry = function
  | HandlerOperation op ->
      let params =
        op.handler_op_params |> List.map string_of_param |> String.concat ", "
      in
      let params_str = Printf.sprintf "(%s)" params in
      let body =
        op.handler_op_body |> List.map string_of_stmt |> String.concat "; "
      in
      Printf.sprintf "operation %s%s { %s }"
        (string_of_ident op.handler_op_name)
        params_str body
  | HandlerReturn ret ->
      let body =
        ret.handler_return_body |> List.map string_of_stmt |> String.concat "; "
      in
      Printf.sprintf "return %s { %s }"
        (string_of_ident ret.handler_return_name)
        body

and string_of_decl_kind = function
  | LetDecl (pat, ty, expr) ->
      let ty_str =
        match ty with None -> "" | Some ty -> ": " ^ string_of_type ty
      in
      Printf.sprintf "let %s%s = %s" (string_of_pattern pat) ty_str
        (string_of_expr expr)
  | VarDecl (pat, ty, expr) ->
      let ty_str =
        match ty with None -> "" | Some ty -> ": " ^ string_of_type ty
      in
      Printf.sprintf "var %s%s = %s" (string_of_pattern pat) ty_str
        (string_of_expr expr)
  | FnDecl fn ->
      let generics =
        ...
        | [] -> ""
        | params ->
            params |> List.map string_of_ident |> String.concat ", "
            |> Printf.sprintf "<%s>"
      in
      let params =
        fn.fn_params |> List.map string_of_param |> String.concat ", "
      in
      let ret =
        match fn.fn_ret_type with
        | None -> ""
        | Some ty -> " -> " ^ string_of_type ty
      in
      let where_clause =
        ...
        | clauses ->
            let clause_str =
              ...
              |> List.map (fun c ->
                     let args =
                       c.constraint_types |> List.map string_of_type
                       |> String.concat ", "
                     in
                     Printf.sprintf "%s(%s)"
                       (string_of_ident c.constraint_trait)
                       args)
              |> String.concat ", "
            in
            " where " ^ clause_str
      ...
      Printf.sprintf "fn %s%s(%s)%s%s%s%s"
        (string_of_ident fn.fn_name)
        generics params ret where_clause effects body
  | TypeDecl decl -> (
      match decl with
      | AliasDecl (name, params, ty) ->
          let params_str =
            ...
            | [] -> ""
            | ps ->
                ps |> List.map string_of_ident |> String.concat ", "
                |> Printf.sprintf "<%s>"
          in
          Printf.sprintf "type alias %s%s = %s" (string_of_ident name)
            params_str (string_of_type ty)
      | NewtypeDecl (name, params, ty) ->
          let params_str =
            ...
            | ps ->
                ...
                |> Printf.sprintf "<%s>"
          in
          Printf.sprintf "type %s%s = new %s" (string_of_ident name) params_str
            (string_of_type ty)
          ...
          let variant_str =
            ...
            |> List.map (fun v ->
                   if v.variant_types = [] then string_of_ident v.variant_name
                     ...
                     v.variant_types |> List.map string_of_type
                     |> String.concat ", "
                     |> Printf.sprintf "%s(%s)" (string_of_ident v.variant_name))
            |> String.concat " | "
          in
          Printf.sprintf "type %s%s = %s" (string_of_ident name) params_str
            variant_str)
  | TraitDecl trait ->
      let params =
        ...
        | clauses ->
            ...
            |> List.map (fun c ->
                   ...
                   Printf.sprintf "%s(%s)"
                     (string_of_ident c.constraint_trait)
                     args)
            |> String.concat ", " |> Printf.sprintf " where %s"
      in
      let items = List.length trait.trait_items in
      Printf.sprintf "trait %s%s%s {items=%d}"
        (string_of_ident trait.trait_name)
        params where_clause items
  | ImplDecl impl ->
      let params =
      ...
      let trait_part =
        ...
        | Some (trait_id, args) ->
            let args_str =
              args |> List.map string_of_type |> String.concat ", "
            in
      ...
      Printf.sprintf "impl %s%s%s%s {items=%d}" trait_part params
        (string_of_type impl.impl_type)
        where_clause items
  | ExternDecl ext ->
  ...
  | EffectDecl eff ->
      let ops =
        ...
        |> List.map (fun op ->
               Printf.sprintf "%s: %s"
                 (string_of_ident op.op_name)
                 (string_of_type op.op_type))
        |> String.concat ", "
      in
      Printf.sprintf "effect %s : %s { %s }"
        (string_of_ident eff.effect_name)
        (string_of_ident eff.effect_tag)
        ops
  | HandlerDecl handler ->
      let entries =
        ...
        |> String.concat "; "
      in
      Printf.sprintf "handler %s { %s }"
        (string_of_ident handler.handler_name)
        entries
  | ConductorDecl conductor ->
      let section_count = List.length conductor.conductor_body in
      Printf.sprintf "conductor %s {sections=%d}"
        (string_of_ident conductor.conductor_name)
        section_count
...
let string_of_visibility = function Public -> "pub " | Private -> ""
  ...
  let base =
    ...
    | Some alias ->
        Printf.sprintf "%s as %s"
          (string_of_ident item.item_name)
          (string_of_ident alias)
  in
  match item.item_nested with
  | None -> base
  | Some nested ->
      let nested_str =
        nested |> List.map string_of_use_item |> String.concat ", "
      in
...
let string_of_use_tree = function
  ...
  | UseBrace (path, items) ->
      let items_str =
        items |> List.map string_of_use_item |> String.concat ", "
      in
...
let string_of_decl decl =
  let attrs =
    ...
    | [] -> ""
    | attrs ->
        attrs |> List.map string_of_attribute |> String.concat " " |> fun s ->
        s ^ " "
  in
  attrs
  ^ string_of_visibility decl.decl_vis
  ^ string_of_decl_kind decl.decl_kind

let string_of_use_decl use_decl =
  (if use_decl.use_pub then "pub " else "")
  ^ "use "
  ^ string_of_use_tree use_decl.use_tree
...
let string_of_compilation_unit cu =
  let parts =
    []
    |> (fun acc ->
         match cu.header with
         | None -> acc
         | Some h -> string_of_module_header h :: acc)
    |> List.rev
  in
