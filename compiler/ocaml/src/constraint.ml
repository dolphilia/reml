...
(** 制約の種類 *)
type constraint_kind = Unify of ty * ty  (** τ₁ = τ₂ の単一化制約 *)

type constraint_ = { kind : constraint_kind; span : span  (** エラー報告用の位置情報 *) }
(** 制約（位置情報付き） *)
...
(** 単一化制約を作成 *)
let unify_constraint t1 t2 span = { kind = Unify (t1, t2); span }
...
let subst_lookup tv subst =
  List.find_opt (fun (tv', _) -> tv.tv_id = tv'.tv_id) subst |> Option.map snd
...
let rec unify subst t1 t2 span =
  ...
  | TVar tv, t | t, TVar tv ->
      (* Occurs check *)
      if occurs_check tv t then Error (occurs_check_error tv t span)
      else (* 新しい代入を追加 *)
        Ok ((tv, t) :: subst)
...
and unify_list subst pairs span =
  List.fold_left
    (fun acc (t1, t2) ->
      match acc with Ok s -> unify s t1 t2 span | err -> err)
    (Ok subst) pairs
  ...
  if List.length sorted1 <> List.length sorted2 then
    ...
    try
      let pairs =
        List.map2
          (fun (n1, t1) (n2, t2) ->
            if n1 <> n2 then raise (Failure "Field name mismatch") else (t1, t2))
          sorted1 sorted2
      in
...
let solve constraints =
  List.fold_left
    (fun acc c ->
      match acc with
      | Ok subst -> (
          match c.kind with Unify (t1, t2) -> unify subst t1 t2 c.span)
      | err -> err)
...
let string_of_constraint c =
  match c.kind with
  | Unify (t1, t2) ->
      Printf.sprintf "Unify(%s, %s) at %d:%d" (string_of_ty t1)
        (string_of_ty t2) c.span.start c.span.end_
