...
let assert_literal_eq (expected : literal) (actual : expr) : unit =
  ...
  | Literal lit -> (
      match (expected, lit) with
      | Ast.Int (exp_s, exp_base), Ast.Int (act_s, act_base) ->
          (* 数値として比較 *)
          let exp_val =
            Int64.of_string
              (if exp_base = Ast.Base10 then exp_s
               else "0x" ^ String.sub exp_s 2 (String.length exp_s - 2))
          in
          let act_val =
            Int64.of_string
              (if act_base = Ast.Base10 then act_s
               else "0x" ^ String.sub act_s 2 (String.length act_s - 2))
          in
...
let test_and_bools () =
  let e =
    make_prim PrimAnd [ make_bool_lit true; make_bool_lit false ] ty_bool
  in
  let result = optimize_expr e in
...
let test_or_bools () =
  let e =
    make_prim PrimOr [ make_bool_lit true; make_bool_lit false ] ty_bool
  in
  let result = optimize_expr e in
  assert_literal_eq (Ast.Bool true) result
