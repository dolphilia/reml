...
let run_with_mode mode =
  ...
  let mk_tag name =
    { Effect_profile.effect_name = name; effect_span = span }
  in
  let effect_set =
...
let compare_with_golden () =
  ...
  if String.trim diag_dictionary <> String.trim diag_monomorph then (
    let path = write_actual_snapshot "residual-leak-mismatch" diag_dictionary in
    failwith
      (Printf.sprintf
  ...
  if String.trim audit_dictionary <> String.trim audit_monomorph then (
    let path =
      write_actual_snapshot_ext "residual-leak-audit-mismatch" ".actual.jsonl" audit_dictionary
    in
    failwith
      (Printf.sprintf
         "辞書モードとモノモルフィゼーションモードで監査出力が一致しません。\n\
         辞書モード出力: %s"
         path));
  ...
  if not (Sys.file_exists audit_golden_path) then (
    let path =
      write_actual_snapshot_ext "residual-leak-audit" ".actual.jsonl" audit_dictionary
    in
    failwith
      (Printf.sprintf
         "監査ゴールデン %s が存在しません。\n現在の出力を %s に書き出しました。"
