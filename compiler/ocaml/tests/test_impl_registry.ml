...
let test_lookup_generic_impl () =
  ...
  let result =
    lookup "Eq" (TApp (TCon (TCUser "Vec"), TCon (TCInt I64))) registry
  in
  assert (result <> None);

  (* 検索: Eq for Vec<String> → 照合成功（T = String） *)
  let result =
    lookup "Eq" (TApp (TCon (TCUser "Vec"), TCon TCString)) registry
  in
  assert (result <> None);
...
let test_tuple_type_impl () =
  ...
  let result =
    lookup "Eq" (TTuple [ TCon (TCInt I64); TCon TCString ]) registry
  in
  assert (result = Some impl_eq_tuple);

  (* 検索: Eq for (String, i64) → 順序が異なるので失敗 *)
  let result =
    lookup "Eq" (TTuple [ TCon TCString; TCon (TCInt I64) ]) registry
  in
  assert (result = None);
...
let test_record_type_impl () =
  ...
  let result =
    lookup "Eq"
      (TRecord [ ("x", TCon (TCInt I64)); ("y", TCon TCString) ])
      registry
  in
  ...
  let result =
    lookup "Eq"
      (TRecord [ ("y", TCon TCString); ("x", TCon (TCInt I64)) ])
      registry
  in
  assert (result = Some impl_eq_record);
  ...
  let impl_eq_nested =
    {
      trait_name = "Eq";
      impl_type =
        TApp (TCon (TCUser "Vec"), TApp (TCon (TCUser "Vec"), TVar tv_t));
      generic_params = [ tv_t ];
      where_constraints = [];
