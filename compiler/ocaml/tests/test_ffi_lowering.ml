...
let assert_metadata_value case_label table key expected =
  ...
  | Some actual ->
      Printf.eprintf "ケース[%s]: メタデータ %s が %s でした (期待値: %s)\n"
        case_label key actual expected;
      exit 1
  | None ->
...
let linux_case () =
  ...
  let thunk_name = Ffi_stub_builder.thunk_symbol_name ~index:0 plan in
  {
    ...
    expect_stub_substrings =
      [ "reml_ffi_bridge_record_status"; thunk_name ];
...
let windows_case () =
  let contract =
    make_contract
      ~block_target:"ffi-win-block"
      ~target:"x86_64-pc-windows-msvc" ~callconv:"msvc"
      ~ownership:"transferred" ~link_name:"ffi_entry_symbol" ()
  in
  let plan =
...
let macos_case () =
  let contract =
    make_contract
      ~block_target:"ffi-macos-block"
      ~target:"arm64-apple-darwin" ~callconv:"aarch64_aapcscc"
      ~ownership:"borrowed" ()
  in
  let plan =
...
let verify_module_flag case_label llvm_module =
  ...
  | Some md -> (
      ...
      | Some other ->
          Printf.eprintf
            "ケース[%s]: reml.bridge.version が %Ld でした (期待値: 1)\n"
            case_label other;
          exit 1
      | None ->
          Printf.eprintf
            "ケース[%s]: reml.bridge.version が整数定数ではありません\n"
            case_label;
          exit 1)
...
let verify_case case =
  ...
  if Array.length metadata_nodes <> 1 then (
    Printf.eprintf
      "ケース[%s]: reml.bridge.stubs の件数が %d 件でした (期待値: 1)\n"
      case.label (Array.length metadata_nodes);
    exit 1);
  ...
  if stub_call_conv <> expected_call_conv then (
    Printf.eprintf
      "ケース[%s]: スタブ %s の CallConv=%d (期待値=%d)\n"
      case.label stub_name stub_call_conv expected_call_conv;
    exit 1);
  ...
  if thunk_call_conv <> expected_call_conv then (
    Printf.eprintf
      "ケース[%s]: サンク %s の CallConv=%d (期待値=%d)\n"
      case.label thunk_name thunk_call_conv expected_call_conv;
    exit 1);
  ...
  if extern_call_conv <> expected_call_conv then (
    Printf.eprintf
      "ケース[%s]: 外部シンボル %s の CallConv=%d (期待値=%d)\n"
      case.label extern_symbol extern_call_conv expected_call_conv;
    exit 1);
  ...
  if not (List.exists (fun cc -> cc = expected_call_conv) stub_call_sites) then (
    Printf.eprintf
      "ケース[%s]: サンクへの呼び出し CallConv に期待値 %d が含まれません\n"
      case.label expected_call_conv;
    exit 1);
  if
    not
      (List.exists (fun cc -> cc = expected_call_conv) thunk_call_sites)
  then (
    Printf.eprintf
      "ケース[%s]: 外部呼び出し CallConv に期待値 %d が含まれません\n"
      case.label expected_call_conv;
    exit 1);
...
let () =
  ...
  Printf.printf
    "FFI lowering metadata tests passed for %d targets.\n"
    (List.length cases);
  exit 0
