...
let assert_argument_classification name expected actual =
  test_count := !test_count + 1;
  let match_result = match expected, actual with
...
let test_type_size_tuples () =
  ...
  assert_true "(i32, i64) サイズ（12-16バイト）" (tuple_12_size = 12 || tuple_12_size = 16);
  ...
  assert_true "(i64, i8) サイズ（9-16バイト、境界値以下）" (tuple_15_size >= 9 && tuple_15_size <= 16);
  ...
  assert_true "(i64, i64, i8) サイズ（17-24バイト、境界値超過）" (tuple_17_size >= 17 && tuple_17_size <= 24);
...
let test_type_size_records () =
  ...
  assert_true "{x: Bool, y: i32} サイズ（5-8バイト）" (record_mixed_size >= 5 && record_mixed_size <= 8);
...
let test_classify_return_sysv () =
  ...
  assert_return_classification "{x: i64, y: i64} 戻り値 (16バイト)" DirectReturn class_rec_16;

  (* 24バイトレコード → SretReturn *)
  let record_24 = TRecord [("a", ty_i64); ("b", ty_i64); ("c", ty_i64)] in
  let class_rec_24 = classify_struct_return test_target test_ctx record_24 in
  assert_return_classification "{a: i64, b: i64, c: i64} 戻り値 (24バイト)" SretReturn class_rec_24;

  (* 4バイトタプル → DirectReturn *)
  let tuple_4 = TTuple [ty_bool; ty_bool; ty_bool; ty_bool] in
  let class_4 = classify_struct_return test_target test_ctx tuple_4 in
  assert_return_classification "(Bool, Bool, Bool, Bool) 戻り値 (4バイト)" DirectReturn class_4;
  ...
  assert_return_classification "((i64, i64), i64) 戻り値 (24バイト)" SretReturn class_nested;
  ...
  assert_return_classification "{data: String, count: i64} 戻り値 (24バイト)" SretReturn class_fat
...
let test_classify_argument_sysv () =
  ...
  assert_argument_classification "(i64, i64, i64) 引数 (24バイト)" (ByvalArg (reml_type_to_llvm test_ctx tuple_24)) class_24;

  (* 16バイトレコード → DirectArg *)
  let record_16 = TRecord [("x", ty_i64); ("y", ty_i64)] in
  let class_rec_16 = classify_struct_argument test_target test_ctx record_16 in
  assert_argument_classification "{x: i64, y: i64} 引数 (16バイト)" DirectArg class_rec_16;

  (* 24バイトレコード → ByvalArg *)
  let record_24 = TRecord [("a", ty_i64); ("b", ty_i64); ("c", ty_i64)] in
  let class_rec_24 = classify_struct_argument test_target test_ctx record_24 in
  assert_argument_classification "{a: i64, b: i64, c: i64} 引数 (24バイト)" (ByvalArg (reml_type_to_llvm test_ctx record_24)) class_rec_24;

  (* 4バイトタプル → DirectArg *)
  let tuple_4 = TTuple [ty_bool; ty_bool; ty_bool; ty_bool] in
  let class_4 = classify_struct_argument test_target test_ctx tuple_4 in
  assert_argument_classification "(Bool, Bool, Bool, Bool) 引数 (4バイト)" DirectArg class_4;
  ...
  assert_argument_classification "(i64, i64, i8) 引数 (境界値超過)" (ByvalArg (reml_type_to_llvm test_ctx tuple_17)) class_17;

  (* ネストタプル: ((i64, i64), i64) → ByvalArg（24バイト） *)
  let nested_tuple = TTuple [TTuple [ty_i64; ty_i64]; ty_i64] in
  let class_nested = classify_struct_argument test_target test_ctx nested_tuple in
  assert_argument_classification "((i64, i64), i64) 引数 (24バイト)" (ByvalArg (reml_type_to_llvm test_ctx nested_tuple)) class_nested;
  ...
  assert_argument_classification "{data: String, count: i64} 引数 (24バイト)" (ByvalArg (reml_type_to_llvm test_ctx fat_record)) class_fat
...
let test_sret_attribute () =
  ...
  let has_sret = String.contains ir_string 's' &&
  ...
  (* 複数の引数で最初の引数がsret *)
  let fn_ty_multi = Llvm.function_type void_ty [| ptr_ty; Llvm.i32_type llctx; Llvm.i64_type llctx |] in
  let test_fn_multi = Llvm.declare_function "test_sret_multi_fn" fn_ty_multi llmodule in
...
let test_byval_attribute () =
  ...
  (* 属性が設定されたことを確認（LLVM IR文字列に含まれるか） *)
  let ir_string = Llvm.string_of_llvalue test_fn in
  let has_byval = String.contains ir_string 'b' &&
  ...
  (* 複数の引数で第2引数がbyval *)
  let fn_ty_multi = Llvm.function_type void_ty [| Llvm.i32_type llctx; ptr_ty; Llvm.i64_type llctx |] in
  let test_fn_multi = Llvm.declare_function "test_byval_multi_fn" fn_ty_multi llmodule in
