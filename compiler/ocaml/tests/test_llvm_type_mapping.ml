...
(** LLVM 型の文字列表現 *)
let lltype_to_string llty = Llvm.string_of_lltype llty
...
let test_primitive_types () =
  ...
  assert_equal "Bool → i1" "i1" (lltype_to_string bool_ty);

  (* Char → i32 *)
  let char_ty = reml_type_to_llvm ctx ty_char in
  assert_equal "Char → i32" "i32" (lltype_to_string char_ty);

  (* 整数型 *)
  let i8_ty = reml_type_to_llvm ctx ty_i8 in
  assert_equal "i8 → i8" "i8" (lltype_to_string i8_ty);

  let i16_ty = reml_type_to_llvm ctx ty_i16 in
  assert_equal "i16 → i16" "i16" (lltype_to_string i16_ty);

  let i32_ty = reml_type_to_llvm ctx ty_i32 in
  assert_equal "i32 → i32" "i32" (lltype_to_string i32_ty);

  let i64_ty = reml_type_to_llvm ctx ty_i64 in
  assert_equal "i64 → i64" "i64" (lltype_to_string i64_ty);

  let isize_ty = reml_type_to_llvm ctx ty_isize in
  assert_equal "isize → i64 (x86_64)" "i64" (lltype_to_string isize_ty);

  (* 符号なし整数型 *)
  let u8_ty = reml_type_to_llvm ctx ty_u8 in
  assert_equal "u8 → i8" "i8" (lltype_to_string u8_ty);

  let u16_ty = reml_type_to_llvm ctx ty_u16 in
  assert_equal "u16 → i16" "i16" (lltype_to_string u16_ty);

  let u32_ty = reml_type_to_llvm ctx ty_u32 in
  assert_equal "u32 → i32" "i32" (lltype_to_string u32_ty);

  let u64_ty = reml_type_to_llvm ctx ty_u64 in
  assert_equal "u64 → i64" "i64" (lltype_to_string u64_ty);

  let usize_ty = reml_type_to_llvm ctx ty_usize in
  assert_equal "usize → i64 (x86_64)" "i64" (lltype_to_string usize_ty);

  (* 浮動小数型 *)
  let f32_ty = reml_type_to_llvm ctx ty_f32 in
  assert_equal "f32 → float" "float" (lltype_to_string f32_ty);

  let f64_ty = reml_type_to_llvm ctx ty_f64 in
  assert_equal "f64 → double" "double" (lltype_to_string f64_ty);

  (* 単位型 *)
  let unit_ty = reml_type_to_llvm ctx ty_unit in
  assert_equal "() → void" "void" (lltype_to_string unit_ty)
...
let test_composite_types () =
  ...
  assert_equal "タプル (i64, Bool) → { i64, i1 }" "{ i64, i1 }"
  ...
  (* レコード型 { x: i64, y: i64 } *)
  let record_ty =
    reml_type_to_llvm ctx (TRecord [ ("x", ty_i64); ("y", ty_i64) ])
  in
  assert_equal "レコード { x: i64, y: i64 } → { i64, i64 }" "{ i64, i64 }"
  ...
  assert_equal "固定長配列 [i64; 5] → [5 x i64]" "[5 x i64]"
...
let test_target_config () =
  ...
  assert_equal "デフォルトターゲットトリプル" "x86_64-unknown-linux-gnu" default.triple;

  assert_equal "デフォルト DataLayout" "e-m:e-p:64:64-f64:64:64-v128:128:128-a:0:64"
    default.datalayout;

  (* ポインタサイズ *)
  assert_equal "x86_64 ポインタサイズ (64 bits)" "64"
    (string_of_int (pointer_size_bits "x86_64-unknown-linux-gnu"));

  assert_equal "x86_64 ポインタサイズ (8 bytes)" "8"
    (string_of_int (pointer_size_bytes "x86_64-unknown-linux-gnu"));

  (* アラインメント仕様 *)
  let align_spec = get_alignment_spec "x86_64-unknown-linux-gnu" in
  assert_equal "x86_64 i64 アラインメント (8 bytes)" "8"
    (string_of_int align_spec.i64_align);

  assert_equal "x86_64 ポインタアラインメント (8 bytes)" "8"
