...
let normalize_ir_line line =
  ...
  else if String.starts_with ~prefix:"target " line then Some line
    (* declare/defineは関数シグネチャとして比較 *)
  else if
    String.starts_with ~prefix:"declare " line
    || String.starts_with ~prefix:"define " line
  then Some line (* attributes行も保持 *)
  else if String.starts_with ~prefix:"attributes " line then Some line
  ...
  | Ok ast -> (
      ...
      | Ok tast -> (
          try
            ...
            let llvm_module =
              Codegen.codegen_module ~target_name optimized_ir
            in
...
let compare_with_golden name =
  ...
  if not (Sys.file_exists golden_file) then (
    ...
    Out_channel.with_open_text actual_file (fun oc ->
        output_string oc actual_ir);

    exit 1);

  let expected_ir =
    In_channel.with_open_text golden_file In_channel.input_all
  in
  ...
  if normalized_actual = normalized_expected then Printf.printf "✓ %s\n" name
  else (
