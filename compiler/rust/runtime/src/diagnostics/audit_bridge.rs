...
use super::metric_point::{MetricPoint, MetricValue};

pub(crate) const METRIC_CAPABILITY_ID: &str = "metrics.emit";
pub(crate) const METRIC_STAGE: &str = "stable";

/// 指定したメトリクス情報を `AuditEnvelope.metadata` へ転写する。
pub(crate) fn attach_audit(metadata: &mut Map<String, Value>, metric: &MetricPoint) {
    metadata.insert(
...
pub(crate) fn metric_audit_metadata(metric: &MetricPoint) -> Map<String, Value> {
    let mut metadata = Map::new();
    attach_audit(&mut metadata, metric);
    metadata
}
...
mod tests {
    use super::*;
    use crate::diagnostics::metric_point;
    use serde_json::json;
