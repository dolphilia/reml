...
fn iter_flat_map_expands_sequences() {
    ...
    assert_eq!(iter.collect_vec(), vec![1, 10, 2, 20, 3, 30]);
}
...
fn iter_scan_tracks_running_totals() {
    ...
    assert_eq!(iter.collect_vec(), vec![1, 3, 6, 10]);
}
...
fn iter_take_drop_enumerate() {
    let iter = Iter::from_list(vec![10, 20, 30, 40])
        ...
        .take(2)
        .enumerate();
    assert_eq!(iter.collect_vec(), vec![(0, 20), (1, 30)]);
}
...
fn iter_zip_pairs_sequences() {
    ...
    assert_eq!(zipped.collect_vec(), vec![(1, "a"), (2, "b"), (3, "c")]);
}
