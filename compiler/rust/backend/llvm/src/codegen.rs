...
impl ModuleIr {
  pub fn describe(&self) -> String {
    ...
    self.metadata.iter().cloned().for_each(|item| summary.push(item));
    summary.join(" | ")
  }
}
...
impl CodegenContext {
  pub fn new(target_machine: TargetMachine, runtime_symbols: Vec<String>) -> Self {
    ...
    Self {
      ...
      ffi_lowering: FfiLowering::new(TypeMappingContext::new(target_machine.data_layout.clone()), runtime_symbols),
      target_machine,
      functions: Vec::new(),
  ...
  pub fn finish_module(mut self, name: impl Into<String>) -> ModuleIr {
    ModuleIr {
      name: name.into(),
