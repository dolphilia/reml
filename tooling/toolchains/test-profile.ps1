...
Write-Host "`n4. OCamlツール確認" -ForegroundColor Yellow
Write-Host "------------------"
$duneVersion = (dune --version 2>&1)
$ocamlVersion = (ocaml --version 2>&1)
if ($duneVersion -and $ocamlVersion) {
    Write-Host "✅ dune: $duneVersion" -ForegroundColor Green
    Write-Host "✅ ocaml: $ocamlVersion" -ForegroundColor Green
} else {
    Write-Host "❌ OCamlツールが見つかりません" -ForegroundColor Red
}
