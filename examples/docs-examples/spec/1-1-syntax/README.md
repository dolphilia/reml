...
- `module_parser` の再実装に関わるサンプル（`use_nested.reml`, `effect_handler.reml`, `block_scope.reml` など）は `cargo test --manifest-path compiler/rust/frontend/Cargo.toml parser::module -- --nocapture` の結果と紐付け、`reports/spec-audit/ch1/module_parser-YYYYMMDD-parser-tests.md` / `module_parser-YYYYMMDD-dualwrite.md` に保存する。`docs/notes/process/spec-integrity-audit-checklist.md#rust-gap-トラッキング表` の `SYNTAX-002/module_parser` 行とログ名を一致させること。
