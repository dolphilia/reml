...
2. マニフェスト API のテストを実行 (`toml v0.5.11` の checksum 問題で失敗する場合は `compiler/rust/runtime/Cargo.lock` の再取得が必要です):
   ```bash
   cargo test manifest --test manifest --manifest-path compiler/rust/runtime/Cargo.toml
   ```
