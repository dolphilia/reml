(* 簡易SQL Parser - OCaml 5実装 *)
(* SELECT, WHERE, JOIN, ORDER BY対応 *)
(* OCaml 5の代数的効果とAngstromパーサーコンビネーターを使用 *)

open Angstrom

(* 代数的効果を使ったエラーハンドリング *)
effect ParseError : string -> 'a

let with_parse_error f =
  match f () with
  | result -> Ok result
  | effect (ParseError msg) k ->
      Error msg
...
(* パブリックAPI（代数的効果を活用） *)
let parse input =
  with_parse_error (fun () ->
    match parse_string ~consume:All (sc *> select_query <* option () (symbol ";")) input with
    | Ok q -> q
    | Error msg -> perform (ParseError msg)
  )
...
(* テスト *)
let () =
  print_endline "=== OCaml 5 SQL Parser テスト（代数的効果を活用） ===";
  let test_sql = "SELECT name, age FROM users WHERE age > 18 ORDER BY name ASC" in
