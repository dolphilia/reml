...
| `expected/text_unicode.stream_decode.golden` | `compiler/rust/runtime/examples/io/text_stream_decode.rs` を `tests/data/unicode/streaming/sample_input.txt` で実行した結果。BOM 処理と `InvalidSequenceStrategy::Replace` の挙動を把握できる。 |
...
```sh
cargo run --manifest-path compiler/rust/runtime/Cargo.toml --bin text_stream_decode \
