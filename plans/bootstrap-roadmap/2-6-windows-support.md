...
**成果物**: ABI 差分レポート、`notes/llvm-spec-status-survey.md` への追記
...
5.2. **サンプルプログラムの実行**
- `samples/` 以下のプログラムを Windows でビルド
- 実行テスト（出力の検証）
- エラーケースのテスト
...
7.1. **セットアップ手順の文書化**
- `guides/llvm-integration-notes.md` への Windows セクション追加
- 環境変数の設定方法
- トラブルシューティング情報
- よくある質問（FAQ）

7.2. **ABI 差分の文書化**
- `notes/llvm-spec-status-survey.md` への差分レポート追記
