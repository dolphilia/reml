...
- **型安全な設定**：`Core.Config`（[2-7](2-7-config.md)）と CLI ガイドで、宣言 DSL → スキーマ検証 → 差分適用 → 実運用(Audit) の安全線を確立する。Chapter 4 では `Core.Config`/`Core.Data` の共通 API を再整理し、ガイドと仕様の分担を明確化する予定である。【F:4-1-standard-library-outline.md†L24-L30】
- **ツール連携**：`RunConfig.extensions["lsp"]` / 構造化ログ（[2-6](2-6-execution-strategy.md)）と IDE/LSP ガイドで、診断・補完・監査を共通 JSON メタデータに揃える。標準ライブラリ章で `Core.Diagnostics`/`Core.Audit` を定義し、CLI/LSP/監査ログの共通語彙を提供する。【F:4-1-standard-library-outline.md†L24-L30】
- **プラグイン拡張**：`ParserPlugin` / `CapabilitySet`（[2-1](2-1-parser-type.md):I 節）と DSL プラグインガイドで、外部 DSL の登録・互換・署名検証まで一貫して扱う。標準ライブラリ側では `Core.Runtime` と Capability Registry を通じた拡張 API を提供する計画である。【F:4-1-standard-library-outline.md†L24-L30】
