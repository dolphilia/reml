...
Reml (Readable & Expressive Meta Language) はパーサーコンビネーターに最適化された言語です。コンパイラやインタプリタを設計して実行するという難しい工程を最短で実現できることを目指します。Reml のコア仕様と標準APIは、書きやすさ・読みやすさ・エラーの品質が良さに徹底的に追求します。また、実用性能とUnicode前提であることも大切な点です。標準ライブラリ群 (`Core.*`) の最新仕様は Chapter 3 に再編されており、Chapter 2 に残るドキュメントは移行ガイドとして参照できます。
...
* **実験フラグ (`-Z`)**: 代数的効果やハンドラなど、既存ユーザーのワークフローを壊さないよう opt-in で提供する。仕様書には PoC 範囲・残余効果の制約・互換性ポリシーを明記。
* **ベータ統合**: 成熟した機能は `stabilize(feature)` RFC を経てデフォルト化。`@pure` / `@dsl_export` / Capability Registry との整合を証明するテストと、DSL 事例での実装報告を必須とする。
* **正式安定化**: ドキュメントとランタイムガイドを更新し、実験フラグ経由で使用していたコードは互換性を保ったまま移行できるようにする。
